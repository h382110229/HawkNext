...
import type { NodeHawkNextBuiltinProviderConfigSource } from "./hawknext-builtin-provider-config-source.js";
import type { HawkNextBuiltinRelease } from "./hawknext-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type HawkNextBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface HawkNextBuiltinRemoteSynchronizerOptions {
  ...
  readonly fetchRelease: (
    endpointKey: string,
    signal: AbortSignal,
  ) => Promise<HawkNextBuiltinRelease | null>;
  readonly onRefreshResult?: (event: HawkNextBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface HawkNextBuiltinRefreshEvent {
  readonly result: HawkNextBuiltinRefreshResult;
...
export class HawkNextBuiltinRemoteSynchronizer {
  readonly #options: HawkNextBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<HawkNextBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: HawkNextBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<HawkNextBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<HawkNextBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("HawkNext Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: HawkNextBuiltinRefreshEvent): void {
