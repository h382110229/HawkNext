...
import {
  normalizeHawkNextBuiltinEndpointOrigin,
  resolveHawkNextBuiltinCachePaths,
} from "./hawknext-builtin-cache-paths.js";
...
export interface EndpointScopedHawkNextBuiltinSourceOptions {
  readonly bundledFilePath: string;
  readonly environmentConfigRoot: string;
  readonly platform: string;
  readonly appVersion: string;
  readonly resolveEndpointOrigin: () => string | Promise<string>;
  readonly fetchRelease: HawkNextBuiltinRemoteSynchronizerOptions["fetchRelease"];
  readonly onRefreshResult?: HawkNextBuiltinRemoteSynchronizerOptions["onRefreshResult"];
  readonly watch?: boolean;
}

/**
 * 让 Environment 的 HawkNext 控制面 Endpoint 同时决定 Active/LKG 与刷新控制路径。
 * Endpoint 切换只替换当前 Source，不读取上一 Endpoint 的缓存。
 */
export class EndpointScopedHawkNextBuiltinSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #options: EndpointScopedHawkNextBuiltinSourceOptions;
  readonly #listeners = new Set<(reason: string) => void>();
  #current: CurrentEndpointSource | null = null;
  #ensureInFlight: Promise<CurrentEndpointSource> | null = null;
  #disposed = false;

  constructor(options: EndpointScopedHawkNextBuiltinSourceOptions) {
  ...
  async refresh(options?: { readonly force?: boolean }): Promise<HawkNextBuiltinRefreshResult> {
  ...
  async #resolveCurrent(): Promise<CurrentEndpointSource> {
    const endpointOrigin = normalizeHawkNextBuiltinEndpointOrigin(
      await this.#options.resolveEndpointOrigin(),
    );
    const paths = resolveHawkNextBuiltinCachePaths({
      environmentConfigRoot: this.#options.environmentConfigRoot,
      platform: this.#options.platform,
      appVersion: this.#options.appVersion,
      hawknextEndpointOrigin: endpointOrigin,
    });
    ...
    const synchronizer = new HawkNextBuiltinRemoteSynchronizer({
      source,
      controlFilePath: paths.controlFilePath,
      resolveEndpointKey: async () =>
        normalizeHawkNextBuiltinEndpointOrigin(await this.#options.resolveEndpointOrigin()),
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("EndpointScopedHawkNextBuiltinSource 已 dispose");
  }
}
...
class CurrentEndpointSource {
  constructor(
    ...
    readonly source: NodeHawkNextBuiltinProviderConfigSource,
    readonly synchronizer: HawkNextBuiltinRemoteSynchronizer,
