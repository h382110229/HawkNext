...
import type { ProviderConfigLayerSnapshot, ProviderSource } from "@hawknext/provider";
import { atomicWritePrivateTextFile, withFileLock } from "@hawknext/shared/node";
import {
  decodeHawkNextBuiltinRelease,
  encodeHawkNextBuiltinRelease,
  serializeHawkNextBuiltinRelease,
  type HawkNextBuiltinRelease,
} from "./hawknext-builtin-release.js";

export interface NodeHawkNextBuiltinProviderConfigSourceOptions {
  readonly bundledFilePath: string;
  readonly activeFilePath?: string;
  readonly watch?: boolean;
}

export type ApplyHawkNextBuiltinReleaseResult = "updated" | "unchanged" | "stale";

/** Bundled、Active/LKG 与 Remote 共用同一 Release，并最终发布为现有 Config Snapshot。 */
export class NodeHawkNextBuiltinProviderConfigSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #bundledFilePath: string;
  readonly #activeFilePath: string;
  ...
  constructor(options: NodeHawkNextBuiltinProviderConfigSourceOptions) {
    const bundledFilePath = options.bundledFilePath.trim();
    if (!bundledFilePath) throw new Error("HawkNext Built-in bundledFilePath 不能为空");
  ...
  async read(): Promise<ProviderConfigLayerSnapshot> {
    ...
    let release: HawkNextBuiltinRelease;
    try {
  ...
  async applyRemoteRelease(release: HawkNextBuiltinRelease): Promise<ApplyHawkNextBuiltinReleaseResult> {
    this.#assertNotDisposed();
    await this.#ensureWatcher();
    const result = await withFileLock(this.#activeFilePath, async () => {
      ...
      if (release.revision === current.revision) {
        if (serializeHawkNextBuiltinRelease(release) === serializeHawkNextBuiltinRelease(current)) {
          return "unchanged" as const;
        }
        throw new Error(`HawkNext Built-in 相同 revision ${release.revision} 对应不同内容`);
      }
  ...
  async #readAndMaterializeLocked(): Promise<HawkNextBuiltinRelease> {
  ...
  async #writeActiveLocked(release: HawkNextBuiltinRelease): Promise<void> {
    this.#assertNotDisposed();
    await atomicWritePrivateTextFile(
      this.#activeFilePath,
      JSON.stringify(encodeHawkNextBuiltinRelease(release), null, 2),
    );
  }
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("NodeHawkNextBuiltinProviderConfigSource 已 dispose");
  }
}

export function createNodeHawkNextBuiltinProviderConfigSource(
  options: NodeHawkNextBuiltinProviderConfigSourceOptions,
): NodeHawkNextBuiltinProviderConfigSource {
  return new NodeHawkNextBuiltinProviderConfigSource(options);
}

interface ReleaseCandidate {
  readonly release?: HawkNextBuiltinRelease;
  readonly error?: unknown;
}

async function readReleaseCandidate(filePath: string): Promise<ReleaseCandidate | null> {
  try {
    return { release: decodeHawkNextBuiltinRelease(JSON.parse(await readFile(filePath, "utf8"))) };
  } catch (error) {
...
function selectReleaseCandidate(
...
): HawkNextBuiltinRelease {
  if (
    bundled?.release &&
    active?.release &&
    bundled.release.revision === active.release.revision &&
    serializeHawkNextBuiltinRelease(bundled.release) !== serializeHawkNextBuiltinRelease(active.release)
  ) {
  ...
  const valid = [bundled?.release, active?.release].filter(
    (candidate): candidate is HawkNextBuiltinRelease => candidate !== undefined,
  );
  if (valid.length === 0) {
    throw new AggregateError(
      [bundled?.error, active?.error].filter((error) => error !== undefined),
      "Bundled 与 Active HawkNext Built-in Release 均不可用",
    );
  }
...
function snapshotFromRelease(
  release: HawkNextBuiltinRelease,
  sourceKey: string,
): ProviderConfigLayerSnapshot {
  return Object.freeze({
    revision: `hawknext-builtin:${release.revision}:${sourceKey}`,
...
function signatureOf(release: HawkNextBuiltinRelease): string {
  return `${release.revision}:${serializeHawkNextBuiltinRelease(release)}`;
}
