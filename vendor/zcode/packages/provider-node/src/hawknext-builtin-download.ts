import { z } from "zod";
import { decodeHawkNextBuiltinRelease, type HawkNextBuiltinRelease } from "./hawknext-builtin-release.js";
...
export interface HawkNextBuiltinDownloadOptions {
...
export async function downloadHawkNextBuiltinRelease(
  options: HawkNextBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeHawkNextBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`HawkNext Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
