...
export interface ProviderConfigSnapshot {
  ...
  readonly hawknextBuiltinProviders: ProviderConfigMap;
  readonly hawknextBuiltinProviderTemplates: ProviderTemplateMap;
  readonly personalProviders: ProviderConfigMap;
  readonly hawknextBuiltinModelRules: ModelConfigRules;
...
export function createFailClosedAccountProviderConfigSnapshot(
  ...
  const unentitledProviders = new ProviderConfigMap(
    config.hawknextBuiltinProviders.entries().flatMap(([providerId, provider]) =>
  ...
  return createAccountProviderConfigSnapshot(config.hawknextBuiltinRevision, unentitledProviders);
}

export function createAccountProviderConfigSnapshot(
  basedOnHawkNextBuiltinRevision: string,
  providers: ProviderConfigMap,
  states?: AccountProviderStates,
): AccountProviderConfigSnapshot {
  return Object.freeze({
    revision: `account:${JSON.stringify([basedOnHawkNextBuiltinRevision, providers.toJSON(), states])}`,
...
const EMPTY_ACCOUNT_PROVIDER_CONFIG_SNAPSHOT: AccountProviderConfigSnapshot = Object.freeze({
  ...
  basedOnHawkNextBuiltinRevision: "uninitialized",
