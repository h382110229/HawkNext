...
import type { completeModelConfigDataSchema } from "@hawknext/shared/model-config";
import type {
  ...
  resolve(input: ProviderConfigResolverInput): ProviderConfigResolution {
    const accountProviders = new ProviderConfigMap(
      input.accountProviders
        .entries()
        .filter(([providerId]) => input.hawknextBuiltinProviders.has(providerId))
        .map(([providerId, config]) => [providerId, config.withoutGroup()] as const),
    );
    const concreteBuiltinProviders = input.hawknextBuiltinProviders.overlay(accountProviders);
    const providerTemplates = input.hawknextBuiltinProviderTemplates;
    ...
    const effectiveModelRules = ModelConfigRules.composeEffective(
      input.hawknextBuiltinModelRules,
    ...
    for (const providerId of resolveProviderOrder(input, effectiveProviders)) {
      ...
      const models = orderedModelIds.map((modelId): ResolvedProviderModel => {
        ...
        const effectiveBuiltinConfig = input.hawknextBuiltinModelRules.resolve({
          providerId,
          templateId,
          modelId,
          apiType: config.api?.type,
...
function resolveProviderOrder(
  ...
  const familyIds = sourceIds.filter((providerId) => {
    ...
    return group === "zai-family" || group === "cloud-cn-family";
  });
  const familySet = new Set(familyIds);
  const builtinIds = input.hawknextBuiltinProviders
    .keys()
