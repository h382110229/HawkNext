...
export const providerTemplateConfigRuleSchema = providerTemplateDataSchema.extend({
  config: providerConfigDataSchema
    .pick({ logo: true, access: true, api: true, builtinModelIds: true, defaultModel: true, fallbackBaseUrl: true })
    .extend({
  ...
  .superRefine((rule, context) => {
    if (rule.providerId.startsWith("account:") && rule.config.access !== undefined) {
      context.addIssue({
        code: "custom",
        path: ["config", "access"],
        message: "固定 Account Provider 的 Access 只能由 HawkNext Built-in Config 声明",
      });
    }
  });
