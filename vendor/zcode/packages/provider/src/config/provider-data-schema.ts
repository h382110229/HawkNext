import { z } from "zod";
import { sparseShape } from "@hawknext/shared/config-schema";
...
export const providerConfigDataSchema = z
  .object({
    ...
    // llmapi deep-wiring: Auto default + primary/fallback failover
    defaultModel: z.string().min(1).nullable().optional(),
    fallbackBaseUrl: z.string().min(1).nullable().optional(),
  })
  .strict();
