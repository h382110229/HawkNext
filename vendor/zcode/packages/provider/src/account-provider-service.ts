  ...
  async #runRefreshLoop(): Promise<AccountProviderConfigSnapshot> {
    ...
    while (this.#pendingReasons.size > 0) {
      ...
      try {
        ...
        const configuredProviders = config.hawknextBuiltinProviders;
        const { providers, states } = await this.#resolve({
          configRevision: config.hawknextBuiltinRevision,
          configuredProviders,
        ...
        if (
          ...
          currentConfig.hawknextBuiltinRevision !== config.hawknextBuiltinRevision
        ) {
          this.#pendingReasons.add("superseded-resolution");
          continue;
        }
        const basedOnHawkNextBuiltinRevision = config.hawknextBuiltinRevision;
