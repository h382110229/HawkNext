...
/**
 * window.hawknext 类型定义 —— 仅包含需要 main 进程参与的平台操作
 *
...
declare global {
  interface Window {
    hawknext: {
      ...
      /** 通过系统原生另存为对话框保存文件 */
      saveFile?(
        payload: import("@hawknext/shared").SaveFileRequest,
      ): Promise<import("@hawknext/shared").SaveFileResult>;
      /** 将当前页面的 print 媒体版面导出为 PDF（Chromium 打印引擎，矢量文本） */
      printPageToPdf?(): Promise<import("@hawknext/shared").PrintPageToPdfResult>;
      ...
      importChromeBrowserData?(
        options?: import("@hawknext/shared").ChromeBrowserDataImportOptions,
