export const HAWKNEXT_KNOWN_TOOL_NAMES = [
  "Read",
  "Write",
  "Edit",
  "ApplyPatch",
...
export type HawkNextKnownToolName = (typeof HAWKNEXT_KNOWN_TOOL_NAMES)[number];

export type HawkNextToolFamily =
  | "file-read"
  | "file-write"
  | "shell"
  | "search"
...
const TOOL_FAMILY_BY_NAME: Record<HawkNextKnownToolName, HawkNextToolFamily> = {
  Read: "file-read",
  Write: "file-write",
  Edit: "file-write",
...
const TOOL_NAME_BY_LOWER = new Map<string, HawkNextKnownToolName>(
  HAWKNEXT_KNOWN_TOOL_NAMES.map((toolName) => [toolName.toLowerCase(), toolName]),
);

export function normalizeHawkNextToolName(
  value: string | null | undefined,
): HawkNextKnownToolName | null {
...
export function getHawkNextToolFamilyForName(
  value: string | null | undefined,
): HawkNextToolFamily | null {
  const toolName = normalizeHawkNextToolName(value);
  return toolName ? TOOL_FAMILY_BY_NAME[toolName] : null;
}

export function isHawkNextToolFamily(
  value: string | null | undefined,
  family: HawkNextToolFamily,
): boolean {
  return getHawkNextToolFamilyForName(value) === family;
}

export function isHawkNextFileContentWriteToolName(value: string | null | undefined): boolean {
  return normalizeHawkNextToolName(value) === "Write";
}
