...
// 消费者：hawknextTaskServiceAdapter/hawknextSessionProjection/hawknextTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromHawkNextMessageParts 已迁
// hawknext-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { HawkNextSessionGoal } from "./hawknext-protocol/index.js";
...
export function isHawkNextGoalContinuationReminderText(text: string): boolean {
...
export function isHawkNextGoalContinuationReminderMessage(message: HawkNextMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isHawkNextGoalContinuationReminderText(textFromHawkNextMessageParts(message.parts)))
  );
}

export function isHawkNextGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isHawkNextGoalModelOnlyReminderMessage(message: HawkNextMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isHawkNextGoalContinuationReminderMessage(message) ||
      isHawkNextGoalStateReminderText(textFromHawkNextMessageParts(message.parts)))
  );
}

export function isHawkNextModelOnlySyntheticUserMessage(message: HawkNextMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isHawkNextCompactSummaryMessage(message: HawkNextMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getHawkNextUserVisibleMessages(
  messages: readonly HawkNextMessageWithParts[],
  _options: { target?: HawkNextSessionGoal | null } = {},
): HawkNextMessageWithParts[] {
  const visibleMessages: HawkNextMessageWithParts[] = [];
  for (const message of messages) {
    if (isHawkNextModelOnlySyntheticUserMessage(message) || isHawkNextCompactSummaryMessage(message)) {
...
export function getHawkNextGoalIterationByAssistantMessageId(
  messages: readonly HawkNextMessageWithParts[],
  options: { maxGoalIteration?: number; target?: HawkNextSessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareHawkNextMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isHawkNextGoalContinuationReminderMessage(message)) {
...
export interface HawkNextGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getHawkNextGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly HawkNextGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareHawkNextMessagesByCreatedTime(
  left: HawkNextMessageWithParts,
  right: HawkNextMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: HawkNextMessageWithParts, target: HawkNextSessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveHawkNextVisibleSessionTitle(input: {
  title?: string;
  messages: readonly HawkNextMessageWithParts[];
  target?: HawkNextSessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isHawkNextGoalContinuationReminderText(normalizedTitle) &&
    !isHawkNextGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isHawkNextGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromHawkNextMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
