...
import { z } from "zod";
import type { HawkNextTaskMigrationSource, HawkNextTaskMode } from "./hawknext-task-types-core.js";
import { hawknextAgentProviderSchema } from "./hawknext-agent-policy.js";
import { hawknextPermissionResponseSchema } from "./hawknext-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./hawknext-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./hawknext-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const hawknextTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly HawkNextTaskMode[];
const hawknextTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly HawkNextTaskMigrationSource[];
const hawknextTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 HawkNextTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(hawknextTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: hawknextAgentProviderSchema.optional(),
  migrationSource: z.enum(hawknextTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: hawknextTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const hawknextPromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const hawknextTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const hawknextTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  hawknextTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: hawknextTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: hawknextTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
