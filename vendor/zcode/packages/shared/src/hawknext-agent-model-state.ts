// ── 旧协议兼容面（过渡期）──────────────────────────────
// 剩余 5 个导出：旧 configOptions 投影函数（formatModelPickerValue/normalizeAvailableHawkNextMode/
// getHawkNextAgentModeSelectOptions/getHawkNextAgentAvailableModes/
// hawknextSessionSettingsToHawkNextConfigOptions）。
// 消费者：services hawknextConfigOptions、UI hawknextSessionProjection 等旧栈。
import { formatModelPickerValue } from "./model-selection.js";
import type { HawkNextSessionMode, HawkNextSessionSettingsState } from "./hawknext-protocol/index.js";
import type { HawkNextConfigOption, HawkNextTaskModeInfo } from "./hawknext-task-types-core.js";
...
export function normalizeAvailableHawkNextMode(mode: HawkNextSessionMode): string {
  return HAWKNEXT_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

export function getHawkNextAgentModeSelectOptions(): NonNullable<HawkNextConfigOption["options"]> {
...
export function getHawkNextAgentAvailableModes(): HawkNextTaskModeInfo[] {
  return HAWKNEXT_AGENT_MODE_OPTIONS.map((mode) => ({ ...mode }));
}

export function hawknextSessionSettingsToHawkNextConfigOptions(
