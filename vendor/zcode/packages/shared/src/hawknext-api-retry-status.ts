import type { HawkNextApiRetryStatus } from "./hawknext-task-types-core.js";

export function normalizeHawkNextApiRetryStatus(
  value: unknown,
...
export function hawknextApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): HawkNextApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeHawkNextApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = hawknextApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isHawkNextModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function hawknextApiRetryFromStreamRecoveryPayload(
  value: unknown,
): HawkNextApiRetryStatus | undefined {
  const record = asRecord(value);
