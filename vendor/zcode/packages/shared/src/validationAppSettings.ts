...
import { normalizeHawkNextEndpointOrigin } from "./hawknextEndpoint.js";
import {
...
const hawknextInteractionBehaviorSchema = z.enum(["queue", "guide"]);
...
const providerFamilyDomainSchema = z.enum(["zai", "cloud-cn"]);
...
const hawknextEndpointOriginSchema = z.preprocess((value) => {
  ...
  try {
    return normalizeHawkNextEndpointOrigin(trimmed);
  } catch {
    return undefined;
  }
}, z.string().optional());

function sanitizeHawkNextEndpointOrigin(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const raw = value as Record<string, unknown>;
  if (!("hawknextEndpointOrigin" in raw)) {
    return value;
  }
  const parsed = hawknextEndpointOriginSchema.safeParse(raw.hawknextEndpointOrigin);
  if (parsed.success && typeof parsed.data === "string") {
    return { ...raw, hawknextEndpointOrigin: parsed.data };
  }
  const { hawknextEndpointOrigin: _hawknextEndpointOrigin, ...next } = raw;
...
const appSettingsObjectSchema = z.object({
  ...
  hawknextInteractionBehavior: hawknextInteractionBehaviorSchema.default("queue"),
  ...
  hawknextEndpointOrigin: hawknextEndpointOriginSchema.optional(),
});
        ...
        migrateMessageStreamShowReasoningDefault(
          migrateCloseToTrayOnWindowsDefault(
            migrateLegacyLocalePreference(
              sanitizeHawkNextEndpointOrigin(migrateLegacyWorkspaceSession(value)),
            ),
          ),
        ),
      ),
...
export const appSettingsPatchSchema = z.object({
  ...
  hawknextInteractionBehavior: hawknextInteractionBehaviorSchema.optional(),
