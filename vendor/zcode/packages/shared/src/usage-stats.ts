...
import type { HawkNextAccountAccess, HawkNextProviderAccountAccess } from "./hawknext-protocol/index.js";
...
export interface UsageStatsRequest {
  ...
  /** 设置页可传入用户当前选中的  / CloudCN 来源，避免两边都配置时只隐式读取第一家。 */
...
export interface UsageEntitlementRequest {
  ...
  /** 聊天输入区可传入当前选中的内置供应商,确保 CloudCN/ 用量跟随模型选择。 */
  preferredProviderId?: string;
...
export interface UsageEntitlementSnapshot {
  ...
  /**
   * HawkNext 官方 Server MCP 的调用额度（`/api/v1/mcp/usage`）。
...
export interface UsageStatsSnapshot {
   ...
   */
  source?: "cloud-cn-monitor" | "local";
  /** 远端用量来源供应商，用于 UI 展示 CloudCN /  等来源。 */
  sourceProvider?: UsageEntitlementProviderInfo | null;
  /** 工具调用维度（仅 CloudCN tool-usage 接口可用，本地聚合不填）。 */
