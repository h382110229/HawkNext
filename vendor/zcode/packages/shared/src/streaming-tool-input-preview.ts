export interface HawkNextStreamingToolInputState {
  deltaCount?: number;
...
export interface HawkNextStreamingToolInputPreview {
  complete: boolean;
  input: unknown;
  rawInput: string;
}

export type HawkNextStreamingToolInputPreviewMode = "active-live" | "background-summary";

export const HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_EAGER_DELTA_COUNT = 1;
export const HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS = 750;
export const HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH = 8 * 1024;
export const HAWKNEXT_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS = 1_000;
export const HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_TIME_BUDGET_MAX_RAW_INPUT =
  HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH;
...
export function appendHawkNextStreamingToolInputDelta(
  state: HawkNextStreamingToolInputState | undefined,
  delta: string,
): HawkNextStreamingToolInputState {
  return {
    ...state,
...
export function buildHawkNextStreamingToolInputPreview(
  rawInput: string,
  completeInput?: unknown,
): HawkNextStreamingToolInputPreview {
...
export function shouldMaterializeHawkNextStreamingToolInputPreview(
  state: HawkNextStreamingToolInputState,
  options: {
    mode?: HawkNextStreamingToolInputPreviewMode;
    now?: number;
    toolName?: string;
  } = {},
): boolean {
  ...
  if (deltaCount <= HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_EAGER_DELTA_COUNT) {
    return true;
  }
  const lastPreviewAt = state.lastPreviewAt ?? 0;
  if (isHawkNextFileStreamingToolInputPreviewTool(options.toolName)) {
    // 性能修复：Write/Edit 的半截 JSON 会触发全量内容恢复和行级 diff。
    // 大字节 chunk 不能绕过一秒窗口，否则模型输出越快，UI 反而更新越频繁。
    return (
      (options.now ?? Date.now()) - lastPreviewAt >=
      HAWKNEXT_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    );
  }
  ...
  if (rawGrowth >= HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_RAW_GROWTH) {
    return true;
  }
  const intervalElapsed =
    (options.now ?? Date.now()) - lastPreviewAt >=
    HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS;
  if (!intervalElapsed) {
    return false;
  }
  // 性能修复：大 Write/Edit 参数通常会被 provider 以 4KB 左右的慢速 chunk 推送。
  // 若只按时间预算，active 任务仍会每个 chunk 解析累计 JSON；超过小输入范围后改由 raw growth 控制。
  return state.rawInput.length <= HAWKNEXT_ACTIVE_STREAMING_TOOL_INPUT_TIME_BUDGET_MAX_RAW_INPUT;
}

export function isHawkNextFileStreamingToolInputPreviewTool(toolName?: string): boolean {
  const normalized = toolName?.trim().toLowerCase();
  return normalized === "write" || normalized === "edit";
}

export function markHawkNextStreamingToolInputPreviewMaterialized(
  state: HawkNextStreamingToolInputState,
  now = Date.now(),
): void {
