import type { HawkNextPlanStep } from "./hawknext-task-types-core.js";
...
function normalizePlanStatus(value: unknown): HawkNextPlanStep["status"] | null {
...
function parsePlanStep(value: unknown, index: number): HawkNextPlanStep | null {
...
function extractPlanStepsFromValue(value: unknown): HawkNextPlanStep[] | null {
  ...
  const steps = collection
    ...
    .filter((step): step is HawkNextPlanStep => step !== null);
