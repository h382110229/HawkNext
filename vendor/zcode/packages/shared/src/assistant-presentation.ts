import {
  ...
  type HawkNextAssistantMessagePart,
} from "./assistant-message-parts.js";

export interface HawkNextAssistantPresentationToolCall {
  toolId: string;
...
export type HawkNextAssistantPresentationBlock =
  | {
      type: "content";
      content: string;
    }
  ...
  | {
      type: "tool-call";
      toolCall: HawkNextAssistantPresentationToolCall;
    };

export interface HawkNextAssistantPresentation {
  messageParts: HawkNextAssistantMessagePart[];
  blocks: HawkNextAssistantPresentationBlock[];
  latestPart: Extract<HawkNextAssistantPresentationBlock, { type: "content" }> | null;
  historyBlocks: HawkNextAssistantPresentationBlock[];
}

export interface BuildHawkNextAssistantPresentationOptions {
  content: string;
  thought?: string;
  toolCalls?: readonly HawkNextAssistantPresentationToolCall[];
  parts?: readonly HawkNextAssistantMessagePart[];
  streaming?: boolean;
  interrupted?: boolean;
  settling?: boolean;
}
...
function buildFallbackAssistantParts({
  ...
  thought,
  toolCalls,
}: Pick<BuildHawkNextAssistantPresentationOptions, "content" | "thought" | "toolCalls">) {
...
export function buildHawkNextAssistantPresentation({
  content,
  thought,
  toolCalls = [],
  parts,
  ...
  interrupted = false,
  settling = false,
}: BuildHawkNextAssistantPresentationOptions): HawkNextAssistantPresentation {
  const messageParts =
  ...
  const blocks: HawkNextAssistantPresentationBlock[] = [];
  ...
  const latestContentPart =
    streaming || interrupted || settling
      ...
      : getLatestAssistantContentPart(
          blocks
            .filter(
              (block): block is Extract<HawkNextAssistantPresentationBlock, { type: "content" }> =>
                block.type === "content",
            )
            .map((block) => ({ type: "content", content: block.content })),
        );
  let latestPart: Extract<HawkNextAssistantPresentationBlock, { type: "content" }> | null = null;
    ...
    latestPart =
      latestBlockIndex >= 0
        ? (blocks[latestBlockIndex] as Extract<
            HawkNextAssistantPresentationBlock,
            { type: "content" }
          >)
        : null;
  }
