import { HAWKNEXT_FILE_LOCK_TIMEOUT_ERROR_CODE } from "../errors.js";
...
function createFileLockTimeoutError(
  ...
  const error = new Error(
    `Timed out after ${waitedMs}ms waiting for the HawkNext file lock: ${lockFile}`,
  ) as NodeJS.ErrnoException & { cause?: unknown };
  error.code = HAWKNEXT_FILE_LOCK_TIMEOUT_ERROR_CODE;
  error.path = filePath;
  error.syscall = "mkdir";
  error.cause = cause;
  return error;
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    try {
      ...
      if (
        ...
        throw Object.assign(new Error("HawkNext file lock ownership changed during acquire"), {
          code: "EEXIST",
        });
      }
      return async () => {
