...
// 透传到 JSON-RPC `error.data.code`；客户端读回时用 readHawkNextAttachmentFaultCode。

export const HAWKNEXT_ATTACHMENT_FAULT_CODES = {
  /** host 未实现 stat 能力。 */
...
export type HawkNextAttachmentFaultCode =
  (typeof HAWKNEXT_ATTACHMENT_FAULT_CODES)[keyof typeof HAWKNEXT_ATTACHMENT_FAULT_CODES];

const KNOWN_FAULT_CODES = new Set<string>(Object.values(HAWKNEXT_ATTACHMENT_FAULT_CODES));

export function isHawkNextAttachmentFaultCode(value: unknown): value is HawkNextAttachmentFaultCode {
 ...
 */
export class HawkNextAttachmentFaultError extends Error {
  readonly code: HawkNextAttachmentFaultCode;

  constructor(code: HawkNextAttachmentFaultCode, options?: { cause?: unknown; message?: string }) {
    // message 默认就是 fault 码本身，旧版本客户端的文本匹配仍能命中。
    super(
      options?.message ?? code,
      options?.cause === undefined ? undefined : { cause: options.cause },
    );
    this.name = "HawkNextAttachmentFaultError";
    this.code = code;
  }
}
...
/**
 * 从任意错误中读出附件 fault 码：
 * - 同进程抛出的 HawkNextAttachmentFaultError / 带 code 的 Error；
 * - 跨 JSON-RPC 回传的 `error.data.code`；
 * - 旧版本 CLI 只有消息文本时的兼容兜底（见下方注释）。
 */
export function readHawkNextAttachmentFaultCode(error: unknown): HawkNextAttachmentFaultCode | undefined {
  if (!error || typeof error !== "object") return undefined;
  const candidate = error as { code?: unknown; data?: unknown };
  if (isHawkNextAttachmentFaultCode(candidate.code)) return candidate.code;
  if (candidate.data && typeof candidate.data === "object") {
    const data = candidate.data as { code?: unknown };
    if (isHawkNextAttachmentFaultCode(data.code)) return data.code;
  }
  // 兼容兜底：桌面端可能连接尚未带结构化 code 的旧 hawknext-cli。仅在消息「整体等于」
  // 某个 fault 码时命中，不做模糊匹配，避免把包装后的任意文本误判成确定分类。
  if (error instanceof Error && isHawkNextAttachmentFaultCode(error.message)) return error.message;
  return undefined;
}
