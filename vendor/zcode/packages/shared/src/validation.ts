...
import { z } from "zod";
import { hawknextProcessDiagnosticSchema } from "./process-diagnostic.js";
...
import { hawknextProviderSchema } from "./providers.js";
import { hawknextAgentProviderSchema } from "./hawknext-agent-policy.js";
...
} from "./hawknext-protocol/index.js";
import { hawknextTaskModeSchema } from "./hawknext-task-mode-schema.js";
import { PROTOCOL_V4_LIMITS } from "./hawknext-protocol-v4/core.js";
import { errorAttributionSchema } from "./hawknext-protocol-v4/snapshot.js";
import { sessionWorkflowActivitySchema } from "./hawknext-protocol-v4/sessions-index-workflow-activity.js";
import {
...
export { hawknextTaskModeSchema } from "./hawknext-task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("hawknext-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  hawknextBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 HawkNextTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { hawknextProviderSchema };

export const hawknextTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessSpawnedResponseSchema = z.object({
  ...
  provider: hawknextProviderSchema,
  workspacePath: nonEmptyStringSchema,
  command: z.string(),
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: hawknextProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = hawknextProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `hawknextProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: hawknextMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: hawknextToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: hawknextMcpTelemetryEventSchema,
  })
  .strict();
...
export const hawknextTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const hawknextPromptImageAttachmentSchema = z.object({
...
export const hawknextPromptVideoAttachmentSchema = z.object({
...
export const hawknextPromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const hawknextPromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const hawknextPromptAttachmentSchema = z.discriminatedUnion("kind", [
  hawknextPromptImageAttachmentSchema,
  hawknextPromptVideoAttachmentSchema,
  hawknextPromptPdfAttachmentSchema,
  hawknextPromptFileAttachmentSchema,
]);

export const hawknextPersistedToolCallSchema = z.object({
...
const hawknextPersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const hawknextPersistedMessageSchema = z.object({
  ...
  attachments: z.array(hawknextPromptAttachmentSchema).optional(),
  tools: z.array(hawknextPersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(hawknextPersistedMessagePartSchema).optional(),
...
export const hawknextTaskGoalStatusSchema = z.enum(["active", "paused", "budget_limited", "complete"]);

export const hawknextTaskTargetChangedActionSchema = z.enum([
  "set",
  "status_updated",
  "cleared",
  "usage_accounted",
...
export const hawknextTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const hawknextTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: hawknextTaskGoalStatusSchema,
...
export const hawknextTaskGoalChangedPatchSchema = z.object({
  action: hawknextTaskTargetChangedActionSchema,
  source: hawknextTaskTargetChangedSourceSchema,
  target: hawknextTaskGoalSchema.nullable(),
  previousTarget: hawknextTaskGoalSchema.nullable().optional(),
});

export const hawknextTaskTargetStatusSchema = hawknextTaskGoalStatusSchema;
export const hawknextTaskTargetSchema = hawknextTaskGoalSchema;
export const hawknextTaskTargetChangedPatchSchema = hawknextTaskGoalChangedPatchSchema;

export const hawknextTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: hawknextTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: hawknextAgentProviderSchema.optional(),
  migrationSource: hawknextTaskMigrationSourceSchema.optional(),
  ...
  status: hawknextTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: hawknextTaskGoalSchema.nullable().optional(),
});

export const hawknextTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const hawknextPinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(hawknextTaskIndexEntrySchema),
});

const hawknextPersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const hawknextPersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(hawknextPersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const hawknextPersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const hawknextSessionFileSchema = z.object({
  meta: hawknextTaskMetaSchema,
  messages: z.array(hawknextPersistedMessageSchema),
  fileChanges: z.array(hawknextPersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(hawknextPersistedTurnCheckpointSchema).optional(),
});
