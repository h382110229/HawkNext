export type HawkNextBackgroundTaskControlStatus =
  | "pending"
  | "running"
  | "completed"
  | "failed"
...
export interface HawkNextBackgroundTaskControlItem {
  jobId: string;
  toolCallId?: string;
  command: string;
  taskKind: "agent" | "bash";
  cancellable?: boolean;
  title?: string;
  status: HawkNextBackgroundTaskControlStatus;
  startedAt?: number;
  elapsedMs?: number;
  pid?: number;
  stdoutTail?: string;
...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<HawkNextBackgroundTaskControlStatus>([
  "running",
]);
...
type BackgroundTaskControlKind = HawkNextBackgroundTaskControlItem["taskKind"];
 ...
 */
export function resolveHawkNextBackgroundTaskControlKind(
  value: unknown,
): BackgroundTaskControlKind | undefined {
...
function normalizeStatus(value: string | undefined): HawkNextBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // HawkNext Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseHawkNextBackgroundTaskControlItems(
  value: unknown,
): HawkNextBackgroundTaskControlItem[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const jobsById = new Map<string, HawkNextBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveHawkNextBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
    ...
    const job: HawkNextBackgroundTaskControlItem = {
      jobId,
...
export function isActiveHawkNextBackgroundTaskControlItem(
  job: HawkNextBackgroundTaskControlItem,
): boolean {
  return ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES.has(job.status);
}

export function getHawkNextBackgroundTaskControlItemElapsedMs(
  job: HawkNextBackgroundTaskControlItem,
  now = Date.now(),
): number {
...
export function collectVisibleHawkNextBackgroundTaskControlItems(
  jobs: readonly HawkNextBackgroundTaskControlItem[],
  now = Date.now(),
  thresholdMs = 30_000,
): Array<HawkNextBackgroundTaskControlItem & { elapsedMs: number }> {
  return jobs
    .filter(isActiveHawkNextBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getHawkNextBackgroundTaskControlItemElapsedMs(job, now),
    }))
