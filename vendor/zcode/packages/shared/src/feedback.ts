...
export type FeedbackTicketFramework = "hawknext-agent";
...
export interface FeedbackDeviceInfo {
  ...
  osVersion?: string;
  osArch?: string;
  /** 提交反馈时的当前 Agent。单 HawkNext Agent 模式下固定为 HawkNext Agent。 */
  agentProvider?: string;
  /** 提交反馈时归一到反馈平台的框架标识，当前固定为 hawknext-agent。 */
...
export const DEFAULT_FEEDBACK_TICKET_FRAMEWORK: FeedbackTicketFramework = "hawknext-agent";

export const FEEDBACK_TICKET_FRAMEWORK_OPTIONS: {
  value: FeedbackTicketFramework;
  label: string;
}[] = [{ value: "hawknext-agent", label: "HawkNext Agent" }];
