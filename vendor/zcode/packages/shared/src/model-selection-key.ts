...
export function buildNativeSupplierKey(hawknextProvider: HawkNextProvider): string {
  return `${NATIVE_SUPPLIER_KEY_PREFIX}${hawknextProvider}`;
}
...
export function buildGhostSupplierKey(
  hawknextProvider: HawkNextProvider,
...
export function resolveSupplierKeyFromModelDisplayValue(
  ...
  return buildNativeSupplierKey(hawknextProvider);
}
