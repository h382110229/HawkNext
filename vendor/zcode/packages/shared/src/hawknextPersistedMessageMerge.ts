import type { HawkNextPersistedMessage, HawkNextPersistedMessagePart } from "./hawknext-task-types-core.js";

// HawkNext runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveHawkNextAssistants(
  messages: readonly HawkNextPersistedMessage[],
): HawkNextPersistedMessage[] {
  const merged: HawkNextPersistedMessage[] = [];
...
function mergeAssistantPair(
  first: HawkNextPersistedMessage,
  next: HawkNextPersistedMessage,
): HawkNextPersistedMessage {
  ...
  const shiftedNextParts: HawkNextPersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
