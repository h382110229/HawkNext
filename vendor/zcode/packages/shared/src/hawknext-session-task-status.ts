// ── 旧协议兼容面（过渡期）──────────────────────────────
// 剩余 1 个导出：deriveHawkNextTaskStatusFromSessionSnapshot。
// 消费者：hawknextTaskServiceAdapter/hawknextTaskIndexSyncer/hawknextSessionProjection（旧投影栈）。
import type { HawkNextSessionStateSnapshot } from "./hawknext-protocol/index.js";
import { getHawkNextUserVisibleMessages } from "./hawknext-session-visible-content.js";
import type { HawkNextTaskMeta } from "./hawknext-task-types-core.js";
type HawkNextTaskStatus = HawkNextTaskMeta["status"];

function statusFromHawkNextSession(
  status: HawkNextSessionStateSnapshot["session"]["status"],
): HawkNextTaskStatus {
...
function hasBlockingActiveSnapshotRuntime(snapshot: HawkNextSessionStateSnapshot): boolean {
...
function hasActiveSnapshotRuntime(snapshot: HawkNextSessionStateSnapshot): boolean {
...
function hasCompletedVisibleAssistantTurn(snapshot: HawkNextSessionStateSnapshot): boolean {
  const visibleMessages = getHawkNextUserVisibleMessages(snapshot.messages, {
...
export function deriveHawkNextTaskStatusFromSessionSnapshot(
  ...
  const status = statusFromHawkNextSession(snapshot.session.status);
