...
export const ServiceChannels = {
  ...
  /** 跨窗口广播 */
  Broadcast: "broadcast",
  /** HawkNext task wrapper 服务 */
  HawkNextTask: "hawknext-task",
  /** 窗口 Host 聚合 workspace/task 投影与列表写路由 */
  WindowController: "window-controller",
  /** HawkNext Protocol agent 服务 */
  HawkNextAgent: "hawknext-agent",
  /** HawkNext session 应用服务 */
  HawkNextSession: "hawknext-session",
  ...
  /** 插件管理服务 */
  Plugins: "plugins",
  /** 设置页插件管理服务（UI 平台能力面收敛，不再直触 hawknextAgentService） */
...
export const PlatformChannels = {
  /** 打开系统目录选择框 */
  SelectDirectory: "hawknext:select-directory",
  /** 打开系统文件选择框 */
  SelectFile: "hawknext:select-file",
  /** 打开系统多文件选择框 */
  SelectFiles: "hawknext:select-files",
  /** Renderer → Main：写入宿主 ~/.hawknext 临时文本附件 */
  CreateTempTextAttachment: "hawknext:create-temp-text-attachment",
  /** Renderer → Main：通过原生另存为对话框保存文件 */
  SaveFile: "hawknext:save-file",
  /** Renderer → Main：用 Chromium 打印引擎把当前页面 print 媒体版面导出为 PDF */
  PrintToPdf: "hawknext:print-to-pdf",
  /** Main → Renderer：转发远程连接过程日志 */
  RemoteConnectionLog: "hawknext:remote-connection-log",
  /** Main → Renderer：远程 workspace session 已关闭 */
  RemoteSessionClosed: "hawknext:remote-session-closed",
  /** 检查目录是否已在其他窗口打开，如果是则激活该窗口 */
  ActivateOrSetWorkspace: "hawknext:activate-or-set-workspace",
  /** 建立 SSH 远程连接 */
  ConnectRemote: "hawknext:connect-remote",
  /** 取消当前窗口正在进行中的远程连接 */
  CancelPendingRemoteConnection: "hawknext:cancel-pending-remote-connection",
  /** Renderer → Main：绑定远程 logical session 的 canonical workspace context */
  BindRemoteWorkspaceSessionContext: "hawknext:bind-remote-workspace-session-context",
  /** 释放当前窗口里的远程 session */
  DisposeRemoteSession: "hawknext:dispose-remote-session",
  /** Renderer → Main：检查本机 Docker daemon 是否可用 */
  IsDockerAvailable: "hawknext:is-docker-available",
  /** Renderer → Main：列出本机可用的 WSL 发行版 */
  ListWSLDistros: "hawknext:list-wsl-distros",
  /** Renderer → Main：列出当前可连接的 Docker 容器 */
  ListDockerContainers: "hawknext:list-docker-containers",
  /** Renderer → Main：列出 SSH config 里可用于快速填表的 alias */
  ListSSHConfigAliases: "hawknext:list-ssh-config-aliases",
  /** Renderer → Main：从用户目录加载 CLI MCP 配置 */
  LoadMcpFromUserDirectory: "hawknext:load-mcp-from-user-directory",
  /** Renderer → Main：保存 CLI MCP 配置到用户目录 */
  SaveMcpToUserDirectory: "hawknext:save-mcp-to-user-directory",
  /** Renderer 日志转发到 main 进程统一存储 */
  Log: "hawknext:log",
  /** Renderer → Main：同步当前窗口所有 tab 的 workspace 路径 */
  SyncWindowTabs: "hawknext:sync-window-tabs",
  /** Renderer → Main：同步当前窗口的未读 task 数 */
  SyncWindowUnreadCount: "hawknext:sync-window-unread-count",
  /** Renderer → Main：当前窗口 active task，只更新 Main 的临时焦点映射。 */
  SyncActiveTaskSession: "hawknext:sync-active-task-session",
  /** Renderer → Main：同步 main 进程需即时感知的应用设置 */
  SyncAppSettings: "hawknext:sync-app-settings",
  /** Renderer → Main：快捷键设置页录制态开关；true = main 暂时摘除可配置菜单 accelerator */
  SetShortcutRecordingActive: "hawknext:set-shortcut-recording-active",
  /** Main → Renderer：聚焦到指定 workspace 路径的 tab */
  FocusTab: "hawknext:focus-tab",
  /** Main → Renderer：菜单触发新建 tab */
  NewTab: "hawknext:new-tab",
  /** Main → Renderer：菜单或快捷键请求关闭当前上下文 */
  CloseActiveContextRequest: "hawknext:close-active-context-request",
  /** Main → Renderer：内置 webview 请求打开新的浏览器 tab */
  OpenBrowserUrl: "hawknext:open-browser-url",
  /** Main → Renderer：agent 首次 browser 命令建好受控 view，通知 renderer 自动开 browser-use tab */
  BrowserViewReady: "hawknext:browser-view-ready",
  /** Main → Renderer：agent 正在操作某个 browser-use tab，renderer 临时显示状态图标 */
  BrowserViewOperation: "hawknext:browser-view-operation",
  /** Main → Renderer：browser visibility capability 显示/隐藏 IAB 右侧面板 */
  BrowserViewVisibility: "hawknext:browser-view-visibility",
  /** Main → Renderer：Agent 设置/重置目标 tab viewport */
  BrowserViewViewportChanged: "hawknext:browser-view-viewport-changed",
  /** Main → Renderer：截图前请求 owner renderer 准备后台 guest 合成表面。 */
  BrowserViewScreenshotSurfacePrepare: "hawknext:browser-view-screenshot-surface-prepare",
  /** Renderer → Main：目标 guest 连续两个 animation frame 的 viewport 已稳定。 */
  BrowserViewScreenshotSurfaceReady: "hawknext:browser-view-screenshot-surface-ready",
  /** Main → Renderer：截图结束或准备失败，释放临时后台合成层。 */
  BrowserViewScreenshotSurfaceRelease: "hawknext:browser-view-screenshot-surface-release",
  /** Main → Renderer：agent close 命令 detach 受控 guest 后，通知 renderer 卸载对应 tab */
  BrowserViewCloseTab: "hawknext:browser-view-close-tab",
  /** Main → Renderer：预算淘汰时卸载 guest，但保留 logical tab shell。 */
  BrowserViewSuspend: "hawknext:browser-view-suspend",
  /** Main → Renderer：为 suspended shell 重新挂载 guest。 */
  BrowserViewRestore: "hawknext:browser-view-restore",
  /** Main → Renderer：菜单触发新建任务 */
  NewTask: "hawknext:new-task",
  /** Main → Renderer：菜单触发打开工作区 */
  OpenWorkspace: "hawknext:open-workspace",
  /** Main → Renderer：deep link 直接打开指定本地工作区目录 */
  OpenWorkspacePath: "hawknext:open-workspace-path",
  /** Main → Renderer：打开内置反馈对话框 */
  OpenFeedbackDialog: "hawknext:open-feedback-dialog",
  /** Main → Renderer：打开我的工单面板 */
  OpenTicketsPanel: "hawknext:open-tickets-panel",
  /** Main → Renderer：窗口全屏状态变化 */
  WindowFullscreenChanged: "hawknext:window-fullscreen-changed",
  /** Renderer → Main：读取窗口最大化状态与系统原生圆角能力 */
  GetDesktopWindowChromeState: "hawknext:get-desktop-window-chrome-state",
  /** Main → Renderer：窗口最大化状态与系统原生圆角能力变化 */
  DesktopWindowChromeStateChanged: "hawknext:desktop-window-chrome-state-changed",
  /** Main → Renderer：原生窗口控制区安全边距变化 */
  WindowControlsOverlayChanged: "hawknext:window-controls-overlay-changed",
  /** Preload → Main：preload 已同步读到当前窗口控制区安全边距 */
  WindowControlsOverlayReady: "hawknext:window-controls-overlay-ready",
  /** 获取资源管理器快照（CPU / 内存，按基础服务、内置插件、社区插件归类） */
  GetResourceUsageSnapshot: "hawknext:get-resource-usage-snapshot",
  SetResourceUsageSamplingActive: "hawknext:set-resource-usage-sampling-active",
  /** 打开资源管理器窗口（其他窗口触发） */
  OpenResourceManager: "hawknext:open-resource-manager",
  /** 资源管理器「存储」tab：开始扫描本机 .hawknext 占用（main 持有 StorageService，Worker 线程遍历） */
  StorageStartScan: "hawknext:storage-start-scan",
  /** 资源管理器「存储」tab：取消扫描 */
  StorageCancelScan: "hawknext:storage-cancel-scan",
  /** 资源管理器「存储」tab：读取最近一次快照 */
  StorageGetSnapshot: "hawknext:storage-get-snapshot",
  /** 资源管理器「存储」tab：按类别清理 */
  StorageClean: "hawknext:storage-clean",
  /** 资源管理器「存储」tab：在系统文件管理器中定位数据根内的路径 */
  StorageRevealPath: "hawknext:storage-reveal-path",
  /** Main → 资源管理器 renderer：扫描进度快照推送 */
  StorageScanProgress: "hawknext:storage-scan-progress",
  /** Renderer → Main：打开外部 URL（用于 OAuth 跳转浏览器） */
  OpenExternal: "hawknext:open-external",
  /** Renderer → Main：查询当前语言下是否存在可用的用户社群入口 */
  CanOpenCommunity: "hawknext:can-open-community",
  /** Renderer → Main：在系统文件管理器中打开路径 */
  OpenInFileManager: "hawknext:open-in-file-manager",
  /** Renderer → Main：使用系统默认应用打开本地文件 */
  OpenExternalFile: "hawknext:open-external-file",
  /** Renderer → Main：打开 HawkNext Computer Use 权限引导 */
  OpenCuaPermissionOnboarding: "hawknext:open-cua-permission-onboarding",
  /** Renderer → Main：取消当前 renderer 发起的一次权限引导 participant */
  CancelCuaPermissionOnboarding: "hawknext:cancel-cua-permission-onboarding",
  /**
   * Renderer → Main：预热并缓存已验证的 Helper 路径 + bundle 指纹。
   * 必须在拖拽浮窗挂载时调用 —— dragstart 链路里不允许任何异步 I/O。
   */
  PrepareCuaHelperPermissionDrag: "hawknext:prepare-cua-helper-permission-drag",
  /** Renderer → Main：把已验证的 Helper.app 同步拖出到 macOS 权限列表 */
  StartCuaHelperPermissionDrag: "hawknext:start-cua-helper-permission-drag",
  /**
   ...
   * 立刻消失可能打断正在进行的拖拽。
   */
  NotifyCuaHelperPermissionDragEnded: "hawknext:notify-cua-helper-permission-drag-ended",
  /** Renderer → Main：上报 OAuth state 用于 deep link 路由 */
  OAuthRegisterState: "hawknext:oauth-register-state",
  /** Main → Renderer：转发 deep link URL */
  OAuthCallback: "hawknext:oauth-callback",
  /** Main → Renderer：转发支付 deep link URL */
  PaymentCallback: "hawknext:payment-callback",
  /** Main → Renderer：外部分享页请求导入 share code。 */
  ShareImport: "hawknext:share-import",
  /** Renderer → Main：OAuth 回调已处理完成，可继续后置启动流程 */
  OAuthCallbackHandled: "hawknext:oauth-callback-handled",
  /** Renderer → Main：renderer 已就绪，可接收缓存的 deep link */
  RendererReady: "hawknext:renderer-ready",
  /** Renderer → Main：同步当前 renderer 的 telemetry 上下文 */
  SyncTelemetryContext: "hawknext:sync-telemetry-context",
  /** Renderer → Main：通过统一 telemetry 层上报业务事件 */
  ReportTelemetryEvent: "hawknext:report-telemetry-event",
  /** Renderer → Main：上报 ARMS 自定义事件 */
  ReportArmsCustomEvent: "hawknext:report-arms-custom-event",
  /** Renderer → Main：读取 Renderer 用户操作 Trace 灰度配置。 */
  GetRendererActionTraceConfig: "hawknext:get-renderer-action-trace-config",
  /** Main → Renderer：Renderer 用户操作 Trace 灰度配置变化。 */
  RendererActionTraceConfigChanged: "hawknext:renderer-action-trace-config-changed",
  /** Renderer → Main：发送已结束的 ui_action batch。 */
  ReportRendererActionTraceBatch: "hawknext:report-renderer-action-trace-batch",
  /** Renderer → Main：主窗口 renderer 每 60 秒的 heap 读数，单向 send，不需要回执。 */
  ReportRendererHeapSample: "hawknext:report-renderer-heap-sample",
  ReportLocalTtftBatch: "hawknext:report-local-ttft-batch",
  /** E2E preload → Main：读取 sendCustom 最终参数的内存 ring。 */
  ReadFinalArmsCustomEventsE2E: "hawknext:e2e:read-final-arms-custom-events",
  /** E2E preload → Main：清空 sendCustom 最终参数的内存 ring。 */
  ClearFinalArmsCustomEventsE2E: "hawknext:e2e:clear-final-arms-custom-events",
  /** E2E preload → Main：配置只针对目标 event name 的真实网络抑制。 */
  ConfigureFinalArmsCustomEventsE2E: "hawknext:e2e:configure-final-arms-custom-events",
  /** Renderer → Main：触发任务完成/失败的系统通知 */
  ShowTaskNotification: "hawknext:show-task-notification",
  /** Main → Preload：通知 renderer 播放任务通知提示音 */
  TaskNotificationSound: "hawknext:task-notification-sound",
  /** Main → Preload：用户点击了系统通知，携带 taskId 让 renderer 跳转到对应任务 */
  TaskNotificationClick: "hawknext:task-notification-click",
  /** Renderer → Main：导出日志（打包 ~/.hawknext/v2 及外部 agent 日志为 zip 并在 Finder 中显示） */
  ExportLogs: "hawknext:export-logs",
  /** Renderer → Main：截取当前窗口作为反馈附件 */
  CaptureWindowScreenshot: "hawknext:capture-window-screenshot",
  /**
   ...
   */
  BrowserViewAttachGuest: "hawknext:browser-view-attach-guest",
  /** Renderer → Main：重建 `<webview>` 前主动断开旧 guest 的 CDP。 */
  BrowserViewDetachGuest: "hawknext:browser-view-detach-guest",
  /** Renderer → Main：用户显式关闭 Browser tab。 */
  BrowserViewCloseTabFromRenderer: "hawknext:browser-view-close-tab-from-renderer",
  /** Renderer → Main：上报 Browser tab residency/display facts。 */
  BrowserViewReportResidency: "hawknext:browser-view-report-residency",
  /** Renderer → Main：指定 generation 的 guest 已卸载。 */
  BrowserViewSuspendReady: "hawknext:browser-view-suspend-ready",
  /** Renderer → Main：用户访问 suspended tab，请求恢复。 */
  BrowserViewEnsureResident: "hawknext:browser-view-ensure-resident",
  /** Renderer → Main：读取 workspace/task 的持久化 logical shells。 */
  BrowserViewRestoreTabs: "hawknext:browser-view-restore-tabs",
  /** Renderer → Main：自由尺寸拖拽/开关回写目标 guest viewport */
  BrowserViewUpdateViewport: "hawknext:browser-view-update-viewport",
  /** Embedded Browser preload → Main：在网页原生 Dialog 创建前同步请求可信系统框。 */
  EmbeddedBrowserJavaScriptDialog: "hawknext:embedded-browser-javascript-dialog",
  /** Renderer → Main：把自动发现的本机 Chrome Profile 数据一次性导入内置浏览器分区。 */
  ImportChromeBrowserData: "hawknext:import-chrome-browser-data",
  /** Renderer → Main：清理内置浏览器缓存或全部站点数据。 */
  ClearEmbeddedBrowserData: "hawknext:clear-embedded-browser-data",
  /** Main → Renderer：通知有新版本已下载完毕，可以重启安装 */
  UpdateReady: "hawknext:update-ready",
  /** Main → Renderer：用户手动点击"检查更新"后的结果反馈（toast 用） */
  UpdateCheckResult: "hawknext:update-check-result",
  /** Main → Renderer：自动更新持续状态变化（菜单 UI 用） */
  UpdateStateChanged: "hawknext:update-state-changed",
  /** Renderer → Main：主动获取当前自动更新状态（菜单打开时补偿事件丢失） */
  GetUpdateState: "hawknext:get-update-state",
  /** Renderer → Main：开始下载当前已发现的自动更新 */
  DownloadUpdate: "hawknext:download-update",
  /** Renderer → Main：取消当前正在下载的自动更新 */
  CancelUpdateDownload: "hawknext:cancel-update-download",
  /** Renderer → Main：打开独立自动更新窗口 */
  OpenUpdateStatusWindow: "hawknext:open-update-status-window",
  /** Renderer → Main：读取自动更新偏好 */
  GetAutoUpdatePreferences: "hawknext:get-auto-update-preferences",
  /** Renderer → Main：写入“自动下载并安装更新”偏好 */
  SetAutoDownloadAndInstallUpdates: "hawknext:set-auto-download-and-install-updates",
  /** Renderer → Main：查询桌面端正在运行的会话数量 */
  GetDesktopSessionActivity: "hawknext:get-desktop-session-activity",
  /** Renderer → Main：读取当前窗口页面缩放档位 */
  GetDesktopZoomLevel: "hawknext:get-desktop-zoom-level",
  /** Main → Renderer：当前窗口页面缩放档位变化 */
  DesktopZoomLevelChanged: "hawknext:desktop-zoom-level-changed",
  /** Renderer → Main：读取开发态 stdio tap proxy 开关状态 */
  GetHawkNextStdioTapDevState: "hawknext:get-hawknext-stdio-tap-dev-state",
  /** Main → Renderer：本地 setting.json 已由 main 进程更新 */
  SettingsChanged: "hawknext:settings-changed",
  /** Main → Renderer：应用语言已切换 */
  ApplicationLocaleChanged: "hawknext:application-locale-changed",
  /** Renderer → Main：读取宿主系统语言 */
  GetSystemLocale: "hawknext:get-system-locale",
  /** Main → Renderer：更新安装后的版本说明 */
  PostUpdateReleaseNotes: "hawknext:post-update-release-notes",
  /** Renderer → Main：确认版本说明已读 */
  AcknowledgePostUpdateReleaseNotes: "hawknext:ack-post-update-release-notes",
  /** Renderer → Main：跳过当前已发现的自动更新版本 */
  SkipUpdateVersion: "hawknext:skip-update-version",
  /** Renderer → Main：用户确认重启安装更新 */
  QuitAndInstallUpdate: "hawknext:quit-and-install-update",
  /** Renderer → Main：获取系统中已安装的编辑器/终端列表（含图标） */
  GetInstalledEditors: "hawknext:get-installed-editors",
  /** Renderer → Main：按 bundle id 获取系统应用图标 */
  GetApplicationIcon: "hawknext:get-application-icon",
  /** Renderer → Main：用指定编辑器打开路径 */
  OpenInEditor: "hawknext:open-in-editor",
  /** Renderer → Main：执行桌面窗口级命令 */
  ExecuteDesktopCommand: "hawknext:execute-desktop-command",
  /** Renderer → Main：同步应用菜单语言，用于重建原生菜单 */
  SetApplicationLocale: "hawknext:set-application-locale",
  /** Renderer → Main：同步标题栏亮暗色，用于原生窗口控制按钮配色 */
  SetTitleBarTheme: "hawknext:set-title-bar-theme",
  /** Renderer → Main：迁移旧版 Common MCP 配置 */
  MigrateLegacyCommonMcp: "hawknext:migrate-legacy-common-mcp",
  /** Renderer → Main：获取当前设备的稳定标识符（deviceMid） */
  GetDeviceId: "hawknext:get-device-id",
} as const;
...
export const EmbeddedBrowserWebviewChannels = {
  ...
  WheelBoundary: "hawknext:embedded-browser-wheel-boundary",
} as const;
...
/**
 * Electron `<webview>`（partition=persist:hawknext-coding-plan）的 `sendToHost` / `ipc-message` 频道。
 * 官网页通过 preload 注入的 window.hawknextBridge 调用，不经过 main process。
 */
export const CodingPlanWebviewChannels = {
  /** 官网页购买成功后通知 App 刷新 entitlements 并关闭 webview。 */
  PurchaseComplete: "hawknext:coding-plan-purchase-complete",
} as const;
...
export interface CodingPlanPurchaseCompletePayload {
  provider: "zai" | "cloud-cn";
  /** 客户端时间戳，用于 App 侧去重/日志，不参与判等。 */
  timestamp: number;
}

/**
 * 官网页 window.__hawknextLang__ 的取值，与 App IntlProvider 的 Locale 一致。
 ...
 * `hawknext-coding-plan-lang-change` CustomEvent，website 侧（hawknextBridge.onLangChange 或
...
export const InternalChannels = {
  DatabaseStartupState: "hawknext:database-startup-state",
  DatabaseStartupControl: "hawknext:database-startup-control",
  /** main → renderer 转发 MessagePort（通过 webContents.postMessage） */
  ServicePort: "hawknext:service-port",
  /** main → renderer 转发窗口 Host 的 scoped MessagePort */
  ScopedServicePort: "hawknext:scoped-service-port",
  /** renderer → main：scoped MessagePort 已注册，可安全切换 attachment */
  ScopedServicePortReady: "hawknext:scoped-service-port-ready",
  /** preload → renderer：主进程已确认系统通知展示，renderer 可播放提示音 */
  TaskNotificationSound: "hawknext:task-notification-sound",
} as const;

/** @deprecated `/ws` 已忽略该头；保留常量仅供旧客户端兼容。 */
export const HAWKNEXT_RPC_CLIENT_MODE_HEADER = "x-hawknext-rpc-client-mode";
/** desktop 先经受保护 HTTP endpoint 申请，再在 `/ws/host` 握手时一次性消费。 */
export const HAWKNEXT_RPC_HOST_CAPABILITY_HEADER = "x-hawknext-rpc-host-capability";
...
export const HostMessageTypes = {
  ...
  /** Main → Host：全局前台 HawkNext 窗口派生的 producer focus fact。 */
...
export const HostResponseTypes = {
  ...
  /** host → main：声明一个 HawkNext Agent session 当前归属该 host */
