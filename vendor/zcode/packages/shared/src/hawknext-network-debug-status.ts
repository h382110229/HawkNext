import type {
  ...
  TraceId,
  HawkNextStreamEvent,
  HawkNextTaskNetworkDebugStatusType,
} from "./hawknext-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<HawkNextTaskNetworkDebugStatusType>([
...
export function hawknextTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<HawkNextStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): HawkNextTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as HawkNextTaskNetworkDebugStatusType)
    : undefined;
}
