...
/** HawkNext 运行中继续输入时的交互行为 */
export type HawkNextInteractionBehavior = "queue" | "guide";
...
export interface ResourceUsageProcess {
  pid: number;
  /** 进程显示名，如 hawknext-main / hawknext-agent-hawknext-demo / node_repl */
  name: string;
...
export interface AppSettings {
   ...
   * 只影响内置浏览器出口，不影响 HawkNext 自身对后端与模型 API 的请求。默认关闭，重启后生效。
   */
  ...
  /** HawkNext 运行中继续输入时，是排队到下一轮，还是引导到下一次工具调用后运行 */
  ...
  /** 用户通过 WelcomeScreen 成功连接后确认的 ZAI / CloudCN provider family 运行域。 */
  ...
  /** 数据目录的根路径（替代 homedir），默认为 os.homedir()；.hawknext/v2 后缀不变 */
  dataBaseDir?: string;
  ...
  /** 设置页里的临时 endpoint override；正式/测试默认 base url 由 HAWKNEXT_BASE_URL env 管理。 */
