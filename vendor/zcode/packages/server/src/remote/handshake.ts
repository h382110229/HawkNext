import type { HelloMessage, HelloAckMessage } from "@hawknext/shared";
import { HAWKNEXT_VERSION, formatZodError, helloMessageSchema } from "@hawknext/shared";
 ...
 * 1. Read lines from stdout until we find a hawknext-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a hawknext-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no hawknext-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "hawknext-hello"
            ) {
              rejectWithDiagnostics(`Invalid hawknext-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {
