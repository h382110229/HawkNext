import { SocketProtocol, ChannelClient } from "@hawknext/rpc";
import type { IServiceAccessor } from "@hawknext/services";
import { RemoteServiceAccess } from "@hawknext/client";
import {
...
/**
 * Connect to a remote hawknext server via an IRemoteBackend.
 *
 * Steps:
...
async function connectRemoteUnchecked(
  ...
  stream.stderr.on("data", (chunk: Buffer) => {
    // 远端 hawknext-server 的服务日志走 stderr，直接写 host stderr 时可能被结构化日志中继吞掉。
...
function buildRemoteServerCommand(
  ...
  const envParts = [
    ...
    'HAWKNEXT_SERVER_RUNTIME_ROOT="$HOME/.hawknext/server"',
  ];
  ...
  if (appVersion) {
    ...
    envParts.push(`${HAWKNEXT_APP_VERSION_ENV}=${quotePosixShellArg(appVersion)}`);
  }
  if (remoteRuntimeNetwork?.authoritative) {
    envParts.push(`${HAWKNEXT_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY}='1'`);
    if (remoteRuntimeNetwork.httpProxy !== undefined) {
      envParts.push(
        `${HAWKNEXT_REMOTE_HTTP_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.httpProxy)}`,
      );
    }
    if (remoteRuntimeNetwork.noProxy !== undefined) {
      envParts.push(
        `${HAWKNEXT_REMOTE_NO_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.noProxy)}`,
      );
    }
  }
  return `${envParts.join(" ")} ~/.hawknext/server/node ~/.hawknext/server/hawknext-server.cjs`;
}
