...
import { REMOTE_BASE } from "@hawknext/server/remote/deployShared.js";
...
function buildRemoteDeployLockScript(lockDir: string, ownerToken: string): string {
  const acquiredMarker = `hawknext-deploy-lock-acquired:${ownerToken}`;
  const releaseMarker = `hawknext-deploy-lock-release:${ownerToken}`;
  return [
    "set -eu",
