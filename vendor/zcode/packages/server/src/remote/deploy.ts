...
import { deployHawkNextAgentRuntime } from "./hawknextAgentDeploy.js";
import {
  deployNodePtyPrebuilds,
  deployNodeRuntime,
  createRemoteComponentVersionResolver,
  logDeployRequired,
} from "@hawknext/server/remote/remoteAssetDeployDecision.js";
import {
  REMOTE_BASE,
  fileExists,
  formatOptionalValue,
...
import { quotePosixPathArg } from "@hawknext/server/remote/posixShell.js";
import { checkServerBundleRequiredMarkers } from "@hawknext/server/remote/serverBundleDeployCheck.js";
import { deployRuntimeTools } from "@hawknext/server/remote/runtimeToolDeploy.js";
import { REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS } from "@hawknext/server/remote/hawknextAgentOfficialPluginAssets.js";
import {
  ensureRemoteReleaseDirFromCdn,
  selectRemoteAssetManifestComponents,
  type RemoteAssetManifestRef,
} from "@hawknext/server/remote/remoteAssetCache.js";
import {
...
} from "@hawknext/server/remote/remoteAssetInstaller.js";
import {
...
import { detectRemoteAssetTools } from "@hawknext/server/remote/remoteAssetPreflight.js";
import { assertSupportedRemoteEnvironment } from "@hawknext/server/remote/remotePlatformSupport.js";
import { acquireRemoteDeployLock } from "@hawknext/server/remote/remoteDeployLock.js";
import type { RemoteAssetNetworkPort } from "@hawknext/server/remote/remoteAssetNetwork.js";
...
/**
 * Deploy the hawknext server to the remote machine.
...
export async function deployServer(
  ...
  const deployWithDecision = async (
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/hawknext-server.cjs 可启动，不代表随包工具仍存在。
      ...
      await deployHawkNextAgentRuntime(
        backend,
        env,
        {
          ...assetDeployOptions,
    ...
    await installer.installFile({
      componentId: SERVER_BUNDLE_COMPONENT_ID,
      sourceRelativePath: "server/hawknext-server.cjs",
      remotePath: `${REMOTE_BASE}/hawknext-server.cjs`,
    ...
    // 部署 HawkNext Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployHawkNextAgentRuntime(
      backend,
      env,
      {
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    const serverPath = `${REMOTE_BASE}/hawknext-server.cjs`;
    ...
    log("remote version:", JSON.stringify(version), "local:", HAWKNEXT_VERSION);
    if (version !== HAWKNEXT_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${HAWKNEXT_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", HAWKNEXT_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/hawknext-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "glm":
        requiredPaths.add(`glm/${platformArch}/hawknext.cjs`);
