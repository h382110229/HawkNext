...
import {
  ...
  HAWKNEXT_RPC_HOST_CAPABILITY_HEADER,
  HAWKNEXT_VERSION,
...
const log = (...args: unknown[]) =>
  console.log(formatLogPrefix("hawknext-server:http", process.pid), ...args);
...
function resolveServerId(options: HttpServerOptions): string {
  return (
    options.serverId?.trim() || readTrimmedEnv("HAWKNEXT_SERVER_ID") || hostname() || "hawknext-server"
  );
}
...
function resolveServerWorkspaces(options: HttpServerOptions): ServerRemoteWorkspaceInfo[] {
  ...
  const workspacePath = readTrimmedEnv("HAWKNEXT_SERVER_WORKSPACE") || process.cwd();
  return [
    {
      path: workspacePath,
...
function createServerInfo(options: HttpServerOptions): ServerRemoteInfo {
  return {
    ...
    ...(options.name?.trim() || readTrimmedEnv("HAWKNEXT_SERVER_NAME")
      ? { name: options.name?.trim() || readTrimmedEnv("HAWKNEXT_SERVER_NAME") }
      : {}),
    version: HAWKNEXT_VERSION,
    protocolVersion: SERVER_REMOTE_PROTOCOL_VERSION,
    authRequired: options.authRequired ?? Boolean(readTrimmedEnv("HAWKNEXT_SERVER_TOKEN")),
...
const hawknextLiteTokenCookieName = "hawknext_lite_token";
...
function hasValidLiteToken(c: Context, token: string): boolean {
  ...
  if (url.searchParams.get("token") === token) {
    c.header(
      "Set-Cookie",
      `${hawknextLiteTokenCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`,
    );
    return true;
  }
  return parseCookieHeader(c.req.header("cookie")).get(hawknextLiteTokenCookieName) === token;
}
...
export function createHttpServer(
  ...
  app.use("/ws/host", async (c, next) => {
    const capability = c.req.header(HAWKNEXT_RPC_HOST_CAPABILITY_HEADER);
