import { disposeServiceResourcesAndWait, getAppConfigDir } from "@hawknext/services/node";
import {
  HAWKNEXT_VERSION,
...
const log = (...args: unknown[]) =>
  console.error(formatLogPrefix("hawknext-server:stdio", process.pid), ...args);
...
if (process.argv.includes("--version")) {
  process.stdout.write(HAWKNEXT_VERSION + "\n");
  process.exit(0);
}
...
async function main() {
  ...
  // 远端主机没有 Desktop main，没人写 telemetry-state.json，services 发往 HawkNext endpoint
