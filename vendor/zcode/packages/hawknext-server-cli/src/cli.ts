...
import { HAWKNEXT_VERSION } from "@hawknext/shared";
import {
...
export function serviceRegistrationEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.HAWKNEXT_SERVER_SKIP_SERVICE_REGISTRATION !== "1";
}
...
async function runServe(
  ...
  if (shouldRegisterService(daemonRequested, supervisorProcess)) {
    ...
    if (existing && (existing.state === "ready" || existing.state === "starting")) {
      if ((existing.serviceRegistered && !legacyIdentityMigration) || startupMode === "fallback") {
        if (json) stdout(io, existing);
          ...
          stdout(
            io,
            `HawkNext Server ${existing.state} at ${existing.host ?? ""}:${existing.port ?? ""}`,
          );
        return 0;
      }
    ...
    if (!serviceStarted) {
      const child = fork(
        ...
        [...SERVER_SERVICE_ARGS, "--server-root", layout.serverRoot],
        {
          ...
          env: {
            ...process.env,
            HAWKNEXT_DATA_BASE_DIR: layout.dataBaseDir,
            HAWKNEXT_SERVER_ROOT: layout.serverRoot,
          },
        },
      );
      child.unref();
    ...
    const started = await waitForPersistedStatus(
      ...
      () => {
        if (childEarlyExit) {
          throw new Error(
            `HawkNext Server daemon exited before ready (code=${childEarlyExit.code ?? "null"} signal=${childEarlyExit.signal ?? "none"}); check ${layout.statusFile} for details`,
          );
        }
      },
      serviceStarted,
    ...
    else stdout(io, `HawkNext Server ${started.state} at ${started.host ?? ""}:${started.port ?? ""}`);
    process.stdin.pause();
      ...
      launch: (generation, release) => {
        ...
        return fork(corePath, [String(generation)], {
          ...
          env: {
            ...inheritedEnv,
            ...releaseWiring,
            ...(runtimeRoot ? { HAWKNEXT_SERVER_RUNTIME_ROOT: runtimeRoot } : {}),
          },
  ...
  else stdout(io, `HawkNext Server ${status.state} at ${status.host ?? ""}:${status.port ?? ""}`);
...
async function runControl(
  ...
  try {
    ...
    if (command === "status") {
      ...
      result = persisted.status ?? createStoppedServerStatus(HAWKNEXT_VERSION);
...
async function runUninstall(
  ...
  const first = await (io.confirm?.("Type DELETE to uninstall HawkNext Server: ") ??
    Promise.resolve(""));
...
async function finishUninstall(
  ...
  try {
    ...
    // HawkNext allowlist，并把保留项写入结果供用户审计。先落盘 marker，再释放 lock，
    // 让并发 Supervisor 在删除 run 目录的最后窗口也会 fail-closed。
    await writeFile(
      layout.uninstalledFile,
      `${JSON.stringify({ uninstalled: true, uninstalledAt: Date.now(), version: HAWKNEXT_VERSION, preservedPaths }, null, 2)}\n`,
...
async function delegateLegacyCli(argv: readonly string[], io: CliIO): Promise<number> {
  const candidate =
    process.env.HAWKNEXT_LEGACY_CLI_ENTRY?.trim() ||
    join(dirname(fileURLToPath(import.meta.url)), "hawknext.cjs");
  try {
    await access(candidate);
  } catch {
    stdout(io, argv.length ? `Unknown command: ${argv[0]}` : "HawkNext TUI");
