...
import { IHawkNextAgentService } from "@hawknext/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitHawkNextBuiltinProviderConfigFilePath =
    process.env[HAWKNEXT_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const hawknextBuiltinProviderConfigFilePath = explicitHawkNextBuiltinProviderConfigFilePath
    ? explicitHawkNextBuiltinProviderConfigFilePath
    : typeof __HAWKNEXT_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeHawkNextBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __HAWKNEXT_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!hawknextBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 HawkNext Built-in Provider Config，且未设置 ${HAWKNEXT_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IHawkNextAgentService));
