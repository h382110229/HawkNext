...
function getDefaultServerDataRoot(): string {
  const configured = process.env.HAWKNEXT_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".hawknext", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\hawknext-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".hawknext") {
