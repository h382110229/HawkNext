...
const SERVER_INSTALL_OWNERSHIP_PRODUCT = "hawknext-server";
...
async function readOwnership(layout: ServerLayout): Promise<ServerInstallOwnership> {
  ...
  if (!markerStat?.isFile()) {
    throw new Error(`HawkNext Server ownership marker is missing or invalid: ${layout.installFile}`);
  }
  try {
    ...
    throw new Error(`HawkNext Server ownership marker is invalid: ${layout.installFile}`, {
      cause: error,
    });
  }
}
...
export async function validateServerInstallOwnership(
  ...
  const [ownership, canonicalServerRoot] = await Promise.all([
    ...
    realpath(layout.serverRoot).catch((error: unknown) => {
      throw new Error(`HawkNext Server ownership root cannot be resolved: ${layout.serverRoot}`, {
        cause: error,
      });
    }),
  ]);
  if (ownership.canonicalServerRoot !== canonicalServerRoot) {
    throw new Error(
      `HawkNext Server ownership root mismatch: expected ${ownership.canonicalServerRoot}, received ${canonicalServerRoot}`,
    );
  }
  return ownership;
}
