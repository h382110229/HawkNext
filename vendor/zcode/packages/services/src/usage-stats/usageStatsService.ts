...
import { isCodingPlanModelProviderId } from "@hawknext/shared";
...
import {
  CloudCNUsageQuotaProvider,
  type UsageApiAuthorizationRequest,
  type UsageApiAuthorization,
} from "./providers/cloud-cnUsageQuotaProvider.js";
import type { OfficialMcpCredentialSource } from "./providers/hawknextMcpQuotaProvider.js";
...
interface UsageStatsServiceDependencies {
  ...
  /** App Usage 经 HawkNext Protocol 读取 agent 数据库真实统计。 */
  hawknextAgentService: Pick<IHawkNextAgentService, "getAppUsageStats">;
  /**
...
export function createUsageStatsService(
  ...
  const quotaProvider = new CloudCNUsageQuotaProvider({
  ...
  return {
    async getAppUsageSnapshot(request: AppUsageRequest): Promise<AppUsageSnapshot> {
      ...
      // 经 HawkNext Protocol usage/stats 取回。不再读本地 session JSON 估算。
      return dependencies.hawknextAgentService.getAppUsageStats({
        range: request.range,
    ...
    async getCodingPlanUsageSnapshot(
      ...
      if (!isCodingPlanProviderId(request.preferredProviderId)) {
        // Coding Plan 页面只允许预置的 /CloudCN Coding Plan 账号。
