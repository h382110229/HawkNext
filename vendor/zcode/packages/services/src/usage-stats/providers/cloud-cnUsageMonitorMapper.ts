...
import { ESTIMATED_TOKEN_CHAR_DIVISOR } from "@hawknext/shared";

// ============================================================================
// 把 CloudCN monitor 接口数据(model-usage / tool-usage)转成 HawkNext 内部统一
// 的 UsageStatsSnapshot 结构。
//
// monitor 接口直接返回估算 token 数,反推字符数以保持现有 UI 字段语义。
// ============================================================================

export interface CloudCNUsageModelSummaryPayload {
  modelName?: string;
  totalTokens?: number;
  sortOrder?: number;
}

export interface CloudCNUsageModelDataPayload {
  modelName?: string;
  totalTokens?: number;
  tokensUsage?: number[];
  sortOrder?: number;
...
export interface CloudCNUsageModelUsagePayload {
  x_time?: string[];
  granularity?: string;
  ...
  totalUsage?: {
    ...
    modelSummaryList?: CloudCNUsageModelSummaryPayload[];
  };
  modelDataList?: CloudCNUsageModelDataPayload[];
  modelSummaryList?: CloudCNUsageModelSummaryPayload[];
}

export interface CloudCNUsageToolUsagePayload {
  x_time?: string[];
  granularity?: string;
  ...
  totalUsage?: {
    ...
    toolSummaryList?: CloudCNUsageToolSummaryPayload[];
  };
  toolDataList?: CloudCNUsageToolDataPayload[];
  toolSummaryList?: CloudCNUsageToolSummaryPayload[];
}

export interface CloudCNCreditUsageMetricPayload {
  value?: number | string | null;
  trend?: number | string | null;
}

export interface CloudCNCreditUsageActivitySummaryPayload {
  totalTokens?: number;
...
export interface CloudCNCreditUsageActivitySeriesPayload {
  date?: string;
  totalTokens?: number;
  modelCallCount?: number;
  mcpCalls?: number;
...
export interface CloudCNCreditUsageActivityPayload {
  summary?: CloudCNCreditUsageActivitySummaryPayload | null;
  series?: CloudCNCreditUsageActivitySeriesPayload[];
}

export interface CloudCNCreditUsageModelDataPayload {
  modelCode?: string;
  modelName?: string;
  sortOrder?: number;
  totalTokens?: number;
  totalCredits?: number;
  totalCreditsUsage?: CloudCNUsageNumberSeries;
  cachedInputCreditsUsage?: CloudCNUsageNumberSeries;
  uncachedInputCreditsUsage?: CloudCNUsageNumberSeries;
  outputCreditsUsage?: CloudCNUsageNumberSeries;
  tokensUsage?: CloudCNUsageNumberSeries;
  totalTokensUsage?: CloudCNUsageNumberSeries;
  cachedInputTokensUsage?: CloudCNUsageNumberSeries;
  uncachedInputTokensUsage?: CloudCNUsageNumberSeries;
  outputTokensUsage?: CloudCNUsageNumberSeries;
}

export interface CloudCNCreditUsageMcpDataPayload {
  mcpCode?: string;
  mcpName?: string;
  toolCode?: string;
  toolName?: string;
  ...
  creditsUsage?: CloudCNUsageNumberSeries;
  mcpCallCount?: CloudCNUsageNumberSeries;
  usageCount?: CloudCNUsageNumberSeries;
}

type CloudCNUsageNumberSeries = Array<number | string | null | undefined>;
...
export interface CloudCNCreditUsageDetailPayload {
  summary?: {
    cacheHitRate?: CloudCNCreditUsageMetricPayload;
    totalCredits?: CloudCNCreditUsageMetricPayload;
    averageDailyCredits?: CloudCNCreditUsageMetricPayload;
  } | null;
  modelUsage?: {
    xTime?: string[];
    periodTypes?: string[];
    modelDataList?: CloudCNCreditUsageModelDataPayload[];
  } | null;
  mcpUsage?: {
    xTime?: string[];
    periodTypes?: string[];
    mcpDataList?: CloudCNCreditUsageMcpDataPayload[];
  } | null;
}

export interface CloudCNUsageModelPerformancePayload {
  x_time?: string[];
  xTime?: string[];
...
export interface CloudCNUsageToolDataPayload {
  toolCode?: string;
  toolName?: string;
  sortOrder?: number;
  usageCount?: number[];
...
export interface CloudCNUsageToolSummaryPayload {
  toolCode?: string;
  toolName?: string;
  totalUsageCount?: number;
  sortOrder?: number;
...
export interface CloudCNUsageModelUsageEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: CloudCNUsageModelUsagePayload | null;
}

export interface CloudCNUsageToolUsageEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: CloudCNUsageToolUsagePayload | null;
}

export interface CloudCNCreditUsageActivityEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: CloudCNCreditUsageActivityPayload | null;
}

export interface CloudCNCreditUsageDetailEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: CloudCNCreditUsageDetailPayload | null;
}

export interface CloudCNUsageModelPerformanceEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: CloudCNUsageModelPerformancePayload | null;
}

export function buildUsageStatsSnapshotFromMonitor(
  request: UsageStatsRequest,
  modelData: CloudCNUsageModelUsagePayload,
  toolData: CloudCNUsageToolUsagePayload,
): UsageStatsSnapshot {
  ...
  const models: UsageStatsModelUsage[] = summaryList
    .filter((entry): entry is CloudCNUsageModelSummaryPayload => Boolean(entry?.modelName))
    .map((entry) => {
...
export function buildCodingPlanUsageSnapshotFromMonitor(params: {
  ...
  xTime: string[];
  activityData?: CloudCNCreditUsageActivityPayload;
  modelDetailData?: CloudCNCreditUsageDetailPayload;
  toolDetailData?: CloudCNCreditUsageDetailPayload;
  healthData?: CloudCNUsageModelPerformancePayload;
  startDateKey: string;
  endDateKey: string;
  modelData?: CloudCNUsageModelUsagePayload;
  toolData?: CloudCNUsageToolUsagePayload;
...
function buildCodingPlanActivitySnapshot(
  payload: CloudCNCreditUsageActivityPayload | undefined,
...
function buildCodingPlanUsageDetailSnapshot(
  modelDetailData: CloudCNCreditUsageDetailPayload | undefined,
  toolDetailData: CloudCNCreditUsageDetailPayload | undefined,
...
function buildCodingPlanUsageDetailSummary(
  summary: CloudCNCreditUsageDetailPayload["summary"] | undefined,
...
function buildCodingPlanHealthSnapshot(
  payload: CloudCNUsageModelPerformancePayload | undefined,
...
function normalizeModelDataList(
  payload: CloudCNUsageModelUsagePayload,
  xTime: string[],
  granularity: CodingPlanUsageGranularity,
): CodingPlanModelData[] {
  return (payload.modelDataList ?? [])
    .filter((item): item is CloudCNUsageModelDataPayload => Boolean(item?.modelName?.trim()))
    .map((item, index) => ({
...
function normalizeCreditModelDataList(
  payload: NonNullable<CloudCNCreditUsageDetailPayload["modelUsage"]>,
  xTime: string[],
  granularity: CodingPlanUsageGranularity,
): CodingPlanModelData[] {
  return (payload.modelDataList ?? [])
    .filter((item): item is CloudCNCreditUsageModelDataPayload =>
...
function isCreditUsageBreakdownBucket(item: CloudCNCreditUsageModelDataPayload): boolean {
  ...
  // CloudCN usage-detail 的 MODEL 数据有时返回“缓存/未缓存/输出”
...
function normalizeModelSummaryList(
  ...
  return (payload.modelSummaryList ?? payload.totalUsage?.modelSummaryList ?? [])
    .filter((item): item is CloudCNUsageModelSummaryPayload => Boolean(item?.modelName?.trim()))
    .map((item, index) => ({
      modelName: item.modelName!.trim(),
      totalTokens: item.totalTokens ?? 0,
      sortOrder: item.sortOrder ?? index,
...
function normalizeToolDataList(
  payload: CloudCNUsageToolUsagePayload,
  xTime: string[],
  granularity: CodingPlanUsageGranularity,
): CodingPlanToolData[] {
  return (payload.toolDataList ?? [])
    .filter((item): item is CloudCNUsageToolDataPayload =>
...
function normalizeCreditToolDataList(
  payload: NonNullable<CloudCNCreditUsageDetailPayload["mcpUsage"]>,
  xTime: string[],
  granularity: CodingPlanUsageGranularity,
): CodingPlanToolData[] {
  return (payload.mcpDataList ?? [])
    .filter((item): item is CloudCNCreditUsageMcpDataPayload =>
      Boolean(
        item?.mcpCode?.trim() ||
        item?.mcpName?.trim() ||
        item?.toolCode?.trim() ||
...
function normalizeToolSummaryList(payload: CloudCNUsageToolUsagePayload): CodingPlanToolSummary[] {
  return (payload.toolSummaryList ?? payload.totalUsage?.toolSummaryList ?? [])
    .filter((item): item is CloudCNUsageToolSummaryPayload =>
      Boolean(item?.toolCode?.trim() || item?.toolName?.trim()),
    )
    .map((item, index) => ({
      toolCode: item.toolCode?.trim() || item.toolName?.trim() || "unknown",
...
function sumNumbers(values: CloudCNUsageNumberSeries | undefined): number {
  return (values ?? []).reduce<number>((sum, value) => sum + (toFiniteNumber(value) ?? 0), 0);
}

function sumSeries(seriesList: Array<CloudCNUsageNumberSeries | undefined>): number[] | undefined {
...
function pickFirstNonEmptySeries(
  seriesList: Array<CloudCNUsageNumberSeries | undefined>,
): number[] | undefined {
...
function alignUsageSeries(
  sourceXTime: string[],
  targetXTime: string[],
  values: CloudCNUsageNumberSeries | undefined,
  granularity: CodingPlanUsageGranularity,
): number[] {
...
function buildToolUsages(toolData: CloudCNUsageToolUsagePayload): UsageStatsToolUsage[] {
