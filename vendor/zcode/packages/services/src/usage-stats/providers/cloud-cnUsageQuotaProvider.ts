...
import type { ProviderFamilyDomain } from "@hawknext/shared";
...
import type {
  CloudCNUsageModelUsagePayload,
  CloudCNUsageModelUsageEnvelope,
  CloudCNUsageToolUsagePayload,
  CloudCNUsageToolUsageEnvelope,
  CloudCNCreditUsageActivityPayload,
  CloudCNCreditUsageActivityEnvelope,
  CloudCNCreditUsageDetailPayload,
  CloudCNCreditUsageDetailEnvelope,
  CloudCNUsageModelPerformancePayload,
  CloudCNUsageModelPerformanceEnvelope,
} from "./cloud-cnUsageMonitorMapper.js";
import {
...
} from "./cloud-cnUsageMonitorRange.js";
import { fetchCloudCNSubscriptionSummary } from "./cloud-cnSubscriptionProvider.js";
import {
  fetchMcpQuotaSnapshot,
  type OfficialMcpCredentialSource,
} from "./hawknextMcpQuotaProvider.js";
import type { CloudCNUsageQuotaEnvelope } from "./cloud-cnUsageQuotaMapper.js";
import { normalizeLimits, pickPrimaryLimit } from "./cloud-cnUsageQuotaMapper.js";

const CLOUD_CN_QUOTA_PATH = "/api/monitor/usage/quota/limit";
const CODING_PLAN_RESET_BASE_PATH = "/api/v1/coding-plan/reset";
const HAWKNEXT_JWT_TOKEN_KEY = "hawknext-jwt";
const ZAI_OAUTH_ACCESS_TOKEN_KEY = "oauth:cloud-intl:access_token";
const CLOUD_CN_OAUTH_ACCESS_TOKEN_KEY = "oauth:cloud-cn:access_token";
const REQUEST_TIMEOUT_MS = 15_000;
const log = createServiceLogger("usage-stats");
const EMPTY_MODEL_USAGE_PAYLOAD = {} satisfies CloudCNUsageModelUsagePayload;
const EMPTY_TOOL_USAGE_PAYLOAD = {} satisfies CloudCNUsageToolUsagePayload;
const EMPTY_CREDIT_ACTIVITY_PAYLOAD = {} satisfies CloudCNCreditUsageActivityPayload;
const EMPTY_CREDIT_DETAIL_PAYLOAD = {} satisfies CloudCNCreditUsageDetailPayload;
const EMPTY_MODEL_PERFORMANCE_PAYLOAD = {} satisfies CloudCNUsageModelPerformancePayload;
...
interface CloudCNUsageQuotaProviderOptions {
  apiClient: ApiClient;
...
interface TeamPlanContext {
  ...
  projectId: string;
  // Team Plan 用量查询在 zai/cloud-cn 两个 family 上对称存在，
  // 但复制团队项目 API Key 的 host、OAuth token key、鉴权 header 都按 family 分离。
  // 这里必须带上 family，下游 resolveTeamPlanProjectApiKey 才能选对 zai/cloud-cn 的业务域名和 token。
  family: ProviderFamilyDomain;
}

interface CodingPlanResetAuthorization {
  hawknextAuthorization: string;
...
export class CloudCNUsageQuotaProvider {
  private readonly apiClient: ApiClient;
  private readonly accountRequestAuthService: Pick<
    IAccountRequestAuthService,
    "resolveAccessCurrent" | "resolveCurrent" | "assertCurrent"
  >;
  private readonly resolveApiAuthorization?: CloudCNUsageQuotaProviderOptions["resolveApiAuthorization"];
  ...
  constructor(options: CloudCNUsageQuotaProviderOptions) {
  ...
  async getSnapshotForRequest(
    ...
    const teamEntitlement = teamContext
      ? await fetchCloudCNSubscriptionSummary({
    ...
    const quotaData = payload && isSuccessfulCloudCNEnvelope(payload) ? payload.data : null;
  ...
  ): Promise<HawkNextAccountAccess | undefined> {
    ...
    return fetchMcpQuotaSnapshot({
      ...
      requestScope: {
        providerFamily:
          ...
          (isCloudCodingPlanProviderId(resolved.provider.id) ? "zai" : "cloud-cn"),
  ...
  private async getStartPlanSnapshot(
    providerId: string,
    accountAccess: HawkNextAccountAccess | undefined,
  ...
  async getUsageStatsSnapshot(request: UsageStatsRequest): Promise<UsageStatsSnapshot> {
    ...
    if (!resolved) {
      ...
      throw new Error("no_cloud-cn_api_key");
    }
    ...
    const modelData = readSuccessfulCloudCNMonitorData(
      modelPayload,
      "CloudCN model usage failed",
      EMPTY_MODEL_USAGE_PAYLOAD,
    );
    const toolData = readSuccessfulCloudCNMonitorData(
      toolPayload,
      "CloudCN tool usage failed",
  ...
  async getCodingPlanResetStatus(
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeHawkNextApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/status`),
      {
        method: "GET",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  async useCodingPlanReset(request: CodingPlanResetUseRequest): Promise<CodingPlanResetUseResult> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeHawkNextApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/use`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
        headers: {
  ...
  async requestCodingPlanResetOpportunity(
    ...
    try {
      payload = await readCodingPlanResetApiJson(
        this.apiClient,
        buildRuntimeHawkNextApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/opportunity`),
        {
          method: "POST",
          timeoutMs: REQUEST_TIMEOUT_MS,
          headers: {
  ...
  async markCodingPlanResetHistoryRead(request: CodingPlanResetScopeRequest): Promise<void> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeHawkNextApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/history/read`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  private async resolveCodingPlanResetAuthorization(
    ...
    const hawknextJwt = (await this.credentialService?.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim() ?? "";
    if (!hawknextJwt) {
      throw new Error("coding_plan_reset_hawknext_jwt_required");
    }
    // reset 同时支持  与 CloudCN Coding Plan。固定读取 oauth:cloud-cn:access_token
    // 会让只登录  的用户在请求发出前失败；业务 JWT 必须跟随当前 provider family
    // 精确选择，禁止跨 family 回退。Header 仍按后端契约直传且不套 Bearer。
    const codingPlanJwtKey =
      accountAccess.family === "zai" ? ZAI_OAUTH_ACCESS_TOKEN_KEY : CLOUD_CN_OAUTH_ACCESS_TOKEN_KEY;
    ...
    return {
      hawknextAuthorization: /^Bearer\s/i.test(hawknextJwt) ? hawknextJwt : `Bearer ${hawknextJwt}`,
  ...
  private async resolveAuthorization(
    ...
    if (request.allowEnvApiKey !== false && request.requirePreferredProvider !== true) {
      const envApiKey =
        readEnv(this.env, "HAWKNEXT_CLOUD_CN_USAGE_API_KEY") ??
        readEnv(this.env, "CLOUD_CN_USAGE_API_KEY");
      if (envApiKey) {
        return {
          authorization: envApiKey,
          quotaUrl: resolveQuotaUrlFromEnv(this.env) ?? buildCloudCNQuotaUrl(this.env),
          teamContext: null,
          provider: {
            id: "env:cloud-cn-usage",
            name: "CloudCN",
          },
        };
      }
  ...
  private async fetchQuota(
    resolved: ResolvedQuotaAuthorization,
  ): Promise<CloudCNUsageQuotaEnvelope> {
    return readApiJson<CloudCNUsageQuotaEnvelope>(this.apiClient, buildQuotaLimitUrl(resolved), {
      method: "GET",
      timeoutMs: REQUEST_TIMEOUT_MS,
      headers: createCloudCNUsageHeaders(resolved),
    });
  }
  ...
  private async fetchModelUsage(
    ...
    startTime: string,
    endTime: string,
  ): Promise<CloudCNUsageModelUsageEnvelope> {
    return readApiJson<CloudCNUsageModelUsageEnvelope>(
      this.apiClient,
  ...
  private async fetchToolUsage(
    ...
    startTime: string,
    endTime: string,
  ): Promise<CloudCNUsageToolUsageEnvelope> {
    return readApiJson<CloudCNUsageToolUsageEnvelope>(
      this.apiClient,
  ...
  ): Promise<CloudCNCreditUsageActivityEnvelope> {
    const range = resolveCreditUsageActivityTimeRange(timeZone);
    return readApiJson<CloudCNCreditUsageActivityEnvelope>(
      this.apiClient,
  ...
  private async fetchCreditUsageDetail(
    ...
    endTime: string,
    usageType: "MODEL" | "MCP",
  ): Promise<CloudCNCreditUsageDetailEnvelope> {
    return readApiJson<CloudCNCreditUsageDetailEnvelope>(
      this.apiClient,
  ...
  private async fetchModelPerformanceDay(
    ...
    timeZone: string | undefined,
    range: "7d" | "30d",
  ): Promise<CloudCNUsageModelPerformanceEnvelope> {
    const timeRange = resolveModelPerformanceTimeRange(timeZone, range);
    return readApiJson<CloudCNUsageModelPerformanceEnvelope>(
      this.apiClient,
...
function createCodingPlanResetHeaders(
  ...
  const headers: Record<string, string> = {
    Authorization: authorization.hawknextAuthorization,
    "X-CloudCn-Authorization": authorization.codingPlanAuthorization,
  };
  if (!includeTargetScope) {
    return headers;
  }
  headers["CloudCn-Target-Type"] = authorization.teamContext ? "TEAM" : "PERSONAL";
  if (authorization.teamContext) {
    headers["CloudCn-Organization"] = authorization.teamContext.organizationId;
    headers["CloudCn-Project"] = authorization.teamContext.projectId;
  }
  return headers;
}
...
function resolveTeamPlanContext(accountAccess: HawkNextAccountAccess): TeamPlanContext | null {
...
function createCloudCNUsageHeaders(resolved: ResolvedQuotaAuthorization): Record<string, string> {
  const headers: Record<string, string> = {
  ...
  if (resolved.teamContext) {
    ...
    headers["cloud-cn-organization"] = resolved.teamContext.organizationId;
    headers["cloud-cn-project"] = resolved.teamContext.projectId;
  }
  return headers;
}
...
function buildQuotaLimitUrl(resolved: ResolvedQuotaAuthorization): string {
  ...
  // CloudCN Team Plan 的 quota/limit 后端按 type=2 路由到团队套餐。
...
function readSuccessfulCloudCNMonitorData<TData extends object>(
  payload: {
    code?: number;
    message?: string;
    msg?: string;
  ...
  if (!isSuccessfulCloudCNEnvelope(payload)) {
    throw new Error(readCloudCNEnvelopeMessage(payload) || fallbackMessage);
  }
  if (payload.data) {
    return payload.data;
  }
  if (options.allowMissingData === true && hasExplicitCloudCNSuccessSignal(payload)) {
    // CloudCN monitor 的 tool-usage 在团队项目无工具调用时会返回
...
// （如 token expired）仍由 readSuccessfulCloudCNMonitorData 抛错：鉴权/后端错误必须
// 显式暴露给用户，不能伪装成空用量（既有测试已锁定该语义）。
async function readBestEffortCloudCNMonitorData<TData extends object>(
  request: Promise<{
    code?: number;
    message?: string;
    msg?: string;
  ...
  return readSuccessfulCloudCNMonitorData(payload, fallbackMessage, emptyData, {
    allowMissingData: true,
  });
}

function readCloudCNEnvelopeMessage(payload: { message?: string; msg?: string }): string {
  return payload.msg?.trim() || payload.message?.trim() || "";
}

function hasExplicitCloudCNSuccessSignal(payload: { code?: number; success?: boolean }): boolean {
  return payload.success === true || payload.code === 0 || payload.code === 200;
}

function isSuccessfulCloudCNEnvelope(payload: { code?: number; success?: boolean }): boolean {
  const code = payload.code;
  // CloudCN monitor/quota 后端存在 code=0 且 msg=“操作成功”的成功响应。
...
function buildUsageMonitorUrl(
  ...
  if (resolved.teamContext) {
    // CloudCN Team Plan 的 monitor 用量接口和 quota/limit 一样按 type=2 路由。
...
function resolveQuotaUrlFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return readEnv(env, "HAWKNEXT_CLOUD_CN_USAGE_QUOTA_URL") ?? readEnv(env, "CLOUD_CN_USAGE_QUOTA_URL");
}

function resolveCodingPlanApiKeyError(providerId: string | undefined): string {
  if (providerId && isCloudCodingPlanProviderId(providerId)) {
    return "zai_coding_plan_api_key_required";
  }
  return "cloud-cn_coding_plan_api_key_required";
}

function resolveAccountProviderQuotaUrl(providerId: string, env: NodeJS.ProcessEnv): string {
  return isCloudCodingPlanProviderId(providerId) ? buildZaiQuotaUrl(env) : buildCloudCNQuotaUrl(env);
}
...
function buildCloudCNQuotaUrl(env: NodeJS.ProcessEnv = process.env): string {
  return buildCloudCNApiUrl(env, CLOUD_CN_QUOTA_PATH);
}

function buildZaiQuotaUrl(env: NodeJS.ProcessEnv = process.env): string {
  // ZAI usage/quota 与 business login 共用业务域名。
  // 测试环境必须请求 配置的 ZAI Business origin，不能把测试 token 发送到生产 。
