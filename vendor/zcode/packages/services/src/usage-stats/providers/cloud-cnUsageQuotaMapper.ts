import { isActivePersonalCodingPlan } from "#src/cloud-cn/codingPlanEntitlement.js";
import type { UsageQuotaLimit, UsageQuotaUsageDetail } from "@hawknext/shared";

interface CloudCNSubscriptionListItem {
  productId?: string;
  productName?: string;
  status?: string;
...
export interface CloudCNUsageQuotaEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: CloudCNUsageQuotaPayload | null;
}

export interface CloudCNUsageQuotaPayload {
  level?: string;
  limits?: CloudCNUsageQuotaLimitPayload[];
}

interface CloudCNUsageQuotaLimitPayload {
  type?: string;
  unit?: number;
  number?: number;
  usage?: number;
...
export function normalizeLimits(limits: CloudCNUsageQuotaPayload["limits"]): UsageQuotaLimit[] {
...
export function pickCurrentSubscriptionFromList(
  payload: CloudCNSubscriptionListItem[] | null | undefined,
): {
  productId: string;
  productName: string;
...
function pickCurrentSubscriptionRenewTime(
  subscription: CloudCNSubscriptionListItem,
): string | null {
  const nextRenewTime = parseCloudCNLocalDateTime(subscription.nextRenewTime);
  if (!nextRenewTime) {
    return null;
  }
...
function parseValidPeriodEnd(value: string | undefined): string | null {
  ...
  return parseCloudCNLocalDateTime(end);
}

function parseCloudCNLocalDateTime(value: string | null | undefined): string | null {
...
function normalizeUsageDetails(
  details: CloudCNUsageQuotaLimitPayload["usageDetails"],
