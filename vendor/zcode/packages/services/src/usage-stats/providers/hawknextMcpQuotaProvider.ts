/* HawkNext 官方 Server MCP 的调用额度读取（`GET /api/v1/mcp/usage`）。
 *
 ...
 * 同时持有  与 CloudCN Coding Plan（或个人 + Team）的用户，如果不做这层比对，
...
export async function fetchMcpQuotaSnapshot(params: {
  ...
  const url = buildRuntimeHawkNextApiUrl(params.env, MCP_USAGE_PATH);
