...
import type { ApiClient, FeedbackDeviceInfo } from "@hawknext/shared";
import {
...
function resolveApiBaseUrl(explicit?: string): string {
  return (
    explicit?.trim() ||
    process.env.HAWKNEXT_FEEDBACK_API_BASE?.trim() ||
    buildRuntimeHawkNextApiUrl(process.env, "/api/v1")
  );
}
...
function buildDeviceSnapshot(): FeedbackDeviceInfo {
  return {
    ...
    buildCommitId: HAWKNEXT_COMMIT,
    buildTime: HAWKNEXT_BUILD_TIME,
...
export function createFeedbackService(options: CreateFeedbackServiceOptions): IFeedbackService {
  ...
  async function getHawkNextJwtToken(): Promise<string | undefined> {
    return (await options.credentialService.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim() || undefined;
  }

  async function hasHawkNextJwtToken(): Promise<boolean> {
    return Boolean(await getHawkNextJwtToken());
  }
  ...
  const httpClient = new FeedbackHttpClient({
    ...
    getAuthHeaders: async () => {
      ...
      const jwtToken = await getHawkNextJwtToken();
      if (jwtToken) {
