...
import { redactFeedbackText, HAWKNEXT_VERSION, HAWKNEXT_COMMIT } from "@hawknext/shared";
...
export async function createFeedbackDiagnosticArchive(options: {
  ...
  const path = join(outputDir, "hawknext-diagnostic-logs.zip");
  ...
  try {
    ...
    zip.addBuffer(
      Buffer.from(
        [
          "HawkNext diagnostic logs",
          `timestamp: ${now.toISOString()}`,
          `appVersion: ${HAWKNEXT_VERSION}`,
          `commit: ${HAWKNEXT_COMMIT}`,
