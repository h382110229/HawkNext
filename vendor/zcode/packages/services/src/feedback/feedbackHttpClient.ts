import { redactFeedbackText } from "@hawknext/shared";
...
function toFeedbackEnvironment(input: CreateFeedbackTicketInput): Record<string, unknown> {
  ...
  assignDefined(environment, "release_channel", process.env.HAWKNEXT_ENV?.trim() || "stable");
...
async function uploadOssForm(
  ...
  return new Promise((resolve, reject) => {
    ...
    const boundary = `----hawknext-feedback-${randomUUID()}`;
