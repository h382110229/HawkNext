/* eslint-disable max-lines -- hawknextSessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { IHawkNextAgentService } from "#src/hawknext-agent/hawknextAgent.js";
import type { HawkNextTaskIndexSyncer } from "#src/hawknext-agent/hawknextTaskIndexSyncer.js";
...
import type {
  ...
  HawkNextSessionEventsParams,
  HawkNextSessionListParams,
  HawkNextSessionMessagesParams,
  HawkNextSessionReadParams,
  HawkNextSessionReadWorkspacePresentationParams,
  HawkNextSessionResumeParams,
  HawkNextSessionSetModeParams,
  HawkNextSessionSetModelParams,
  HawkNextSessionSetThoughtLevelParams,
  HawkNextTaskTarget,
  HawkNextSessionWorkspaceTarget,
} from "#src/hawknext-session/hawknextSession.js";
import { formatModelPickerValue } from "#src/hawknext-agent/hawknextConfigOptions.js";
import { createHawkNextSessionApiRetryRuntimeTracker } from "#src/hawknext-session/hawknextSessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/hawknext-session/importedClaudeSessionRepair.js";
import { createHawkNextDeferredDraftRegistry } from "#src/hawknext-session/hawknextSessionDraftRegistry.js";
...
interface CreateHawkNextSessionServiceOptions {
...
export function createHawkNextSessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateHawkNextSessionServiceOptions): IHawkNextSessionService {
  const { withApiRetryRuntime } = createHawkNextSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createHawkNextDeferredDraftRegistry();

  function notifySyncer(
    target: HawkNextTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: HawkNextSessionStateSnapshot) {
  ...
  // hawknextSessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[hawknext-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: HawkNextSessionStateSnapshot,
    params: HawkNextSessionResumeParams | HawkNextSessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends HawkNextSessionCreateParams | HawkNextSessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: HawkNextSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: HawkNextSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: HawkNextSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: HawkNextSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[hawknext-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[hawknext-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 HawkNext task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[hawknext-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: HawkNextSessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[hawknext-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[hawknext-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[hawknext-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: HawkNextSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: HawkNextSessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[hawknext-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: HawkNextSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: HawkNextSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: HawkNextTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[hawknext-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: HawkNextTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: HawkNextTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[hawknext-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: HawkNextSessionSetModelParams) {
    ...
    async setThoughtLevel(params: HawkNextSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: HawkNextSessionSetModeParams) {
