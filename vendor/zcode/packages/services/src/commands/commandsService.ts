...
import {
  ...
  HAWKNEXT_COMMAND_AGENT_SOURCES,
...
const DEFAULT_COMMAND_AGENT_SOURCE: CommandAgentSource = HAWKNEXT_COMMAND_AGENT_SOURCE;
const COMMAND_AGENT_SOURCE_ORDER: readonly CommandAgentSource[] = HAWKNEXT_COMMAND_AGENT_SOURCES;
...
const HAWKNEXT_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  agentSource: "hawknextAgent",
  directorySource: "hawknext",
  userDirectorySegments: [".hawknext", "commands"],
  workspaceDirectorySegments: [".hawknext", "commands"],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // HawkNext Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让 user .hawknext 抢在 workspace .agents 前面。
    for (const agentSource of agentSources) {
      const descriptors =
        agentSource === HAWKNEXT_COMMAND_AGENT_SOURCE
    ...
    const pluginCommands =
      !params.agentSource || params.agentSource === HAWKNEXT_COMMAND_AGENT_SOURCE
        ? await discoverPluginCommands(enabledOverrides)
        : [];

    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as HawkNextCommand[],
...
async function discoverPluginCommandsRecursive(
  ...
  for (const entry of entries) {
    ...
    if (!entry.toLowerCase().endsWith(HAWKNEXT_COMMAND_DESCRIPTOR.fileExtension)) {
      continue;
    }
    ...
    try {
      ...
      const parsed = CommandFileParser.parseCommandFile(
        content,
        fullPath,
        HAWKNEXT_COMMAND_DESCRIPTOR.format,
      );
      if (!parsed) {
        continue;
      }
      const name = getCommandName(rootDir, fullPath, HAWKNEXT_COMMAND_DESCRIPTOR);
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // `.hawknext` 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
    if (descriptor.directorySource === "hawknext" && discoveredCount > 0) {
      break;
    }
  }
}
