...
// to @hawknext/hawknext-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @hawknext/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @hawknext/services root
...
// at compile time; Vite never resolves @hawknext/hawknext-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@hawknext/hawknext-cua/broker";
...
} from "@hawknext/hawknext-cua/broker/ports";
