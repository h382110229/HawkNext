...
const DEV_ROOT_ENV = "HAWKNEXT_CUA_DEV_ROOT";
const EXPECTED_PACKAGE_NAME = "@hawknext/hawknext-cua";
...
async function requireExpectedPackage(
  ...
  try {
    ...
    const contract = pkg.hawknextCuaRuntime;
    if (
  ...
  throw new WindowsCuaDevRuntimeResolutionError(
    "invalid-package",
    `Windows CUA development root package.json must name ${EXPECTED_PACKAGE_NAME} and expose a valid hawknextCuaRuntime contract.`,
    PACKAGE_JSON,
  );
}
