...
interface GitCommitMessageCurrentModelProvider {
  ...
  }): Promise<HawkNextWorkspaceGenerateTextParams["selection"] | null>;
}
...
interface GitCommitMessageTextGenerator {
  generateText(params: {
    ...
    prompt: string;
    querySource: string;
  }): Promise<{ text: string; selection: HawkNextWorkspaceGenerateTextParams["selection"] }>;
}
...
export class GitCommitMessageGenerator {
  ...
  private async resolveCurrentModel(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<HawkNextWorkspaceGenerateTextParams["selection"]> {
