...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@hawknext/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/hawknext/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "HawkNext Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@hawknext.local",
    GIT_COMMITTER_NAME: "HawkNext Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@hawknext.local",
  };
}
