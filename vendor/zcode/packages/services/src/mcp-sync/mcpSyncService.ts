...
const HAWKNEXT_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  source: "hawknext",
  directorySource: "hawknext",
  userConfigDirSegments: [".hawknext", "cli"],
  workspaceConfigDirSegments: [".hawknext"],
...
export function createMcpSyncService(
  ...
  return {
    ...
    async checkRemoteUserMcpWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(dirname(getUserHawkNextMcpConfigPath()));
    },
...
function getUserHawkNextMcpConfigPath(): string {
  return buildUserConfigPath(HAWKNEXT_MCP_DESCRIPTOR);
}
...
async function collectEffectiveUserMcpRecords(): Promise<UserMcpRecord[]> {
  const hawknextRecords = await readUserMcpRecordsFromFile(HAWKNEXT_MCP_DESCRIPTOR);
  if (hawknextRecords.length > 0) {
    return sortMcpRecords(hawknextRecords);
  }
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  if (payload.action === "set-enabled") {
    ...
    const location =
      payload.location ??
      buildDirectoryMcpLocation(HAWKNEXT_MCP_DESCRIPTOR, scope, payload.projectPath);
  ...
  await writeHawkNextServersToFile(scope, nextServers, payload.projectPath);
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  const hawknextServers = await readDirectoryServersFromFile(
    HAWKNEXT_MCP_DESCRIPTOR,
    scope,
    workspacePath,
  );
  if (hawknextServers.length > 0) {
    return hawknextServers;
  }
...
async function writeHawkNextServersToFile(
  scope: Exclude<McpScope, "common">,
  servers: Record<string, Record<string, unknown>>,
  workspacePath?: string,
): Promise<void> {
  const filePath = buildDirectoryConfigPath(HAWKNEXT_MCP_DESCRIPTOR, scope, workspacePath);
  const current = (await readJsonObject(filePath)) ?? {};
  const next = writeServerMapToJson(current, HAWKNEXT_MCP_DESCRIPTOR.configKeyName, servers);
  await writeTextAtomic(filePath, `${JSON.stringify(next, null, 2)}\n`);
}

async function readUserCliConfig(): Promise<Record<string, unknown>> {
  return (await readJsonObject(getUserHawkNextMcpConfigPath())) ?? {};
}

async function writeUserCliConfig(config: Record<string, unknown>): Promise<void> {
  await writeTextAtomic(getUserHawkNextMcpConfigPath(), `${JSON.stringify(config, null, 2)}\n`);
}
...
async function importMcpServers(params: {
  ...
  const targetPath = getUserHawkNextMcpConfigPath();
  const current = (await readJsonObject(targetPath)) ?? {};
  const targetServers = readServerMapFromJson(current, HAWKNEXT_MCP_DESCRIPTOR.configKeyName);
  const existingByName = await collectEffectiveUserMcpRecordByName();
  ...
  if (changed) {
    await writeTextAtomic(
      targetPath,
      `${JSON.stringify(writeServerMapToJson(current, HAWKNEXT_MCP_DESCRIPTOR.configKeyName, targetServers), null, 2)}\n`,
    );
  }
