...
export function createSkillSyncService(options?: { maxArchiveBytes?: number }): ISkillSyncService {
  ...
  return {
    ...
    async listRemoteUserSkillStatuses(params): Promise<SkillSyncRemoteStatusResult> {
      const root = getUserHawkNextSkillRoot();
    ...
    async checkRemoteUserSkillWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(getUserHawkNextSkillRoot());
    },
...
async function collectUserSkillCandidates(): Promise<SkillSyncCandidate[]> {
  ...
  const userHawkNextSkillRoot = getUserHawkNextSkillRoot();
  const rawHawkNextCandidates = await collectUserSkillCandidatesInRoot(userHawkNextSkillRoot);
  const hawknextDirectoryNames = await collectCoveredSkillDirectoryNamesInRoot(userHawkNextSkillRoot);
  const hawknextSkillNameKeys = new Set(
    rawHawkNextCandidates.map((candidate) => normalizeSkillNameKey(candidate.name)),
  );
  const hawknextCandidates = await dedupeCandidatesByCanonicalSkillPath(
    rawHawkNextCandidates,
    seenSkillRealpaths,
  );
  const agentsCandidates = await dedupeCandidatesByCanonicalSkillPath(
    (await collectUserSkillCandidatesInRoot(getUserAgentsSkillRoot())).filter(
      (candidate) =>
        !hawknextDirectoryNames.has(candidate.directoryName) &&
        !hawknextSkillNameKeys.has(normalizeSkillNameKey(candidate.name)),
    ),
    seenSkillRealpaths,
  );

  return [...hawknextCandidates, ...agentsCandidates].sort((left, right) =>
...
async function collectUserSkillDirectoryPathByName(): Promise<Map<string, string>> {
  const result = await collectSkillDirectoryPathByName(getUserHawkNextSkillRoot());
...
async function importArchive(
  ...
  const targetRoot = getUserHawkNextSkillRoot();
  ...
  try {
    ...
    // 远端 SkillsService 会同时读取用户级 .hawknext/skills 和 .agents/skills。
    // 同名 skill 已在兼容目录存在时也必须跳过，避免同步后在 .hawknext 下生成重复来源。
    const existingSkillPathByName = await collectUserSkillDirectoryPathByName();
