...
export {
  ...
  HAWKNEXT_E2E_FS_FAULTS_ALLOW_ENV,
  HAWKNEXT_E2E_FS_FAULTS_ENV,
...
export {
  ...
  HAWKNEXT_WINDOWS_APP_INSTALL_DIR_ENV,
} from "./paths.js";
...
export { createHawkNextAgentService } from "./hawknext-agent/hawknextAgentService.js";
export { createHawkNextTaskServiceAdapter } from "./hawknext-agent/hawknextTaskServiceAdapter.js";
export { createHawkNextSessionService } from "./hawknext-session/hawknextSessionService.js";
export {
  resolveDefaultHawkNextAgentCommand,
  HawkNextAgentProcessManager,
} from "./hawknext-agent/hawknextAgentProcessManager.js";
export type {
  HawkNextAgentCommand,
  HawkNextAgentCommandResolver,
  HawkNextAgentCommandResolverContext,
  HawkNextAgentProcessManagerOptions,
} from "./hawknext-agent/hawknextAgentProcessManager.js";
export { HawkNextProtocolClient } from "./hawknext-agent/hawknextProtocolClient.js";
export type { HawkNextProtocolTransport } from "./hawknext-agent/hawknextProtocolTransport.js";
export { HawkNextStdioTransport } from "./hawknext-agent/hawknextStdioTransport.js";
export {
  getHawkNextStdioTapDevLogDir,
  readHawkNextStdioTapDevState,
  setHawkNextStdioTapDevEnabled,
} from "./hawknext-agent/hawknextStdioTapDevConfig.js";
export type { HawkNextStdioTapDevState } from "@hawknext/shared";
export {
...
import { IHawkNextTaskService } from "./session/hawknextTaskService.js";
import { IHawkNextAgentService } from "./hawknext-agent/hawknextAgent.js";
import type { CuaOperationStateReporter } from "./hawknext-agent/cuaOperationTurnTracker.js";
import { IHawkNextSessionService } from "./hawknext-session/hawknextSession.js";
import {
...
import { createHawkNextAgentService } from "./hawknext-agent/hawknextAgentService.js";
import type { HawkNextAgentCommandResolver } from "./hawknext-agent/hawknextAgentProcessManager.js";
import { buildAgentTelemetrySpawnEnv } from "./hawknext-agent/agentTelemetryEnv.js";
import { resolveHawkNextAgentPresentationSurface } from "./hawknext-agent/hawknextAgentPresentationSurface.js";
import { createHawkNextTaskServiceAdapter } from "./hawknext-agent/hawknextTaskServiceAdapter.js";
import { createHawkNextSessionService } from "./hawknext-session/hawknextSessionService.js";
import { createHawkNextTaskIndexSyncer } from "./hawknext-agent/hawknextTaskIndexSyncer.js";
...
import { readLegacyHawkNextConfigProviders } from "./model-provider/legacyHawkNextConfigProviderReader.js";
...
import { fetchHawkNextBuiltinRemoteRelease } from "./model-provider/hawknextBuiltinRemoteConfig.js";
import {
  createProviderRuntimeFromConfigRuntime,
...
import { buildHelperOpenArgs, isCuaLocalDevelopmentRuntime } from "@hawknext/hawknext-cua/broker/server";
...
import {
  ...
  isPotentialHawkNextCuaAgentMcpServer,
...
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@hawknext/hawknext-cua/broker/helperConstants";
import { resolveBrokerSocketPath } from "@hawknext/hawknext-cua/broker/socketPath";
import {
  DEFAULT_HAWKNEXT_MODEL_CONTEXT_BUDGET_STRATEGY,
  resolveSafeEndpointHostname,
  HAWKNEXT_JWT_INVALID_BROADCAST_CHANNEL,
  formatLogPrefix,
  ...
  resolveRuntimeHawkNextEndpointOrigin,
  type BrowserBackendDescriptor,
  type BrowserClientMode,
  type BrowserCommand,
  isHawkNextCuaMcpCommand,
  isHawkNextCuaMcpPackageArg,
  isHawkNextCuaInternalFeatureEnabled,
  HAWKNEXT_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  type HawkNextAutomation,
  type HawkNextAutomationRun,
  getCapturedHawkNextAgentTelemetryEnv,
  HAWKNEXT_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV,
  CLOUD_INTL_PROVIDER_ID,
  hawknextAccountAccessSchema,
  hawknextProviderAccountAccessSchema,
  HAWKNEXT_VERSION,
  HAWKNEXT_ENV,
  buildRuntimeHawkNextApiUrl,
} from "@hawknext/shared";

// 这些 conversation-share 实现依赖 Node 文件系统；仅通过 @hawknext/services/node 暴露，
...
export function shouldEnableDefaultCuaProductHelper(
  ...
  // CUA 已随正式版默认开启（isHawkNextCuaInternalFeatureEnabled 默认 ON，仅显式 0/false/off 关闭；2026-08 注释更正——旧注释称默认关闭已过期）。显式开启后 macOS 使用既有产品 Helper，Windows 使用安装包内 runtime；
  // 两端都保持按需启动。关闭时不创建 host、不探测资源、不产生子进程或权限提示。
  const env = options.env ?? process.env;
  if (!isHawkNextCuaInternalFeatureEnabled(env)) return false;
  ...
  return {
    async resolveMcpServers(servers, context) {
      ...
      return options.isResolverCurrent?.(resolver, context) === false
        ? servers?.filter((server) => !isPotentialHawkNextCuaAgentMcpServer(server))
        : resolved;
    },
    async restart() {
      ...
      if (!resolver) {
        throw new Error("HawkNext Computer Use is not enabled (plugin off or not product mode).");
      }
...
export const HAWKNEXT_CUA_BUNDLED_HELPER_APP_PATH_ENV = "HAWKNEXT_CUA_BUNDLED_HELPER_APP_PATH";

export function resolveBundledCuaHelperAppPath(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const injectedPath = env[HAWKNEXT_CUA_BUNDLED_HELPER_APP_PATH_ENV]?.trim();
  if (injectedPath) {
    return injectedPath;
  }
  const resourcesPath = (
...
export function hasGlobalCliHawkNextCuaServer(env: NodeJS.ProcessEnv = process.env): boolean {
  const home = env.HOME?.trim() || homedir();
  const configPath = join(home, ".hawknext", "cli", "config.json");
  let parsed: unknown;
  try {
  ...
  return Object.entries(parsed.mcp.servers).some(([name, config]) =>
    isGlobalCliHawkNextCuaServer(name, config),
  );
}

function isGlobalCliHawkNextCuaServer(name: string, config: unknown): boolean {
  if (!isRecord(config)) return false;
  if (config.enabled === false) return false;
  if (typeof config.type === "string" && config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 与 desktop/services resolver 和 CLI bootstrap 共用 @hawknext/shared 的单一事实源，避免第三处
  // 判定漂移：git/.git/本地路径形态的 hawknext-cua 若这里漏判，全局 CLI env 注入不会带 broker
  // socket/token，agent 会回退成 Python/uvx 自己持有 macOS TCC（违反 product broker 边界）。
  if (typeof config.command === "string" && isHawkNextCuaMcpCommand(config.command)) {
    return true;
  }
  return (
    Array.isArray(config.args) &&
    config.args.some((arg) => typeof arg === "string" && isHawkNextCuaMcpPackageArg(arg))
  );
}
...
export async function buildCuaProductHelperAgentEnv(
  ...
  //
  // fail-closed 理由：CLI 全局配置里存在 hawknext-cua 时，返回空 env 会让 agent 按原始
  ...
  try {
    ...
    if (host.waitForTransport) {
      ...
      return {
        ...
        [HAWKNEXT_CUA_PLUGIN_AUTHORITY_ENV_KEY]: transport.pluginAuthority,
      };
    }
    ...
    if (reserved && !hasCuaProductHelperAgentEnvUnavailable(host)) {
      ...
      return {
        ...
        [HAWKNEXT_CUA_PLUGIN_AUTHORITY_ENV_KEY]: reserved.pluginAuthority,
      };
    }
    ...
    return {
      ...
      [HAWKNEXT_CUA_PLUGIN_AUTHORITY_ENV_KEY]: handle.pluginAuthority,
    };
  } catch (error) {
    ...
    logger.warn(
      undefined,
      `Computer Use Helper broker_unavailable; disabling workspace hawknext-cua MCP server for this agent spawn (${cuaHelperStartErrorDetail(error)})`,
    );
    return {
...
export function createLocalServices(options: {
  ...
  hawknextAgentSpawnFallbackCwd?: string;
  /** desktop-attached remote server 从 Desktop Host 收到的一次性 Agent 网络配置。 */
  remoteAgentNetwork?: {
    httpProxy?: string;
    noProxy?: string;
  };
  /** 所属 Environment 的 HawkNext Built-in Provider Config 物理路径。 */
  ...
  // 导致 bun 这类只在 shell profile 里追加的命令在 HawkNext Agent/终端里不可见。
  // 这里在所有本地服务启动前统一修正运行时环境，并顺带把内置 rg 注入 PATH，
  // 让 HawkNext Agent、终端、认证 runtime 共用同一套命令解析结果。
  initializeRuntimeProcessEnv(options?.runtimeProcessEnvPatch);

  const desktopContextPromptEnabledRaw =
    process.env[HAWKNEXT_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]?.trim();
  ...
  const resolveCurrentHawkNextEndpointOrigin = async () =>
    resolveRuntimeHawkNextEndpointOrigin(process.env, {
      overrideOrigin: (await settingService.get()).hawknextEndpointOrigin,
    });
  ...
  const hawknextJwtLogoutHandlerRef: {
  ...
  const apiClient = createNodeApiClient({
    ...
    onHawkNextJwtInvalid: (input, headers) => hawknextJwtLogoutHandlerRef.current?.(input, headers),
    isHawkNextJwtRequest: (input, headers) =>
      isCurrentOAuthCredentialRequest({ input, headers, credentialService }),
    resolveHawkNextEndpointOrigin: resolveCurrentHawkNextEndpointOrigin,
  });
  ...
  const accountProviderCredentialService = createAccountProviderCredentialService({
    ...
    resolveProviderApiKey: (family, accessToken) =>
      accountProviderApiKeyResolver.resolveProviderApiKey(
        family === "zai" ? CLOUD_INTL_PROVIDER_ID : CLOUD_CN_PROVIDER_ID,
        accessToken,
      ),
  });
  const resolveLegacyTeamOrganization = createLegacyTeamOrganizationResolver({
    ...
    loadOAuthTokenSet: (family) =>
      oauthCredentialRepo.loadTokenSet(family === "zai" ? CLOUD_INTL_PROVIDER_ID : CLOUD_CN_PROVIDER_ID),
  });
  ...
  const providerConfigRuntime = createProviderConfigRuntime({
    hawknextBuiltinFilePath: options.hawknextBuiltinProviderConfigFilePath,
    hawknextBuiltinEnvironment: {
      environmentConfigRoot: resolveAppConfigDir(),
      platform: clientConfigPlatform,
      appVersion: HAWKNEXT_VERSION,
      resolveEndpointOrigin: resolveCurrentHawkNextEndpointOrigin,
      onRefreshResult: (event) => {
        if (event.result === "updated")
          providerConfigLog.info(undefined, "HawkNext Built-in CDN 配置已更新", event);
        else providerConfigLog.debug(undefined, "HawkNext Built-in 刷新检查", event);
      },
      fetchRelease: (endpointOrigin, signal) =>
        fetchHawkNextBuiltinRemoteRelease({
          apiClient,
          endpointOrigin,
          signal,
          appVersion: HAWKNEXT_VERSION,
    ...
    onHawkNextBuiltinRefreshError: (error) => {
      providerConfigLog.warn(undefined, "HawkNext Built-in Config 远端刷新失败", { error });
    },
    ...
    // 已发布 config.json 保存的是 HawkNext 用户配置；清理第三方 ACP 不能移除这条升级路径。
    // Repository 仅在新 Personal 配置不存在时导入，并保留旧文件以便回滚。
    readLegacyProviders: () => readLegacyHawkNextConfigProviders(),
  });
  ...
  let providerConnectivityAgentService:
    | Pick<IHawkNextAgentService, "testModelConnectivity">
    | undefined;
  ...
  // mcpSync/hooks 里引用 hawknextAgentService 的闭包是惰性调用，声明顺序不影响初始化。
  ...
  const mcpSyncService = createMcpSyncService({
    ...
    listMcpServerStatuses: (params) => hawknextAgentService.listMcpServerStatuses(params),
  });
  ...
  const hooksService = createHooksService({
    grantWorkspaceHookTrust: (params) => hawknextAgentService.grantWorkspaceHookTrust(params),
  });
  ...
  // createDefaultCuaProductHelper() 在 hawknextAgentService 存在之前就要组装 resolver，
  // 但"是否有活跃 turn"这个信号只有 hawknextAgentService 建好之后才能查询。用前向引用占位——resolver 真正
  ...
  const isCuaEnabledForContext = (context?: CuaProductMcpServerResolverContext): boolean =>
    // 保留 main 原有 gate 行为（避免回归）：dev/internal 特性开启时（HAWKNEXT_CUA_DEV_MODE=1 或
    // HAWKNEXT_CUA_PRODUCT_HELPER=1）即视为启用，不依赖 config.json 显式 enable——main 的 bootstrap
    // 用 isHawkNextCuaInternalFeatureEnabled 门控 bundled plugin，与 feat 的 workspace enablement 不同。
    // 生产路径（dev mode off）回落到官方插件 workspace enablement 判定（与 feat 一致）。
    isHawkNextCuaInternalFeatureEnabled(process.env) ||
    isOfficialCuaPluginEnabledForWorkspace({
      env: process.env,
  ...
  const createManagedDefaultCuaProductHelper = (
    ...
    const helper = createDefaultCuaProductHelper({
      // 转发活跃-turn 查询（前向引用，hawknextAgentService 建好后赋值）。
  ...
  //
  // dev 判定直接用上游的 isCuaLocalDevelopmentRuntime（@hawknext/hawknext-cua/broker/server，
  // 即本文件已经用来 import buildHelperOpenArgs 的那个 subpath，可正常导入）。
  //
  // 行为等价性（别误读成安全加固）：上游是 `COMPILED_LOCAL_DEVELOPMENT_RUNTIME &&
  // HAWKNEXT_RUNTIME_ENV!=="production"`，而那个编译期常量只有 scripts/build-cua-helper-app.mjs
  // 会用 define 折叠（Helper bundle）；desktop host bundle 没有该 define，于是回退成
  // `process.env.NODE_ENV !== "production"` —— 正是复制品写的那一项。所以在**当前**打包形态下
  // 两者逐字等价，关门靠的是 HAWKNEXT_RUNTIME_ENV=production（打包态显式注入且不传 NODE_ENV）。
  //
  // 换成上游的收益是消除漂移面：折叠点、因子个数与 fail-closed 方向都由上游一处决定，
  // 哪天 host bundle 也补上 __HAWKNEXT_LOCAL_DEVELOPMENT_RUNTIME__ define（Helper 侧已经有），
  // 编译期门自动生效，不需要再回来改这里。
  ...
  const launchStandaloneCuaHelperForStatus = async (): Promise<string | null> => {
    ...
    const home = process.env.HAWKNEXT_HOME?.trim() || join(homedir(), ".hawknext");
  ...
  const cuaPipSessionService = createCuaPipSessionService({
    ...
    resolveCredentials: async () => {
      ...
      // PiP 客户端以 role=presentation 声明，资格由 Helper 按对端（HawkNext 主进程）签名 identifier 裁决。
  ...
  const cuaPermissionService: ICuaPermissionService = {
    async getStatus(
      ...
      if (
        ...
        return {
          available: false,
          reason: "HawkNext Computer Use is not enabled (plugin off or not product mode).",
        };
      }
      ...
      if (!host || !host.running) {
        ...
        if (!stable) {
          return {
            available: false,
            reason:
              "HawkNext Computer Use is not running; it will start automatically on first Computer Use use.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
        // standalone Helper 上直接查权限真值（身份模式，无 token）。
        try {
          const { callBrokerMethod } = await import("@hawknext/hawknext-cua/broker/helperHealth");
          ...
          return {
            available: false,
            reason: "HawkNext Computer Use is starting up; retry in a moment.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
      }
      try {
        ...
        if (!helper || !isDefaultCuaProductHelperCurrent(helper)) {
          return {
            available: false,
            reason: "HawkNext Computer Use lifecycle is disposed.",
          };
        }
    ...
    async restartHelper(
      ...
      try {
        ...
        return {
          ok: false,
          reason: `Failed to restart HawkNext Computer Use: ${
            error instanceof Error ? error.message : String(error)
          }`,
        };
      }
  ...
  const hawknextAgentService = createHawkNextAgentService({
    ...
    commandResolver: options?.hawknextAgentCommandResolver,
    presentationSurface: resolveHawkNextAgentPresentationSurface({
    ...
    // HawkNextAgentProcessManager，导致 host 永远不向 main 上报 Agent spawn/exit，进程监控器
    // 因而看不到实际运行的 Agent，也无法验证只读到可写升级是否复用同一进程。
    processLifecycleReporter: options?.processLifecycleReporter,
    spawnFallbackCwd: options?.hawknextAgentSpawnFallbackCwd,
    ...
    // adapter 的 fetch wrapper。判定实现与 CLI 侧共用 @hawknext/shared 的同一份，避免分叉。
    // origin 解析复用 resolveCurrentHawkNextEndpointOrigin——与闲时任务同口径（含 settings
    ...
    officialMcpTrustedOrigins: createOfficialMcpTrustedOriginRegistry({
      ...
      resolveHawkNextApiOrigin: resolveCurrentHawkNextEndpointOrigin,
    }),
    cuaOperationStateReporter: shouldEnableCuaOperationStateReporter({
      ...
      : undefined,
    // HawkNext 只发布 turn/session 事实；面板 terminal policy 由 producer coordinator 决定。
    ...
    resolveSpawnEnv: async (context) => {
      ...
      if (!helper && cuaPluginEnabled && process.platform === "darwin") {
        ...
        cuaProductHelperEnv = {
          ...
          [HAWKNEXT_CUA_PLUGIN_AUTHORITY_ENV_KEY]: randomBytes(16).toString("hex"),
        };
      ...
      const telemetryEnv = getCapturedHawkNextAgentTelemetryEnv();
      ...
      return {
        ...
        ...buildAgentEndpointOriginEnv(await resolveCurrentHawkNextEndpointOrigin()),
        // broker 凭据（socket/token）注入 agent spawn env，让内置 hawknext-cua plugin 的
        // computer-use MCP server 经 __hawknext-plugin-host 恢复 token 后连上 broker。
        ...
        ...createNodeProviderRuntimePathEnv({
          ...
          hawknextBuiltinFilePath: await providerConfigRuntime.resolveHawkNextBuiltinActiveFilePath(),
    ...
    ...(isDesktopAttachedRemote
      ...
      : {
          ...
          resolveSessionRuntimePreferences: async (scope) => {
            ...
            const settings = await settingService.get();
            const modelContextBudgetStrategy = DEFAULT_HAWKNEXT_MODEL_CONTEXT_BUDGET_STRATEGY;
            return {
  ...
  providerConnectivityAgentService = hawknextAgentService;
  ...
  hasActiveTurnRef = () => hawknextAgentService.hasActiveCuaOperationTurn();
  // desktop-continuous UI 直接订阅 hawknextSessionService，绕开 HawkNext task adapter 的
  ...
  const hawknextTaskIndexSyncer = createHawkNextTaskIndexSyncer({
    agentService: hawknextAgentService,
    taskIndexRepo,
  });
  ...
  const hawknextSessionService = createHawkNextSessionService({
    agentService: hawknextAgentService,
  ...
  const gitCommitMessageGenerator = new GitCommitMessageGenerator({
    ...
    textGenerator: {
      async generateText(params) {
        return await hawknextAgentService.generateWorkspaceText({
  ...
  // task wrapper 由 HawkNext task service adapter 提供；核心 session 状态由 HawkNext agent server 维护。
  const hawknextTaskService = createHawkNextTaskServiceAdapter({
    hawknextAgentService,
    taskIndexRepo,
    taskIndexSyncer: hawknextTaskIndexSyncer,
    settingService,
    cuaProductMcpServerResolver,
  ...
  const hawknextJwtLogoutLogger = createServiceLogger("hawknext-jwt-logout");
  hawknextJwtLogoutHandlerRef.current = (input, headers) => {
      ...
      .then((invalidated) => {
        ...
        if (invalidated) {
          void broadcastService.send({
            channel: HAWKNEXT_JWT_INVALID_BROADCAST_CHANNEL,
            payload: {},
          });
        }
      })
      .catch((error) => {
        hawknextJwtLogoutLogger.warn("HawkNext JWT logout failed", { error });
      });
  };
  ...
  const offPeakCredentialResolverDeps = {
    ...
    resolveAccountProvider: async () => {
      ...
      const staticAccess = hawknextProviderAccountAccessSchema.parse(config.access.toJSON());
      const access = await accountRequestAuthService.resolveAccessCurrent(staticAccess);
      if (!access) return null;
      return {
        providerId: provider.providerId,
        access: hawknextAccountAccessSchema.parse(access),
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    apiClient,
    baseUrl: buildRuntimeHawkNextApiUrl(process.env, "/api/v1"),
    tokenProvider: async (): Promise<string | null> => {
      ...
      return tokenSet?.hawknextJwtToken ?? tokenSet?.accessToken ?? null;
    },
  });
  ...
  const services = new ServiceCollection()
    ...
    .register(IHawkNextTaskService, hawknextTaskService)
    .register(IHawkNextAgentService, hawknextAgentService)
    .register(IHawkNextSessionService, hawknextSessionService)
    ...
    .register(
      ...
      createClientConfigService({
        ...
        resolveRequestContext: async () => ({
          endpointOrigin: await resolveCurrentHawkNextEndpointOrigin(),
          appVersion: HAWKNEXT_VERSION,
      ...
      (() => {
        ...
        const offPeakTaskService = new OffPeakTaskService({
          ...
          resolveModelSelection: async (input) => {
            ...
            // 不能靠 Registry/JSON 顺序在  与 CloudCN 间猜测。
            const selection = {
              providerId: provider.providerId,
              modelId,
          ...
          stopRunningTask: async (params) => {
            await hawknextTaskService.stopGeneration({
        ...
        // 回写前向引用，供 hawknextAgentService 的 offPeak/create、offPeak/list 协议 handler 调用。
    ...
    .register(IPluginManagementService, createPluginManagementService({ hawknextAgentService }))
...
/** 仅给同一事件账号返回当前 HawkNext JWT；不缓存、不修改登录凭据。 */
export function createTelemetryAuthorizationLoader(
  ...
  return async (userId) => {
    ...
    try {
      ...
      if (provider !== "zai" && provider !== "cloud-cn") return null;
      ...
      const jwt = (await credentialService.load("hawknext-jwt"))?.trim();
...
): () => Promise<import("@hawknext/shared").OAuthLoginAttribution | null> {
...
export function disposeServiceResources(services: ServiceCollection): void {
  ...
  const disposableServices = [
    ...
    services.getOptional(IHawkNextTaskService),
    services.getOptional(IHawkNextAgentService),
    services.getOptional(IHawkNextSessionService),
...
export async function disposeServiceResourcesAndWait(services: ServiceCollection): Promise<void> {
  ...
  // 旧的同步 dispose 会在 host 退出时丢掉强杀 timer，导致 hawknext-cli/app-server 变成孤儿进程。
  const disposableServices = [
    services.getOptional(ITerminalService),
    services.getOptional(IHawkNextTaskService),
    services.getOptional(IHawkNextAgentService),
