...
import type { ConversationRow } from "@hawknext/shared/hawknext-protocol-v4";
import {
  PROTOCOL_V4_LIMITS,
  HAWKNEXT_ATTACHMENT_FAULT_CODES,
  readHawkNextAttachmentFaultCode,
} from "@hawknext/shared/hawknext-protocol-v4";
import { Emitter } from "@hawknext/rpc";

import type { IHawkNextAgentService } from "../hawknext-agent/hawknextAgent.js";
import type { IHawkNextSessionService } from "#src/hawknext-session/hawknextSession.js";
...
interface ConversationShareServiceOptions {
  hawknextAgentService: ConversationShareAgentService;
  ...
  hawknextSessionService?: Pick<IHawkNextSessionService, "createSession" | "listSessions">;
...
function hasUnsupportedArtifactReference(value: unknown): boolean {
  if (typeof value === "string") return /^hawknext-artifact:\/\//iu.test(value);
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 hawknext-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readHawkNextAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === HAWKNEXT_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === HAWKNEXT_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === HAWKNEXT_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === HAWKNEXT_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === HAWKNEXT_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === HAWKNEXT_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readHawkNextAttachmentFaultCode(error);
  return (
    faultCode === HAWKNEXT_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === HAWKNEXT_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly hawknextAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly hawknextSessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.hawknextAgentService = options.hawknextAgentService;
    ...
    this.hawknextSessionService = options.hawknextSessionService;
    ...
    // 兜底写死生产站 ，于是测试环境（API base 走
    // 配置的 HawkNext origin）导入后回链仍指向生产站，点分割线打开的是另一个环境的分享。
    // 改用与 API base 同一个环境解析器（buildRuntimeHawkNextApiUrl 也走它），保证同环境。
    // 优先级不变：显式 option > HAWKNEXT_CONVERSATION_SHARE_WEB_URL > 按环境推导。
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.HAWKNEXT_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeHawkNextEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".hawknext-share-imports.json");
    ...
    if (this.hawknextSessionService) {
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.hawknextAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".hawknext-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".hawknext-share-import.json");
    ...
    try {
      ...
      if (
        ...
        const sessions = await this.hawknextSessionService.listSessions({ workspacePath, limit: 100 });
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.hawknext-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.hawknextSessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .hawknext-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".hawknext-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.hawknextAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.hawknextAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      hawknextAgentService: agentService,
  ...
  private async cleanupAbandonedImports(): Promise<void> {
    ...
    for (const entry of imports) {
      ...
      if (typeof marker.sessionId !== "string" || !marker.sessionId.startsWith("share-import-"))
        continue;
      try {
        const sessions = await this.hawknextSessionService.listSessions({
