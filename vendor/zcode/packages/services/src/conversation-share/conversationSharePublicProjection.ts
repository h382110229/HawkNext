...
import type { ConversationShareArtifactDescriptor } from "@hawknext/shared";
import type { ArtifactRow, ConversationRow } from "@hawknext/shared/hawknext-protocol-v4";
...
function assertSafeString(value: string): void {
  if (/^hawknext-artifact:\/\//iu.test(value)) {
...
function isPublicArtifactRef(value: string): boolean {
  return /^hawknext-artifact:\/\/share\/[A-Za-z0-9._~-]+$/u.test(value);
}
  ...
  switch (row.kind) {
    ...
    case "artifact": {
      ...
      return {
        ...
        sha256: row.sha256,
        ref: `hawknext-artifact://share/${artifactVersionId}`,
        state: row.state,
      };
    }
    case "timelineMarker":
  ...
  for (const [index, row] of input.rows.entries()) {
    ...
    if (row.kind === "turnHeader") {
      ...
      if (
        ...
        row.ref !== `hawknext-artifact://share/${row.artifactVersionId}`
      ) {
        throwProjectionError(
          "invalid_conversation",
