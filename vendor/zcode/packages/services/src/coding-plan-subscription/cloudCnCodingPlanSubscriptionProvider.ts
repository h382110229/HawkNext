/* eslint-disable max-lines -- Coding Plan provider 需要集中维护 CloudCN 支付宝与  Stripe/PayPal 接口映射，拆分会让共享鉴权和响应解包更难追踪。 */
import type {
  ApiClient,
  ApiRequestInit,
  ForceUpdateConfig,
  ...
  HawkNextModelContextBudgetStrategy,
...
import {
  ...
  HAWKNEXT_DYNAMIC_WORKFLOW_MODE_ENV,
} from "@hawknext/shared";
import type { ICredentialService } from "../credential/credential.js";
import { readApiJson } from "../providers/api/apiJson.js";
import { createServiceLogger } from "../logger/serviceLogger.js";
import {
  ensureCloudCNTeamPlanProjectApiKeyWithStatus,
  type CloudCNTeamPlanApiKeyEnsureResult,
  type CloudCNTeamPlanBizContext,
} from "#src/cloud-cn/teamPlanApiKey.js";

const CLOUD_CN_CODING_PLAN_API_PREFIX = "/api/biz";
const ZAI_CODING_PLAN_PAY_API_PREFIX = "/api/pay";
const HAWKNEXT_CLIENT_CONFIG_API_PREFIX = "/api/v1/client/configs";
...
interface HawkNextClientConfigEnvelope {
  code?: number;
  msg?: string;
  success?: boolean;
  data?: {
...
interface CloudCNCodingPlanSubscriptionProviderOptions {
  apiClient: ApiClient;
...
interface CloudCNCustomerInfoResponse {
  organizations?: Array<{
...
export class CloudCNCodingPlanSubscriptionProvider {
  ...
  private clientConfigSnapshot: HawkNextClientConfigEnvelope | null = null;
  private clientConfigSnapshotExpiresAt = 0;
  private clientConfigRequest: Promise<HawkNextClientConfigEnvelope> | null = null;

  constructor(options: CloudCNCodingPlanSubscriptionProviderOptions) {
  ...
  // ─────────── family 维度抽象（供 CloudCodingPlanSubscriptionProvider 覆盖）───────────
  // 原 enterprise 读方法（getEnterprisePricing / enrichEnterprisePricingTeamProjects 等）
  // 把 family 硬编码成 cloud-cn：providerId=cloudCnCodingPlan、host=resolveCloudCNApiOrigin、
  // token=loadCloudCNAccessToken。zai family 对称化时无法复用。
  // 这里把 family 维度抽成 protected 虚方法，cloud-cn 默认实现保持原行为，zai 子类覆盖即可。

  /** 当前 family 的 Coding Plan providerId（cloudCnCodingPlan / cloudCodingPlan）。 */
  protected codingPlanProviderId(): CodingPlanSubscriptionProviderId {
    return BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan;
  }

  /** 当前 family 的业务域名（cloud-cn 域 / zai 域）。 */
  protected resolveFamilyEnterpriseHost(): string {
    return resolveCloudCNApiOrigin(process.env);
  }

  /** 当前 family 的业务 OAuth token。 */
  protected async loadFamilyEnterpriseToken(): Promise<string> {
    return this.loadCloudCNAccessToken();
  }

  /** 当前 family 的业务鉴权头。 */
  protected createFamilyEnterpriseAuthHeaders(token: string): Record<string, string> {
    return createCloudCNLoginAuthHeaders(token);
  }
  ...
  async getOffPeakClientConfig(options?: { forceRefresh?: boolean }): Promise<OffPeakClientConfig> {
    if (process.env["HAWKNEXT_OFFPEAK_MOCK"] === "1") {
   ...
   *   1. 本地覆盖（HAWKNEXT_DYNAMIC_WORKFLOW_MODE）在任何网络动作之前裁决，命中即返回——
  ...
  async getDynamicWorkflowClientConfig(options?: {
    ...
    if (normalizeDynamicWorkflowMode(process.env[HAWKNEXT_DYNAMIC_WORKFLOW_MODE_ENV])) {
  ...
  async getModelContextBudgetStrategy(): Promise<HawkNextModelContextBudgetStrategy> {
    // 3.12.2：预算统一为 preflight-v1；保留兼容方法，但不能再为每次建会话等待远端配置。
    return DEFAULT_HAWKNEXT_MODEL_CONTEXT_BUDGET_STRATEGY;
  }
  ...
  async preview(request: CodingPlanPreviewRequest): Promise<CodingPlanPreviewResponse> {
    return this.post<CodingPlanPreviewResponse>(
      ...
      resolvePreviewPath(request.providerId),
      {
        ...
        // Coding Plan 试算接口默认按 Maas 渠道处理，不显式标记会丢失 hawknext 来源归因。
        salesChannel: request.salesChannel ?? "hawknext",
      },
    );
  }
  ...
  async productInfo(request: CodingPlanProductInfoRequest): Promise<CodingPlanProductInfo> {
    const url = new URL(
      `${resolveCodingPlanHost(request.providerId)}${CLOUD_CN_CODING_PLAN_API_PREFIX}/product/info`,
    );
  ...
  async createSign(request: CodingPlanCreateSignRequest): Promise<CodingPlanAgreementResponse> {
    if (request.providerId && isCloudCodingPlanProviderId(request.providerId)) {
  ...
  async checkPayment(
    ...
    const url = new URL(
      `${resolveCodingPlanHost(request.providerId)}${CLOUD_CN_CODING_PLAN_API_PREFIX}/pay/check`,
    );
  ...
  async checkPendingOrders(
    ...
    return this.get<CodingPlanPendingOrderCheckResponse>(
      request.providerId,
      new URL(
        `${resolveCodingPlanHost(request.providerId)}${CLOUD_CN_CODING_PLAN_API_PREFIX}/pay/check-pending-orders`,
      ),
    );
  }
  ...
  async getEnterprisePricing(
    ...
    if (request.authenticated === false) {
      ...
      >(
        this.apiClient,
        `${host}${CLOUD_CN_CODING_PLAN_API_PREFIX}/campaign/partner/enterprise/pricing`,
        {
          method: "GET",
    ...
    const response = await this.get<EnterpriseCodingPlanPricingResponse>(
      providerId,
      new URL(`${host}${CLOUD_CN_CODING_PLAN_API_PREFIX}/subscription/enterprise/v2/pricing`),
    );
  ...
  async getEnterpriseBalance(): Promise<EnterpriseCodingPlanBalanceResponse> {
    return this.get<EnterpriseCodingPlanBalanceResponse>(
      ...
      new URL(
        `${resolveCloudCNEnterpriseHost()}${CLOUD_CN_CODING_PLAN_API_PREFIX}/subscription/enterprise/v2/balance`,
      ),
    );
  }
  ...
  async getEnterprisePendingOrders(): Promise<EnterpriseCodingPlanPendingOrder[]> {
    const url = new URL(
      `${resolveCloudCNEnterpriseHost()}${CLOUD_CN_CODING_PLAN_API_PREFIX}/subscription/enterprise/v2/orders/pending`,
    );
  ...
  async checkEnterpriseOrderStatus(
    ...
    const url = new URL(
      `${resolveCloudCNEnterpriseHost()}${CLOUD_CN_CODING_PLAN_API_PREFIX}/subscription/enterprise/v2/order/${encodeURIComponent(request.orderNo)}/status`,
    );
  ...
  private async post<T>(
    ...
    const payload = await readCodingPlanApiJson<RemoteEnvelope<T>>(
      this.apiClient,
      `${endpoint.host}${CLOUD_CN_CODING_PLAN_API_PREFIX}${path}`,
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  private async getClientConfigs(): Promise<HawkNextClientConfigEnvelope> {
    ...
    // client/configs 和其他 HawkNext 平台接口必须共享运行时 endpoint；
    // E2E/测试环境会通过 HAWKNEXT_BASE_URL 指向本地 mock，硬编码线上域名会让套餐状态不可控。
    const url = resolveCodingPlanClientConfigUrl(process.env);
    url.searchParams.set("app_version", HAWKNEXT_VERSION);
    ...
    this.clientConfigRequest = readCodingPlanApiJson<HawkNextClientConfigEnvelope>(
      this.apiClient,
      url,
      {
        method: "GET",
  ...
  private async resolveEndpointConfig(
    ...
    const normalizedProviderId =
      providerId ?? BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan;
    if (isCloudCodingPlanProviderId(normalizedProviderId)) {
      const authorization = await this.loadZaiAuthorization();
      return {
        providerId: normalizedProviderId,
        host: resolveCloudCodingPlanHost(),
        headers: createZaiLoginAuthHeaders(authorization),
      };
    }

    const token = await this.loadCloudCNAccessToken();
    return {
      providerId: normalizedProviderId,
      host: resolveCloudCNApiOrigin(process.env),
      headers: createCloudCNLoginAuthHeaders(token),
    };
  }

  private async loadCloudCNAccessToken(): Promise<string> {
    const token = (
      await this.credentialService.load(`oauth:${CLOUD_CN_PROVIDER_ID}:access_token`)
    )?.trim();
    if (!token) {
      throw new Error("cloud-cn_oauth_required");
    }
    const hawknextJwtToken = (await this.credentialService.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim();
    if (hawknextJwtToken && token === hawknextJwtToken) {
      // 旧版 CloudCN OAuth callback 曾把 hawknext JWT 同时写进
      // oauth:cloud-cn:access_token，付费套餐预览会拿它去打  并报令牌过期。
      // 这里在服务边界拦截旧污染状态，避免继续向 CloudCN 业务接口发送错误凭据。
      log.warn(undefined, "CloudCN access token is stale hawknext JWT; login required");
      throw new Error("cloud-cn_oauth_required");
    }
    return token;
  }
  ...
  protected async enrichEnterprisePricingTeamProjects(
    ...
    const host = this.resolveFamilyEnterpriseHost();
    const customerInfo = await readCodingPlanApiJson<RemoteEnvelope<CloudCNCustomerInfoResponse>>(
      this.apiClient,
      `${host}${CLOUD_CN_CODING_PLAN_API_PREFIX}/customer/getCustomerInfo`,
      {
        method: "GET",
        timeoutMs: REQUEST_TIMEOUT_MS,
    ...
    const teamProjects = resolveCloudCNTeamProjects(customerInfo.data);
  ...
  protected async ensureSubscribedTeamPlanProjectApiKeys(
    ...
    for (const teamContext of teamContexts) {
      try {
        const result = await ensureCloudCNTeamPlanProjectApiKeyWithStatus({
        ...
        // 多团队套餐每个项目都需要独立 hawknext-team-api-key。
  ...
  protected async loadZaiAuthorization(): Promise<string> {
    const token = (
      await this.credentialService.load(`oauth:${CLOUD_INTL_PROVIDER_ID}:access_token`)
    )?.trim();
    if (!token) {
...
function resolveCodingPlanClientConfigUrl(env: NodeJS.ProcessEnv): URL {
  return new URL(buildRuntimeHawkNextApiUrl(env, HAWKNEXT_CLIENT_CONFIG_API_PREFIX));
}
...
function createTeamPlanProjectApiKeyPrewarmStatusFromEnsureResult(
  result: CloudCNTeamPlanApiKeyEnsureResult,
...
): CloudCNTeamPlanBizContext[] {
  const result: CloudCNTeamPlanBizContext[] = [];
...
function resolveCloudCNTeamProjects(
  customerInfo: CloudCNCustomerInfoResponse | null | undefined,
): EnterpriseCodingPlanProjectContext[] {
  ...
  for (const organization of customerInfo?.organizations ?? []) {
    ...
    for (const project of organization.projects ?? []) {
      const projectId = project.projectId?.trim() ?? "";
      if (!projectId || !isCloudCNTeamCodingPlanProject(project)) {
        continue;
      }
      result.push({
        organizationId,
...
function isCloudCNTeamCodingPlanProject(
  project: NonNullable<
    NonNullable<CloudCNCustomerInfoResponse["organizations"]>[number]["projects"]
  >[number],
): boolean {
...
export function createCloudCNLoginAuthHeaders(token: string): Record<string, string> {
  return {
    // CloudCN 登录态业务接口要求 Authorization 直接传 accessToken。
...
export function createZaiLoginAuthHeaders(token: string): Record<string, string> {
  return {
    //  provider connection 已把 access_token 持久化为业务 JWT。
...
function resolveCodingPlanHost(providerId: CodingPlanSubscriptionProviderId | undefined): string {
  return providerId && isCloudCodingPlanProviderId(providerId)
    ? resolveCloudCodingPlanHost()
    : resolveCloudCNApiOrigin(process.env);
}

function resolveCloudCodingPlanHost(): string {
  //  Coding Plan 的 /api/biz 与 /api/pay 业务接口必须跟随产品环境。
  // 业务 token 必须发送到 .env 配置的 ZAI Business origin；未覆盖时默认 。
  return resolveZaiBusinessBaseUrl(process.env);
}

function resolveCloudCNEnterpriseHost(): string {
  // 企业套餐 token 必须发送到 .env 配置的 CloudCN origin；未覆盖时默认 ，
  // 服务端返回空响应后被归一成系统繁忙。
  return resolveCloudCNApiOrigin(process.env);
}

function resolvePreviewPath(providerId: CodingPlanSubscriptionProviderId | undefined): string {
  return providerId && isCloudCodingPlanProviderId(providerId) ? "/pay/zai-preview" : "/pay/preview";
}

function assertZaiPaymentProvider(providerId: CodingPlanSubscriptionProviderId): void {
  if (!isCloudCodingPlanProviderId(providerId)) {
    throw new Error(CODING_PLAN_ZAI_OVERSEAS_PAYMENT_REQUIRED);
  }
}
...
function normalizeStaticProductProviderIds<T>(
  ...
  for (const [legacyId, currentId] of [
    ["builtin:cloud-cn-coding-plan", BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan],
...
function unwrapClientConfigProducts(
  payload: HawkNextClientConfigEnvelope,
): CodingPlanStaticProductsConfig {
  if (payload.code !== undefined && payload.code !== 0) {
    throw new Error(payload.msg?.trim() || "HawkNext client config request failed");
  }
  const products = payload.data?.configs?.codingPlanStaticProducts;
  if (!products || typeof products !== "object") {
    throw new Error("HawkNext client config missing Coding Plan products");
  }
...
function unwrapClientConfigTeamProducts(
  ...
  if (!products || typeof products !== "object") {
    throw new Error("HawkNext client config missing Coding Plan team products");
  }
  for (const providerProducts of Object.values(products)) {
    if (
      ...
      throw new Error("HawkNext client config has invalid Coding Plan team products");
    }
  }
...
function unwrapClientConfigStartPlanPreview(
  ...
  if (
    ...
    throw new Error("HawkNext client config invalid Start Plan preview");
  }
  return {
...
function resolveCodingPlanProviderName(providerId: CodingPlanSubscriptionProviderId): string {
  return isCloudCodingPlanProviderId(providerId) ? "" : "CloudCN";
}
 ...
 * 来自 HawkNext Built-in Provider / Model Config。
 * mock 模式（HAWKNEXT_OFFPEAK_MOCK=1）只替代产品曝光与套餐状态；模型候选仍来自 Registry。
 */
export function resolveOffPeakClientConfig(
  ...
  if (env["HAWKNEXT_OFFPEAK_MOCK"] === "1") {
    return {
      enabled: hasModels,
      modelSelectionView,
      // HAWKNEXT_OFFPEAK_MOCK_NO_PLAN=1 演示「非 coding plan 锁定」态；缺省视为已订阅。
      codingPlanActive: env["HAWKNEXT_OFFPEAK_MOCK_NO_PLAN"] !== "1",
    };
  }
