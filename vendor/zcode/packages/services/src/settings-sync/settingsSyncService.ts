...
const HAWKNEXT_PLUGIN_MANIFEST_PATH = [".hawknext-plugin", "plugin.json"] as const;
...
function getWorkspaceHawkNextCommandRoot(workspacePath: string): string {
  return join(workspacePath, ".hawknext", "commands");
}

function getUserHawkNextCommandRoot(): string {
  return join(resolveUserHomeDir(), ".hawknext", "commands");
}

function getWorkspaceHawkNextPluginRoot(workspacePath: string): string {
  return join(workspacePath, ".hawknext", "plugins");
}

function getUserHawkNextPluginRoot(): string {
  return join(resolveUserHomeDir(), ".hawknext", "plugins");
}

function getUserHawkNextCliConfigPath(): string {
  return join(resolveUserHomeDir(), ".hawknext", "cli", "config.json");
}

function getWorkspaceHawkNextConfigPath(workspacePath: string): string {
  return join(workspacePath, ".hawknext", "config.json");
}
...
function getUserHawkNextAgentsFilePath(): string {
  return join(resolveUserHomeDir(), ".hawknext", "AGENTS.md");
}
...
function resolveTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserHawkNextSkillRoot();
  }
  return workspacePath ? getWorkspaceHawkNextSkillRoot(workspacePath) : null;
}
...
function resolveCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceHawkNextSkillRoot(workspacePath)
      : null
...
function resolveCommandTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserHawkNextCommandRoot();
  }
  return workspacePath ? getWorkspaceHawkNextCommandRoot(workspacePath) : null;
}
...
function resolveCommandCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceHawkNextCommandRoot(workspacePath)
      : null
    : getUserHawkNextCommandRoot();
}
...
function resolvePluginTargetRootForScope(
  ...
  if (targetScope === "global") {
    return getUserHawkNextPluginRoot();
  }
  return workspacePath ? getWorkspaceHawkNextPluginRoot(workspacePath) : null;
}
...
function resolvePluginCandidateTargetRoot(
  ...
  return sourceScope === "workspace"
    ? workspacePath
      ? getWorkspaceHawkNextPluginRoot(workspacePath)
      : null
    : getUserHawkNextPluginRoot();
}
...
function resolvePluginConfigPathForScope(
  ...
  if (targetScope === "global") {
    return getUserHawkNextCliConfigPath();
  }
  return workspacePath ? getWorkspaceHawkNextConfigPath(workspacePath) : null;
}
...
async function findPluginManifestPath(pluginPath: string): Promise<string | null> {
  const hawknextManifestPath = join(pluginPath, ...HAWKNEXT_PLUGIN_MANIFEST_PATH);
  if (await pathExists(hawknextManifestPath)) {
    return hawknextManifestPath;
  }
...
function readHawkNextMcpServers(parsed: Record<string, unknown>): Record<string, McpServerConfig> {
...
async function collectExistingMcpServerNameKeys(
  ...
  if (targetConfigPath) {
    for (const name of Object.keys(
      readHawkNextMcpServers(await readJsonFileOrEmpty(targetConfigPath)),
    )) {
...
async function addMcpServerToHawkNextConfig(
  filePath: string,
  name: string,
  config: McpServerConfig,
): Promise<void> {
  ...
  const servers = readHawkNextMcpServers(parsed);
...
async function importMcpServersForAgent(
  ...
  for (const candidate of candidates) {
    ...
    try {
      await addMcpServerToHawkNextConfig(selectedConfigPath, candidate.name, candidate.config);
      importedCount += 1;
...
async function getClaudeAgentsFileMigrationStatus(): Promise<SettingsSyncClaudeAgentsFileMigrationStatus> {
  ...
  const targetPath = getUserHawkNextAgentsFilePath();
...
async function copyClaudeAgentsFileToHawkNextAgentsFile(params: {
  overwrite?: boolean;
...
export function createSettingsSyncService(
  ...
  return {
    ...
    async copyClaudeAgentsFileToHawkNextAgentsFile(
      request = {},
    ): Promise<SettingsSyncClaudeAgentsFileCopyResult> {
      const result = await copyClaudeAgentsFileToHawkNextAgentsFile({
