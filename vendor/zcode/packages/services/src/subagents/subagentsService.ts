...
const PLUGIN_MANIFEST_PATHS = [
  join(".hawknext-plugin", "plugin.json"),
...
async function discoverPluginAgents(params: {
  ...
  const storageRoot = await resolveHawkNextStorageRoot(params.storageOptions);
...
async function readPluginConfig(options?: SubagentStorageOptions): Promise<PluginConfigSummary> {
  try {
    const configPath = join(resolveUserHomeDir(options), ".hawknext", "cli", "config.json");
...
async function readEnabledPluginRecords(
  ...
  const records = installed.filter(
    (record) =>
      ...
      !(
        record.id.endsWith(`@${HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE_ID}`) &&
  ...
  for (const cacheRoot of await scanOfficialPluginCacheRoots(pluginStorageRoot)) {
    const id = `${cacheRoot.name}@${HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE_ID}`;
...
export function createSubagentsService(options?: SubagentsServiceOptions): ISubagentsService & {
  ...
  return {
    ...
    async getPrimaryUserAgentsDirectory(_params: { provider: HawkNextProvider }): Promise<{
      path: string;
    }> {
