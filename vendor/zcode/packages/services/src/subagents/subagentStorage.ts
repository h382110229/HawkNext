...
export async function resolveUserSubagentRoot(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveHawkNextStorageRoot(options), "agents");
}

export function resolveWorkspaceSubagentRoot(workspacePath: string): string {
  return join(workspacePath, ".hawknext", "agents");
}

export async function resolveSubagentStateFile(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveHawkNextStorageRoot(options), "v2", "agents-state.json");
}

export async function resolveHawkNextStorageRoot(options?: SubagentStorageOptions): Promise<string> {
...
async function readUserCliConfig(
  ...
  try {
    const raw = await readFile(
      join(resolveUserHomeDir(options), ".hawknext", "cli", "config.json"),
      "utf8",
    );
