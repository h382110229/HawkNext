...
export interface ProviderConfigRuntimeOptions {
  ...
  readonly hawknextBuiltinRemote?: NodeProviderConfigRuntimeOptions["hawknextBuiltinRemote"];
  readonly hawknextBuiltinEnvironment?: NodeProviderConfigRuntimeOptions["hawknextBuiltinEnvironment"];
 ...
 * 配置迁移保留 HawkNext 用户的供应商数据，文件运行时由 @hawknext/provider-node 唯一实现。
 */
export class ProviderConfigRuntime {
  ...
  constructor(options: ProviderConfigRuntimeOptions) {
    const runtimeOptions: NodeProviderConfigRuntimeOptions = {
      hawknextBuiltinFilePath: options.hawknextBuiltinFilePath,
      hawknextBuiltinActiveFilePath: options.hawknextBuiltinActiveFilePath,
      hawknextBuiltinRemote: options.hawknextBuiltinRemote,
      hawknextBuiltinEnvironment: options.hawknextBuiltinEnvironment,
      onHawkNextBuiltinRefreshError: options.onHawkNextBuiltinRefreshError,
  ...
  resolveHawkNextBuiltinActiveFilePath(): Promise<string> {
    return this.#runtime.resolveHawkNextBuiltinActiveFilePath();
  }

  refreshHawkNextBuiltin(options?: { readonly force?: boolean }) {
    return this.#runtime.refreshHawkNextBuiltin(options);
  }

  onDidCheckHawkNextBuiltin(listener: () => Promise<void>): () => void {
    return this.#runtime.onDidCheckHawkNextBuiltin(listener);
  }
