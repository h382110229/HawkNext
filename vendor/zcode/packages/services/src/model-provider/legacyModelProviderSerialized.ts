...
/**
 * 各 HawkNext Agent Provider 的模型槽位映射（按 provider 区分）。
...
export interface ModelProviderEndpoints {
  ...
  /** @deprecated Gemini custom provider 已统一走 endpoints.official-model + compat，仅保留旧数据兼容读取。 */
  gemini?: string;
...
export type ModelProviderSupportedFormat = "anthropic" | "official-model" | "responses" | "gemini";

export type ModelProviderApiFormat =
  | "anthropic-messages"
  | "official-model-chat-completions"
  | "official-model-responses";

export type ModelProviderCatalogSourceId = "china-llm-hawknext-dev";

export type ModelProviderKind = "anthropic" | "official-model" | "official-model-compatible";
...
const modelSupportedFormatSchema = z.enum(["anthropic", "official-model", "responses", "gemini"]);
...
export const modelProviderKindSchema = z.enum(["anthropic", "official-model", "official-model-compatible"]);

export const modelProviderCatalogSourceIdSchema = z.enum(["china-llm-hawknext-dev"]);
...
export const modelProviderCatalogFileSchema = z.object({
  schemaVersion: z.literal("hawknext.model-providers.v1"),
...
export const modelProviderStoreFileSchema = z.object({
  schemaVersion: z.literal("hawknext.model-providers.v2"),
...
export function stripLegacyClaudeProviderMappings(
  ...
  // 后续 HawkNext CLI 等 provider 的槽位配置；这里只删历史子字段，未知后续 key 原样保留。
...
export function resolveModelProviderDefaultKind(
  ...
  if (paths?.["official-model-compatible"] !== undefined) {
    return "official-model-compatible";
  }
  if (paths?.["official-model"] !== undefined) {
    return "official-model";
  }
  ...
  switch (apiFormat) {
    case "anthropic-messages":
      return "anthropic";
    case "official-model-responses":
      return "official-model";
    case "official-model-chat-completions":
      return "official-model-compatible";
  }
}
...
export function resolveModelProviderKindApiFormat(kind: ModelProviderKind): ModelProviderApiFormat {
  switch (kind) {
    ...
    case "official-model":
      return "official-model-responses";
    case "official-model-compatible":
      return "official-model-chat-completions";
  }
}
...
export function normalizeModelProviderBaseUrlForKind(
  ...
  const suffixes: Record<ModelProviderKind, string[]> = {
    ...
    // OfficialModel Responses 的 SDK baseURL 通常包含 /v1，运行时只会追加 /responses。
    // 不能把 /v1/responses 整段剥掉，否则用户填写 https://host/v1 会被展示/落盘成 https://host。
    officialModel: ["/responses"],
    "official-model-compatible": ["/chat/completions"],
  };
...
export function resolveModelProviderRuntimeBaseUrl(
  ...
  // 否则 OfficialModel-only provider 会被错误探测/同步成 Anthropic 可用。
  if (paths[kind] === undefined) {
    return "";
  }
  // catalog 的 baseURL + path 表示完整请求地址，而 OfficialModel/Anthropic
...
function getDefaultModelSupportedFormatsFromLegacyEndpoints(
  endpoints: Pick<ModelProviderEndpoints, "anthropic" | "official-model">,
): ModelProviderSupportedFormat[] {
  const formats: ModelProviderSupportedFormat[] = [];
  if (endpoints.anthropic?.trim()) {
    formats.push("anthropic");
  }
  if (endpoints.officialModel?.trim()) {
    formats.push("official-model");
  }
  return formats;
}
...
export function migrateLegacyModelProviderConfig(
  ...
  // 旧 provider 可能同时配置 Anthropic 与 OfficialModel，迁移阶段先保留历史路径，
  ...
  const officialModelEndpoint = provider.endpoints.officialModel?.trim();
  if (officialModelEndpoint) {
    legacyEntries.push([defaultKind === "official-model" ? "official-model" : "official-model-compatible", officialModelEndpoint]);
  }
  ...
  const migratedModels = provider.models.map((modelId) => {
    ...
    const disabledReason =
      kinds.length === 0 && formats.includes("gemini")
        ? "legacy gemini format is not supported by hawknext.model-providers.v2"
        : undefined;
...
export function getDefaultModelSupportedFormatsFromEndpoints(
  endpoints: Partial<
    Pick<ModelProviderEndpoints, "anthropic" | "official-model" | "gemini" | "baseURL" | "paths">
  >,
): ModelProviderSupportedFormat[] {
  const formats: ModelProviderSupportedFormat[] = [];
...
export function resolveModelProviderApiFormat(
  ...
  if (
    ...
    provider.apiFormat === "official-model-chat-completions" ||
    provider.apiFormat === "official-model-responses"
  ) {
  ...
  // 仍按 Anthropic-compatible 主链路优先，避免 Claude 语义 provider 被误切到 OfficialModel。
  if (provider.endpoints.paths?.anthropic !== undefined) {
    return "anthropic-messages";
  }

  if (provider.endpoints.paths?.["official-model"] !== undefined) {
    return "official-model-responses";
  }

  if (provider.endpoints.paths?.["official-model-compatible"] !== undefined) {
    return "official-model-chat-completions";
  }
