...
import { AccountProviderService, createAccountProviderConfigResolver } from "@hawknext/provider";
import {
  type ApiClient,
...
import {
  validateCloudCNAccountProviderAvailability,
...
export function createAccountProviderConnectionResolver(
  ...
  return async ({ configuredProviders, reasons = [] }) => {
    ...
    for (const family of ["zai", "cloud-cn"] as const) {
...
export function createCodingPlanFamilyAvailabilityResolver(
  ...
  return ({ family, providers, selections }) => {
    ...
    return family === "zai"
      ...
      : validateCloudCNAccountProviderAvailability(providers, context);
  };
}
