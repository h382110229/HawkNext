...
import { type CloudCNTeamPlanBizContext } from "#src/cloud-cn/teamPlanApiKey.js";
import {
  fetchPersonalCodingPlanEntitlement,
  fetchTeamCodingPlanEntitlement,
} from "#src/cloud-cn/codingPlanEntitlement.js";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { readApiJson } from "../providers/api/apiJson.js";
import { normalizeApiKeyForHeader } from "../providers/api/index.js";
import { resolveCloudCNStartPlanHawkNextJwt } from "./cloud-cnStartPlanHawkNextJwt.js";
import {
...
import {
  createCloudCNLoginAuthHeaders,
  createZaiLoginAuthHeaders,
} from "../coding-plan-subscription/cloudCnCodingPlanSubscriptionProvider.js";

const REQUEST_TIMEOUT_MS = 15_000;
const log = createServiceLogger("coding-plan-availability");
const CLOUD_CN_SUBSCRIPTION_LIST_PATH = "/api/biz/subscription/list";
...
function buildZaiSubscriptionListUrl(): string {
  ...
  // availability 校验必须和 OAuth business login 共用同一套 HAWKNEXT_ENV 域名分流，只允许 origin 不同。
...
interface CloudCNCustomerInfoEnvelope {
  code?: number;
  success?: boolean;
  msg?: string;
  data?: CloudCNCustomerInfo | null;
}
...
async function validateCodingPlanProviderAvailability(
  ...
  return validateSubscriptionListAvailability(
    provider,
    context,
    buildCloudCNApiUrl(process.env, CLOUD_CN_SUBSCRIPTION_LIST_PATH),
  );
}
...
export async function validateCloudCNAccountProviderAvailability(
  providers: readonly CodingPlanAvailabilityProvider[],
  context: CodingPlanAvailabilityContext,
): Promise<Partial<Record<string, CodingPlanAvailabilityResult>>> {
  const startProvider = findAvailabilityProvider(providers, "cloud-cn", "start-plan");
  const individualProvider = findAvailabilityProvider(
    providers,
    "cloud-cn",
    "individual-coding-plan",
  );
  const teamProvider = findAvailabilityProvider(providers, "cloud-cn", "team-coding-plan");
  return validateFamilyAccountProviders({
    family: "cloud-cn",
    context,
    startProvider,
...
// zai/cloud-cn Team Plan 对称化。原硬编码 cloud-cn host/token/header，
// zai team plan 永远不校验 customerInfo（被移出团队也不禁用）。
// 泛化为 family-aware：按 family 选 host（zai 用配置的 ZAI Business origin）、token（oauth:cloud-intl:）、header（Bearer）。
// 已验证 zai 域名 getCustomerInfo 返回结构与 cloud-cn 同构（organizations/projects）。
async function validateSelectedTeamPlanAvailability(
  ...
  const oauthProvider = family === "zai" ? CLOUD_INTL_PROVIDER_ID : CLOUD_CN_PROVIDER_ID;
  const token = (
  ...
  const hawknextJwtToken = (await context.credentialService?.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim();
  // CloudCN 旧版本可能把 hawknext-jwt 误写进 oauth access token；
  // 但  的 business JWT 本身就是合法 Bearer token，不能套用这个 stale-token 防御。
  ...
  try {
    ...
    // cloud-cn 走 resolveCloudCNApiOrigin。路径 /api/biz/customer/getCustomerInfo 两边同构。
    const host =
      family === "zai"
        ? buildRuntimeZaiBusinessUrl(process.env, "")
        : resolveCloudCNApiOrigin(process.env);
    const authHeaders =
      family === "zai" ? createZaiLoginAuthHeaders(token) : createCloudCNLoginAuthHeaders(token);
    const payload = await readApiJson<CloudCNCustomerInfoEnvelope>(
      context.apiClient!,
    ...
    const resolvedTeamContext = resolveCloudCNTeamProjectContext(
      payload.data,
      selectedTeamContext,
    );
...
// zai/cloud-cn Team Plan 对称化。原仅解析 cloud-cn selectedKey，
...
function resolveCloudCNTeamProjectContext(
  customerInfo: CloudCNCustomerInfo | null | undefined,
  selectedContext: {
    organizationId: string | null;
    projectId: string;
  },
): CloudCNTeamPlanBizContext | null {
...
async function resolveStartPlanAuthorization(
  ...
  if (provider.family === "cloud-cn") {
    const hawknextJwtToken = await resolveCloudCNStartPlanHawkNextJwt({
      credentialService: context.credentialService,
      provider,
    });
    return {
      value: hawknextJwtToken ? `Bearer ${hawknextJwtToken}` : "",
      missingReason: "coding_plan_not_authenticated",
    };
  }

  const credentialJwt = await loadZaiProviderConnectionHawkNextJwtToken(context);
...
async function loadZaiProviderConnectionHawkNextJwtToken(
  context: CodingPlanAvailabilityContext,
): Promise<string> {
  return (await context.credentialService?.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim() || "";
}
...
function hasActiveStartPlan(plans: ZaiStartPlanPlan[] | undefined): boolean {
  return Boolean(
    plans?.some((plan) => {
      ...
      // billing/balance 的 plans 真实返回的是 `plan_id=hawknext-v3-start-plan`
      // 和 `name=HawkNext V3 Start Plan`；只认 `name === "start plan"` 的话，
