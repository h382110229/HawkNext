import { CLOUD_CN_PROVIDER_ID, resolveCloudCNApiOrigin, CLOUD_INTL_PROVIDER_ID } from "@hawknext/shared";
import { ZAI_API_HOST } from "../providers/api/apiEndpoints.js";
import {
  DEFAULT_ORG_NAME,
  DEFAULT_PROJECT_NAME,
  HAWKNEXT_API_KEY_NAME,
...
export class AccountProviderApiKeyResolver {
  ...
  async resolveProviderApiKey(
    ...
    try {
      if (provider === CLOUD_CN_PROVIDER_ID) {
        return await this.resolveBizApiKey(resolveCloudCNApiOrigin(process.env), accessToken);
      }
  ...
  private async resolveZaiApiKey(oauthAccessToken: string): Promise<string | null> {
    // Provider Connection 已把  access token 持久化为业务 token。
  ...
  private async resolveBizApiKey(
    ...
    let apiKeyEntry = apiKeys.find((item) => item.name === HAWKNEXT_API_KEY_NAME) ?? null;

    if (!apiKeyEntry) {
      apiKeyEntry = await this.fetchRemoteData<RemoteApiKeySummary>(listUrl, {
        method: "POST",
        headers: createBizAuthHeaders(authorization),
        body: JSON.stringify({ name: HAWKNEXT_API_KEY_NAME }),
      });
    }
    ...
    if (!secretKey) {
      //  请求必须使用 copy 接口返回的 secretKey，裸 apiKey 不能用于模型鉴权。
