...
export class ProviderRuntime {
  ...
  constructor(dependencies: ProviderRuntimeDependencies) {
    ...
    this.#disposeBuiltinRecovery = this.#configRuntime.onDidCheckHawkNextBuiltin(async () => {
      const [config, account] = await Promise.all([
        this.configService.read(),
        accountSource.read(),
      ]);
      if (!this.#disposed && config.hawknextBuiltinRevision !== account.basedOnHawkNextBuiltinRevision) {
  ...
  return {
    ...
    refreshSources: async (reason) => {
      const sourceResults = await Promise.allSettled([
        configRuntime.refreshHawkNextBuiltin({ force: true }),
