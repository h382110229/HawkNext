...
import { DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS } from "@hawknext/shared";
...
const SKILL_SETTINGS_DIR = join(resolveUserHomeDir(), ".hawknext", "v2");
const SKILL_CLI_SETTINGS_DIR = join(resolveUserHomeDir(), ".hawknext", "cli");
const SKILL_CLI_CONFIG_FILE = join(SKILL_CLI_SETTINGS_DIR, "config.json");
const GIT_MARKER = ".git";
const HOME_PREFIX = "~/";
const HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE = "hawknext-plugins-official";
const HAWKNEXT_INLINE_PLUGIN_MARKETPLACE = "inline";
const HAWKNEXT_PLUGIN_MANIFEST_PATH = join(".hawknext-plugin", "plugin.json");
...
/** 对齐 apps/hawknext-cli/packages/adapters/src/skills/index.ts:19 */
...
/** HawkNext Agent 工作区级技能目录。 */
function getWorkspaceHawkNextSkillRoot(workspacePath: string): string {
  return join(workspacePath, ".hawknext", "skills");
}

/** 兼容目录: workspace 级 `.agents/skills`, 仅在同层 `.hawknext/skills` 没读到技能时 fallback。 */
function getWorkspaceAgentsSkillRoot(workspacePath: string): string {
  return join(workspacePath, ".agents", "skills");
}

/** HawkNext Agent 用户级技能目录。 */
function getUserHawkNextSkillRoot(): string {
  return join(resolveUserHomeDir(), ".hawknext", "skills");
}
...
async function isUserAgentsSkillCoveredByHawkNext(params: {
  skillPath: string;
  rootPath: string;
  userHawkNextSkillNameKeys: Set<string>;
}): Promise<boolean> {
  const { skillPath, rootPath, userHawkNextSkillNameKeys } = params;
  if (rootPath !== getUserAgentsSkillRoot()) {
    return false;
  }
  if (await exists(join(getUserHawkNextSkillRoot(), basename(dirname(skillPath)), SKILL_FILE_NAME))) {
    return true;
  }
  return userHawkNextSkillNameKeys.has(await readSkillNameKey(skillPath));
}

/**
 * 从 workspacePath 向上走到 worktree 根（含 .git 标记），把每一层的
 * `.hawknext/skills` 与 `.agents/skills` 都收集起来。
 * 对齐 apps/hawknext-cli/packages/adapters/src/skills/roots.ts:60-72。
...
async function resolveAncestorWorkspaceRoots(workspacePath: string): Promise<string[]> {
  ...
  for (const dir of baseDirectories) {
    ...
    // 当成 `.hawknext` 的 fallback，导致同层 `.hawknext` 只要有一个技能，`/`、`$` 和设置页
    // 就会整根漏掉 `.agents` 技能，形成“模型可执行但 UI 无法引用”的发现语义分裂。
    roots.push(getWorkspaceHawkNextSkillRoot(dir));
...
async function scanOfficialPluginCacheRoots(pluginStorageRoot: string): Promise<string[]> {
  const cacheRoot = join(pluginStorageRoot, "cache", HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE);
...
async function findPluginManifestPath(rootPath: string): Promise<string | null> {
  for (const manifestPath of [
    join(rootPath, HAWKNEXT_PLUGIN_MANIFEST_PATH),
...
async function resolvePluginSkillRootDescriptors(): Promise<SkillRootDescriptor[]> {
  ...
  const candidates: PluginRootCandidate[] = [
    ...config.dirs.map((dir) => ({
      defaultEnabled: true,
      marketplace: HAWKNEXT_INLINE_PLUGIN_MARKETPLACE,
  ...
  for (const candidate of candidates) {
    ...
    // 仍会从 cache 贡献技能。
    if (
      candidate.marketplace === HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE &&
...
async function discoverSkills(params: {
  ...
  if (params.includeUserSkills) {
    // 用户级技能是全局资源，`.hawknext/skills` 里只要存在一个技能就截断
    // `.agents/skills` 会导致外部 Agent 的全局技能在导入后从设置页消失。
    roots.push({
      scope: "user" as const,
      rootPath: getUserHawkNextSkillRoot(),
    });
    roots.push({
      scope: "user" as const,
  ...
  const userHawkNextSkillNameKeys = params.includeUserSkills
    ? await collectSkillNameKeysInRoot(getUserHawkNextSkillRoot())
    : new Set<string>();
  ...
  for (const rootPath of scanRootPaths) {
    ...
    for (const skillPath of skillPaths) {
      if (
        await isUserAgentsSkillCoveredByHawkNext({
          skillPath,
          rootPath: root.rootPath,
          userHawkNextSkillNameKeys,
        })
      ) {
        continue;
      }
      ...
      // 这些字段不影响 HawkNext 读取 name/description，继续报 warning 只会制造无操作价值的噪音。
...
function resolveCapabilities(options?: SkillsServiceOptions): SkillsCapability {
  const isDesktopRuntime = options?.isDesktopRuntime ?? Boolean(process.env.HAWKNEXT_PROCESS_LABEL);
  if (isDesktopRuntime) {
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async copyToCommon(params: {
      ...
      const commonRoot =
        skill.scope === "workspace"
          ? getWorkspaceHawkNextSkillRoot(params.workspacePath)
          : getUserHawkNextSkillRoot();
    ...
    async removeFromCommon(params: {
      ...
      const userCommonRoot = getUserHawkNextSkillRoot().replaceAll("\\", "/").toLowerCase();
      const workspaceCommonRoot = getWorkspaceHawkNextSkillRoot(params.workspacePath)
        .replaceAll("\\", "/")
        .toLowerCase();
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.hawknext/skills/<name>` 下的目录项本身。
      ...
      // 收集工作区各层级（沿 worktree 向上）的 .hawknext/skills 与 .agents/skills，外加用户级两根。
      const allowedRootCandidates = await resolveAncestorWorkspaceRoots(params.workspacePath);
      allowedRootCandidates.push(getUserHawkNextSkillRoot());
