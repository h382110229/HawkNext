...
} from "@hawknext/shared/workspace-hook-discovery";
import { parseWorkspaceHookTrustStoreContent } from "@hawknext/shared/workspace-hook-trust-store-file";
...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "hawknext") {
    return workspacePath ? join(baseDir, ".hawknext") : join(baseDir, ".hawknext", "cli");
  }
...
function getConfigPath(source: SettingsDirectorySource, workspacePath?: string): string {
  return join(
    ...
    source === "hawknext" ? HAWKNEXT_CONFIG_FILE : SETTINGS_FILE,
  );
}
...
async function readUserHawkNextSource(): Promise<WorkspaceHookSourceInput | undefined> {
  const path = getConfigPath("hawknext");
...
async function readPersistentWorkspaceHookTrustDigests(
  ...
  const userConfig = (await readJsonFile<Record<string, unknown>>(getConfigPath("hawknext"))) ?? {};
...
async function loadHooksImpl(
  ...
  const hooks = [
    ...
    ...fromUserHawkNextSource({
      ...
      runtimeRoot,
      workspacePath,
      location: buildLocation("hawknext"),
    }),
...
async function writeHawkNextHooksConfig(
  workspacePath: string | undefined,
  hooks: Hook[],
): Promise<void> {
  const configPath = getConfigPath("hawknext", workspacePath);
  const existingConfig = (await readJsonFile<HawkNextConfigFile>(configPath)) ?? {};
  ...
  await atomicWriteWorkspaceHookConfig(configPath, {
    ...
    hooks: {
      ...
      events: toHawkNextHooksEvents(hooks),
    },
  });
}
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".hawknext", "config.json");
  const userHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      (!hook.location || (hook.location.source === "hawknext" && hook.location.scope === "user")),
  );
  const projectHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      hook.location?.source === "hawknext" &&
      hook.location.scope === "project" &&
      (!hook.configuredState ||
        resolve(hook.configuredState.sourcePath) === currentProjectConfigPath),
  );
  await writeHawkNextHooksConfig(undefined, userHooks);
  await writeHawkNextHooksConfig(params.workspacePath, projectHooks);
}
