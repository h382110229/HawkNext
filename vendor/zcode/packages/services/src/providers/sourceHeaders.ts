...
import {
  ...
  normalizeHawkNextSourceHeaderValue,
  HAWKNEXT_ENV,
...
export { HAWKNEXT_SOURCE_HEADERS };

interface HawkNextSourceHeaderOptions {
  appVersion?: string;
  arch?: string;
...
function normalizePrintableHeaderValue(value: string | undefined): string | undefined {
  return normalizeHawkNextSourceHeaderValue(value);
}
...
export function buildHawkNextSourceHeaders(
  options: HawkNextSourceHeaderOptions = {},
): Record<string, string> {
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  const appVersion = normalizePrintableHeaderValue(options.appVersion ?? HAWKNEXT_VERSION);
  const releaseChannel = normalizePrintableHeaderValue(options.releaseChannel ?? HAWKNEXT_ENV);
  ...
  return buildHawkNextSourceHeadersFromContext({
    appVersion,
    arch,
    clientLanguage,
    clientTimezone,
