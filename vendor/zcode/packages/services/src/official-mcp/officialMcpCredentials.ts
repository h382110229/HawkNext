/*
 * HawkNext 官方 Server MCP 的凭证解析与身份头构造。
 *
 ...
 *
 * 与 Off-Peak 的三处有意差异：
 *   1. 显式产出 CloudCn-Target-Type（Off-Peak 侧当前没有生产者）；
 *   2. 不存在任何 mock 凭证分支（官方 MCP 无 mock 网关，测试用依赖注入替换来源）；
 *   3. 失败原因使用 official_* 分类，不复用 Off-Peak 的 reason 字符串。
 *
 * 凭证通道：Coding Plan 凭证走 `X-CloudCn-Authorization` + MaaS 登录 JWT，
 ...
 *
 * 必须按 provider family 精确选择、**禁止跨 family 回退**：拿 ZAI 的业务 JWT 去打 CloudCN 的
 * Coding Plan 只会得到一次注定失败的请求，而且失败原因会指向"没有套餐"这种误导结论。
 * 这几行与 cloud-cnUsageQuotaProvider 的 reset 通道逻辑等价但独立（见文件头说明）。
 */
function maasJwtCredentialKey(providerFamily: "zai" | "cloud-cn"): string {
...
function resolveSelectedProvider(
  ...
  const candidates = registry.providers.flatMap((provider) => {
    const parsed = hawknextProviderAccountAccessSchema.safeParse(provider.config.access);
...
function resolveSelectedPlan(
  ...
  access: HawkNextAccountAccess | null,
  ...
  if (access.planKind === "team-coding-plan") {
    ...
    return {
      ...
      plan: {
        ...
        providerId,
        planScope: teamScope,
        wireScope: providerFamily === "cloud-cn" ? teamScope : null,
      },
    };
  }
  return {
...
> {
  const [registry, activeProviderValue, jwtValue] = await Promise.all([
    ...
    deps.credentialService.load(HAWKNEXT_JWT_TOKEN_KEY),
  ]);
  const activeProvider = activeProviderValue?.trim();
  const jwt = jwtValue?.trim() ?? "";
  if ((activeProvider !== "zai" && activeProvider !== "cloud-cn") || !jwt) {
 ...
 * 解析当前选中连接的官方 MCP 凭证。
 *
 * 防竞态：Registry、动态 Account Access、active provider、hawknext JWT 与 MaaS JWT
 * 都可能在解析期间变化。这里在前后各取一次并比对，任一不一致就整轮
 * 重来，绝不拼接两代凭证——既包括"hawknext JWT 来自 ZAI 而 MaaS JWT 来自 CloudCN"（跨 family 混搭），
...
export async function resolveOfficialMcpCredentials(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    // hawknext JWT 是全局登录身份镜像；只校验 selectedKey 会把 ZAI JWT 与 CloudCN key 拼到同一请求。
...
export function createOfficialMcpAuthHeadersResolver(deps: OfficialMcpCredentialResolverDeps): {
  ...
  return {
    // request 仅为契约对齐（host handler 已在此之前完成可信校验，见 hawknextAgentService）；
