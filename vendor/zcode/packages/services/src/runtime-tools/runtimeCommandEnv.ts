...
import { buildHawkNextToolEnvPassthroughEnv, sanitizeHawkNextRuntimeEnvInPlace } from "@hawknext/shared";
...
function buildLoginShellEnvPatch(
  ...
  return {
    ...patch,
    ...buildHawkNextToolEnvPassthroughEnv(snapshot),
  };
}
...
export function buildRuntimeProcessEnvPatch(
  ...
  const toolEnvPassthroughPatch = buildHawkNextToolEnvPassthroughEnv({
    ...normalizedBaseEnv,
    ...loginShellEnvPatch,
  });
  ...
  const envPatch: Record<string, string> = {
    ...
    // encoding is set. HawkNext/Bash tool output is consumed as UTF-8, so force Python
...
export function initializeRuntimeProcessEnv(
  ...
  // 网络变量会先封存为 HAWKNEXT_TOOL_ENV_PASSTHROUGH_JSON，只有 Bash/tool 子进程边界才恢复原名。
  sanitizeHawkNextRuntimeEnvInPlace(process.env);
