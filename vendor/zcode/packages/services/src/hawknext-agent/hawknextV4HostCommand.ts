...
class HawkNextV4CommandRejectedError extends Error {
  readonly code = "HAWKNEXT_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "HawkNextV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new HawkNextV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new HawkNextV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
