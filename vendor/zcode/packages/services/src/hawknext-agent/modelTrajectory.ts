...
} from "#src/session/hawknextTaskService.js";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import {
  readTrajectoryFileTail,
  resolveModelIODirs,
  sanitizeSessionSegment,
} from "#src/hawknext-agent/modelTrajectoryFileTail.js";
import type { TrajectoryFileTail } from "#src/hawknext-agent/modelTrajectoryFileTail.js";
...
/**
 * 解析 ~/.hawknext/cli/{debug,rollout} 下的 model-io JSONL，按 sessionId 还原某个 task 的模型调用轨迹。
 *
 * 设计说明：
 * - model-io 由 adapters/model/runner-debug.ts 落盘；一个 session 一个
 *   `model-io-<sanitizedSessionId>.jsonl`。
 * - HawkNext Agent 把 taskId 当作 sessionId（见 hawknextTaskServiceAdapter），所以这里只读取
...
function mapRecord(record: Record<string, unknown>): HawkNextModelTrajectoryRecord {
  ...
  const mapped: HawkNextModelTrajectoryRecord = {
...
function mapMessages(value: unknown): HawkNextModelTrajectoryMessage[] {
...
function mapResponseToolCall(rawToolCall: unknown): HawkNextModelTrajectoryContentPart {
...
function mapUsage(value: unknown): HawkNextModelTrajectoryUsage | undefined {
