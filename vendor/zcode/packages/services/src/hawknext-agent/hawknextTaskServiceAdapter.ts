/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 HawkNext session 的协议适配。 */
...
import {
  coalesceConsecutiveHawkNextAssistants,
  ...
  generateTraceId,
  getHawkNextGoalActiveIterationCount,
  getHawkNextGoalIterationByAssistantMessageId,
  getHawkNextUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeHawkNextApiRetryStatus,
  attachHawkNextBackgroundTaskNotificationToRaw,
  collectHawkNextBackgroundTaskNotificationsByToolUseId,
  mergeHawkNextBackgroundTaskControlItems,
  parseHawkNextBackgroundTaskControlItems,
  parseHawkNextBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveHawkNextVisibleSessionTitle,
  textFromHawkNextMessageParts,
  HAWKNEXT_AGENT_PROVIDER,
  hawknextBackgroundTaskNotificationToolUpdateStatus,
  appendHawkNextStreamingToolInputDelta,
  buildHawkNextStreamingToolInputPreview,
  createHawkNextToolProjectionMemory,
  finalizeHawkNextToolProjectionInput,
  forgetHawkNextToolProjectionMetadata,
  isHawkNextModelRetryRecoveryProgressPayload,
  markHawkNextStreamingToolInputPreviewMaterialized,
  resolveHawkNextToolProjectionMetadata,
  hawknextApiRetryFromModelNetworkStatusPayload,
  hawknextApiRetryFromStreamRecoveryPayload,
  hawknextTaskNetworkDebugStatusFromPayload,
  shouldMaterializeHawkNextStreamingToolInputPreview,
  type HawkNextApiRetryStatus,
  type HawkNextAssistantMessageFeedback,
  type HawkNextBackgroundTaskNotificationInfo,
  type HawkNextBackgroundTaskControlItem,
  type HawkNextBackgroundTurnAttribution,
  type HawkNextCancelTaskCommandResult,
  type HawkNextConfigOption,
  type HawkNextEnqueueTaskCommandResult,
  type HawkNextError,
  type HawkNextGoalVerificationTimelineMeta,
  type HawkNextImportSessionsResult,
  type HawkNextImportableSessionCandidate,
  type HawkNextUsage,
  type HawkNextPersistedMessage,
  type HawkNextPersistedMessagePart,
  type HawkNextPersistedToolCall,
  type HawkNextPromptAttachment,
  type HawkNextProvider,
  type HawkNextSessionFile,
  type HawkNextTaskGoal,
  type HawkNextTaskGoalStats,
  type HawkNextTaskMode,
  type HawkNextPlanStep,
  type HawkNextSlashCommand,
  type HawkNextStreamEvent,
  type HawkNextTaskCreateResult,
  type HawkNextTaskMeta,
  type HawkNextTaskClientMode,
  type HawkNextTaskRuntimeCommand,
  type HawkNextTaskSnapshot,
  type HawkNextTaskSnapshotBody,
  type HawkNextTaskSnapshotRefContent,
  type HawkNextTaskSnapshotToolCallsSlice,
  type HawkNextTaskTokenUsageResult,
  type HawkNextTodoGroup,
  type HawkNextTurnSteerCommandKind,
  type HawkNextTurnSteerSource,
  type HawkNextWorkspaceEvent,
  type HawkNextWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  hawknextContextUsageBreakdownSchema,
  hawknextSessionSettingsStateSchema,
  type HawkNextDeliveryKind,
  type HawkNextMessagePart,
  type HawkNextMessageWithParts,
  type ModelSelection,
  type HawkNextPermissionOption,
  type HawkNextPermissionRequestParams,
  type HawkNextPermissionRequest,
  type HawkNextSessionEvent,
  type HawkNextSessionMode,
  type HawkNextSessionSettingsState,
  type HawkNextSessionStateSnapshot,
  type HawkNextStateUpdatedNotification,
  type HawkNextContextCompactionTimelineMeta,
  type HawkNextTimelineMeta,
  type HawkNextTimelineStatus,
  type HawkNextTimelineTrigger,
  type HawkNextToolProjectionMemory,
  type HawkNextUserInputRequestParams,
  type HawkNextUserInputResponse,
  type HawkNextAgentMcpServer,
} from "@hawknext/shared";
import type {
  HawkNextTaskListQuery,
  HawkNextTaskListResult,
  HawkNextWorkspaceEventSubscriptionParams,
  IHawkNextTaskService,
  HawkNextArchivedTaskDeletionResult,
  HawkNextTaskReadyOutcome,
  HawkNextTaskTerminalOutcome,
} from "../session/hawknextTaskService.js";
...
import type {
  HawkNextTaskIndexReadyEvent,
  HawkNextTaskIndexSyncer,
  HawkNextTaskIndexTerminalEvent,
} from "./hawknextTaskIndexSyncer.js";
import { readModelTrajectory } from "./modelTrajectory.js";
import { errorAttributionSchema, type CommandPayloadMap } from "@hawknext/shared/hawknext-protocol-v4";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./hawknextV4HostCommand.js";
...
import {
  ...
  getHawkNextAgentAvailableModes,
  normalizeAvailableHawkNextMode,
  settingsToConfigOptions,
} from "./hawknextConfigOptions.js";
...
interface CreateHawkNextTaskServiceAdapterOptions {
  hawknextAgentService: IHawkNextAgentService;
  ...
  taskIndexSyncer: HawkNextTaskIndexSyncer;
...
type WorkspaceEventInput = string | HawkNextWorkspaceEventSubscriptionParams;
type HawkNextSendPromptRuntimeCommand = Extract<HawkNextTaskRuntimeCommand, { type: "send_prompt" }>;
type HawkNextTerminalStreamEvent =
  | Extract<HawkNextStreamEvent, { type: "task_complete" }>
  | Extract<HawkNextStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: HawkNextProvider = HAWKNEXT_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: HawkNextSlashCommand[] = [];
const logger = createServiceLogger("hawknext-task-service");
...
interface LiveToolProjection {
  ...
  tool: HawkNextPersistedToolCall;
  toolId: string;
}
...
function formatHawkNextAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveHawkNextAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.HAWKNEXT_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(homedir(), ".hawknext", "cli", "log");
  return join(logDir, `hawknext-${formatHawkNextAgentLogDate(now)}.jsonl`);
}

export function createHawkNextTaskServiceAdapter(
  options: CreateHawkNextTaskServiceAdapterOptions,
): IHawkNextTaskService {
  const errorEmitter = new Emitter<HawkNextError>();
  const taskEmitters = new Map<string, Emitter<HawkNextStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<HawkNextStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, HawkNextTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, HawkNextApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, HawkNextBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, HawkNextToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`HawkNext task service adapter does not support IHawkNextTaskService.${name} yet.`),
      {
        code: "HAWKNEXT_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: HawkNextBackgroundTurnAttribution,
  ): HawkNextBackgroundTurnAttribution {
  ...
  async function resolveProductMcpServers(
    servers: HawkNextAgentMcpServer[] | undefined,
  ): Promise<HawkNextAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    params: {
      ...
      messageId?: string;
      content: string;
      attachments?: HawkNextPromptAttachment[];
      toolDenylist?: string[];
      clientId?: string;
      clientMode?: HawkNextTaskClientMode;
      logReason?: string;
    ...
    // HawkNext task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 HawkNext session 事件回投 HawkNext Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "HawkNext task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.hawknextAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.hawknextAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "HawkNext task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "HawkNext task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: HawkNextTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: HawkNextSendPromptRuntimeCommand,
  ): HawkNextSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: HawkNextSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: HawkNextSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "HawkNext task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: HawkNextTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is HawkNextSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "HawkNext task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "HawkNext task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "HawkNext task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<HawkNextStreamEvent, { type: "tool_call_update" }>["status"],
  ): HawkNextPersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: HawkNextStreamEvent): void {
    ...
    const nextTool: HawkNextPersistedToolCall =
  ...
  function persistedToolId(tool: HawkNextPersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: HawkNextPersistedToolCall | undefined,
    liveTool: HawkNextPersistedToolCall,
  ...
  function findToolLocation(
    messages: readonly HawkNextPersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly HawkNextPersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly HawkNextPersistedMessagePart[] | undefined,
    toolIndex: number,
  ): HawkNextPersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: HawkNextPersistedMessage,
    toolIndex: number,
    tool: HawkNextPersistedToolCall,
  ): HawkNextPersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    meta: HawkNextTaskMeta,
    messages: HawkNextPersistedMessage[],
  ): HawkNextPersistedMessage[] {
    ...
    if (changed) {
      logger.debug(undefined, "HawkNext snapshot 合并 live tool projection", {
        event: "hawknext_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`HawkNext session target is not loaded: ${taskId}`), {
        code: "HAWKNEXT_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): HawkNextToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createHawkNextToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<HawkNextWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<HawkNextStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<HawkNextStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<HawkNextStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: HawkNextStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: HawkNextTaskMeta | undefined,
    reason: HawkNextWorkspaceTaskListChanged["reason"],
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.hawknextAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.hawknextAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 HawkNextSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.hawknextAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.hawknextAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: HawkNextSessionStateSnapshot): HawkNextTaskMeta {
    const target = {
  ...
  function rememberIndexedTaskMeta(meta: HawkNextTaskMeta): HawkNextTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 HawkNext session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: HawkNextTaskMeta): Promise<HawkNextTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 HawkNext session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: HawkNextStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: HawkNextStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToHawkNext(
    snapshot: HawkNextSessionStateSnapshot,
    options?: { includeEmptyPendingElicitations?: boolean },
  ): HawkNextTaskSnapshot {
    const meta = snapshotToMeta(snapshot);
    const activeGoalIterationCount = getSnapshotGoalActiveIterationCount(snapshot);
    const goalIterationByAssistantMessageId = getHawkNextGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectHawkNextBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveHawkNextAssistants(
              getHawkNextUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: HawkNextPermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<HawkNextStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseHawkNextBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: HawkNextAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 HawkNext snapshot 到 task index 失败", error);
      });
      const snapshotEvent: HawkNextStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: HawkNextTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: HawkNextTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeHawkNextAgentServiceAndWait(): Promise<void> {
    const agentService = options.hawknextAgentService as IHawkNextAgentService & {
  ...
  const service: IHawkNextTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.hawknextAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.hawknextAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<HawkNextTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: HawkNextSessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.hawknextAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.hawknextAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.hawknextAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        // 迁移属 v4 生命周期收口。
        snapshot = await options.hawknextAgentService.createSession({
          ...target,
          sessionTraceId: createSessionTraceId(),
          mode: toHawkNextMode(params.mode),
          model: requestedSelection,
          thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<HawkNextEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: HawkNextTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<HawkNextEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<HawkNextCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "HawkNext task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "HawkNext task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "HawkNext task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "HawkNext task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "HawkNext task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.hawknextAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[hawknext-task-service] goalSession start", {
      ...
      const result = await options.hawknextAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[hawknext-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[hawknext-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.hawknextAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<HawkNextTaskMeta> {
    ...
    async listTasks(params): Promise<HawkNextTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<HawkNextTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: HawkNextTaskListQuery): Promise<HawkNextTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<HawkNextTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<HawkNextTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<HawkNextTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<HawkNextTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const hawknextSnapshot = limitTaskSnapshotMessages(
        snapshotToHawkNext(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(hawknextSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[hawknext-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(hawknextSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...hawknextSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[hawknext-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[hawknext-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[hawknext-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<HawkNextTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<HawkNextTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<HawkNextTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<HawkNextTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<HawkNextConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.hawknextAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<HawkNextSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as HawkNextAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<HawkNextImportableSessionCandidate[]> {
    ...
    async importClaudeSessions(params: {
      ...
      workspaceIdentity?: string;
      sessionIds: string[];
    }): Promise<HawkNextImportSessionsResult> {
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.hawknextAgentService.createSession({
          ...
          // 导入后的任务必须是真实 HawkNext session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toHawkNextMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<HawkNextConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.hawknextAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toHawkNextMode(params.value as HawkNextTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<HawkNextConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.hawknextAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<HawkNextConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveHawkNextAgentCurrentLogFilePath();
      // 返回 HawkNext Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // HawkNext Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[HawkNextTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<HawkNextTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 HawkNext Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.hawknextAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.hawknext-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<HawkNextArchivedTaskDeletionResult> {
      const result: HawkNextArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<HawkNextTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[HawkNextTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[HawkNextTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[HawkNextTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[HawkNextTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[HawkNextTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<HawkNextTaskMeta> {
    ...
    async setTaskUnread(params): Promise<HawkNextTaskMeta> {
    ...
    async archiveTask(params): Promise<HawkNextTaskMeta> {
    ...
    async unarchiveTask(params): Promise<HawkNextTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<HawkNextTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<HawkNextStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<HawkNextTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<HawkNextTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<HawkNextStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 HawkNextStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.hawknextAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toHawkNextDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<HawkNextWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.hawknextAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 hawknext-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeHawkNextAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: HawkNextTaskMeta, overlay: TaskOverlay): HawkNextTaskMeta {
  return {
    ...meta,
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<HawkNextTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: HawkNextTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 hawknext-cli。
...
function toHawkNextMode(mode: HawkNextTaskMode | undefined): HawkNextSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `hawknext-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<HawkNextPersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<HawkNextGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `hawknext-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `hawknext-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: HawkNextPersistedMessage[],
  timelineMessages: HawkNextPersistedMessage[],
): HawkNextPersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: HawkNextSessionStateSnapshot): number {
  ...
  return getHawkNextGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toHawkNextDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<HawkNextStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseHawkNextBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = hawknextBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachHawkNextBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: HawkNextMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, HawkNextBackgroundTaskNotificationInfo>,
): HawkNextPersistedMessage {
  const tools: HawkNextPersistedToolCall[] = [];
  const parts: HawkNextPersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: HawkNextTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly HawkNextMessagePart[],
): HawkNextPromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<HawkNextMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is HawkNextPromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<HawkNextMessagePart, { type: "file" }>,
): HawkNextPromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<HawkNextMessagePart, { type: "text" }>,
): HawkNextTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<HawkNextMessagePart, { type: "compaction" }>,
): HawkNextTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<HawkNextMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, HawkNextBackgroundTaskNotificationInfo>,
): HawkNextPersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // HawkNext Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachHawkNextBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? hawknextBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly HawkNextMessagePart[]): string {
  return textFromHawkNextMessageParts(parts);
}

function reasoningFromParts(parts: readonly HawkNextMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<HawkNextMessagePart, { type: "reasoning" }> => part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function sessionTodosToPlanSteps(
  todos: HawkNextSessionStateSnapshot["todos"],
): HawkNextPlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: HawkNextSessionStateSnapshot["goalStats"],
): HawkNextTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: HawkNextSessionStateSnapshot["todoGroups"],
): HawkNextTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = hawknextSessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableHawkNextMode(parsedSettings.data.mode.current),
      availableModes: getHawkNextAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: HawkNextToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, HawkNextBackgroundTaskControlItem[]>,
  ...
  // HawkNext Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 HawkNext prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: HawkNextStreamEvent = {
  ...
  if (event.type === "turn.completed") {
    const events: HawkNextStreamEvent[] = [];
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "HawkNext compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "HawkNext session failed",
...
): Extract<HawkNextStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isHawkNextModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendHawkNextStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeHawkNextStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildHawkNextStreamingToolInputPreview(state.rawInput);
    markHawkNextStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildHawkNextStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeHawkNextToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "HawkNext streaming tool input projected", {
    ...details,
    event: "hawknext.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<HawkNextStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<HawkNextMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): HawkNextContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<HawkNextStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveHawkNextToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeHawkNextToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: HawkNextStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // HawkNext Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 HawkNext Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetHawkNextToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // HawkNext Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: HawkNextPermissionRequestParams,
): HawkNextPermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: HawkNextSessionStateSnapshot["projection"]["pendingPermissions"][number],
): HawkNextPermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: HawkNextUserInputRequestParams,
): HawkNextStreamEvent {
  const questions =
...
): Extract<HawkNextStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: HawkNextSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<HawkNextStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: HawkNextSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<HawkNextStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, HawkNextBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: HawkNextBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): HawkNextBackgroundTaskControlItem[] | null {
  const parsedJobs = parseHawkNextBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeHawkNextBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): HawkNextPermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as HawkNextPermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<HawkNextStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = hawknextTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromHawkNextGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromHawkNextGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseHawkNextBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<HawkNextStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): HawkNextApiRetryStatus | null | undefined {
...
function recordAgentModelNetworkTelemetry(event: HawkNextSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `hawknext_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<HawkNextStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: HawkNextSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: HawkNextSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // HawkNext Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = hawknextContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): HawkNextUsage | undefined {
...
function timelineStatusValue(value: unknown): HawkNextTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): HawkNextTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): HawkNextTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): HawkNextTurnSteerCommandKind | undefined {
