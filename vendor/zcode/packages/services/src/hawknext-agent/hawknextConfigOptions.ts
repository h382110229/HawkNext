import type {
  ...
  ModelSelection,
  HawkNextSessionMode,
  HawkNextSessionSettingsState,
  HawkNextSessionStateSnapshot,
  HawkNextTaskModeInfo,
} from "@hawknext/shared";
import {
  formatModelPickerValue as formatSharedModelSelection,
  getHawkNextAgentAvailableModes as getSharedHawkNextAgentAvailableModes,
  normalizeAvailableHawkNextMode as normalizeSharedAvailableHawkNextMode,
...
function resolveTaskMetaModelSelectionFromSnapshot(
  snapshot: Pick<HawkNextSessionStateSnapshot, "messages" | "settings">,
): ModelSelection | undefined {
...
export function normalizeAvailableHawkNextMode(mode: HawkNextSessionMode): string {
  return normalizeSharedAvailableHawkNextMode(mode);
}

export function getHawkNextAgentAvailableModes(): HawkNextTaskModeInfo[] {
  return getSharedHawkNextAgentAvailableModes();
}

export function settingsToConfigOptions(settings: HawkNextSessionSettingsState): HawkNextConfigOption[] {
  // service 侧曾经维护了一份三项 mode 白名单，edit 模式上线后没有同步，
  // 切换模式时 mode_update 会把 UI 菜单覆盖成 build/plan/yolo。这里统一复用 shared 的事实源。
  return hawknextSessionSettingsToHawkNextConfigOptions(settings);
}
