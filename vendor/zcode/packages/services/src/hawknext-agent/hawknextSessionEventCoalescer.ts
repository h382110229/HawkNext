import type { HawkNextSessionEvent } from "@hawknext/shared";
import type { HawkNextAgentServiceEvent } from "#src/hawknext-agent/hawknextAgent.js";
...
type SessionServiceEvent = Extract<HawkNextAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: HawkNextAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: HawkNextSessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: HawkNextSessionEvent,
  next: HawkNextSessionEvent,
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: HawkNextAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
