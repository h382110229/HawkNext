import { resolveHawkNextAgentSpawnCwd } from "#src/hawknext-agent/hawknextAgentSpawnCwd.js";
import type { HawkNextAgentStorageStartupSnapshot } from "#src/hawknext-agent/hawknextAgent.js";
/* eslint-disable max-lines -- hawknextAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseHawkNextProcessDiagnostic,
  HAWKNEXT_AGENT_LIFECYCLE_LOG_MARKER,
  HAWKNEXT_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  HAWKNEXT_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  HAWKNEXT_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@hawknext/shared/process-diagnostic";
import {
  HAWKNEXT_AGENT_RUNTIME,
  HAWKNEXT_AGENT_PROVIDER,
  HAWKNEXT_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveHawkNextRuntimeEnv,
  sanitizeHawkNextRuntimeEnv,
} from "@hawknext/shared";
import {
  findHawkNextAgentRuntimeBinary,
  findHawkNextAgentRuntimeNodeBundle,
...
import { HawkNextProtocolClient } from "./hawknextProtocolClient.js";
import { HawkNextStdioTransport } from "./hawknextStdioTransport.js";
import { readHawkNextStdioTapDevState } from "./hawknextStdioTapDevConfig.js";
import type { HawkNextAgentPresentationSurface } from "./hawknextAgentPresentationSurface.js";
...
export interface HawkNextAgentCommand {
...
export interface HawkNextAgentCommandResolverContext {
  presentationSurface?: HawkNextAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type HawkNextAgentCommandResolver = (
  context: HawkNextAgentCommandResolverContext,
) => Promise<HawkNextAgentCommand | null> | HawkNextAgentCommand | null;

export interface HawkNextAgentProcessManagerOptions {
  commandResolver?: HawkNextAgentCommandResolver;
  presentationSurface?: HawkNextAgentPresentationSurface;
...
interface ManagedHawkNextAgentProcess {
  ...
  runtimeIdentity: HawkNextAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.HAWKNEXT_E2E_ARTIFACT_DIR?.trim();
  if (env.HAWKNEXT_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "hawknext-e2e-coverage-preload.cjs");
 ...
 */
interface HawkNextAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: HawkNextAgentRuntimeIdentity;
}

export interface HawkNextAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: HawkNextAgentRuntimeIdentity;
...
interface HawkNextAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface HawkNextAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("hawknext-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("HAWKNEXT_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildHawkNextAgentSpawnPreflight(
  command: HawkNextAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<HawkNextAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveHawkNextAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceHawkNextAgentCommand(
  context: HawkNextAgentCommandResolverContext,
): HawkNextAgentCommand | null {
  const distEntrypoint = findUpward("apps/hawknext-cli/packages/cli/dist/hawknext.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.HAWKNEXT_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "hawknext.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 hawknext-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/hawknext-cli/packages/cli/src/main.ts");
...
function resolveDeployedHawkNextAgentBinaryCommand(
  context: HawkNextAgentCommandResolverContext,
): HawkNextAgentCommand | null {
  // 旧 resolver 只识别 HAWKNEXT_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 hawknext-server.cjs 单文件部署到 ~/.hawknext/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 hawknext-agent 已经部署到 ~/.hawknext/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "HawkNext agent server command is not configured"。
  // 这里复用 findHawkNextAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.hawknext/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findHawkNextAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: HAWKNEXT_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeHawkNextAgentCommand(
  context: HawkNextAgentCommandResolverContext,
): HawkNextAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 hawknext-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 hawknext.cjs，
  ...
  const bundlePath = findHawkNextAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...HAWKNEXT_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultHawkNextAgentCommand(
  context: HawkNextAgentCommandResolverContext,
): HawkNextAgentCommand | null {
  const command = process.env.HAWKNEXT_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.HAWKNEXT_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.HAWKNEXT_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 hawknext.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceHawkNextAgentCommand(context) ??
    resolveElectronRuntimeHawkNextAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedHawkNextAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: HawkNextAgentCommand | null,
  presentationSurface: HawkNextAgentCommandResolverContext["presentationSurface"],
): HawkNextAgentCommand | null {
...
function wrapHawkNextAgentCommandWithStdioTapDevProxy(
  command: HawkNextAgentCommand,
  workspaceKey: string,
): HawkNextAgentCommand {
  const tapState = readHawkNextStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/hawknext-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("HawkNext stdio tap proxy enabled but script not found");
    return command;
  }
...
export class HawkNextAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedHawkNextAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedHawkNextAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<HawkNextProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: HawkNextAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): HawkNextAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: HawkNextAgentCommandResolver;
  private readonly presentationSurface: HawkNextAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: HawkNextAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: HawkNextAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<HawkNextAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<HawkNextAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 HawkNextAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: HawkNextAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultHawkNextAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("HawkNext agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedHawkNextAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedHawkNextAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("HawkNext agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedHawkNextAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedHawkNextAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedHawkNextAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("HawkNext agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("HawkNext agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessForShutdown(managed: ManagedHawkNextAgentProcess): Promise<void> {
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`HawkNext agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<HawkNextProtocolClient> {
    if (this.disposed) {
      throw new Error("HawkNext agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("HawkNext agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("HawkNext agent process start wait completed", {
        workspaceKey,
  ...
  getExistingClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): HawkNextProtocolClient | undefined {
    const managed = this.processesByWorkspaceKey.get(resolveWorkspaceKey(params));
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "HawkNext agent server command is not configured. Set HAWKNEXT_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("HawkNext agent process start was cancelled.");
    }
    const effectiveCommand = wrapHawkNextAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("HawkNext agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("HawkNext agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildHawkNextAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 HAWKNEXT_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 HawkNext 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveHawkNextRuntimeEnv(process.env);
    log("HawkNext agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeHawkNextRuntimeEnv(process.env),
        [HAWKNEXT_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new HawkNextStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseHawkNextProcessDiagnostic(line);
    ...
    const client = new HawkNextProtocolClient(transport, {
    ...
    const runtimeIdentity: HawkNextAgentRuntimeIdentity = {
    ...
    const managed: ManagedHawkNextAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("HawkNext agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `HawkNext agent process error${this.processLifecycleReporter?.onError ? ` ${HAWKNEXT_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("HawkNext agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `HawkNext agent process exited unexpectedly${this.processLifecycleReporter ? ` ${HAWKNEXT_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "HawkNext agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("HawkNext agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("HawkNext agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<HawkNextAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("HawkNext agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "HAWKNEXT_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("HawkNext agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("HawkNext agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，hawknext-cli 会残留为孤儿进程。
