import { resolveWorkspaceKey, HAWKNEXT_AGENT_PROVIDER } from "@hawknext/shared";
import type { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { IHawkNextAgentService, HawkNextAgentWorkspaceTarget } from "#src/hawknext-agent/hawknextAgent.js";
...
export async function repairSubagentTaskIndex(params: {
  ...
  agentService: Pick<IHawkNextAgentService, "listSessions">;
  taskIndexRepo: Pick<TaskIndexRepo, "listTaskMetas" | "updateTaskState">;
  isCurrent: () => boolean;
  onRemoved: () => void;
}): Promise<void> {
  const { target, taskIndexRepo, agentService, isCurrent } = params;
  const rows = await taskIndexRepo.listTaskMetas({ ...target, provider: HAWKNEXT_AGENT_PROVIDER });
  const ids = rows
