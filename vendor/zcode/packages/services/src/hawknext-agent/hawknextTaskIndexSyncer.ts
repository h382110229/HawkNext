...
import { repairSubagentTaskIndex } from "#src/hawknext-agent/repairSubagentTaskIndex.js";
import {
  deriveHawkNextTaskStatusFromSessionSnapshot,
  generateTraceId,
  getHawkNextUserVisibleMessages,
  isHawkNextGoalContinuationReminderText,
  isHawkNextModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@hawknext/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { HawkNextWorkspaceEventSubscriptionParams } from "#src/session/hawknextTaskListTypes.js";
import type {
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./hawknextConfigOptions.js";

const logger = createServiceLogger("hawknext-task-index-syncer");
...
export interface HawkNextTaskIndexTerminalEvent {
...
export interface HawkNextTaskIndexReadyEvent {
  target: HawkNextAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface HawkNextTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: HawkNextAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，hawknextSessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: HawkNextWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: HawkNextWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: HawkNextAgentSessionTarget, model: string): Promise<HawkNextTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: HawkNextTaskMeta | undefined,
    reason: HawkNextWorkspaceTaskListChanged["reason"],
    options?: Pick<HawkNextWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): HawkNextWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): HawkNextTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
function buildBaselineMetaFromSummary(
  target: HawkNextAgentWorkspaceTarget,
...
export function createHawkNextTaskIndexSyncer(
  options: CreateHawkNextTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<HawkNextWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<HawkNextTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<HawkNextTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === HAWKNEXT_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<HawkNextWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<HawkNextWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: HawkNextAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: HawkNextAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 hawknextSessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: HawkNextAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 hawknextSessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 hawknextSessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 hawknextSessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useHawkNextConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 HawkNextConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useHawkNextConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: HawkNextAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: HawkNextSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isHawkNextGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isHawkNextModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: HawkNextSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getHawkNextUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<HawkNextMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromHawkNextGoal(
  goal: NonNullable<HawkNextSessionStateSnapshot["projection"]["target"]>,
): HawkNextTaskGoal {
  return {
