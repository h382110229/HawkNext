...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@hawknext/rpc";
import {
...
export type HawkNextAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface HawkNextAgentV4ConnectionContext {
  connectionId: string;
  clientMode: HawkNextAgentV4ClientMode;
  role?: "terminal-client" | "trusted-host-relay";
}

const TRUSTED_CONNECTION_FIELD = "__hawknextTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__hawknextTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: HawkNextAgentV4ConnectionContext;
};

interface TrustedHawkNextAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedHawkNextAgentV4UnsubscribeRoute;
};

export function readTrustedHawkNextAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedHawkNextAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedHawkNextAgentV4Connection(
  value: unknown,
): HawkNextAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: HawkNextAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedHawkNextAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: HawkNextAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: HawkNextAgentWorkspaceTarget): HawkNextAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: HawkNextAgentWorkspaceTarget;
  connection: HawkNextAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: HawkNextAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: HawkNextAgentV4ConnectionContext): HelloMessage {
...
export interface HawkNextAgentConnectionScope {
  readonly service: IHawkNextAgentService;
...
export function createHawkNextAgentConnectionScope(
  base: IHawkNextAgentService,
  context: HawkNextAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): HawkNextAgentV4ConnectionContext => {
    const downstream = readTrustedHawkNextAgentV4Connection(params);
  ...
  const ownershipKey = (kind: SubscriptionKind, target: HawkNextAgentWorkspaceTarget, topic: string) =>
    `${kind}\0${workspaceKey(target)}\0${topic}`;
  const routedEventKey = (kind: SubscriptionKind, target: HawkNextAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: HawkNextAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies HawkNextAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: HawkNextAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedHawkNextAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedHawkNextAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: HawkNextAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: HawkNextAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: HawkNextAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IHawkNextAgentService> = {
