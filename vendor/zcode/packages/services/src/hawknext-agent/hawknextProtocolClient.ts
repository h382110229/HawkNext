import { HawkNextStorageStartupGate } from "#src/hawknext-agent/hawknextStorageStartupGate.js";
...
import type {
  ...
  HawkNextProtocolTrace,
} from "@hawknext/shared";
import type { V4Method } from "@hawknext/shared/hawknext-protocol-v4";
import type { z } from "zod";
import type { HawkNextProtocolTransport } from "./hawknextProtocolTransport.js";

/** 客户端可发的方法名：旧 hawknextProtocolMethods + v4/*（并存，收敛为 v4）。 */
type HawkNextProtocolClientMethod = HawkNextProtocolMethod | V4Method;

interface HawkNextProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface HawkNextProtocolRequestTimeoutEvent {
  method: HawkNextProtocolClientMethod;
  requestId: HawkNextProtocolRequestId;
  timeoutMs: number;
}
...
interface HawkNextProtocolClientRequestOptions {
...
const DEFAULT_HAWKNEXT_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class HawkNextProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "HawkNextProtocolClientError";
  }
}

export class HawkNextProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: HawkNextProtocolClientMethod,
    readonly requestId: HawkNextProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`HawkNext Protocol request timed out: ${method}`);
    this.name = "HawkNextProtocolRequestTimeoutError";
  }
}

export class HawkNextProtocolClient implements IDisposable {
  readonly storageStartup: HawkNextStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<HawkNextProtocolNotification>();
  private readonly requestEmitter = new Emitter<HawkNextProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<HawkNextProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "HawkNext Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: HawkNextProtocolTransport,
    options?: HawkNextProtocolClientOptions,
  ) {
    this.storageStartup = new HawkNextStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_HAWKNEXT_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`HawkNext agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new HawkNextProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: HawkNextProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: HawkNextProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: HawkNextProtocolRequestId; result: unknown }).id,
    ...
    if ("error" in message && "id" in message) {
      ...
      this.rejectResponse(
        errorMessage.id,
        new HawkNextProtocolClientError(
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as HawkNextProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as HawkNextProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as HawkNextProtocolNotification);
    }
  }

  private resolveResponse(id: HawkNextProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`HawkNext Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: HawkNextProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("HawkNext Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("HawkNext Protocol client is disposed");
    }
  }
}
