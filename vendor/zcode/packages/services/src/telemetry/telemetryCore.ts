...
function sessionCreateEventId(userId: string, sessionId: string): string {
  const bytes = createHash("sha256")
    .update(JSON.stringify(["hawknext:session_create:v1", userId, sessionId]))
    .digest()
    .subarray(0, 16);
...
function resolveTelemetryStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".hawknext", "v2", "telemetry-state.json");
  }
...
function resolveTelemetryLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".hawknext", "v2", "telemetry-state.lock");
  }
...
export function createTelemetryCore(dependencies: TelemetryCoreDependencies = {}) {
  ...
  const appVersion = dependencies.appVersion ?? HAWKNEXT_VERSION;
  ...
  async function sendReport(
    ...
    if (!HAWKNEXT_TELEMETRY_ENABLED || !HAWKNEXT_TELEMETRY_REPORT_ENDPOINT) {
      return;
    }
    ...
    const endpoint = String(
      rewriteHawkNextEndpointUrl(
        HAWKNEXT_TELEMETRY_REPORT_ENDPOINT,
        (await dependencies.resolveHawkNextEndpointOrigin?.()) ?? HAWKNEXT_TELEMETRY_REPORT_ENDPOINT,
      ),
    );

    const headers = buildHawkNextSourceHeadersFromContext({
      appVersion,
      platform,
      arch: dependencies.arch ?? process.arch,
      osVersion,
      releaseChannel: dependencies.releaseChannel ?? HAWKNEXT_ENV,
