...
import { DATA_BASE_DIR_FORBIDDEN_WINDOWS_INSTALL_DIR_ERROR_CODE } from "@hawknext/shared";

let _dataBaseDir: string | null = null;
export const HAWKNEXT_WINDOWS_APP_INSTALL_DIR_ENV = "HAWKNEXT_WINDOWS_APP_INSTALL_DIR";
const envDataBaseDir = process.env.HAWKNEXT_DATA_BASE_DIR?.trim() || null;
...
/** Get the current base directory. Priority: setDataBaseDir() > env HAWKNEXT_DATA_BASE_DIR > homedir(). */
...
/** {dataBaseDir}/.hawknext */
export function getHawkNextDataRootDir(): string {
  return join(getDataBaseDir(), ".hawknext");
}

/** 非项目对话共享的真实工作目录；默认 ~/.hawknext/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getHawkNextDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.hawknext/v2 */
export function getAppConfigDir(): string {
  return join(getHawkNextDataRootDir(), "v2");
}
...
function collectWindowsForbiddenAppInstallDirs(
  ...
  const candidates = [
    ...
    readEnvValue(env, HAWKNEXT_WINDOWS_APP_INSTALL_DIR_ENV),
    programFiles ? win32.join(programFiles, "HawkNext") : null,
    programFilesX86 ? win32.join(programFilesX86, "HawkNext") : null,
    programW6432 ? win32.join(programW6432, "HawkNext") : null,
    localAppData ? win32.join(localAppData, "Programs", "HawkNext") : null,
  ];
...
export function getExportLogStageDir(): string {
  return join(getHawkNextDataRootDir(), "export-log-stage");
}

export function getExportLogDir(): string {
  return join(getHawkNextDataRootDir(), "export-log");
}

export function getFeedbackRootDir(): string {
  return join(getHawkNextDataRootDir(), "feedback");
}
...
export function getGitCheckpointIndexRootDir(): string {
  return join(getHawkNextDataRootDir(), "git-checkpoint-index");
}

/** ~/.hawknext/v2/tasks-index.sqlite */
...
/** 与 HawkNext session 持久化一致：使用 workspaceKey 的 SHA-256 前 12 位 */
...
/** ~/.hawknext/v2/sessions/{workspaceHash} */
...
/** ~/.hawknext/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.hawknext/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .hawknext/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".hawknext", "v2");
  const newDir = join(newBaseDir, ".hawknext", "v2");
