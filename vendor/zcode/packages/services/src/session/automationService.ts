import type {
  HawkNextAutomation,
  HawkNextAutomationCreateParams,
  HawkNextAutomationRun,
  HawkNextAutomationScheduleRule,
  HawkNextAutomationUpdateParams,
} from "@hawknext/shared";
import { resolveWorkspaceKey } from "@hawknext/shared";
...
function assertValidAutomationScheduleRule(rule: HawkNextAutomationScheduleRule): void {
...
export class AutomationService {
  ...
  async create(params: HawkNextAutomationCreateParams): Promise<HawkNextAutomation> {
  ...
  async update(
    ...
    // 同时顺手修复历史上已经存在的同类脏数据。
    const normalizedParams: HawkNextAutomationUpdateParams = hasIntervalCarrier
    ...
    const options: {
      ...
      lifecycleStatus?: HawkNextAutomation["lifecycleStatus"];
      resetRetry?: boolean;
    } = {};
    ...
    const effectiveDirectScheduleRule = hasDirectScheduleRule
      ? directScheduleRule
      : carrierScheduleRule;
    const normalizedWithSchedule: HawkNextAutomationUpdateParams =
  ...
  async runNow(
    automationId: string,
    scope?: AutomationWorkspaceScope,
  ): Promise<{ automation: HawkNextAutomation; run: HawkNextAutomationRun } | null> {
    const workspaceKey = resolveScopeKey(scope);
    const existing = await this.repo.get(automationId, workspaceKey);
    if (!existing) return null;
