...
function appendHawkNextAgentIndexedProviderFilter(
  where: string[],
...
interface TaskIndexStatePatch {
  ...
  unreadAt?: number;
  model?: string;
  status?: HawkNextTaskMeta["status"];
  lastError?: HawkNextTaskMeta["lastError"];
  target?: HawkNextTaskMeta["target"];
  updatedAt?: number;
}
...
const GROUPED_WORKSPACE_BOOTSTRAP_ONCE_KEY = "__hawknext_internal_grouped_workspace_bootstrap_once__";
const DEFAULT_TASK_GROUP_COLOR: HawkNextTaskGroupColor = "gray";
const WORKSPACE_BOOTSTRAP_TASK_GROUP_COLORS = [
  ...
  "blue",
  "purple",
] satisfies HawkNextTaskGroupColor[];
...
function isTerminalTaskStatus(status: HawkNextTaskMeta["status"]): boolean {
  return status === "completed" || status === "error";
}

function shouldPreserveNewerTerminalStatus(
  existingMeta: HawkNextTaskMeta | null,
  incomingMeta: HawkNextTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): HawkNextTaskMeta {
  const workspaceIdentity = resolveTaskIndexRowWorkspaceIdentity(row);
  try {
    const parsed = hawknextTaskMetaSchema.safeParse(JSON.parse(row.meta_json));
    if (parsed.success) {
      return {
        ...(parsed.data as HawkNextTaskMeta),
  ...
  return {
    ...
    mode: row.mode as HawkNextTaskMeta["mode"],
    model: row.model ?? undefined,
    provider: row.provider === HAWKNEXT_AGENT_PROVIDER ? HAWKNEXT_AGENT_PROVIDER : undefined,
    migrationSource: (row.migration_source as HawkNextTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as HawkNextTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: HawkNextTaskMeta): string {
...
function normalizeWorkspaceBootstrapScopes(
  scopes: Array<{
    workspacePath: string;
    workspaceIdentity?: string;
    workspacePurpose?: import("@hawknext/shared").WorkspacePurpose;
  }>,
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): HawkNextTaskListItem {
...
function isTaskGroupColor(value: string): value is HawkNextTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): HawkNextTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): HawkNextTaskGroupColor {
...
function groupedTopNodeOrderRef(node: HawkNextGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareGroupedNodes(
  left: HawkNextGroupedTaskViewNode,
  right: HawkNextGroupedTaskViewNode,
): number {
...
function compareCronGroupTasks(left: HawkNextTaskListItem, right: HawkNextTaskListItem): number {
...
function compareGroupTasks(
  left: HawkNextTaskListItem,
  right: HawkNextTaskListItem,
...
export class TaskIndexRepo {
  ...
  private normalizeGroupedTopNodeOrders(
    nodes: HawkNextGroupedTaskViewNode[],
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendHawkNextAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): HawkNextTaskMeta {
  ...
  async syncTaskMetaAtGroupedTop(params: {
    ...
    titleOverridden?: boolean;
    searchableText?: string;
  }): Promise<{ meta: HawkNextTaskMeta; initializedGroupedOrder: boolean }> {
      ...
      try {
        ...
        const meta: HawkNextTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 HawkNext session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 HawkNext 任务。
   ...
   */
  private ensureCronGroupMembership(meta: HawkNextTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<HawkNextTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: HawkNextTaskMeta): Promise<HawkNextTaskMeta> {
    await this.ensureReady();
  ...
  async clearTaskUnreadIfMatches(params: {
  ...
  }): Promise<{ meta: HawkNextTaskMeta; cleared: boolean }> {
    await this.ensureReady();
    return this.enqueueWrite(params, () => {
      ...
      try {
        ...
        const nextMeta: HawkNextTaskMeta = {
          ...current,
          unreadAt: undefined,
        };
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<HawkNextTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async queryTaskList(
    params: HawkNextTaskListQuery & { provider?: HawkNextProvider },
  ): Promise<HawkNextTaskListResult> {
    await this.ensureReady();
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<HawkNextTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: HawkNextGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: HawkNextGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    params: HawkNextGroupedTaskViewQuery & { provider?: HawkNextProvider },
  ): Promise<HawkNextGroupedTaskView> {
    await this.ensureReady();
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 HawkNext Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendHawkNextAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
  ...
  async queryGroupedTaskViewStructure(params: {
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: HawkNextGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
    ...
    const orderRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupViewNodeOrderRow[];
    const topLevelOrders: HawkNextGroupedTaskViewStructureTopOrder[] = [];
  ...
  async applyGroupedTaskViewOrder(
    params: HawkNextGroupedTaskViewOrderInput & { provider?: HawkNextProvider },
  ): Promise<HawkNextGroupedTaskView> {
    await this.ensureReady();
    ...
    const validateTaskRef = (task: HawkNextGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 HawkNext Agent 视图只接受当前 glm task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: HawkNextGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: HawkNextGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: HawkNextGroupedTaskRef[] = [];
