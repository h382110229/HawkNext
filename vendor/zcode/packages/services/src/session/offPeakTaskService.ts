...
import {
  ...
  type HawkNextOffPeakTaskCreateParams,
...
import type { ModelSelection, ModelSelectionValidation } from "@hawknext/provider";
...
function isValidCreateParams(params: HawkNextOffPeakTaskCreateParams): boolean {
  return (
...
> {
  ...
  if (failureStage === "client_validation") {
    ...
    errorCode = error.bihawkNext === undefined ? "" : String(error.bihawkNext);
    if (error.bihawkNext === 3101) errorCategory = "eligibility_3101";
    else if (error.bihawkNext === 3103) errorCategory = "quota_3103";
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: HawkNextOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: HawkNextOffPeakTaskCreateParams = {
      ...params,
    ...
    let created: HawkNextOffPeakTask;
    try {
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null> {
  ...
  ): Promise<HawkNextOffPeakTask | null> {
    const existing = await this.deps.repo.get(offPeakTaskId);
  ...
  async list(): Promise<HawkNextOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null> {
  ...
  ): Promise<HawkNextOffPeakTask[]> {
    const repaired: HawkNextOffPeakTask[] = [];
  ...
  private async settleOne(task: HawkNextOffPeakTask): Promise<void> {
