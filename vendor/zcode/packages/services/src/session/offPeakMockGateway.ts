...
/* off-peak 进程内 mock 网关。
   ...
   与联调完全一致，联调时删掉 HAWKNEXT_OFFPEAK_MOCK 开关即可。
...
export function isOffPeakMockEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env["HAWKNEXT_OFFPEAK_MOCK"] === "1";
}
...
function readEnvOptions(env: NodeJS.ProcessEnv): OffPeakMockGatewayOptions {
  ...
  const scenarioValue = env["HAWKNEXT_OFFPEAK_MOCK_SCENARIO"];
  const scenario =
    ...
    scenarioValue === "invalid-ticket"
      ...
      : undefined;
  return {
    ...(num("HAWKNEXT_OFFPEAK_MOCK_READY_DELAY_MS") !== undefined
      ? { readyDelayMs: num("HAWKNEXT_OFFPEAK_MOCK_READY_DELAY_MS") }
      : {}),
    ...(num("HAWKNEXT_OFFPEAK_MOCK_READY_TTL_MS") !== undefined
      ? { readyTtlMs: num("HAWKNEXT_OFFPEAK_MOCK_READY_TTL_MS") }
      : {}),
    ...(num("HAWKNEXT_OFFPEAK_MOCK_ACTIVE_MS") !== undefined
      ? { activeMs: num("HAWKNEXT_OFFPEAK_MOCK_ACTIVE_MS") }
      : {}),
    ...(num("HAWKNEXT_OFFPEAK_MOCK_QUEUE_429_COUNT") !== undefined
      ? { queue429Count: num("HAWKNEXT_OFFPEAK_MOCK_QUEUE_429_COUNT") }
      : {}),
    ...(num("HAWKNEXT_OFFPEAK_MOCK_QUOTA_COUNT") !== undefined
      ? { quotaExhaustedCount: num("HAWKNEXT_OFFPEAK_MOCK_QUOTA_COUNT") }
      : {}),
    ...(num("HAWKNEXT_OFFPEAK_MOCK_AVAILABILITY_BLOCK_MS") !== undefined
      ? {
          availabilityBlockedMs: num("HAWKNEXT_OFFPEAK_MOCK_AVAILABILITY_BLOCK_MS"),
        }
      : {}),
    ...(num("HAWKNEXT_OFFPEAK_MOCK_RETRY_AFTER_S") !== undefined
      ? { retryAfterS: num("HAWKNEXT_OFFPEAK_MOCK_RETRY_AFTER_S") }
      : {}),
    ...(num("HAWKNEXT_OFFPEAK_MOCK_NEXT_POLL_S") !== undefined
      ? { nextPollS: num("HAWKNEXT_OFFPEAK_MOCK_NEXT_POLL_S") }
      : {}),
    ...(scenario ? { scenario } : {}),
  };
}
...
/** 固定端口（可用 HAWKNEXT_OFFPEAK_MOCK_PORT 覆盖）：多窗口多 host 时共享同一份 mock 票据状态。 */
...
export async function startOffPeakMockGateway(
  ...
  async function handleMessages(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (config.scenario === "foreground-subagents" || config.scenario === "capture") {
      ...
      capturedRequests.push({
        ...
        stage,
        ticketId,
        ...(typeof req.headers["cloud-cn-organization"] === "string"
          ? { organization: req.headers["cloud-cn-organization"] }
          : {}),
        ...(typeof req.headers["cloud-cn-project"] === "string"
          ? { project: req.headers["cloud-cn-project"] }
          : {}),
      });
      const scripted =
  ...
  const requestedPort =
    options?.port ??
    (Number(process.env["HAWKNEXT_OFFPEAK_MOCK_PORT"]) > 0
      ? Number(process.env["HAWKNEXT_OFFPEAK_MOCK_PORT"])
      : DEFAULT_MOCK_PORT);
  try {
