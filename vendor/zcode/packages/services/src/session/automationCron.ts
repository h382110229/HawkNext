...
import type { HawkNextAutomation, HawkNextAutomationScheduleRule } from "@hawknext/shared";
...
export function scheduleRuleDefinition(rule: HawkNextAutomationScheduleRule): string {
...
export function isOneShotAutomation(
  automation: Pick<HawkNextAutomation, "recurring" | "maxRuns">,
): boolean {
...
export function buildRelativeDelaySchedule(
  delayMinutes: number,
  from = Date.now(),
): Pick<HawkNextAutomation, "cronExpr" | "scheduleRule"> {
...
export function computeInitialAutomationNextRunAt(
  automation: Pick<HawkNextAutomation, "cronExpr" | "recurring" | "scheduleRule">,
  from = Date.now(),
): number | null {
...
export function computeScheduleRuleNextRunAt(
  rule: HawkNextAutomationScheduleRule,
  from = Date.now(),
): number | null {
...
export function inferMinuteIntervalScheduleRule(
  cronExpr: string,
  anchorAt: number,
): HawkNextAutomationScheduleRule | undefined {
...
export function buildIntervalScheduleRule(
  intervalUnit: HawkNextAutomationScheduleRule["unit"],
  interval: number,
  cronExpr: string,
  anchorAt: number,
...
export function computeAutomationNextRunAt(
  automation: Pick<HawkNextAutomation, "cronExpr" | "scheduleRule">,
  from?: number,
): number | null {
