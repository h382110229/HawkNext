/* eslint-disable max-lines -- HawkNext task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@hawknext/rpc";
import { ServiceChannels } from "@hawknext/shared";
import type { CommandPayloadMap } from "@hawknext/shared/hawknext-protocol-v4";
import { createServiceDescriptor } from "#src/descriptors.js";
import type {
  HawkNextImportSessionsResult,
  HawkNextImportableSessionCandidate,
  HawkNextSessionCompactResult,
  HawkNextSessionGoalAction,
  HawkNextSessionGoalResult,
  HawkNextTaskCreateResult,
  HawkNextTaskMeta,
  HawkNextStreamEvent,
  HawkNextTaskMode,
  HawkNextConfigOption,
  HawkNextError,
  HawkNextAssistantMessageFeedback,
  HawkNextPromptAttachment,
  HawkNextProvider,
  HawkNextWorkspaceEvent,
  TraceId,
  HawkNextSessionFile,
  HawkNextAgentMcpServer,
  HawkNextTaskSnapshot,
  HawkNextTaskSnapshotBody,
  HawkNextTaskSnapshotToolCallsSlice,
  HawkNextTaskSnapshotRefContent,
  HawkNextEnqueueTaskCommandResult,
  HawkNextCancelTaskCommandResult,
  HawkNextTaskClientMode,
  HawkNextTaskTokenUsageResult,
...
import type {
  ...
  HawkNextTaskListSortBy,
  HawkNextTaskListWorkspaceScope,
  HawkNextWorkspaceEventSubscriptionParams,
  HawkNextGroupedTaskView,
  HawkNextGroupedTaskViewOrderInput,
  HawkNextGroupedTaskViewQuery,
  HawkNextGroupedTaskViewStructure,
  HawkNextTaskGroup,
  HawkNextTaskGroupColor,
} from "#src/session/hawknextTaskListTypes.js";

export interface HawkNextTaskSnapshotWithEtagResult {
  snapshot: HawkNextTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface HawkNextArchivedTaskDeletionResult {
...
export type HawkNextModelTrajectoryContentPart =
...
export interface HawkNextModelTrajectoryMessage {
  role: string;
  parts: HawkNextModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface HawkNextModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type HawkNextModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface HawkNextModelTrajectoryCallSource {
  kind: HawkNextModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface HawkNextModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: HawkNextModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: HawkNextModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: HawkNextModelTrajectoryContentPart[];
    usage?: HawkNextModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface HawkNextModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 HawkNext Agent 落盘 model-io）。 */
  available: boolean;
  records: HawkNextModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export type {
  HawkNextTaskListKind,
  HawkNextTaskListQuery,
  HawkNextTaskListResult,
  HawkNextTaskListSortBy,
  HawkNextTaskListWorkspaceScope,
  HawkNextWorkspaceEventSubscriptionParams,
  HawkNextGroupedTaskRef,
  HawkNextGroupedTaskView,
  HawkNextGroupedTaskViewNode,
  HawkNextGroupedTaskViewOrderInput,
  HawkNextGroupedTaskViewQuery,
  HawkNextGroupedTaskViewStructure,
  HawkNextGroupedTaskViewStructureMember,
  HawkNextGroupedTaskViewStructureTopOrder,
  HawkNextGroupedTaskViewTopLevelNodeRef,
  HawkNextTaskGroup,
  HawkNextTaskGroupColor,
} from "#src/session/hawknextTaskListTypes.js";

/** 一个 task 输入轮次的终态结果，供后台派发（定时任务）回写运行结果。 */
export interface HawkNextTaskTerminalOutcome {
  taskId: string;
...
export interface HawkNextTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IHawkNextTaskService — HawkNext task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * HawkNext Agent server 维护，新功能应优先走 IHawkNextSessionService。
 */
export interface IHawkNextTaskService {
  // ---- 生命周期 ----
  ...
  /** 创建 HawkNext session 并同步 task 索引。 */
  createTask(params: {
    workspacePath: string;
    workspaceIdentity?: string;
    provider?: HawkNextProvider;
  ...
  }): Promise<HawkNextTaskCreateResult>;
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<HawkNextEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: HawkNextTaskClientMode;
  }): Promise<HawkNextCancelTaskCommandResult>;
  ...
  }): Promise<HawkNextSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<HawkNextSessionGoalResult>;
   ...
   */
  listTaskList(params: HawkNextTaskListQuery): Promise<HawkNextTaskListResult>;

  /** 创建最小 task group；完整 delete 后续由 group 管理功能补齐 */
  createTaskGroup(params?: {
    title?: string;
    color?: HawkNextTaskGroupColor;
  }): Promise<HawkNextTaskGroup>;

  /** 重命名 task group；workspaceScopes 只用于通知当前可见 grouped 视图刷新 */
  renameTaskGroup(params: {
    groupId: string;
    title: string;
    workspaceScopes?: HawkNextTaskListWorkspaceScope[];
  }): Promise<HawkNextTaskGroup>;
  ...
  listGroupedTaskViewStructure(params: {
    workspaceScopes: HawkNextTaskListWorkspaceScope[];
  }): Promise<HawkNextGroupedTaskViewStructure>;

  /** 一次性提交 grouped 视图最终排序和 membership，服务层用 sqlite transaction 落库 */
  applyGroupedTaskViewOrder(params: HawkNextGroupedTaskViewOrderInput): Promise<HawkNextGroupedTaskView>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<HawkNextTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<HawkNextTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<HawkNextTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<HawkNextTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<HawkNextTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskMeta(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<HawkNextTaskMeta | null>;

  /** 读取当前 active task 内存态的配置选项，不做 workspace 预热兜底 */
  getTaskConfigOptions(params: { taskId: string }): Promise<HawkNextConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: HawkNextAssistantMessageFeedback | null;
  }): Promise<HawkNextSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: HawkNextTaskMode }): Promise<void>;
  ...
  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.hawknext/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 HawkNext Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<HawkNextModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<HawkNextTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 HawkNext Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  deleteArchivedTasks(params: {
    ...
    workspaceIdentity?: string;
    taskIds: string[];
  }): Promise<HawkNextArchivedTaskDeletionResult>;
  ...
  onDynamicWorkspaceEvent(
    workspace: string | HawkNextWorkspaceEventSubscriptionParams,
  ): Event<HawkNextWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<HawkNextError>;
}

export const IHawkNextTaskService = createServiceDescriptor<IHawkNextTaskService>(
  ServiceChannels.HawkNextTask,
);
