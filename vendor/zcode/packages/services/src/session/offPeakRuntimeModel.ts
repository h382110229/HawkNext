...
export class OffPeakCredentialsUnavailableError extends OffPeakPermanentDispatchError {
  constructor(readonly missing: "jwt" | "codingPlanApiKey") {
    super(
      missing === "jwt"
        ? "off-peak requires hawknext login (jwt missing)"
...
export interface OffPeakCredentialSnapshot {
  ...
  providerFamily: "zai" | "cloud-cn";
  providerId: string;
  /** CloudCN Team 组织/项目身份；仅两者同时存在时才允许进入请求头。 */
...
interface OffPeakCredentialResolverDeps {
  ...
  resolveAccountProvider(): Promise<{
    ...
    readonly access: HawkNextAccountAccess;
...
function resolveSelectedOffPeakCodingPlan(
  ...
  if (access.planKind === "individual-coding-plan") {
    return {
      kind: access.family === "zai" ? "zai-personal" : "cloud-cn-personal",
  ...
  return {
    kind: access.family === "zai" ? "zai-team" : "cloud-cn-team",
    providerFamily: access.family,
    providerId,
 ...
 *
 * 派发凭据必须与 UI 选中的 Coding Plan 保持一致，避免 ZAI/Team 任务误用个人 CloudCN key。
...
export async function resolveOffPeakCredentials(
  ...
  if (options.allowMockCredentials !== false && env["HAWKNEXT_OFFPEAK_MOCK"] === "1") {
    if (env["HAWKNEXT_OFFPEAK_MOCK_NO_PLAN"] === "1") {
      throw new OffPeakCodingPlanUnavailableError("connection_unavailable");
    }
    ...
    return {
      ...
      kind: "cloud-cn-personal",
      providerFamily: "cloud-cn",
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (activeProvider !== selection.providerFamily) {
      // hawknext JWT 是当前 App 登录身份的全局镜像；只校验 selectedKey 会把
      // ZAI JWT 与 CloudCN key（或反向）拼到同一请求，服务端只能在取号时才拒绝。
      throw new OffPeakCodingPlanUnavailableError("provider_identity_mismatch");
    }
    const jwt = (await deps.credentialService.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim() ?? "";
    if (!jwt) {
...
/**
 * CloudCN Team 的组织/项目必须和鉴权凭证来自同一次 selected connection 解析。
 *
 * Off-Peak 过去只传 JWT 与 Coding Plan key，服务端没有 CloudCN access key，
...
export function buildOffPeakPlanIdentityHeaders(
  ...
  if (credentials.kind !== "cloud-cn-team") {
    return {};
  }
  ...
  return {
    "cloud-cn-organization": organizationId,
    "cloud-cn-project": projectId,
  };
}
 ...
 * 复用同一份票据状态），真实模式指向 hawknext API origin。node 服务装配与 host 派发两侧
      ...
      if (!originPromise) {
        originPromise = (async () => {
          if (!isOffPeakMockEnabled(env)) {
            return new URL(buildRuntimeHawkNextApiUrl(env, "/")).origin;
          }
