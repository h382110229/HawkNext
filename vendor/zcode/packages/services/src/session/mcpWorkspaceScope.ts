...
import type { HawkNextAgentMcpServer } from "@hawknext/shared";
...
function isFilesystemServer(
  server: HawkNextAgentMcpServer,
): server is Extract<HawkNextAgentMcpServer, { command: string }> {
  return (
    "command" in server &&
...
export function appendWorkspaceToFilesystemMcpServers(
  mcpServers: HawkNextAgentMcpServer[] | undefined,
  workspacePath: string,
): HawkNextAgentMcpServer[] | undefined {
