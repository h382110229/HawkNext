...
import {
  ...
  hawknextTaskModeSchema,
  type HawkNextAutomation,
  type HawkNextAutomationCreateParams,
  type HawkNextAutomationDispatchStatus,
  type HawkNextAutomationLifecycleStatus,
  type ModelSelection,
  type HawkNextAutomationRun,
  type HawkNextAutomationRunDispatchStatus,
  type HawkNextAutomationRunOutcome,
  type HawkNextAutomationTrigger,
  type HawkNextAutomationUpdateParams,
...
function rowToAutomation(row: AutomationRow): HawkNextAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as HawkNextAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as HawkNextAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as HawkNextAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): HawkNextAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: HawkNextAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): HawkNextAutomation["mode"] | undefined {
  const parsed = hawknextTaskModeSchema.safeParse(mode);
  return parsed.success ? parsed.data : undefined;
}

function assertValidAutomationMode(mode: unknown): void {
  if (mode === undefined || mode === null) return;
  if (!hawknextTaskModeSchema.safeParse(mode).success) {
    // 读取兼容历史脏数据不代表允许继续写脏数据；Repo 是绕过 RPC 时的最终持久化边界。
    throw new Error(`Invalid automation mode: ${String(mode)}`);
  }
}

function rowToRun(row: AutomationRunRow): HawkNextAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as HawkNextAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as HawkNextAutomationRunDispatchStatus,
    outcome: (row.outcome as HawkNextAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  // 互相覆盖全局值，导致 repo 与裸 SQL 操作在并发窗口内写进真实库 ~/.hawknext/v2（历史脏数据
  ...
  async create(
    params: HawkNextAutomationCreateParams,
    options: { nextRunAt: number | null; lifecycleStatus?: HawkNextAutomationLifecycleStatus },
  ): Promise<HawkNextAutomation> {
  ...
  async get(automationId: string, workspaceKey?: string): Promise<HawkNextAutomation | null> {
    await this.ensureReady();
    const row = this.getRow(automationId, workspaceKey);
  ...
  async update(
    automationId: string,
    params: HawkNextAutomationUpdateParams,
    options?: {
      nextRunAt?: number | null;
      lifecycleStatus?: HawkNextAutomationLifecycleStatus;
      resetRetry?: boolean;
    },
    workspaceKey?: string,
  ): Promise<HawkNextAutomation | null> {
    assertValidAutomationMode(params.mode);
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<HawkNextAutomation[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: HawkNextAutomation[] = [];
  ...
  async ensureRunClaimed(params: {
    ...
    workspaceKey: string;
    scheduledAt: number | null;
    trigger: HawkNextAutomationTrigger;
  }): Promise<void> {
    await this.ensureReady();
    const now = Date.now();
    this.getDatabase()
  ...
  async markRunDispatch(params: {
    runId: string;
    dispatchStatus: HawkNextAutomationRunDispatchStatus;
  ...
  async listRuns(automationId: string, workspaceKey?: string): Promise<HawkNextAutomationRun[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
  ...
  async getRun(runId: string): Promise<HawkNextAutomationRun | null> {
    await this.ensureReady();
    const row = this.getDatabase()
