...
interface ImportedClaudeHistoryRepairResult {
  ...
  createdAt?: number;
  updatedAt?: number;
  messages: HawkNextSessionImportMessage[];
...
interface ImportedClaudeSessionRepairCreateParams {
  ...
  workspaceIdentity?: string;
  sessionId: string;
  mode: HawkNextSessionStateSnapshot["session"]["mode"];
  model: HawkNextSessionStateSnapshot["settings"]["model"]["current"];
  thoughtLevel?: string;
...
function countAssistantMessages(messages: readonly HawkNextSessionImportMessage[]): number {
  return messages.filter((message) => message.role === "assistant").length;
}

function shouldRepairImportedClaudeSnapshot(
  snapshot: Pick<HawkNextSessionStateSnapshot, "messages" | "runtime" | "session">,
): boolean {
  ...
  if (!snapshot.session.sessionId.startsWith("claude-import-") && !hasLegacyFixedMessageIds) {
    ...
    // 避免普通 HawkNext session 被同名 legacy 备份误回填成 Claude 历史。
    return false;
  }
...
export async function repairImportedClaudeSessionSnapshot<T>(params: {
  ...
  // 早期导入可能已经创建了真实 HawkNext session，但没有把 Claude 历史写入
  // hawknext-cli sessionStore，或用了全局 msg_import_* 导致串会话。这里统一用同名 sessionId
