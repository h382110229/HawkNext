...
import type { HawkNextImportSessionsResult, HawkNextTaskMeta } from "@hawknext/shared";
...
export async function importClaudeNativeSessions(params: {
  ...
  sessionIds: string[];
  createImportedSession?: (source: ClaudeNativeImportedSessionSource) => Promise<HawkNextTaskMeta>;
  onTaskImported: (meta: HawkNextTaskMeta) => void;
  ...
  const result: HawkNextImportSessionsResult = {
    imported: [],
    skipped: [],
    failed: [],
  };
