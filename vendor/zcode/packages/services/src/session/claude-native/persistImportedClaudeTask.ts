...
export function buildSearchableTextFromMessages(messages: HawkNextSessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function persistImportedClaudeTask(params: {
  ...
  const indexMeta: HawkNextTaskMeta = {
    ...meta,
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.hawknext/v2/sessions/{hash}/{taskId}.json。现在真实 HawkNext session 承担续聊，legacy snapshot
