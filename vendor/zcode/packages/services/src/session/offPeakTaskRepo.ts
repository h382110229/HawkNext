...
/* off-peak 任务仓库：off_peak_tasks 的 sqlite schema、状态机守卫写入与调度认领。
   ...
   禁止往 automations 表或 HawkNextAutomation 类型上加字段。 */
...
import {
  ...
  type HawkNextOffPeakTaskStatus,
...
function rowToTask(row: OffPeakTaskRow): HawkNextOffPeakTask {
  ...
  return {
    ...
    prompt: row.prompt,
    permissionMode: row.permission_mode as HawkNextTaskMode,
    ...
    status: row.status as HawkNextOffPeakTaskStatus,
...
function readOffPeakModelSelection(row: OffPeakTaskRow): HawkNextOffPeakTask["modelSelection"] | null {
...
function serializeOffPeakModelSelection(
  selection: NonNullable<HawkNextOffPeakTask["modelSelection"]>,
): string {
...
export class OffPeakTaskRepo {
   ...
   */
  async create(
    params: HawkNextOffPeakTaskCreateParams,
    options?: {
      /** 测试注入用；缺省 Date.now()。 */
      now?: number;
  ...
  async invalidateModelSelection(
    offPeakTaskId: string,
    modelSelection: NonNullable<HawkNextOffPeakTask["modelSelection"]>,
    options?: { now?: number },
  ...
  async updateEditableFields(
    ...
    params: {
      ...
      modelSelection?: HawkNextOffPeakTask["modelSelection"] | null;
    },
    options?: { now?: number },
  ): Promise<HawkNextOffPeakTask | null> {
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<HawkNextOffPeakTask[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: HawkNextOffPeakTask[] = [];
  ...
  async listNonTerminal(): Promise<HawkNextOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
  ...
  async listUnsettledTerminal(): Promise<HawkNextOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
