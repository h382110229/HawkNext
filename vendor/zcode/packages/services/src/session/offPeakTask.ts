import type {
  ...
  HawkNextOffPeakTask,
  HawkNextOffPeakTaskCreateParams,
  ModelSelection,
...
export interface IOffPeakTaskService {
  ...
  createTask(params: HawkNextOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<HawkNextOffPeakTask | null>;
  list(): Promise<HawkNextOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<HawkNextOffPeakTask | null>;
}
