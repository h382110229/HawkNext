...
const providerNames: Readonly<Record<string, string>> = {
  "builtin:cloud-cn": "cloud-cn-api",
  "builtin:zai": "zai-api",
  "builtin:cloud-cn-start-plan": "account:cloud-cn-start-plan",
  "builtin:zai-start-plan": "account:zai-start-plan",
  "builtin:cloud-cn-coding-plan": "account:cloud-cn-individual-coding-plan",
...
function decodeLegacySelection(row: LegacySelectionRow): ModelSelection | undefined {
  ...
  } else if (["glm", "hawknext"].includes(provider)) {
    // 旧 provider=glm/hawknext 是执行后端，不是供应商身份。
    return undefined;
  }
