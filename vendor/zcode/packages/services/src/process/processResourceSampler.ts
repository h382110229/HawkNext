...
export interface HostResourceUsageAgent {
  ...
  children: readonly HawkNextProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: HawkNextProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatHawkNextAgentProcessName(owner.agent.provider, owner.agent.workspacePath)
