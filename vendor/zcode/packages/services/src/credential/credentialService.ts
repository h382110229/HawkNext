...
async function readAll(credentialsFile = getCredentialsFile()): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read HawkNext credentials: ${credentialsFile}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`HawkNext credentials are corrupt: ${credentialsFile}`, { cause: error });
  }
}
