...
// Conversation share 的具体实现依赖 Node 文件系统，只能从 @hawknext/services/node 引入；
...
// 若 value 导出 createOnboardingRecordService，会连带 fs/atomicFileUtils → @hawknext/shared/node →
...
// HawkNext task wrapper service — task 列表/置顶/归档等 app 侧包装状态入口。
export { IHawkNextTaskService } from "./session/hawknextTaskService.js";
export type {
  HawkNextArchivedTaskDeletionResult,
  HawkNextModelTrajectory,
  HawkNextModelTrajectoryCallSource,
  HawkNextModelTrajectoryCallSourceKind,
  HawkNextModelTrajectoryContentPart,
  HawkNextModelTrajectoryMessage,
  HawkNextModelTrajectoryRecord,
  HawkNextModelTrajectoryUsage,
...
export type { HawkNextTaskListItem } from "./session/hawknextTaskListTypes.js";
...
// HawkNext agent service — IHawkNextAgentService is both a type (interface) and value (descriptor)
export {
  IHawkNextAgentService,
  type HawkNextAgentLocalRuntimeChildProcesses,
  HAWKNEXT_AGENT_RUNTIME_UNAVAILABLE_CODE,
} from "./hawknext-agent/hawknextAgent.js";
export {
  isHawkNextAgentMcpStatusModeUnsupportedError,
  HAWKNEXT_AGENT_MCP_STATUS_MODE_UNSUPPORTED_ERROR_CODE,
  HawkNextAgentMcpStatusModeUnsupportedError,
} from "./hawknext-agent/hawknextAgentErrors.js";
export {
  createHawkNextAgentConnectionScope,
  readTrustedHawkNextAgentV4Connection,
} from "./hawknext-agent/hawknextAgentConnectionScope.js";
export type {
  HawkNextAgentConnectionScope,
  HawkNextAgentV4ClientMode,
  HawkNextAgentV4ConnectionContext,
} from "./hawknext-agent/hawknextAgentConnectionScope.js";
export type {
  ...
  HawkNextAgentStorageStartupSnapshot,
  HawkNextAgentRuntimeLifecycleEvent,
  HawkNextAgentRuntimePolicy,
  HawkNextAgentReadSessionParams,
  HawkNextAgentResumeSessionParams,
  HawkNextAgentRunAutomationNowResult,
...
// HawkNext session service — app-facing session facade without HawkNext Agent naming.
export { IHawkNextSessionService } from "./hawknext-session/hawknextSession.js";
export type {
  HawkNextSessionCreateParams,
  HawkNextSessionEventsParams,
  HawkNextSessionInitializeResult,
  ...
  HawkNextSessionSubscribeParams,
  HawkNextTaskTarget,
  HawkNextSessionWorkspaceTarget,
} from "./hawknext-session/hawknextSession.js";
...
// Storage（资源管理器「存储」tab）：数据类型在 @hawknext/shared；这里只导出服务接口与卷分组纯函数
...
// 设置页插件管理薄服务（UI 平台能力面不再直触 hawknextAgentService）
