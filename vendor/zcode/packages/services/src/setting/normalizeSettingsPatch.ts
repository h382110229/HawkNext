import type { AppSettings } from "@hawknext/shared";
...
export function normalizeSettingsPatch(patch: Partial<AppSettings>): Partial<AppSettings> {
  ...
  if (
    "hawknextEndpointOrigin" in normalizedPatch &&
    typeof normalizedPatch.hawknextEndpointOrigin === "string"
  ) {
    // 非生产 endpoint override 需要支持 Reset 清空；RPC/JSON 对 undefined 不稳定时，用空串也能回到默认生产域。
    const trimmedHawkNextEndpointOrigin = normalizedPatch.hawknextEndpointOrigin.trim();
    normalizedPatch.hawknextEndpointOrigin =
      trimmedHawkNextEndpointOrigin.length > 0 ? trimmedHawkNextEndpointOrigin : undefined;
  }
