/* eslint-disable max-lines -- OAuth 凭据仓储集中维护 ZAI/CloudCN 登录镜像 key 边界，拆分会让鉴权事实源更难追踪。 */
...
import { CLOUD_CN_PROVIDER_ID, isCredentialDecryptError, CLOUD_INTL_PROVIDER_ID } from "@hawknext/shared";
...
const KNOWN_OAUTH_PROVIDER_IDS = [CLOUD_CN_PROVIDER_ID, CLOUD_INTL_PROVIDER_ID] as const;
...
export class OAuthCredentialRepo {
  ...
  async loadTokenSet(provider: OAuthProviderId): Promise<OAuthTokenSet | null> {
    try {
      ...
      const hawknextJwtToken =
        provider === CLOUD_INTL_PROVIDER_ID || provider === CLOUD_CN_PROVIDER_ID
          ? await this.credentialService.load(HAWKNEXT_JWT_TOKEN_KEY)
          : null;

      return {
        accessToken,
        ...(refreshToken ? { refreshToken } : {}),
        ...(hawknextJwtToken ? { hawknextJwtToken } : {}),
      };
    } catch (error) {
      if (!isCredentialDecryptError(error)) {
        throw error;
  ...
  async saveTokenSet(provider: OAuthProviderId, tokenSet: OAuthTokenSet): Promise<void> {
    ...
    if (provider === CLOUD_INTL_PROVIDER_ID || provider === CLOUD_CN_PROVIDER_ID) {
      if (tokenSet.hawknextJwtToken) {
        // CloudCN Start Plan 与  Start Plan 一样消费 hawknext JWT。
        // JWT 必须在 OAuth callback 阶段随 tokenSet 落盘，后续 balance/runtime 只读取它，
        // 不能再拿 CloudCN access token 拼另一个 /oauth/token body 临时兑换。
        await this.credentialService.save(HAWKNEXT_JWT_TOKEN_KEY, tokenSet.hawknextJwtToken);
      } else {
        await this.credentialService.delete(HAWKNEXT_JWT_TOKEN_KEY);
      }
    }
  }
  ...
  private async loadUserProfileFromKey(
    ...
    try {
      ...
      if (provider === CLOUD_INTL_PROVIDER_ID && typeof parsed === "object" && parsed !== null) {
  ...
  async saveUserProfile(provider: OAuthProviderId, profile: OAuthUserProfile): Promise<void> {
    ...
    const persistProfile =
      provider === CLOUD_INTL_PROVIDER_ID && profile.rawProfile ? profile.rawProfile : profile;
  ...
  async clearProvider(provider: OAuthProviderId): Promise<void> {
    ...
    await this.credentialService.delete(refreshTokenKey(provider));
    await this.credentialService.delete(userInfoKey(provider));
    if (shouldClearHawkNextJwtOnLogout(provider)) {
      await this.credentialService.delete(HAWKNEXT_JWT_TOKEN_KEY);
    }
  }
  ...
  private async clearCorruptOAuthSession(): Promise<void> {
    ...
    // 等价于强制登出已注册 OAuth provider：先清 provider 命名空间与共享 hawknext JWT，
...
function shouldClearHawkNextJwtOnLogout(provider: OAuthProviderId): boolean {
  return provider === CLOUD_INTL_PROVIDER_ID || provider === CLOUD_CN_PROVIDER_ID;
}
