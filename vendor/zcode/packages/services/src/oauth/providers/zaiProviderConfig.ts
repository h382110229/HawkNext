...
const ZAI_OAUTH_PROVIDER_CONFIG: Omit<OAuthProviderRuntimeConfig, "appSecret"> = {
  ...
  userinfoUrl: "/api/oauth/userinfo",
  businessLoginUrl: "/api/auth/z/login",
...
export function createZaiProviderRuntimeConfig(env: NodeJS.ProcessEnv): OAuthProviderRuntimeConfig {
  return {
    ...
    authorizeUrl:
      ...
      buildRuntimeCloudOAuthUrl(env, "/api/oauth/authorize"),
    tokenUrl:
      readEnv(env, "ZAI_OAUTH_TOKEN_URL") ?? buildHawkNextApiUrlFromEnv(env, "/api/v1/oauth/token"),
...
export function resolveZaiUserinfoUrl(env: NodeJS.ProcessEnv): string {
  return (
    readEnv(env, "ZAI_OAUTH_USERINFO_URL") ?? buildRuntimeCloudOAuthUrl(env, "/api/oauth/userinfo")
  );
}
