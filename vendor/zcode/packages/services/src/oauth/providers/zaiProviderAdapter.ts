...
const log = (...args: unknown[]) => console.log(formatLogPrefix("cloudOAuth", process.pid), ...args);
...
function normalizeBackendAvatarUrl(avatar: string | undefined): string | undefined {
  ...
  // ZAI 后端现在稳定返回可展示 URL 或 base64，客户端继续拼  前缀会改坏服务端语义。
...
function sanitizeTokenPayloadForLog(tokenPayload: ZaiBackendTokenPayload): unknown {
  ...
  const cloudAccessToken = tokenPayload.data.zai?.access_token;
      ...
      ...(tokenPayload.data.zai
        ? {
            zai: {
              ...
              ...(cloudAccessToken
                ? {
                    access_token: maskAccessToken(cloudAccessToken),
                    accessTokenLength: cloudAccessToken.length,
                  }
                : {}),
            },
          }
...
export class ZaiProviderAdapter implements OAuthProviderAdapter {
  readonly providerId = CLOUD_INTL_PROVIDER_ID;
  ...
  constructor(
    ...
    this.businessTokenResolver = new ZaiBusinessTokenResolver({
      ...
      // 如果继续硬编码生产 ，本地测试登录会在 OAuth token 成功后失败。
      loginUrl: config.businessLoginUrl ?? "/api/auth/z/login",
  ...
  async normalizePolledTokenSet(tokenSet: OAuthTokenSet): Promise<OAuthTokenSet> {
    // CLI flow 的 ready.access_token 仍是  OAuth token，而 Desktop
    // oauth:cloud-intl:access_token 的既有契约是 /api/auth/z/login 返回的业务 token。
  ...
  async exchangeToken(
    ...
    log("token request", {
      ...
      body: {
        provider: CLOUD_INTL_PROVIDER_ID,
    ...
    try {
      tokenPayload = await readApiJson<ZaiBackendTokenPayload>(
        ...
        this.config.tokenUrl,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          // hawknext OAuth token 后端现在同时服务  和 CloudCN。
    ...
    const hawknextJwtToken = tokenPayload.data?.token;
    if (!hawknextJwtToken) {
    ...
    //  业务接口只认 /api/auth/z/login 返回的平台 JWT。
    // 这里在登录阶段完成转换，让 oauth:cloud-intl:access_token 持久化的就是业务 token，后续不再做兜底二次交换。
