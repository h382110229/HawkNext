...
import { resolveCloudCNUserinfoUrl } from "#src/oauth/providers/cloud-cnProviderConfig.js";
...
export async function isCurrentOAuthCredentialRequest(options: {
  ...
  const currentJwt = (await options.credentialService.load("hawknext-jwt"))?.trim() ?? "";
  if (currentJwt && authorization === `Bearer ${currentJwt}`) return true;

  // 原观察器只识别 HawkNext JWT，业务 access token 的 userinfo 401
  // 只会变成普通请求错误。仅扩展用户/团队身份查询，避免支付和 API key 接口跟随全局退出。
  const provider = await options.credentialService.load("oauth:active_provider");
  if (provider !== CLOUD_CN_PROVIDER_ID && provider !== CLOUD_INTL_PROVIDER_ID) return false;
  ...
  const urls =
    provider === CLOUD_CN_PROVIDER_ID
      ? [() => buildCloudCNApiUrl(env, customerInfoPath), () => resolveCloudCNUserinfoUrl(env)]
