...
import {
  refreshLegacyCloudCNCachedProfile,
...
function resolveInactiveOAuthProvider(provider: OAuthProviderId): OAuthProviderId | null {
  if (provider === CLOUD_INTL_PROVIDER_ID) {
    return CLOUD_CN_PROVIDER_ID;
  }
  if (provider === CLOUD_CN_PROVIDER_ID) {
    return CLOUD_INTL_PROVIDER_ID;
  }
  return null;
}
...
export class OAuthService implements IOAuthService {
  ...
  async restoreCachedSessionState(): Promise<OAuthCachedSessionRestoreResult> {
    ...
    if (!profile) {
      // zai / cloud-cn 这类 OAuth token 生命周期很短，启动时如果强依赖远端 userinfo 校验，
    ...
    // 启动缓存恢复只需要检查共享 hawknext JWT；若通过 loadActiveTokenSet 连带读取
    // provider access token，会把原本后台执行的 CloudCN profile 迁移重新阻塞到首屏恢复链路。
    const hawknextJwtToken = (await this.credentialService.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim() ?? "";
    if (hawknextJwtToken && resolveJwtExpiration(hawknextJwtToken, this.now()).kind === "expired") {
      serviceLog.info("cached session invalidated because hawknext JWT expired", {
    ...
    if (activeProvider === CLOUD_CN_PROVIDER_ID) {
      const migrationGeneration = this.oauthSessionGeneration;
      void refreshLegacyCloudCNCachedProfile({
        adapter,
        cachedProfile: profile,
        loadTokenSet: () =>
          this.loadCloudCNCachedProfileMigrationTokenSet(migrationGeneration, profile),
        now: this.now,
        runWithAdapterError: (run) => this.runWithAdapterError(adapter, run),
        saveProfile: (nextProfile) =>
          this.saveCloudCNCachedProfileMigration(migrationGeneration, profile, nextProfile),
      })
    ...
    if (activeProvider === CLOUD_INTL_PROVIDER_ID) {
      if (!hawknextJwtToken) {
        // sidebar 登录入口之前只看缓存 user_info，会把“缺少 hawknext-jwt”的状态误判成已登录。
        // 这里补充 hawknext-jwt 门槛，确保没有后端 JWT 时统一按未登录处理。
        log("restoreCachedSession skipped: missing hawknext-jwt:", activeProvider);
        return { status: "signed-out" };
      }
    }
  ...
  private async invalidateExpiredCachedSession(
    ...
    const invalidated = await this.runSessionMutation(async () => {
      ...
      const currentJwt = (await this.credentialService.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim() ?? "";
      if (
  ...
  private async loadCloudCNCachedProfileMigrationTokenSet(
    expectedGeneration: number,
    ...
    return this.runSessionMutation(async () => {
      if (this.oauthSessionGeneration !== expectedGeneration) {
        log("restoreCachedSession skipped stale CloudCN token migration");
        return null;
      }

      const activeProvider = await this.repo.getActiveProvider();
      const currentProfile = await this.repo.loadUserProfile(CLOUD_CN_PROVIDER_ID);
      if (
        activeProvider !== CLOUD_CN_PROVIDER_ID ||
        !currentProfile ||
        !isSameOAuthProfile(currentProfile, expectedCachedProfile)
      ) {
        // 迁移请求目标固定是 CloudCN，token 也必须固定读取 CloudCN 命名空间；
        // 发请求前先复核持久化快照，避免切换到 ZAI 后把其他 provider token 发给 CloudCN。
        log("restoreCachedSession skipped stale CloudCN token migration:", activeProvider);
        return null;
      }

      return this.repo.loadTokenSet(CLOUD_CN_PROVIDER_ID);
    });
  }

  private async saveCloudCNCachedProfileMigration(
    expectedGeneration: number,
    expectedCachedProfile: OAuthUserProfile,
    ...
    await this.runSessionMutation(async () => {
      if (this.oauthSessionGeneration !== expectedGeneration) {
        log("restoreCachedSession skipped stale CloudCN profile migration");
        return;
      }

      const activeProvider = await this.repo.getActiveProvider();
      if (activeProvider !== CLOUD_CN_PROVIDER_ID) {
        log("restoreCachedSession skipped stale CloudCN profile migration:", activeProvider);
        return;
      }

      const currentProfile = await this.repo.loadUserProfile(CLOUD_CN_PROVIDER_ID);
      if (!currentProfile || !isSameOAuthProfile(currentProfile, expectedCachedProfile)) {
        // CloudCN 旧缓存迁移在后台完成，期间用户可能 logout、切到 ZAI，
        // 或重新登录 CloudCN。只有当前缓存仍是启动时那份旧缓存时，旧迁移结果才允许落盘。
        log("restoreCachedSession skipped outdated CloudCN profile migration");
        return;
      }

      await this.repo.saveUserProfile(CLOUD_CN_PROVIDER_ID, nextProfile);
    });
  }
  ...
  private async persistOAuthSession(
    ...
    if (inactiveProvider) {
      await assertCurrent();
      // ZAI 与 CloudCN 是互斥身份域。切换 provider 时必须先清旧 provider，
      // 再保存当前 token；反序会让 clearProvider 误删共享的 hawknext-jwt。
  ...
  async restoreSession(): Promise<UserInfo | null> {
    ...
    if (!tokenSet && adapter.loadLegacyTokenSet) {
      ...
      // 多 provider 改造后，旧版 CloudCN 仍可能只保留 legacy token key。
  ...
  async startOAuthWithPolling(provider: OAuthProviderId): Promise<OAuthStartResponse> {
    if (provider !== CLOUD_INTL_PROVIDER_ID && provider !== CLOUD_CN_PROVIDER_ID) {
      return this.startOAuthInternal(provider);
    }
    ...
    const initUrl = buildHawkNextApiUrlFromEnv(this.env, "/api/v1/oauth/cli/init");
    ...
    if (provider === CLOUD_CN_PROVIDER_ID) {
      // CloudCN CLI callback 的失败页会截断原有 Desktop deep link 回调体验。
      // flow 仍由 Host 轮询，但浏览器回调恢复到官网中转页，再透传到 hawknext://oauth/callback。
      authorizeUrl.searchParams.set("redirect", buildDesktopOAuthRedirectUriFromEnv(this.env));
    } else if (provider === CLOUD_INTL_PROVIDER_ID) {
      //  后端 init 仍可能返回 provider-specific callback，导致回跳行为与 CloudCN 不一致。
      // Desktop 统一改写为官网中转页，再由官网透传到 hawknext://oauth/callback。
    ...
    this.pendingState = {
      ...
      polling: {
        ...
        pollIntervalMs,
        pollToken,
        pollUrl: buildHawkNextApiUrlFromEnv(
          this.env,
  ...
  async pollPendingOAuth(): Promise<OAuthCallbackResult | null> {
    ...
    try {
      const result = await this.runPendingSessionCompletion(pending, async () => {
        ...
        const cloudCn = isUnknownRecord(ready.cloudCn) ? ready.cloudCn : null;
        const providerAccessToken =
          pending.provider === CLOUD_INTL_PROVIDER_ID
            ? readTrimmedString(zai?.access_token)
            : readTrimmedString(cloudCn?.access_token) || readTrimmedString(cloudCn?.accessToken);
        const hawknextJwtToken = readTrimmedString(ready.token);
        const userId = readTrimmedString(user?.user_id);
        if (!hawknextJwtToken || !providerAccessToken || !userId) {
          throw new Error("OAuth flow 查询响应无效");
        }
        ...
        const refreshToken =
          pending.provider === CLOUD_CN_PROVIDER_ID
            ? readTrimmedString(cloudCn?.refresh_token) ||
              readTrimmedString(cloudCn?.refreshToken)
            : undefined;
  ...
  async logout(provider?: OAuthProviderId): Promise<void> {
    ...
    const loggedOutIdentity = await this.runSessionMutation(async () => {
      ...
      this.oauthSessionGeneration += 1;
      // ZAI/CloudCN provider 的 Unlink 已收敛为 App logout。
