...
import { createHawkNextTaskServiceAdapter } from "../src/hawknext-agent/hawknextTaskServiceAdapter.js";
import {
...
test("current Project Memory catalog and files remain readable", async () => {
  const dir = await mkdtemp(join(tmpdir(), "hawknext-current-memory-"));
  setDataBaseDir(dir);
  try {
...
test("opening the task index leaves retired ACP IDs and user rows untouched", async () => {
  const dir = await mkdtemp(join(tmpdir(), "hawknext-acp-index-"));
...
test("missing sessions report the owner error even when a valid ACP snapshot exists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "hawknext-acp-snapshot-"));
  setDataBaseDir(dir);
  ...
  type Options = Parameters<typeof createHawkNextTaskServiceAdapter>[0];
  const service = createHawkNextTaskServiceAdapter({
    hawknextAgentService: {
      async resumeSession() {
        throw ownerError;
      },
      disposeAll() {},
    } as unknown as Options["hawknextAgentService"],
    taskIndexRepo: {
      async getTaskMeta() {
        return meta;
      },
...
test("current session recovery preserves Desktop and replayable projections", async () => {
  const snapshot = hawknextSessionStateSnapshotSchema.parse({
    protocol: { name: HAWKNEXT_PROTOCOL_NAME, version: HAWKNEXT_PROTOCOL_VERSION },
    session: {
...
test("current Provider configuration starts without an old config migration callback", async () => {
  const dir = await mkdtemp(join(tmpdir(), "hawknext-provider-current-"));
  const runtime = createProviderConfigRuntime({
    hawknextBuiltinFilePath: fileURLToPath(
      new URL("../../../config/provider/hawknext-builtin.json", import.meta.url),
    ),
  ...
  try {
    ...
    assert.ok(config.hawknextBuiltinRevision);
