...
for (const clientMode of ["desktop-continuous", "web-remote-replayable"] as const) {
  test(`previously imported Claude history becomes a real session for ${clientMode}`, async () => {
    const dir = await mkdtemp(join(tmpdir(), "hawknext-import-recovery-"));
    setDataBaseDir(dir);
    const meta = {
    ...
    type CreateInput = Parameters<Options["hawknextAgentService"]["createSession"]>[0];
    const created: CreateInput[] = [];
    let session: HawkNextSessionStateSnapshot | undefined;
    ...
    const service = createHawkNextTaskServiceAdapter({
      ...
      hawknextAgentService: {
        ...
        async createSession(input: CreateInput) {
          created.push(input);
          session = hawknextSessionStateSnapshotSchema.parse({
