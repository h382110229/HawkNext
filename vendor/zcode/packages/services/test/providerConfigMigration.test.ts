...
import { readLegacyHawkNextConfigProviders } from "../src/model-provider/legacyHawkNextConfigProviderReader.js";
...
const legacyConfig = {
  provider: {
    "custom-example": {
      ...
      npm: "@ai-sdk/official-model-compatible",
      enabled: false,
      options: {
...
async function setup() {
  const dir = await mkdtemp(join(tmpdir(), "hawknext-provider-migration-"));
  setDataBaseDir(dir);
  ...
  const runtime = createProviderConfigRuntime({
    ...
    readLegacyProviders: async () => {
      reads += 1;
      return readLegacyHawkNextConfigProviders();
    },
...
test("startup migrates published HawkNext config into personal config without changing the source", async () => {
