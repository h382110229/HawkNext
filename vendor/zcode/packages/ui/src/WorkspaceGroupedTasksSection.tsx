...
import type { HawkNextGroupedTaskView, HawkNextTaskGroupColor } from "@hawknext/services";
import { OFF_PEAK_DEFAULT_GROUP_ID, type HawkNextTaskMeta } from "@hawknext/shared";
...
function getGroupedTaskViewSignature(view: HawkNextGroupedTaskView): string {
  return view.nodes
    .map((node) =>
      node.type === "task"
        ? `t:${taskKey(node.task)}`
...
export function WorkspaceGroupedTasksSection({
  ...
  const groupedDraftTask = useHawkNextSessionStore(
    (state) =>
      selectWorkspaceHawkNextState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .groupedDraftTask,
  );
  const groupedDraftFocusVersion = useHawkNextSessionStore(
    (state) =>
      selectWorkspaceHawkNextState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .draftFocusVersion,
  );
  const clearGroupedDraftTask = useHawkNextSessionStore((state) => state.clearGroupedDraftTask);
  const {
  ...
  const dragOriginViewRef = useRef<HawkNextGroupedTaskView | null>(null);
  const dragPreviewViewRef = useRef<HawkNextGroupedTaskView | null>(null);
  ...
  const handleStartRenameTask = useCallback((task: HawkNextTaskMeta) => {
    setRenamingTaskKey(taskKey(task));
    setRenameDraft(task.title ?? "");
  }, []);

  const handleMoveTaskToGroup = useCallback(
    (task: HawkNextTaskMeta, groupId: string | null) => {
  ...
  const handleSubmitRenameTask = useCallback(async () => {
    ...
    try {
      const meta = await workspaceServices.services.hawknextTaskService.renameTask({
        taskId: task.taskId,
  ...
  const handleMarkTaskAsUnread = useCallback(
    (task: HawkNextTaskMeta) => {
      ...
      void workspaceServices.services.hawknextTaskService
        .setTaskUnread({
          taskId: task.taskId,
          workspacePath: task.workspacePath,
          unread: true,
  ...
  const handleUpdateGroupColor = useCallback(
    (groupId: string, color: HawkNextTaskGroupColor) => {
  ...
  const setViewWithGroupedTaskAnimation = useCallback(
    (nextView: HawkNextGroupedTaskView) => {
