...
export function trajectoryToolOutputs(message: HawkNextModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: HawkNextModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: HawkNextModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: HawkNextModelTrajectoryMessage): {
  names: string;
  ids: string;
} {
