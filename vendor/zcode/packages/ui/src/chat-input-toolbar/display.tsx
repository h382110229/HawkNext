...
import {
  ...
  type HawkNextConfigSelectValue,
...
import { HAWKNEXT_MODE_OPTION_DESCRIPTION_IDS, HAWKNEXT_MODE_OPTION_LABEL_IDS } from "./display-help.js";
...
function isConfigSelectValueInOptions(option: HawkNextConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: HawkNextConfigOption): string | undefined {
...
function getConfigSelectItemTestId(
  ...
  entry: HawkNextConfigSelectValue,
): string | undefined {
  if (option.category === "thought_level") {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 HawkNext Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<HawkNextConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: HawkNextProvider | undefined,
  entry: Pick<HawkNextConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return HAWKNEXT_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return HAWKNEXT_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
