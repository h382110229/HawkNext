...
import type { SessionCreateSource } from "@hawknext/shared";
...
import { localizeConversationShareUrl } from "@hawknext/shared";
import type {
...
import { prepareWorkspaceWithHawkNextSessionService } from "@/hooks/useWorkspacePrepare.js";
import {
  createCodingPlanFunnelContext,
  resolveCodingPlanEntryPlanState,
} from "@/lib/codingPlanFunnelTelemetry.js";
import { decodeCustomModelValue, encodeCustomModelValue } from "@/lib/hawknextCustomModelValue.js";
import { parseModelPickerValue } from "@/lib/hawknextSessionProjection.js";
...
import type { GroupedDraftTaskState } from "@/store/hawknextSessionStoreTypes.js";
import {
...
export function SessionPane({
  ...
  const { conversationShareService, modelSelectionService, hawknextSessionService, hawknextTaskService } =
    useServices();
  ...
  const [sendSubmissionError, setSendSubmissionError] = useState<HawkNextUiError | null>(null);
  ...
  const workspaceConfigOptions = useHawkNextSessionStore(
  ...
  useEffect(() => {
    ...
    useHawkNextSessionStore.getState().invalidateDraftRuntime(workspacePath, workspaceIdentity);
  ...
  const promoteGroupedDraftTask = useHawkNextSessionStore((state) => state.promoteGroupedDraftTask);
  ...
  const theme = useHawkNextStoreWithDefault((state) => state.theme, "system");
  const codePreviewSettings = useHawkNextStoreWithDefault(
  ...
  const dispatchCommand = useCallback(
    async (
      ...
      const groupedDraftTask =
        type === "createSession"
          ? useHawkNextSessionStore.getState().getWorkspaceState(workspacePath, workspaceIdentity)
              .groupedDraftTask
          : null;
    ...
    async (reference?: ConversationSelectionReference, forceNew = false) => {
      ...
      try {
        ...
        if (targetChildSessionId) {
          try {
            await hawknextSessionService.readSession({
              workspacePath,
              ...(workspaceIdentity ? { workspaceIdentity } : {}),
  ...
  const dispatchSendText = useCallback(
    (text: string, options?: ConversationComposerSendOptions) => {
      const createSource = useHawkNextSessionStore
        .getState()
    ...
    async (
      ...
      try {
        ...
        setSendSubmissionError({
          code: runtimeModelUnavailable ? "HAWKNEXT_RUNTIME_MODEL_UNAVAILABLE" : "SEND_FAILED",
  ...
  const handleRecoverCustomModelSelection = useCallback(
    async (value: string, sourceModel: ModelSelectionSource | null) => {
      ...
      const displayProvider = provider ?? HAWKNEXT_AGENT_PROVIDER;
      ...
      const store = useHawkNextSessionStore.getState();
      ...
      try {
        await hawknextTaskService.restartWorkspaceProcess({
          workspacePath,
          workspaceIdentity,
        ...
        const prepareResult = await prepareWorkspaceWithHawkNextSessionService({
          workspacePath,
          workspaceIdentity,
          provider: displayProvider,
          hawknextSessionService,
          ...
          >
            <ConversationTimeline
              ...
              headerSlot={
                ...
                // rows 为空，但只读块必须留下来显示「需要更新 HawkNext」，不能整块消失。
                importedShare &&
