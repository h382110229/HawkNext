import { BufferWriter, serialize } from "@hawknext/rpc";
...
interface AttachmentUploadAgent {
  ...
  attachmentCommitV4(params: HawkNextAgentAttachmentTerminalParams): Promise<V4AttachmentPutResult>;
...
function measureAttachmentChannelRequestBytes(method: string, params: unknown): number {
  ...
  serialize(writer, [100, 2_147_483_647, ServiceChannels.HawkNextAgent, method]);
...
export async function uploadAttachmentTransaction(
  ...
  const beginParams: HawkNextAgentAttachmentBeginParams = {
    ...common,
  ...
  try {
    ...
    for (let chunkIndex = begin.nextChunkIndex; chunkIndex < totalChunks; chunkIndex += 1) {
      ...
      const chunkParams: HawkNextAgentAttachmentChunkParams = {
        ...common,
        chunkIndex,
    ...
    const terminal = common satisfies HawkNextAgentAttachmentTerminalParams;
