import { HAWKNEXT_AGENT_PROVIDER, type PlanIdentitySnapshot, type HawkNextProvider } from "@hawknext/shared";
...
function resolveLegacyConversationModelValue(params: {
  ...
  if (!configProvider || !modelName || configProvider === HAWKNEXT_AGENT_PROVIDER) {
...
/** V4 config.provider 是实际模型 provider id；agentProvider 表示 HawkNext 运行时。 */
export function buildV4ConversationPromptTelemetryExtraDetail(params: {
  agentProvider?: HawkNextProvider;
  configProvider?: string | null;
  modelName?: string | null;
  askMode?: string | null;
  ...
  const agentProvider = params.agentProvider ?? HAWKNEXT_AGENT_PROVIDER;
  ...
  return {
    ...
    // agent 表示 HawkNext 运行时，不能用模型 provider id 替代。
    agent: agentProvider,
  };
}
