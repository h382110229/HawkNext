...
import type { ConversationTelemetryFact } from "@hawknext/shared/hawknext-protocol-v4";
...
export interface ConversationPromptTelemetrySeed {
  localTtft?: import("@hawknext/shared").LocalTtftContext;
...
function toLegacyNetworkEvent(
...
): Extract<HawkNextStreamEvent, { type: "task_network_debug_status" }> {
  return {
  ...
  } as Extract<HawkNextStreamEvent, { type: "task_network_debug_status" }>;
}
...
function toToolLifecycleEvent(input: {
  ...
  if (!input.hasStarted && fact.phase !== "completed" && fact.phase !== "failed") {
    ...
    } as Extract<HawkNextStreamEvent, { type: "tool_call" }>;
  }
  return {
    ...
    raw: {},
    ...(skillMetadata ? { skillMetadata } : {}),
  } as Extract<HawkNextStreamEvent, { type: "tool_call_update" }>;
}

function toUsage(fact: Extract<ConversationTelemetryFact, { kind: "usage.delta" }>): HawkNextUsage {
  return {
...
function mergeUsage(left: HawkNextUsage | null, right: HawkNextUsage): HawkNextUsage {
...
export class ConversationTelemetrySupervisor {
  ...
  private processPromptFact(
    ...
    switch (fact.kind) {
      ...
      case "stream.chunk": {
        ...
        } as Extract<HawkNextStreamEvent, { type: "agent_thought_chunk" | "agent_message_chunk" }>;
  ...
  private handleTerminal(
    ...
    const terminalEvent = // 修复原因：旧 adapter 把所有 TurnComplete（包括 cancelled）先投影成
      ...
      ) as Extract<HawkNextStreamEvent, { type: "task_complete" | "task_error" }>;
    this.reportFinalizedSteps(
      lifecycle,
      recordAgentStepTelemetryEvent({
        taskId: lifecycle.taskKey,
  ...
  private handleCompaction(
    ...
    const timeline: HawkNextContextCompactionTimelineMeta = {
      version: 1,
      kind: "synthetic",
