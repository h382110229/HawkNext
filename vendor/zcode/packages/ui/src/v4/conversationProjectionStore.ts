...
const RUNTIME_RECYCLED_ERROR = "HawkNext agent runtime 已被回收，重连未成功";
 ...
 *
 * 三种文案都来自同一次回收：transport 关闭时 HawkNextProtocolClient.rejectAll 打断在途请求
...
function isRuntimeRecycleError(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("HawkNext agent transport closed") ||
    message.includes("HawkNext Protocol client disposed") ||
    message.includes("HawkNext Protocol client is disposed") ||
    message.includes("HawkNext Agent runtime is not running")
  );
}
...
export class ConversationProjectionStore {
  ...
  /**
   * 有界退避重连。重订阅走 start-if-needed（hawknextAgentService.subscribeConversationV4），
