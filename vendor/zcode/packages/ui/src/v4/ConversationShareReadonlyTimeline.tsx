 ...
 * 公开页 bundle 体积与无 Desktop 宿主（window.hawknext / PlatformProvider / tab store）的运行环境；
...
import { getCompactToolCallStatusMessageId, type Locale } from "@hawknext/shared";
import type {
  ArtifactRow,
  AssistantTextRow,
  ConversationRow,
...
import { HawkNextIntlProvider, useHawkNextIntl } from "@/i18n/IntlProvider.js";
...
function resolveImportedArtifactPath(
  ...
  // 取舍：这里只校验形状（.hawknext-share/<dir>/shared-artifacts/<file> 四段），刻意不把
  ...
  // .hawknext-share/*/shared-artifacts/ 内，无越权读放大。
...
export function ConversationShareReadonlyTimeline({
  ...
  const labels: ReadonlyLabels =
    locale === "zh-CN"
      ? {
          ...
          unsupportedRows: "部分内容需要更新 HawkNext 查看",
        }
      : {
          history: "Reasoning",
          ...
          unsupportedRows: "Some content requires a newer version of HawkNext",
        };
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <HawkNextIntlProvider initialLocale={locale}>
