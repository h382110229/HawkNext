...
interface BuildTaskListParams {
  ...
  terminalStatusByTaskId?: ReadonlyMap<
    string,
    Extract<HawkNextTaskMeta["status"], "completed" | "error">
  >;
...
function mergeTaskIndexRowWithSession(
  taskIndexTask: HawkNextTaskMeta,
  sessionTask: HawkNextTaskMeta,
): HawkNextTaskMeta {
...
export function buildTaskListResult(params: BuildTaskListParams): BuildTaskListResult {
  ...
  filtered.sort((a, b) => compareHawkNextTaskListItems(a, b, params.sortBy));
