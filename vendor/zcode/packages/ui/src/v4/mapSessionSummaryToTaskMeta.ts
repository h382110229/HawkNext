// sessions-index 的 SessionSummary → 侧栏实时 detail HawkNextTaskMeta 映射。
...
import type { TraceId, HawkNextProvider, HawkNextTaskMeta } from "@hawknext/shared";
import type { SessionSummary } from "@hawknext/shared/hawknext-protocol-v4";
import {
...
function phaseToStatus(phase: SessionSummary["phase"]): HawkNextTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
interface MapSessionSummaryOptions {
  ...
  previous?: HawkNextTaskMeta;
}

/**
 * SessionSummary → 实时 detail HawkNextTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as HawkNextProvider } : {}),
      ...(status ? { status } : {}),
