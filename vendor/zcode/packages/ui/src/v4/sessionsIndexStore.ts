...
import { isHawkNextFileLockTimeoutError } from "@hawknext/shared";
import { HAWKNEXT_AGENT_RUNTIME_UNAVAILABLE_CODE } from "@hawknext/services";
...
function isTransientSubscribeError(error: unknown): boolean {
  if (isHawkNextFileLockTimeoutError(error)) {
    return true;
  }
  const code =
