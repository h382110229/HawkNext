...
import type { HawkNextElicitationRequest, HawkNextPermissionOption, HawkNextProvider } from "@hawknext/shared";
...
} from "@/store/hawknextSessionStore.js";
import type { ElicitationFormDraft } from "@/store/hawknextSessionStoreTypes.js";
...
function buildV4ElicitationProgressKey(request: HawkNextElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  const notificationEnabled = useHawkNextStoreWithDefault((state) => state.notificationEnabled, true);
  const localElicitationDraft = useHawkNextSessionStore((state) => {
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useHawkNextSessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: HawkNextPermissionOption, feedback?: string) => {
