...
import { useToolbarConfigOptions } from "@/hooks/useHawkNextConfig.js";
...
function resolveFamilyForPlanProviderId(providerId: string | null | undefined): {
  ...
  switch (providerId?.trim()) {
    ...
    case BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan:
      return { family: "cloud-cn", kind: "personalCoding" };
    case BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan:
      return { family: "cloud-cn", kind: "teamCoding" };
    case BUILTIN_MODEL_PROVIDER_IDS.cloudCnStartPlan:
      return { family: "cloud-cn", kind: "start" };
    default:
      return null;
  }
}
...
function resolveV4ContextPlanConnection(params: {
  ...
  if (providerFamily.kind === "start") {
    ...
    if (
      ...
      providerId !== BUILTIN_MODEL_PROVIDER_IDS.cloudCnStartPlan
    ) {
      return { kind: "none" };
    }
    return {
...
function resolveContextTeamUsageSourceFromEntitlementSnapshot({
  ...
  accountAccess?: HawkNextProviderAccountAccess | HawkNextAccountAccess | null;
  ...
  // 原 createCloudCNTeamPlanConnectionKey + cloudCnCodingPlan providerId 硬编码，
  // zai team snapshot 也生成 cloud-cn 前缀 sourceId（与设置页/usage sources 不一致）。
  // 从 snapshot.provider.id 反查 family，生成对应前缀。
  const familySpec = resolveModelProviderFamilySpecByProviderId(snapshot.provider?.id ?? "");
  const family: ProviderFamilyDomain = familySpec?.id ?? "cloud-cn";
  if (!accountAccess) {
    return null;
  }
  ...
  return {
    ...
    label: `${familySpec?.id === "zai" ? "ZAI" : "CloudCN"} - ${displayName}`,
  };
}
...
function resolveContextCodingPlanUsageSource(params: {
  ...
  // 原类型/守卫硬绑 cloudCnCodingPlan，zai team 上下文永远返回 null。
  // 放开为 zai/cloud-cn 两种 codingPlan providerId。
  providerId?:
    | typeof BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan
    | typeof BUILTIN_MODEL_PROVIDER_IDS.zaiTeamCodingPlan;
  ...
  return (
    buildCodingPlanUsageSources({
      accountAccesses: {
        [resolveModelProviderFamilySpecByProviderId(params.providerId ?? "")?.id ?? "cloud-cn"]:
...
function V4ComposerModelControlsImpl({
  ...
  // 配置面存活服务读（过渡归宿 = 配置面 v4 化）：连接方式选中键喂 CloudCN Team Plan 门控豁免。
  ...
  const modelOption = modelSelectionView?.providers.some((provider) => provider.models.length > 0)
    ? ({
        ...
        currentValue: "",
        options: [],
      } satisfies HawkNextConfigOption)
    : undefined;
  ...
  // 原 hook 不传 family，默认只拉 cloud-cn 企业 pricing，
    ...
    () =>
      contextPlanConnection.kind === "teamCoding"
        ? resolveContextCodingPlanUsageSource({
            ...
            // 原硬取 cloudCnCodingPlan 的 entitlement snapshot，
  ...
  const teamEntitlement = useUsageEntitlement({
    ...
    // 原硬编码 cloudCnCodingPlan，zai family team plan 选中时
    // contextCodingPlanUsageTeamSource.providerId 是 cloudCodingPlan，但这里仍传 cloudCnCodingPlan
    ...
    // / resolveV4ContextPlanConnection 按 family 正确产出 cloudCodingPlan/cloudCnCodingPlan）。
    preferredProviderId:
  ...
  const thoughtOption = useMemo<HawkNextConfigOption | null>(() => {
...
import { isApiKeyAccess } from "@hawknext/provider";
