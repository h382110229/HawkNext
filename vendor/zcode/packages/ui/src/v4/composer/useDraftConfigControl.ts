...
import { HAWKNEXT_AGENT_PROVIDER, resolveExecutionState } from "@hawknext/shared";
...
import type { SessionConfigState } from "@hawknext/shared/hawknext-protocol-v4";
import type { IModelSelectionService } from "@hawknext/services";
...
import { submissionModeSchema } from "@hawknext/shared/hawknext-protocol-v4";
...
function shouldHydrateWorkspaceCatalog(params: {
  configOptions: readonly HawkNextConfigOption[];
  sessionId: string | null;
  slashCommands: readonly HawkNextSlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const hawknextSessionService = useHawkNextSessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithHawkNextSessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useHawkNextSessionStore.getState();
