...
// 复用 treemappingActivity 的既有解析规则，不复活 hawknextChatMessages。
//
...
import type { ConversationRow, ToolCallRow } from "@hawknext/shared/hawknext-protocol-v4";
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(hawknextAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [hawknextAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
