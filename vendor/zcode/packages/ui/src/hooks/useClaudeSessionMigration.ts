...
import type { HawkNextImportSessionsResult, HawkNextImportableSessionCandidate } from "@hawknext/shared";
import { logger } from "@/logger.js";
import { useHawkNextTaskService } from "@/hooks/useHawkNextTaskService.js";
...
export function useClaudeSessionMigration(params: {
  ...
  const hawknextTaskService = useHawkNextTaskService(
  ...
  const [candidates, setCandidates] = useState<HawkNextImportableSessionCandidate[]>([]);
  const [selectedSessionIds, setSelectedSessionIds] = useState<string[]>([]);
  const [scanError, setScanError] = useState<string | null>(null);
  const [importError, setImportError] = useState<string | null>(null);
  const [lastImportResult, setLastImportResult] = useState<HawkNextImportSessionsResult | null>(null);
  ...
  const scan = useCallback(async () => {
    ...
    try {
      ...
      const nextCandidates = await hawknextTaskService.scanImportableClaudeSessions({
  ...
  const importSessions = useCallback(
    async (sessionIds: string[]) => {
      ...
      try {
        ...
        const result = await hawknextTaskService.importClaudeSessions({
          workspacePath: effectiveWorkspacePath,
          ...(effectiveWorkspaceIdentity ? { workspaceIdentity: effectiveWorkspaceIdentity } : {}),
          sessionIds,
        });
