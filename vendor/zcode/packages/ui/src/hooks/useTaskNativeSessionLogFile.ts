...
function supportsTaskNativeSessionLogFile(_provider: HawkNextProvider | null | undefined): boolean {
 ...
 *
 * 路径规则统一通过 hawknextTaskService 解析，避免 UI 层猜 provider 自己的目录结构。
 */
export function useTaskNativeSessionLogFile(
  workspacePath: string,
  taskId: string | null,
  providerHint?: HawkNextProvider | null,
  workspaceIdentity?: string,
  options: { enabled?: boolean } = {},
) {
  const hawknextTaskService = useHawkNextTaskService(workspacePath, undefined, workspaceIdentity);
  ...
  }, [enabled, hawknextTaskService, providerHint, taskId, workspaceIdentity, workspacePath]);
