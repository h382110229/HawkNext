...
import type { HawkNextSessionEndedSubagent, HawkNextSessionSubagentsResult } from "@hawknext/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: HawkNextSessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly HawkNextSessionEndedSubagent[],
  incoming: readonly HawkNextSessionEndedSubagent[],
): HawkNextSessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const hawknextAgentService = services.hawknextAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<HawkNextSessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof hawknextAgentService?.listSessionSubagents !== "function") {
      ...
      return hawknextAgentService.listSessionSubagents({
