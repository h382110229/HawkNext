...
function areResolvedTaskMetasEqual(left: HawkNextTaskMeta | null, right: HawkNextTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: HawkNextTaskMeta | null) {
  const stableTaskMetaRef = useRef<HawkNextTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useHawkNextSessionStore((state) =>
    selectWorkspaceHawkNextState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 hawknextTaskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task
