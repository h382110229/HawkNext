...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, HawkNextPersistedFileSnapshot>,
  file: HawkNextTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: HawkNextPersistedFileChange | null;
  summary: HawkNextTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, HawkNextPersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 HawkNext Agent 已持久化的单轮文件快照，
