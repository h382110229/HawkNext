...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly HawkNextTaskMeta[];
  optimisticTasks: readonly HawkNextTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareHawkNextTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useHawkNextSessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useHawkNextSessionStore.getState();
