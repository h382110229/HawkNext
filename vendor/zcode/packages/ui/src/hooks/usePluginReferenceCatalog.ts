...
interface PluginReferenceCatalogState {
  entries: HawkNextPluginReferenceCatalogEntry[];
...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<HawkNextPluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<HawkNextPluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<HawkNextPluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    const subscription = services.hawknextAgentService.onAgentRuntimeRestarted((event) => {
  ...
  useEffect(() => {
    ...
    let request: Promise<HawkNextPluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<HawkNextPluginsReferenceCatalogResult>> | undefined;
