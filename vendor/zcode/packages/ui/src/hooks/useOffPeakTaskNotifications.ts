...
import type { HawkNextOffPeakTaskStatus } from "@hawknext/shared";
import type { IOffPeakTaskService } from "@hawknext/services";
...
function notifiableStatus(status: HawkNextOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, HawkNextOffPeakTaskStatus>());
