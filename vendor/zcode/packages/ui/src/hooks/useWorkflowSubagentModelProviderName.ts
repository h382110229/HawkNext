...
import type { HawkNextConfigOption } from "@hawknext/shared";
 ...
 * （见 hawknextSessionSettingsToConfigOptions），那种「名字」在这里就当作没有——屏幕上绝不出现
 * providerId（团队套餐的它是一个 UUID）。
 */
function workflowSubagentProviderNameLookup(
  configOptions: readonly HawkNextConfigOption[] | null | undefined,
...
export function useWorkflowSubagentModelProviderName(
  ...
  const configOptions = useHawkNextSessionStore((state) =>
    workspacePath === undefined
      ? undefined
      : selectWorkspaceHawkNextState(state, workspacePath, workspaceIdentity).configOptions,
  );
