...
import { isRemoteWorkspaceIdentity, HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID } from "@hawknext/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/hawknextSessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 hawknextSessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 hawknext-cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useHawkNextSessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID,
