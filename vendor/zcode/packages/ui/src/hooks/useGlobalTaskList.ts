...
function buildWorkspaceScopes(workspaceTabs: WorkspaceTabState[]): HawkNextTaskListWorkspaceScope[] {
  const scopes = new Map<string, HawkNextTaskListWorkspaceScope>();
...
export function useGlobalTaskList(params: {
  ...
  const taskListVersionSignature = useHawkNextSessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs
        .map((tab) => {
          const workspace = selectWorkspaceHawkNextState(
            state,
            tab.workspacePath,
