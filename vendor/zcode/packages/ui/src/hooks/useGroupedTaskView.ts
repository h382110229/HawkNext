...
import { selectWorkspaceHawkNextState, useHawkNextSessionStore } from "@/store/hawknextSessionStore.js";
import { buildTaskEntityKey, buildTaskWorkspaceKey } from "@/lib/taskQueryCache.js";
import { mergeTaskWithOptimisticMeta } from "@/lib/hawknextTaskMetaMerge.js";
import {
...
function collectViewWorkspaceScopes(
  ...
  const addTask = (task: HawkNextTaskMeta) => {
...
function mergeGroupedTaskViewWithOptimistic(params: {
  ...
  const optimisticTaskByKey = new Map<string, HawkNextTaskMeta>();
  const placementTaskByKey = new Map<string, HawkNextTaskMeta>();
  ...
  const groupMissingTasksByGroupId = new Map<string, HawkNextTaskMeta[]>();
  const topMissingTasks: HawkNextTaskMeta[] = [];
...
function collectGroupedViewTaskKeys(view: HawkNextGroupedTaskView): Set<string> {
...
function groupedNodeIdentityKey(node: HawkNextGroupedTaskView["nodes"][number]): string {
  if (node.type === "group") {
...
function areGroupedNodesEquivalent(
  previous: HawkNextGroupedTaskView["nodes"][number],
  next: HawkNextGroupedTaskView["nodes"][number],
): boolean {
...
function stabilizeGroupedView(
  previous: HawkNextGroupedTaskView,
  next: HawkNextGroupedTaskView,
): HawkNextGroupedTaskView {
...
function nodeToTopLevelRef(
  node: HawkNextGroupedTaskView["nodes"][number],
): HawkNextGroupedTaskViewTopLevelNodeRef {
  if (node.type === "group") {
...
function viewToOrderInput(params: { view: HawkNextGroupedTaskView }): HawkNextGroupedTaskViewOrderInput {
  return {
...
const groupedViewCacheBySignature = new Map<string, HawkNextGroupedTaskView>();

function readCachedGroupedView(signature: string): HawkNextGroupedTaskView | undefined {
  return groupedViewCacheBySignature.get(signature);
}

function writeCachedGroupedView(signature: string, view: HawkNextGroupedTaskView): void {
...
export function useGroupedTaskView(params: { workspaceTabs: WorkspaceTabState[] }) {
  ...
  const [view, setView] = useState<HawkNextGroupedTaskView>(
  ...
  const taskListVersionSignature = useHawkNextSessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs.map((tab) => {
        const workspaceKey = buildTaskWorkspaceKey(tab.workspacePath, tab.workspaceIdentity);
        const workspaceState = selectWorkspaceHawkNextState(
          state,
          tab.workspacePath,
  ...
  const clearPromotedGroupedDraftTask = useHawkNextSessionStore(
  ...
  const displayedViewRef = useRef<HawkNextGroupedTaskView>({ nodes: [] });
  ...
  const [remoteDataLoader] = useState(
    () =>
      new GroupedRemoteDataSingleFlight<{
        structure: HawkNextGroupedTaskViewStructure;
    ...
    try {
      ...
      const remoteData = await remoteDataLoader.load(remoteDataKey, async () => {
        const [structureResult, membershipResult] = await Promise.all([
          services.hawknextTaskService.listGroupedTaskViewStructure({
            workspaceScopes: scopes,
          }),
          fetchTaskListMembershipSets({
            service: services.hawknextTaskService,
  ...
  useEffect(() => {
    ...
    const promotedGroupTasks: HawkNextTaskMeta[] = [];
    const settledRootTasks: HawkNextTaskMeta[] = [];
    ...
    void services.hawknextTaskService
  ...
  useEffect(() => {
    const disposables = scopes.map((scope) =>
      services.hawknextTaskService.onDynamicWorkspaceEvent(scope)((event) => {
  ...
  }, [invalidateRemoteData, scopes, services.hawknextTaskService]);
  ...
  const createGroup = useCallback(async (): Promise<HawkNextTaskGroup> => {
    setSaving(true);
    try {
      const group = await services.hawknextTaskService.createTaskGroup();
  ...
  }, [invalidateRemoteData, refresh, services.hawknextTaskService]);
  ...
  const renameGroup = useCallback(
    async (groupId: string, title: string) => {
      ...
      const optimisticView: HawkNextGroupedTaskView = {
      ...
      setSaving(true);
      try {
        const renamedGroup = await services.hawknextTaskService.renameTaskGroup({
          groupId,
          title: nextTitle,
    ...
    [invalidateRemoteData, scopes, services.hawknextTaskService, view],
  );

  const updateGroupColor = useCallback(
    async (groupId: string, color: HawkNextTaskGroupColor) => {
      const previousView = view;
      const groupNode = view.nodes.find(
        (node) => node.type === "group" && node.group.id === groupId,
      );
      ...
      setSaving(true);
      try {
        const updatedGroup = await services.hawknextTaskService.updateTaskGroupColor({
          groupId,
          color,
          workspaceScopes: collectViewWorkspaceScopes(optimisticView),
        });
  ...
  const applyOrder = useCallback(
    async (nextView: HawkNextGroupedTaskView) => {
      const previousView = view;
      setView(nextView);
      setSaving(true);
      try {
        ...
        await services.hawknextTaskService.applyGroupedTaskViewOrder(
          viewToOrderInput({
            view: nextView,
          }),
        );
    ...
    [invalidateRemoteData, refresh, services.hawknextTaskService, view],
  );
  ...
  const ungroupGroup = useCallback(
    async (groupId: string) => {
      ...
      const nextView: HawkNextGroupedTaskView = {
      ...
      try {
        ...
        await services.hawknextTaskService.deleteTaskGroup({
          groupId,
    ...
    [applyOrder, invalidateRemoteData, refresh, scopes, services.hawknextTaskService, view],
  );
