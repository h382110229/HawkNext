...
import { hawknextSessionSnapshotToTaskMeta } from "@/lib/hawknextSessionProjection.js";

function resolveImmediateActiveTaskSnapshotMeta(
  previousSnapshotMeta: HawkNextTaskMeta | null,
  taskId: string | null,
  taskMetaFromLists?: HawkNextTaskMeta | null,
) {
...
export function useActiveTaskSnapshotMeta(
  ...
  const [snapshotMeta, setSnapshotMeta] = useState<HawkNextTaskMeta | null>(null);
  ...
  useEffect(() => {
    ...
    void hawknextSessionService
      // active header 只需要 session meta/标题兜底，走 HawkNext Protocol 的轻量读取，
      ...
      .then((snapshot) => {
        ...
        setSnapshotMeta(hawknextSessionSnapshotToTaskMeta(snapshot));
      })
      .catch(() => {
        if (cancelled) {
          return;
  ...
  }, [hawknextSessionService, taskId, taskMetaFromLists, workspaceIdentity, workspacePath]);
