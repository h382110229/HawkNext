...
import type { IHawkNextTaskService } from "@hawknext/services";
import type { HawkNextTaskSnapshot } from "@hawknext/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<IHawkNextTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<HawkNextTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IHawkNextTaskService["getTaskSnapshotWithEtag"]>[0];

const hawknextTaskServiceProxyCache = new WeakMap<IHawkNextTaskService, IHawkNextTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IHawkNextTaskService,
  Map<string, { etag: string; snapshot: HawkNextTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "hawknext-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: HawkNextTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: HawkNextTaskSnapshot }>();
...
function writePersistedSnapshotEntry(key: string, etag: string, snapshot: HawkNextTaskSnapshot): void {
  ensurePersistedSnapshotCacheLoaded();
...
function createHawkNextTaskServiceProxy(service: IHawkNextTaskService): IHawkNextTaskService {
...
/** 获取 HawkNext task wrapper 服务实例 */
export function useHawkNextTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): IHawkNextTaskService {
  // HawkNext task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.hawknextTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = hawknextTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createHawkNextTaskServiceProxy(rawService);
  hawknextTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
