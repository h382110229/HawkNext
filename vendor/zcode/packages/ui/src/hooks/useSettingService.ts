...
import { APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL, type AppSettings } from "@hawknext/shared";
...
export function useSettings() {
  const { broadcastService, settingService, hawknextAgentService } = useServices();
  ...
  const update = useCallback(
    async (patch: Partial<AppSettings>) => {
      ...
      if (
        ...
        const syncResults = await Promise.allSettled([
          hawknextAgentService.syncAppRuntimePreferences(preferences),
        ]);
    ...
    [broadcastService, settingService, settingsStore, hawknextAgentService, platform, refresh],
  );
