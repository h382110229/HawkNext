...
import type { IPlatformService, TaskNotificationPayload } from "@hawknext/shared";
import type { ConversationSnapshot, SessionSummary } from "@hawknext/shared/hawknext-protocol-v4";
...
export function useWorkspaceTerminalTaskNotifications({
  ...
  useEffect(() => {
    ...
    const store = acquireSessionsIndex(registryScope, hawknextAgentService);
    const syncState = () => {
      setIndexState({
        signature,
