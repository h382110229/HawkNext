...
import { createUuid } from "@hawknext/shared";
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@hawknext/shared";
...
export function useAppPanels(options: {
  ...
  const { hawknextAgentService, hawknextSessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await hawknextAgentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,
