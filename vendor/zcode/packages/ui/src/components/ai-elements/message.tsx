 ...
 * Modified by HawkNext: local integration, formatting and adaptations.
...
import type { EditorInfo, FileStat, OpenInEditorOptions } from "@hawknext/shared";
...
import { createHawkNextFileCitationRemarkPlugin } from "@/lib/hawknextFileCitationRemarkPlugin.js";
import { windowsFileLinkEscapeRemarkPlugin } from "@/lib/windowsFileLinkEscapeRemarkPlugin.js";
import { projectHawkNextFileCitations } from "@/lib/hawknextFileCitation.js";
import { rewriteMarkdownArtifactImageSources } from "@hawknext/shared";
...
export type MessageResponseProps = {
  ...
  /** 仅 Assistant 正文开启：把完整 hawknext-file-citation 投影为现有文件链接。 */
  renderHawkNextFileCitations?: boolean;
};
...
export function buildMessageStreamdownRenderKey(params: {
  ...
  return [
    ...
    params.renderHawkNextFileCitations ? "citations" : "plain",
...
export const messageResponsePropsAreEqual = (
  ...
  nextProps.renderHawkNextFileCitations === prevProps.renderHawkNextFileCitations &&
...
export const MessageResponse = memo(
  ({
    ...
    onOpenFileLink,
    onOpenExternalUrl,
    renderHawkNextFileCitations = false,
    workspacePath,
    workspaceHomePath,
    workspaceIdentity,
    ...
    const projectedCitationMarkdown = useMemo(
      () =>
        renderHawkNextFileCitations
          ? projectHawkNextFileCitations(rawMarkdown, { streaming: renderStreaming }).visibleText
          : rawMarkdown,
      [rawMarkdown, renderStreaming, renderHawkNextFileCitations],
    );
    ...
    const messageRemarkPlugins = useMemo<PluggableList>(
      () => [
        ...
        ...(renderHawkNextFileCitations && workspacePath
          ? [createHawkNextFileCitationRemarkPlugin(workspacePath, workspaceHomePath)]
          : []),
      ],
      [renderHawkNextFileCitations, workspaceHomePath, workspacePath],
    );
    ...
    const boundaryResetKey = useMemo(
      () =>
        renderStreaming
          ...
          : `${streamdownMode}:${renderHawkNextFileCitations ? "citations" : "plain"}:${hashMarkdownCacheKey(targetMarkdown)}`,
      [renderStreaming, renderHawkNextFileCitations, streamdownMode, targetMarkdown],
    );
