...
import type { HawkNextGroupedTaskViewNode } from "@hawknext/services";
...
function getTopLevelNodeKey(node: HawkNextGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: HawkNextGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: HawkNextGroupedTaskViewNode, index: number) => ReactNode;
}) {
