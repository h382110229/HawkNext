...
import type { HawkNextGroupedTaskView, HawkNextGroupedTaskViewNode } from "@hawknext/services";
import type { HawkNextTaskMeta } from "@hawknext/shared";
import type { GroupedDraftTaskPlacement } from "@/store/hawknextSessionStoreTypes.js";
import { taskKey } from "@/workspace-grouped-tasks/ids.js";

function cloneView(view: HawkNextGroupedTaskView): HawkNextGroupedTaskView {
  return {
...
function cloneNodes(view: HawkNextGroupedTaskView): HawkNextGroupedTaskViewNode[] {
  return [...view.nodes];
}

function cloneGroupNode(
  node: Extract<HawkNextGroupedTaskViewNode, { type: "group" }>,
): Extract<HawkNextGroupedTaskViewNode, { type: "group" }> {
...
function filterGroupedViewByTaskKeys(
  ...
  let changed = false;
  const nodes: HawkNextGroupedTaskView["nodes"] = [];
...
function getGroupedTaskGroupIds(view: HawkNextGroupedTaskView): string[] {
...
function getTaskGroupIdInView(view: HawkNextGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function findTopLevelTaskIndex(view: HawkNextGroupedTaskView, targetTaskKey: string): number {
  return view.nodes.findIndex(
    (node) => node.type === "task" && taskKey(node.task) === targetTaskKey,
  );
}

function findGroupIndex(view: HawkNextGroupedTaskView, groupId: string): number {
  return view.nodes.findIndex((node) => node.type === "group" && node.group.id === groupId);
}

function findGroupIdByTaskKey(view: HawkNextGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function getNodeIdAfterGroup(
  view: HawkNextGroupedTaskView,
  groupId: string,
): HawkNextGroupedTaskViewNode | null {
...
function insertTaskBeforeTopLevelNode(
  view: HawkNextGroupedTaskView,
  task: HawkNextTaskMeta,
  beforeNode: HawkNextGroupedTaskViewNode | null,
): HawkNextGroupedTaskView {
  const nodes = cloneNodes(view);
  const insertIndex =
    beforeNode === null
