...
/* eslint-disable max-lines -- footer 套餐徽标、升级入口与 entitlement 探测共用同一份
   provider 选择与 family 过滤上下文，拆文件会让 zai/cloud-cn 对称性难以追踪。 */
...
export function useWorkspaceSidebarFooterUsageSummaryState({
  ...
  const selectedSupplierKey = useHawkNextSessionStore((state) =>
    workspacePath
      ? selectWorkspaceHawkNextState(state, workspacePath, workspaceIdentity).selectedSupplierKey
      : "",
  );
  ...
  const cloudCnProvider = availableCodingPlanProviders.find(
  ...
  const cloudCnProviderFingerprint = resolveEntitledAccountProviderAccessFingerprint(
    providerSettingsView,
    BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
  );
  ...
  const cloudCnTeamProvider = resolveEntitledAccountProviderAccess(
    providerSettingsView,
  ...
  const selectedProviderFamilyId = selectedProviderIdFromSupplierKey
    ...
    : null;
  // providerFamilyDomain 是当前登录/运行 family 边界；CloudCN Team selectedKey
  // 会在切换到  后保留，footer 若不按当前 domain 过滤会把头像旁徽标误显示成 Team。
  ...
  const cloudCnFamilyAllowed = providerFamilyDomain !== "zai";
  // 原只有 cloud-cnFamilyAllowed 单变量，zai family 下 enterprise products 完全不拉。
  // zai team plan 对称化需要 zai family 也独立拉一份 enterprise pricing。
  const zaiFamilyAllowed = providerFamilyDomain !== "cloud-cn";
  const cloudCnEnterpriseProducts = useEnterpriseCodingPlanProducts({
    // footer badge 和升级入口都需要识别 Team Plan。
    // Team 项目上下文只在企业 pricing/customerInfo 返回，账号级头像徽标也不能被当前连接方式卡住。
    enabled:
      enabled && !providerSourcesLoading && cloudCnFamilyAllowed && Boolean(cloudCnTeamProvider),
    authenticated: true,
    family: "cloud-cn",
  });
  ...
  const subscribedTeamProducts = useMemo(
    () => [
      ...(cloudCnEnterpriseProducts.snapshot?.productList.filter(
    ...
    [cloudCnEnterpriseProducts.snapshot?.productList, zaiEnterpriseProducts.snapshot?.productList],
  );
      ...
      buildCodingPlanUsageSources({
        accountAccesses: {
          ...(zaiTeamProvider?.access
            ...
            : {}),
          ...(cloudCnTeamProvider?.access
            ? {
                cloudCn: cloudCnTeamProvider.access,
              }
            : {}),
        },
        subscribedTeamProducts,
      }),
    [cloudCnTeamProvider?.access, subscribedTeamProducts, zaiTeamProvider?.access],
  );
  ...
  const cloudCnEntitlement = useUsageEntitlement({
    enabled:
      enabled && !providerSourcesLoading && cloudCnFamilyAllowed && Boolean(cloudCnProvider),
    includeSubscription: true,
    ...
    cacheKey: buildUsageEntitlementCacheKey({
      ...
      providerFingerprint: cloudCnProviderFingerprint,
    }),
    refreshOnMount: false,
  });
  const teamEntitlement = useUsageEntitlement({
    // 原硬绑 cloudCnCodingPlan providerId 判断，zai team source 的 providerId
    // 是 cloudCodingPlan，永远进不到 team 分支，导致 zai team 额度不查询、badge 不显示。
    ...
    includeSubscription: true,
    preferredProviderId:
      currentUsageSource?.providerId ?? BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
  ...
  useEffect(() => {
    for (const refresh of [
      ...
      cloudCnEntitlement.refresh,
      teamEntitlement.refresh,
    ]) {
      void refresh({ silent: true, reason: "access" });
    }
  }, [zaiEntitlement.refresh, cloudCnEntitlement.refresh, teamEntitlement.refresh]);
  const profilePlanBadge = resolveSidebarFooterProfilePlanBadge({
    individualEntitlements: [
      ...(providerFamilyDomain !== "cloud-cn"
        ...
        : []),
      ...(cloudCnFamilyAllowed
        ? [
            {
              providerId: BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
              snapshot: cloudCnEntitlement.snapshot,
              loading: cloudCnEntitlement.loading,
            },
          ]
        : []),
    ],
    ...
    hasTeamPlanEntitlement:
      providerFamilyDomain === "zai"
        ...
        : providerFamilyDomain === "cloud-cn"
          ? Boolean(cloudCnTeamProvider)
          : Boolean(zaiTeamProvider || cloudCnTeamProvider),
  });
  const providerEntitlements = [
    ...
    currentUsageSource.audience === "individual"
      ? [
          {
            sourceId: BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
            providerId: BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
            accountAccess: currentUsageSource.accountAccess,
            ...cloudCnEntitlement,
          },
      ...
      : []),
    // 原 team 分支硬判 cloudCnCodingPlan providerId，zai team source 走不进来。
    // 改为统一按 audience === "team" 路由，覆盖 zai/cloud-cn 两种 family 的 team source。
  ...
  const selectedUpgradeProviderId: SidebarUsageCodingPlanProviderId | undefined =
    ...
    selectedProviderId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan
      ? selectedProviderId
      : undefined;
