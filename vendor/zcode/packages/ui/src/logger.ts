import { formatLogPrefix } from "@hawknext/shared";
  ...
  return (
    (
      globalThis as typeof globalThis & {
        __HAWKNEXT_RENDERER_DISABLE_LOGGING__?: boolean;
      }
    ).__HAWKNEXT_RENDERER_DISABLE_LOGGING__ === true
  );
}
...
function log(level: LogLevel, ...args: unknown[]) {
  ...
  // ui 包单独 typecheck 时拿不到 desktop renderer 注入的 window.hawknext 声明，
  ...
  if (level !== "debug" && typeof window !== "undefined") {
    ...
    (window as DesktopLogBridgeWindow).hawknext?.log?.(level, args);
  }
}
...
export function logMemoryDiagnostics(line: string): void {
  ...
  if (typeof window !== "undefined") {
    const bridge = (window as DesktopLogBridgeWindow).hawknext?.log;
    if (bridge) {
      bridge("info", [line]);
      return;
    }
