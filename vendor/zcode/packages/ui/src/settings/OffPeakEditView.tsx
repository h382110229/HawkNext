...
interface OffPeakEditViewProps {
  editing: HawkNextOffPeakTask | null;
  ...
  onDeleteHistory?: (task: HawkNextOffPeakTask) => void;
...
export function OffPeakEditView({
  ...
  const thoughtLevelOption = useMemo<HawkNextConfigOption | null>(
    () =>
    ...
    >
      ...
      {tab === "history" ? (
        <OffPeakHistoryTab
          ...
          {...(onOpenSession
            ? {
                onOpenSession: (task: HawkNextOffPeakTask) =>
                  task.sessionId
                    ? onOpenSession({
