...
import { RENDERER_HAWKNEXT_ENDPOINT_URLS } from "@/lib/rendererHawkNextEndpoint.js";
...
interface CodingPlanEmbeddedWebviewDialogProps {
   ...
   * 官网页购买成功后的回调。
   *
   * 官网页通过 preload 注入的 window.hawknextBridge.notifyPurchaseComplete({ provider })
   * 发送 hawknext:coding-plan-purchase-complete 频道消息，本组件在 webview 的
...
export function CodingPlanEmbeddedWebviewDialog({
  ...
  const theme = useHawkNextStoreWithDefault((state) => state.theme, "zai-dark");
  const userId = useHawkNextStoreWithDefault((state) => state.user?.id ?? null, null);
  ...
  const computedWebviewUrl = useMemo(() => {
    ...
    const origin = resolveCodingPlanEmbeddedOrigin({
      endpointOrigin: RENDERER_HAWKNEXT_ENDPOINT_URLS.origin,
      e2eStoreBridgeEnabled: env.VITE_HAWKNEXT_E2E_STORE_BRIDGE === "1",
    ...
    async (webview: ElectronWebviewTag | null = webviewRef.current) => {
      ...
      try {
        ...
        const credentials: CodingPlanEmbeddedCredentials =
          provider === "zai"
            ? {
                cloudAccessToken: values[0],
                hawknextJwtToken: values[1],
              }
            : {
                cloudCnAccessToken: values[0],
                // CloudCN OAuth callback 同样会落盘 hawknext JWT；官网用它在
                // hawknext-plan 域查 billing/balance 判定 Start Plan 状态。
                hawknextJwtToken: values[1],
              };
  ...
  const handleWebviewRef = useCallback(
    (element: ElectronWebviewTag | null) => {
      ...
      // ipc-message: 官网页通过 preload 的 window.hawknextBridge.notifyPurchaseComplete
      // 发回购买完成信号（hawknext:coding-plan-purchase-complete）。
      ...
      const handleIpcMessage = (event: ElectronWebviewIpcMessageEvent) => {
        ...
        if (raw?.provider !== "zai" && raw?.provider !== "cloud-cn") {
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          ...
          <webview
            ...
            partition="persist:hawknext-coding-plan"
            src={webviewUrl}
            className={cn(
