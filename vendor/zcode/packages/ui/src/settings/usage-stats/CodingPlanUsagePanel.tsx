...
import { BUILTIN_MODEL_PROVIDER_IDS } from "@hawknext/shared";
...
export function CodingPlanUsagePanel({
  ...
  const cloudCnEntitlement = useUsageEntitlement({
    enabled:
      !loadingSources &&
      effectiveProviderId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan &&
      Boolean(individualProviderFingerprint) &&
      !effectiveSourceIsTeamPlan,
    includeSubscription: true,
  ...
  const teamEntitlement = useUsageEntitlement({
    // 原硬绑 cloudCnCodingPlan providerId，zai team source 永远进不来。
    // 改为按 effectiveSourceIsTeamPlan 路由，providerId 按 effectiveSource 动态取，
    // zai team → cloudCodingPlan，cloud-cn team → cloudCnCodingPlan。
...
function CodingPlanQuotaCards({
  ...
  accountAccess:
    | import("@hawknext/shared").HawkNextProviderAccountAccess
    | import("@hawknext/shared").HawkNextAccountAccess
    | undefined;
            ...
            <div key={card.key} className="min-w-0 flex-1 rounded-xl bg-surface/70 p-4">
              ...
              <div className="flex min-h-5 min-w-0 items-center gap-1">
                ...
                {card.key === "serverMcp" ? (
                  <ControlHintTooltip
                    title={intl.formatMessage({
                      id: "sidebar.usage.plan.hawknextMcpDescription",
                    })}
                    standalone
                  >
                    <button
                      ...
                      data-hawknext-mcp-info="usage-stats"
                    >
...
function shouldShowCodingPlanUsageDetailSummary({
  ...
  // 但 CloudCN 官网不展示积分摘要卡；只有真实 credits 数据才展示这组三卡。
