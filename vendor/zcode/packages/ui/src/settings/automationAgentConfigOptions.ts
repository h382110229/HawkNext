import type { HawkNextConfigOption, HawkNextProvider } from "@hawknext/shared";
...
export function buildAutomationModeOption(currentValue: string): HawkNextConfigOption {
  return {
    id: "mode",
    name: "Mode",
    category: "mode",
...
export function resolveAutomationModelItem(
  ...
  // workspace runtime 也可能返回 hawknext-official-model-compatible/model 这类包装值，provider 对不上时
...
export function buildAutomationThoughtLevelOption(
  runtimeOption: HawkNextConfigOption | undefined,
  currentValue: string,
