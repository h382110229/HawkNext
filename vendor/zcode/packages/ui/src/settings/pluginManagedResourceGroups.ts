import type {
  ...
  HawkNextPluginInfo,
  HawkNextPluginComponentKind,
  HawkNextPluginsDescribeResult,
} from "@hawknext/shared";
import { isPluginCommand, isUserCommand, HAWKNEXT_COMMAND_AGENT_SOURCE } from "@hawknext/shared";
...
const COMPONENT_KIND_ORDER: HawkNextPluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: HawkNextPluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<HawkNextPluginComponentKind, PluginComponentDisplayGroup>();
...
export interface PluginMcpServerItem {
  active: boolean;
  authorization?: HawkNextMcpServerStatusSnapshot["authorization"];
  error?: string;
  failureKind?: HawkNextMcpServerStatusSnapshot["failureKind"];
  id: string;
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === HAWKNEXT_COMMAND_AGENT_SOURCE &&
...
export function filterLocalMcpServers(servers: HawkNextMcpServer[], query: string): HawkNextMcpServer[] {
  return servers.filter((server) => {
    if (server.source !== "hawknextagentmcp") {
      return false;
    }
...
function toPluginMcpServerDisplayName(plugin: HawkNextPluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: HawkNextPluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: HawkNextMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
