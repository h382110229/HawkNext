...
import { compareDocumentPluginPriority, isPluginCommand, isUserCommand } from "@hawknext/shared";
...
export function selectBuiltInPlugins(
  ...
  installedPlugins: readonly HawkNextInstalledPluginSummary[],
): HawkNextPluginInfo[] {
...
interface PluginSettingsGroups {
  installed: HawkNextPluginInfo[];
  builtIn: HawkNextPluginInfo[];
}
...
export function selectPluginsForScope(
  plugins: readonly HawkNextPluginInfo[],
  _installedPlugins: readonly HawkNextInstalledPluginSummary[],
  _scope: HawkNextPluginScope,
): HawkNextPluginInfo[] {
...
export function selectSkillsForScope(
  ...
  scopedPlugins: readonly Pick<HawkNextPluginInfo, "id" | "name" | "enabled">[],
...
export function selectCommandsForScope(
  commands: readonly HawkNextCommand[],
  scopedPlugins: readonly Pick<HawkNextPluginInfo, "id" | "name" | "enabled">[],
  scope: HawkNextPluginScope,
): HawkNextCommand[] {
  const enabledPlugins = scopedPlugins.filter((plugin) => plugin.enabled);
