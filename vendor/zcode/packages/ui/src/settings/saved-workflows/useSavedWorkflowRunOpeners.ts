...
import type { HawkNextSavedWorkflowRun } from "@hawknext/shared";
import type {
...
export function useSavedWorkflowRunOpeners(options: {
  ...
  resolveTarget: (run: HawkNextSavedWorkflowRun) => SavedWorkflowRunOpenTarget | null;
  ...
  handleOpenRun: (run: HawkNextSavedWorkflowRun, workflowName: string) => void;
  handleOpenArtifact: (run: HawkNextSavedWorkflowRun, artifactId: string) => void;
} {
  const { onOpenWorkflowArtifact, onOpenWorkflowRun, resolveTarget } = options;

  const handleOpenRun = useCallback(
    (run: HawkNextSavedWorkflowRun, workflowName: string) => {
  ...
  const handleOpenArtifact = useCallback(
    (run: HawkNextSavedWorkflowRun, artifactId: string) => {
      if (!run.parentSessionId) return;
      const target = resolveTarget(run);
      if (!target) return;
      // 行上的 chip 载荷（`HawkNextSavedWorkflowRun.artifacts`）带最新版的 `contentType`：
