...
import {
  ...
  type HawkNextSavedWorkflowEntry,
  type HawkNextSavedWorkflowMeta,
  type HawkNextSavedWorkflowRun,
  type HawkNextWorkflowsGetResult,
} from "@hawknext/shared";
import type { IHawkNextAgentService, HawkNextAgentSavedWorkflowTarget } from "@hawknext/services";
...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<HawkNextWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: HawkNextSavedWorkflowMeta): MetaDraft {
  return {
...
interface SavedWorkflowDetailViewProps {
  ...
  target: HawkNextAgentSavedWorkflowTarget;
  agentService: IHawkNextAgentService;
  name: string;
  /** 面包屑「自动化 › <项目名> › <工作流名>」里的项目一级。 */
  projectLabel: string;
  entry: HawkNextSavedWorkflowEntry | undefined;
  runs: readonly HawkNextSavedWorkflowRun[];
  now: number;
  busy: boolean;
  canOpenRun: boolean;
  onBack: () => void;
  ...
  onMove?: () => void;
  onDelete: () => void;
  onOpenRun: (run: HawkNextSavedWorkflowRun) => void;
  /** 产物 chip → `workflow-artifact` tab；缺席即 chips 只读。 */
  onOpenArtifact?: (run: HawkNextSavedWorkflowRun, artifactId: string) => void;
  onMetaSaved: () => void;
  /** 传入即在运行历史每行渲染项目列（全局工作流跨项目历史）。 */
  resolveRunProject?: (run: HawkNextSavedWorkflowRun) => SavedWorkflowRunProject | null;
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: HawkNextSavedWorkflowMeta = {
      description,
