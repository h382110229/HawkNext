...
export function SavedWorkflowProjectGroup({
  ...
  const agentService = services.hawknextAgentService;
  ...
  const launch = useCallback(
    async (entry: HawkNextSavedWorkflowEntry, args: Record<string, unknown>) => {
  ...
  const handleCardPromote = useCallback(
    (entry: HawkNextSavedWorkflowEntry) => void handlePromote(entry),
    [handlePromote],
  );
