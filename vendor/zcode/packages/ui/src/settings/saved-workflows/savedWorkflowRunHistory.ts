...
import type { HawkNextSavedWorkflowRun, HawkNextSavedWorkflowRunStatus } from "@hawknext/shared";
...
): Map<string, HawkNextSavedWorkflowRun> {
  const byName = new Map<string, HawkNextSavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: HawkNextSavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: HawkNextSavedWorkflowRun, now: number): number {
