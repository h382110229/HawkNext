...
function SavedWorkflowLastRunBadge({
  ...
  run: HawkNextSavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: HawkNextSavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: HawkNextSavedWorkflowEntry) => void;
  onRun: (entry: HawkNextSavedWorkflowEntry) => void;
  onRevise: (entry: HawkNextSavedWorkflowEntry) => void;
  onCopyPath: (entry: HawkNextSavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: HawkNextSavedWorkflowEntry) => void;
  onDelete: (entry: HawkNextSavedWorkflowEntry) => void;
}
