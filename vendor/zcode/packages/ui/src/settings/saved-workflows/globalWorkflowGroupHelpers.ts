...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type HawkNextSavedWorkflowRun } from "@hawknext/shared";
import type { HawkNextAgentSavedWorkflowTarget } from "@hawknext/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: HawkNextAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: HawkNextSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
