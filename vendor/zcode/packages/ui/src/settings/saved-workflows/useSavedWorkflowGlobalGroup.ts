...
// max-lines 400；载体是 `useServices().hawknextAgentService`，RPC 一律带 `{ scope: "global" }`。
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import type { HawkNextSavedWorkflowEntry, HawkNextSavedWorkflowRun } from "@hawknext/shared";
...
export function useSavedWorkflowGlobalGroup({
  ...
  const { hawknextAgentService: agentService, fileWatcherService } = useServices();
  ...
  const [launchEntry, setLaunchEntry] = useState<HawkNextSavedWorkflowEntry | null>(null);
  const [moveEntry, setMoveEntry] = useState<HawkNextSavedWorkflowEntry | null>(null);
  ...
  // 目录监听：list 回的绝对目录（`~/.hawknext/workflows`）本机可直接 watch。
  ...
  const handleRun = useCallback(
    (entry: HawkNextSavedWorkflowEntry) => {
      launcher.clearError();
  ...
  const handleDelete = useCallback(
    async (entry: HawkNextSavedWorkflowEntry) => {
  ...
  const handleCardDelete = useCallback(
    (entry: HawkNextSavedWorkflowEntry) => void handleDelete(entry),
    [handleDelete],
  );
  const handleCardMove = useCallback((entry: HawkNextSavedWorkflowEntry) => setMoveEntry(entry), []);
  const handleOpen = useCallback(
    (entry: HawkNextSavedWorkflowEntry) => onOpenDetail(entry.name),
    [onOpenDetail],
  );
  // 全局档的目标项目按 `run.cwd` 反查已打开项目；没打开就两个入口都关掉
  // （实例必须开在发起它的项目）。门与实参构造与项目档共用（产物不需要 toolCallId）。
  const resolveRunTarget = useCallback(
    (run: HawkNextSavedWorkflowRun) => findProjectByCwd(localProjects, run.cwd) ?? null,
    [localProjects],
  );
