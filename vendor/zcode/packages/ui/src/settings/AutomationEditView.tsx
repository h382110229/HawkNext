...
import {
  ...
  type HawkNextAutomationScheduleRule,
...
interface AutomationEditViewProps {
  /** null = 新建；否则编辑。 */
  editing: HawkNextAutomation | null;
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: HawkNextAutomation) => Promise<void> | void;
...
function resolveRunStatus(run: HawkNextAutomationRun): RunStatusKind {
...
function normalizeAutomationScheduleRule(
  rule: HawkNextAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: HAWKNEXT_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<HawkNextAutomationScheduleRule | undefined>(
    () =>
