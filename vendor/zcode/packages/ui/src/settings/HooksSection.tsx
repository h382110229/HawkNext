...
import type { Hook, HookConfig, HawkNextInstalledPluginSummary, HawkNextPluginInfo } from "@hawknext/shared";
import { Button } from "@/components/ui/button.js";
import { toast } from "@/components/ui/toast.js";
import { useConfirmDialog } from "@/hooks/useConfirmDialog.js";
import { useWorkspaceServicesResolution } from "@/hooks/useWorkspaceServices.js";
import { useHawkNextSessionService } from "@/hooks/useHawkNextSessionService.js";
import { useHawkNextIntl } from "@/i18n/IntlProvider.js";
import { invalidateDeferredDraftSessionForRuntimeChange } from "@/lib/hawknextDraftSkillInvalidation.js";
...
function isEditableHook(hook: Hook): boolean {
  ...
  return hook.editable ?? (!hook.location || hook.location.source === "hawknext");
}

// workspace-hook-trust：editable=false 且 source=hawknext 的行是「上游/祖先 hawknext.json
// 里的只读工作区 Hook」。它们不是外部格式兼容导入源，塞进 Legacy 会让 Import 按钮
// 必然失败（importHook 拒绝 source=hawknext），也违反「只读但可逐条 Trust」的约定。
// 这类行应留在 Installed 分组，由信任状态门控 Switch，走行内 Trust 流程。
function isReadOnlyHawkNextHook(hook: Hook): boolean {
  return hook.editable === false && (hook.location?.source ?? "hawknext") === "hawknext";
}

function isInCompatibilitySection(hook: Hook): boolean {
  return !isEditableHook(hook) && !isReadOnlyHawkNextHook(hook);
}
...
function buildPluginHookRows(
  plugins: readonly Pick<HawkNextPluginInfo, "enabled" | "hookDetails" | "id" | "name">[],
  installedPlugins: readonly Pick<HawkNextInstalledPluginSummary, "id" | "scope">[],
...
export function HooksSection({ workspacePath, workspaceIdentity }: HooksSectionProps) {
  ...
  const hawknextSessionService = useHawkNextSessionService(
  ...
  const editableHooks = useMemo(
    () =>
      hooksState.hooks.filter(
        (hook) =>
          (isEditableHook(hook) || isReadOnlyHawkNextHook(hook)) &&
  ...
  const invalidateDraft = useCallback(
    ...
    [targetWorkspaceIdentity, targetWorkspacePath, hawknextSessionService],
  );
