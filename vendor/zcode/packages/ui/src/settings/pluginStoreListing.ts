...
export function resolveUniquePluginListingByName(
  plugins: readonly Pick<HawkNextAvailablePluginSummary, "name" | "listing">[],
  name: string,
): HawkNextPluginStoreListing | undefined {
  const normalizedName = name.trim().toLocaleLowerCase();
  let match: HawkNextPluginStoreListing | undefined;
  let count = 0;
...
export interface StorePluginItem {
  ...
  orphaned: boolean;
  listing?: HawkNextPluginStoreListing;
  summary?: HawkNextAvailablePluginSummary;
  /** 运行时信息（仅已发现的已安装插件有）：启用态、组件、manifest 回退字段。 */
  info?: HawkNextPluginInfo;
  installedMeta?: HawkNextInstalledPluginSummary;
}

export type PluginUpdateStatus = NonNullable<HawkNextInstalledPluginSummary["updateStatus"]>;
...
const OFFICIAL_MARKETPLACE_ORDER: readonly string[] = [HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE_ID];
...
export function sortMarketplaceSources(
  marketplaces: readonly HawkNextPluginMarketplaceSummary[],
  locale: string,
): HawkNextPluginMarketplaceSummary[] {
...
export function buildStoreItems(input: {
  marketplaces: HawkNextPluginMarketplaceSummary[];
  marketplaceAvailabilityKnown: boolean;
  availablePlugins: HawkNextAvailablePluginSummary[];
  installedPlugins: HawkNextInstalledPluginSummary[];
  plugins: HawkNextPluginInfo[];
  restorableBuiltins: HawkNextAvailablePluginSummary[];
}): StorePluginItem[] {
