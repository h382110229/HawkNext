import type { HawkNextCommand, HawkNextSlashCommand } from "@hawknext/shared";
...
function commandToSlashCommand(command: HawkNextCommand): HawkNextSlashCommand | null {
  if (!command.enabled) {
    return null;
  }
...
export function mergeSlashCommandsAfterCommandRefresh(
  currentSlashCommands: readonly HawkNextSlashCommand[],
  refreshedCommands: readonly HawkNextCommand[],
  ...
  const customCommands: HawkNextSlashCommand[] = [];
