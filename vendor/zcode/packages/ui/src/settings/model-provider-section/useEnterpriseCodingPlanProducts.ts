...
/**
 * 原 hook 只服务 cloud-cn family，getStaticTeamProducts 硬编码
 * 读 cloudCnCodingPlan bucket、getEnterprisePricing 不传 family。
 * zai family 对称化后，hook 接受 family 参数：
 *   - 按 family 读 static bucket（cloudCodingPlan / cloudCnCodingPlan）
 *   - 传 family 给 service.getEnterprisePricing，service 据此路由到对应 provider
 * 缺省 family 时保持 cloud-cn，向后兼容既有调用点。
 */
