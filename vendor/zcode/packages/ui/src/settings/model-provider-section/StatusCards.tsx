...
import {
  CloudCNRegistrationHint,
  isCloudCNUnregisteredAuthError,
} from "./CloudCNRegistrationHint.js";
...
export function CodingPlanStatusPanel({
  ...
  quotaResetAccountAccess?: HawkNextProviderAccountAccess | HawkNextAccountAccess;
  ...
  const shouldShowCloudCNRegistrationHint =
    isChecking &&
    providerIcon === CLOUD_CN_PROVIDER_ID &&
    isCloudCNUnregisteredAuthError(authError);
  ...
  const statusMeta =
    isPurchased && isStartPlanProvider ? (
      ...
      />
    ) : shouldShowCloudCNRegistrationHint ? (
      <CloudCNRegistrationHint onOpenRegistration={onOpenRegistration} />
          ...
          {infoDescription ? (
            <ControlHintTooltip title={infoDescription} standalone>
              <button
                ...
                data-hawknext-mcp-info="model-settings"
              >
...
function findUsageLimit(
  ...
  return limits.find(
    (limit) =>
      // zai team plan 返回 CREDIT_LIMIT，cloud-cn 返回 TOKENS_LIMIT，
