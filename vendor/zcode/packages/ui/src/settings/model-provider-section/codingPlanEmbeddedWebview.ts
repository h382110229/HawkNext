...
import type { CodingPlanWebviewLocale } from "@hawknext/shared";
...
type CodingPlanWebsiteProvider = "zai" | "cloud-cn";
...
export interface CodingPlanEmbeddedCredentials {
  ...
  cloudCnAccessToken?: string | null;
}
...
const CODING_PLAN_WEBVIEW_CREDENTIAL_LOCAL_STORAGE_KEYS = [
  "oauth:cloud-intl:access_token",
  "hawknext-jwt",
  "oauth:cloud-cn:access_token",
] as const;
const CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY = "hawknext:coding-plan:report-context";

export function resolveCodingPlanWebsiteProvider(
  providerId: CodingPlanProviderId,
): CodingPlanWebsiteProvider {
  return isCloudCodingPlanProviderId(providerId) ||
...
export function resolveCodingPlanEmbeddedOrigin({
  ...
  if (
    ...
    return normalizeHawkNextEndpointOrigin(normalizedOverride);
  }
  const normalizedEndpointOrigin = normalizeHawkNextEndpointOrigin(endpointOrigin);
...
export function buildCodingPlanEmbeddedWebviewUrl({
  ...
  const url = new URL("/coding-plan", normalizeHawkNextEndpointOrigin(origin));
...
export function createCodingPlanAuthInjectionScript({
  ...
  // App 当前 locale，写入 window.__hawknextLang__ 供 hawknextBridge.getLang() 读取，
  ...
  const values: Record<string, string | null> =
    provider === "zai"
      ? {
          "oauth:cloud-intl:access_token": credentials.cloudAccessToken?.trim() || null,
          "hawknext-jwt": credentials.hawknextJwtToken?.trim() || null,
          "oauth:cloud-cn:access_token": null,
        }
      : {
          "oauth:cloud-intl:access_token": null,
          // hawknext-jwt 是 hawknext-plan 域通用凭证（CloudCN OAuth callback 同样落盘），
          // 官网用它查 billing/balance 判定 Start Plan 是否使用中；CloudCN 分支缺失注入
          // 会导致官网 Start Plan 卡因查不到权益而误显示「已过期」。业务接口仍走
          // oauth:cloud-cn:access_token，互不污染。
          "hawknext-jwt": credentials.hawknextJwtToken?.trim() || null,
          "oauth:cloud-cn:access_token": credentials.cloudCnAccessToken?.trim() || null,
        };
  ...
  return `(() => {
  ${storageUpdates}
  const hawknextTheme = ${JSON.stringify(theme)};
  document.documentElement.classList.toggle("dark", hawknextTheme === "zai-dark");
  document.documentElement.classList.toggle("theme-zai-light", hawknextTheme === "zai-light");
  document.documentElement.classList.toggle("theme-zai-dark", hawknextTheme === "zai-dark");
  localStorage.setItem("hawknext-theme", hawknextTheme);
  localStorage.setItem("hawknext:coding-plan:embedded", "app");
  // 写入当前 App locale，供官网 hawknextBridge.getLang() 读取。
  // 注意：这是注入 webview 执行的原始 JS，不能用 TS 语法（如 as any）。
  window.__hawknextLang__ = ${JSON.stringify(resolvedLocale)};
  const hawknextReportContext = ${JSON.stringify(normalizedReportContext)};
  window.__hawknextReportContext__ = hawknextReportContext;
  localStorage.setItem(${JSON.stringify(CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY)}, JSON.stringify(hawknextReportContext));
  window.dispatchEvent(new CustomEvent("hawknext-coding-plan-auth-ready", {
    detail: { ...${JSON.stringify({ provider, locale: resolvedLocale })}, reportContext: hawknextReportContext },
  }));
})()`;
}
...
export function createCodingPlanCredentialClearScript(): string {
  ...
  delete window.__hawknextReportContext__;
})()`;
}

export function createCodingPlanScrollbarHideScript(): string {
  return `(() => {
  const styleId = "hawknext-coding-plan-hide-scrollbar";
 ...
 * 重写 window.__hawknextLang__ 并派发 hawknext-coding-plan-lang-change 事件，
 * 官网侧（hawknextBridge.onLangChange 或 window 监听）据此无感切换语言。
 */
export function createCodingPlanLangInjectionScript(locale: CodingPlanWebviewLocale): string {
  ...
  window.__hawknextLang__ = ${JSON.stringify(resolvedLocale)};
  window.dispatchEvent(new CustomEvent("hawknext-coding-plan-lang-change", {
...
export function getCodingPlanCredentialKeys(provider: CodingPlanWebsiteProvider): string[] {
  // hawknext-jwt 对两个 provider 都加载：它是 hawknext-plan 域通用凭证，
  // CloudCN OAuth callback 同样落盘（见 resolveCloudCNStartPlanHawkNextJwt）。
  return provider === "zai"
    ? [`oauth:${CLOUD_INTL_PROVIDER_ID}:access_token`, "hawknext-jwt"]
    : [`oauth:${CLOUD_CN_PROVIDER_ID}:access_token`, "hawknext-jwt"];
}
