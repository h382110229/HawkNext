...
export function useCodingPlanProducts(
  ...
  const supportedProvider =
    ...
    isCloudCodingPlanProviderId(providerId);
...
async function loadCodingPlanProductsForTest(
  ...
  // CloudCN/ 套餐预览接口对连发请求会偶发返回“系统繁忙”；这里合并进行中请求，并只短缓存成功结果，
...
function buildZaiStartStaticProducts(preview: StartPlanPreviewConfig): CodingPlanStaticProduct[] {
  ...
  const previewName = preview.name.trim() || " Start";
  ...
  return [
    {
      ...
      description: "Free Coding Plan entry for connected  users.",
    ...
    },
    {
      ...
      productName: previewName,
      productSmallTitle: "Free Coding Plan",
      description: "Free Coding Plan entry for signed-in  users.",
      productEquityList: equityList,
      priceUnit: "quarter",
      displayOrder: 0,
      priceCurrency: "USD",
...
async function loadCodingPlanStaticProductListForTest(
  ...
  try {
    ...
    //  - Coding Plan 的购买列表只展示付费升级项，避免把 Start 当成可购买套餐重复显示。
...
function isCodingPlanOAuthRequiredMessage(message: string): boolean {
  ...
  return (
    normalized === "cloud-cn_oauth_required" ||
