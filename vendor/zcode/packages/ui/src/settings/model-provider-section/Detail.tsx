...
import type { SavePersonalModelDraftInput } from "@hawknext/provider";
...
export function ModelProviderSectionDetail({
  ...
  onOpenCloudCNRegistration: () => void;
  ...
  if (effectiveSelectedPlanNavItem) {
    ...
    const purchaseChoiceBannersVisible =
      ...
      (selectedNavItem.oauthProviderId === CLOUD_INTL_PROVIDER_ID ||
        (selectedNavItem.oauthProviderId === CLOUD_CN_PROVIDER_ID &&
    ...
    const planSupplementalContent =
      isStartPlanProvider && hasActivePaidPlan ? null : anonymousPurchaseChoiceBannersVisible ||
        ...
        <CodingPlanPurchaseChoiceBanners
          ...
          teamVisible={selectedNavItem.oauthProviderId !== CLOUD_INTL_PROVIDER_ID}
        />
      ) : accessBanner ? (
    ...
    if (shouldShowDedicatedProviderDetail && dedicatedProvider) {
      const statusPanel = (
        <CodingPlanStatusPanel
          ...
          onOpenRegistration={onOpenCloudCNRegistration}
...
function CodingPlanPurchaseChoiceBanners({
  ...
  const enterpriseProducts = useEnterpriseCodingPlanProducts({
    ...
    enabled:
      teamVisible && pricingProviderId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
  ...
  if (audience === "personal") {
    ...
    if (product) {
      return {
        ...
        // 不能让 formatter 默认落到 CNY，否则  Global 入口会错误显示 RMB。
        currency:
          product.product.priceCurrency ??
          (providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan ? "CNY" : "USD"),
      };
    }
    if (
      soldOutVisible &&
