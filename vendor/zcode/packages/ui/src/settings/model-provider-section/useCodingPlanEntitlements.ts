...
import type { ProviderSettingsView } from "@hawknext/services";
import {
...
function resolveEntitlementAccountAccess(
  access: HawkNextProviderAccountAccess | undefined,
  selection: ProviderFamilyConnectionSelection | undefined,
): HawkNextProviderAccountAccess | HawkNextAccountAccess | undefined {
...
export function useCodingPlanEntitlements({
  ...
  const cloudCnFamily = useProviderFamilyEntitlements({
    familySpec: getModelProviderFamilySpec("cloud-cn"),
    selection: connectionSelections.cloudCn,
    providerSettingsView,
  });
  ...
  const refresh = useCallback(
    (options: UsageEntitlementRefreshOptions = {}) => {
      ...
      if (cloudCnFamily.codingEnabled) {
        refreshJobs.push(cloudCnFamily.coding.refresh(options));
      }
      if (cloudCnFamily.startEnabled) {
        refreshJobs.push(cloudCnFamily.start.refresh(options));
      }
      return Promise.all(refreshJobs).then(() => undefined);
    },
    [
      cloudCnFamily.coding.refresh,
      cloudCnFamily.codingEnabled,
      cloudCnFamily.start.refresh,
      cloudCnFamily.startEnabled,
  ...
  const providerFingerprint = useMemo(
    () =>
      [
        ...
        cloudCnFamily.codingFingerprint,
        cloudCnFamily.startEnabled ? cloudCnFamily.startProviderFingerprint : "",
      ]
        .filter(Boolean)
        .join("|"),
    [
      cloudCnFamily.codingFingerprint,
      cloudCnFamily.startEnabled,
      cloudCnFamily.startProviderFingerprint,
      zaiFamily.codingFingerprint,
      zaiFamily.startEnabled,
  ...
  return useMemo(
    () => ({
      entitlements: {
        ...
        [cloudCnFamily.codingPlanProviderId]: {
          snapshot: cloudCnFamily.coding.snapshot,
          loading: cloudCnFamily.coding.loading,
          error: cloudCnFamily.coding.error,
        },
        [cloudCnFamily.startPlanProviderId]: {
          snapshot: cloudCnFamily.startEnabled ? cloudCnFamily.start.snapshot : null,
          loading: cloudCnFamily.startEnabled ? cloudCnFamily.start.loading : false,
          error: cloudCnFamily.startEnabled ? cloudCnFamily.start.error : null,
        },
      },
      enabledStartPlanProviderIds: [
        ...(zaiFamily.startEnabled ? [zaiFamily.startPlanProviderId] : []),
        ...(cloudCnFamily.startEnabled ? [cloudCnFamily.startPlanProviderId] : []),
      ],
      refresh,
    }),
    [cloudCnFamily, refresh, zaiFamily],
  );
}
