...
export function resolveCodingPlanEntitlementState({
  ...
  accountAvailability?: import("@hawknext/provider").AccountProviderState["availability"];
  accountUnavailableReason?: import("@hawknext/provider").AccountProviderState["unavailableReason"];
  ...
  if (currentSubscription) {
    return {
      // /CloudCN 的真实套餐状态来自 subscription/list。
      status: "purchased",
      ...(entitlement?.error
...
function filterStartPlanItemsByEntitlement({
  ...
  // 原变量名 hasCloudCNTeamPlan 暗示只服务 cloud-cn，但逻辑
  // （entitlement 或 subscribedTeamProducts）本身是 family 无关的。
  // 重命名为中性名称，并在下方 filter 去掉 familySpec.id === "cloud-cn" 守卫，
 ...
 * 原 appendSubscribedTeamPlanItems 硬编码找 cloudCnCodingPlan，
 * zai team plan items 无对应展示基线。zai/cloud-cn 对称化后，team item 的
...
function appendSubscribedTeamPlanItems({
  ...
  // 原实现先 items.find(cloudCnCodingPlan)，不存在时直接 return items。
  // 当设置页只展示 zai family（providerFamilyDomain === "zai"）时，codingPlanItems 里
  // 没有 cloudCnCodingPlan，这个守卫会让 appendSubscribedTeamPlanItems 整体短路，
  // zai teamPlan item 永远不生成 → pickFamilyModeNavigationItem 找不到 saved team item
  // → selectedNavItem=null → 右侧 Plan Card 永远卡在 "加载中"。
  // 对称化：去掉 cloud-cn 硬编码前置守卫，entitlement/fallback/product 三个 builder
  // 各自按 family 解析对应 codingPlanItem，不存在就跳过该 family。

  // entitlement + fallback 两个 builder 原来只对 cloudCnCodingPlanItem 调用，
  ...
  const productTeamItems: TeamPlanNavItem[] = subscribedTeamProducts.flatMap((product) => {
    ...
    // 缺省 cloud-cn，向后兼容未标记 family 的旧数据。
    ...
    return projectContexts.flatMap((projectContext) => {
      ...
      // 去重 key 必须包含 family 维度，否则 zai/cloud-cn 相同 productId+org+project 会互相覆盖。
      ...
      return [
        {
          ...
          subscriptionRenewTime: null,
          subscriptionExpireTime: null,
          // Team Plan 项目没有可用 hawknext-team-api-key 时，不能继续当作已启用连接方式。
  ...
  // 原写法硬编码 items.findIndex(cloudCnCodingPlanItem.key) 作为插入点，
  // zai-only 视图下 cloudCnCodingPlanItem 不存在会 throw（.key 访问 undefined）。
...
function buildEntitlementTeamPlanItems(
  ...
  // 原硬编码读 cloudCnCodingPlan bucket + cloud-cn team key。
  // zai/cloud-cn 对称化后，按 family 读对应 codingPlan bucket、生成对应前缀 team key。
