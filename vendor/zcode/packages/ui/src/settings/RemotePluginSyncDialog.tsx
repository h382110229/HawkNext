...
import type {
  ...
  RemoteTarget,
  HawkNextPluginOptionValue,
  HawkNextPluginInfo,
  HawkNextPluginMarketplaceSummary,
  HawkNextPluginUserConfigOption,
  HawkNextPluginsOverviewResult,
} from "@hawknext/shared";
import type { IPluginSyncService, IHawkNextAgentService } from "@hawknext/services";
...
interface RemotePluginSyncPluginOptions {
  configuredOptions: Record<string, HawkNextPluginOptionValue>;
  userConfig: Record<string, HawkNextPluginUserConfigOption>;
}
...
interface RemotePluginSyncDialogProps {
  ...
  localHawkNextAgentService?: RemotePluginSyncAgentService | null;
  remoteHawkNextAgentService?: RemotePluginSyncAgentService | null;
...
function buildMarketplaceRemotePluginSyncCandidates(
  overview: HawkNextPluginsOverviewResult,
  plugins: readonly HawkNextPluginInfo[],
): RemotePluginSyncCandidate[] {
...
function buildMarketplaceRemotePluginStatuses(
  candidates: readonly RemotePluginSyncCandidate[],
  remoteOverview: HawkNextPluginsOverviewResult,
...
function componentTypesFromPluginInfo(plugin: HawkNextPluginInfo | undefined): string[] {
  return plugin?.components?.map((component) => component.kind) ?? [];
}

function buildRemotePluginSyncPluginOptions(
  plugin: HawkNextPluginInfo | undefined,
...
function serializeMarketplaceSourceInput(
  marketplace: HawkNextPluginMarketplaceSummary,
): string | undefined {
...
async function loadRemotePluginSyncCandidates(params: {
  ...
  if (!params.localHawkNextAgentService || !params.remoteHawkNextAgentService) {
  ...
  const [localOverview, localPluginsResult, remoteOverview] = await Promise.all([
    params.localHawkNextAgentService.getPluginsOverview({
      workspacePath: params.localWorkspacePath,
    }),
    params.localHawkNextAgentService.listPlugins({
      workspacePath: params.localWorkspacePath,
    }),
    params.remoteHawkNextAgentService.getPluginsOverview({
...
function isPathPluginOption(option: HawkNextPluginUserConfigOption | undefined): boolean {
  return option?.type === "file" || option?.type === "directory";
}

function isPortablePluginOptionValue(
  value: HawkNextPluginOptionValue,
  localOption: HawkNextPluginUserConfigOption | undefined,
  remoteOption: HawkNextPluginUserConfigOption | undefined,
): boolean {
...
function buildPortablePluginOptionsSyncPlan(
  ...
  remotePlugin: HawkNextPluginInfo,
): { options: Record<string, HawkNextPluginOptionValue>; skippedCount: number } {
  const options: Record<string, HawkNextPluginOptionValue> = {};
  let skippedCount = 0;
...
function arePluginOptionsEqual(
  left: Record<string, HawkNextPluginOptionValue>,
  right: Record<string, HawkNextPluginOptionValue>,
): boolean {
...
async function syncPortablePluginOptions(
  ...
  const remoteHawkNextAgentService = params.remoteHawkNextAgentService;
  if (!remoteHawkNextAgentService) {
    emitRemotePluginSyncProgress(
      params,
      row,
      "syncing",
  ...
  const remotePlugins = await awaitRemotePluginSyncStep(
    ...
    () => remoteHawkNextAgentService.listPlugins(workspace),
  );
  ...
  await awaitRemotePluginSyncStep(
    ...
    () =>
      remoteHawkNextAgentService.configurePlugin({
        ...workspace,
...
async function syncMarketplaceRemotePlugin(
  ...
  try {
    throwIfRemotePluginSyncStopped(params, row);
    if (!params.remoteHawkNextAgentService || !marketplacePlugin) {
    ...
    const installResult = await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        remoteHawkNextAgentService.installPlugin({
          ...workspace,
    ...
    await awaitRemotePluginSyncStep(
      ...
      () =>
        remoteHawkNextAgentService.setPluginEnabled({
          ...workspace,
          enabled: candidate.enabled,
          pluginId: candidate.pluginId,
          scope: REMOTE_PLUGIN_SYNC_CONFIG_SCOPE,
...
async function prepareRemoteMarketplaceSource(
  params: {
    ...
    onItemProgress?: (event: RemotePluginSyncProgressEvent) => void;
    remotePluginSyncService: IPluginSyncService;
    remoteHawkNextAgentService: RemotePluginSyncAgentService;
    selectedMarketplacePluginNames: ReadonlyMap<string, string[]>;
    signal?: AbortSignal;
    stopControl?: RemotePluginSyncStopControl;
  },
  ...
  if (marketplaceSourceInput) {
    await awaitRemotePluginSyncStep(
      ...
      (operationId) =>
        params.remoteHawkNextAgentService.addPluginMarketplace({
          ...workspace,
          operationId,
      ...
      const result = await syncSelectedRemotePlugins({
        ...
        stopControl: {
          cancelOperation: async (operationId) => {
            if (!remoteHawkNextAgentService?.cancelPluginOperation) {
              return;
            }
            await remoteHawkNextAgentService.cancelPluginOperation({ operationId });
          },
