 ...
 * Supports the unified HawkNext Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToHawkNextAgentMcpServer,
...
import { isHawkNextAgentMcpStatusModeUnsupportedError, type IMcpSyncService } from "@hawknext/services";
...
const DEFAULT_MCP_SOURCE: ServerScope = "hawknextagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 hawknextAgentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isHawkNextAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "hawknextagentmcp" || !server.enabled) {
      continue;
    }
    ...
    if (!hawknextAgentServer) {
      continue;
    }
    seenNames.add(server.name);
    result.push(hawknextAgentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: HawkNextAgentMcpServer[]): {
  mode: "status";
  mcpServers: HawkNextAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: HawkNextMcpServer[]): string {
  return servers
    .filter((server) => server.source === "hawknextagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: HawkNextPluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, HawkNextAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<HawkNextMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForHawkNext("hawknext");
  ...
  async function handleSave(form: FormState, prev?: HawkNextMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: HawkNextMcpServer) {
  ...
  function handleEdit(server: HawkNextMcpServer) {
  ...
  function handleOpenAuthorization(server: HawkNextMcpServer) {
