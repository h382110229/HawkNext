...
import { TID_PLUGIN_STORE_BROWSE } from "@hawknext/shared";
import type { HawkNextPluginInfo, HawkNextPluginScope, HawkNextPluginUserConfigOption } from "@hawknext/shared";
...
function PluginList({
  ...
  const getPluginOptionValue = (
    ...
    key: string,
    option: HawkNextPluginUserConfigOption,
  ...
  const savePluginOptions = async (plugin: HawkNextPluginInfo) => {
  ...
  const renderPluginRows = (items: HawkNextPluginInfo[]) => (
  ...
  return (
    <section className="space-y-6" aria-labelledby="plugin-installed-title">
      ...
      <RemoteSyncDialogs
        ...
        remoteHawkNextAgentService={targetServiceResolution.services.hawknextAgentService}
