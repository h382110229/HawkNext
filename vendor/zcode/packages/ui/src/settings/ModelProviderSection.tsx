...
function resolveCodingPlanIntentProviderId(
  ...
  switch (target?.providerId) {
    ...
    case BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan:
    case BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan:
    case BUILTIN_MODEL_PROVIDER_IDS.cloudCnStartPlan:
...
function shouldRefreshCodingPlanEntitlementsAfterSave(
  ...
  const isCodingPlanProvider =
    ...
    nextProvider.providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan ||
    nextProvider.providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan ||
    nextProvider.providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnStartPlan;
...
function resolveBuiltinPresetOAuthProvider(
  ...
  if (
    presetId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan ||
    presetId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan ||
    presetId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnStartPlan
  ) {
...
export function ModelProviderSection({
  ...
  const requestLoginEntry = useHawkNextStore((state) => state.requestLoginEntry);
  const setUser = useHawkNextStore((state) => state.setUser);
  const oauthError = useHawkNextStore((state) => state.oauthError);
  const setOAuthError = useHawkNextStore((state) => state.setOAuthError);
  const {
  ...
  // zai 与 cloud-cn Team Plan 对称化。原仅 cloud-cn 调 hook，
  ...
  // 原仅检查 cloud-cn selectedKey 是否为 team plan，zai team key
  // 永远不会触发已购团队 fallback（断裂）。改为任一 family 有持久化 team key 即显示。
  const showPurchasedTeamPlanFallback = Boolean(
    effectiveConnectionSelections.cloudCn?.kind === "team-coding-plan" ||
  ...
  const refreshCodingPlanPurchaseTokenState = useCallback(
    async (
      ...
      const [activeProvider, zaiToken, cloudCnToken] = await Promise.all([
        credentialService.load("oauth:active_provider"),
        credentialService.load(`oauth:${CLOUD_INTL_PROVIDER_ID}:access_token`),
        credentialService.load(`oauth:${CLOUD_CN_PROVIDER_ID}:access_token`),
      ]);
      if (options.shouldApply && !options.shouldApply()) {
        return null;
      }
      const normalizedActiveProvider =
        activeProvider === CLOUD_INTL_PROVIDER_ID || activeProvider === CLOUD_CN_PROVIDER_ID
          ? activeProvider
          : null;
      setActiveOAuthProvider(normalizedActiveProvider);
      setCodingPlanPurchaseTokenAuthenticatedByProviderId({
        [BUILTIN_MODEL_PROVIDER_IDS.zaiIndividualCodingPlan]:
          normalizedActiveProvider === CLOUD_INTL_PROVIDER_ID && (zaiToken?.trim().length ?? 0) > 0,
        [BUILTIN_MODEL_PROVIDER_IDS.zaiTeamCodingPlan]:
          normalizedActiveProvider === CLOUD_INTL_PROVIDER_ID && (zaiToken?.trim().length ?? 0) > 0,
        [BUILTIN_MODEL_PROVIDER_IDS.zaiStartPlan]:
          normalizedActiveProvider === CLOUD_INTL_PROVIDER_ID && (zaiToken?.trim().length ?? 0) > 0,
        [BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan]:
          normalizedActiveProvider === CLOUD_CN_PROVIDER_ID &&
          (cloudCnToken?.trim().length ?? 0) > 0,
        [BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan]:
          normalizedActiveProvider === CLOUD_CN_PROVIDER_ID &&
          (cloudCnToken?.trim().length ?? 0) > 0,
  ...
  const handleCodingPlanLogin = useCallback(
    (
      ...
      // ZAI/CloudCN provider 不再有独立 connection，Connect 必须切换 App active provider。
  ...
  const handleCodingPlanDisconnect = useCallback(
    async (presetId: BuiltinModelProviderId, providerId: OAuthProviderId, providerName: string) => {
      if (providerId !== CLOUD_CN_PROVIDER_ID && providerId !== CLOUD_INTL_PROVIDER_ID) {
        return;
      }
      ...
      try {
        ...
        // ZAI/CloudCN provider 已恢复为 App 登录镜像。
    ...
    <ModelProviderSectionLayout
      ...
      onRefresh={() => {
        void refreshModelProviderSection({
          refresh,
          // 手动刷新设置页时也要同时刷新  / CloudCN Team Plan 快照；
          // 原来只刷新 CloudCN， Team Plan 购买或订阅变化后会继续显示旧项目。
          refreshTeamPlanProducts: refreshAuthenticatedEnterpriseProducts,
        });
        refreshCodingPlanEntitlements();
      }}
    ...
    >
      ...
      {templatePickerOpen ? (
        ...
        <ModelProviderSectionDetail
          ...
          onSelectNavItem={handleSelectNavItem}
          onOpenCloudCNRegistration={() => {
            // 未注册提示来自一次失败的 OAuth checking 状态；跳转注册后要恢复普通状态，避免提示卡住。
            setOAuthError(null);
            setPresetSubscriptionProviderId((current) =>
              current === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan ? null : current,
            );
            platform.openExternal(CLOUD_CN_REGISTRATION_URL);
          }}
