...
import { markHawkNextJwtInvalidRestart } from "@/root/hawknextJwtInvalidRestartMarker.js";
...
export function useRootOAuthEffects({
  ...
  useEffect(() => {
    ...
    async function restoreOAuthSessionInBackground() {
      ...
      try {
        // zai / cloud-cn 的 OAuth token 生命周期较短，启动时如果仍走远端校验，
  ...
  useEffect(() => {
    ...
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel !== HAWKNEXT_JWT_INVALID_BROADCAST_CHANNEL || disposed) {
        return;
      }
      void (async () => {
        ...
        markHawkNextJwtInvalidRestart();
        if (typeof window !== "undefined" && !("hawknext" in window)) {
