...
import type { Locale, LocalePreference } from "@hawknext/shared";
import { DEFAULT_LOCALE } from "@hawknext/shared";
import type { BroadcastMessage, IBroadcastService, ISettingService } from "@hawknext/services";
import {
...
const LOCALE_PREFERENCE_KEY = "hawknext-locale-preference";
 ...
 */
export function HawkNextIntlProvider({
  children,
  settingService,
  broadcastService,
  initialLocale,
...
/** 获取 intl 上下文 */
export function useHawkNextIntl(): IntlContextValue {
  const ctx = useContext(IntlContext);
  if (!ctx) {
    throw new Error("useHawkNextIntl 必须在 HawkNextIntlProvider 内使用");
  }
  return ctx;
}
