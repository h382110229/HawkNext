...
import type { GitChangeSourceId, WorkspacePurpose } from "@hawknext/shared";
import { useHawkNextStore } from "@/store/StoreProvider.js";
import { getVisibleTaskMetas, useHawkNextSessionStore } from "@/store/hawknextSessionStore.js";
...
import { HAWKNEXT_PRODUCT_DOCS_URL } from "@/lib/productDocs.js";
...
import { useWorkspaceShellHawkNextState } from "@/app-shell/useWorkspaceShellHawkNextState.js";
...
export function App({
  ...
  const { intl, locale, setLocale } = useHawkNextIntl();
  ...
  const workspaceReadOnlyReason = workspaceReadOnly
    ...
    : undefined;
  const { workspaceShellHawkNextState, reloadSessionDisabled } = useWorkspaceShellHawkNextState(
    workspaceAbsPath,
    workspaceIdentity,
  );
  const activeTaskId = workspaceShellHawkNextState.activeTaskId;
  ...
  const draftSessionId = useHawkNextSessionStore(
  ...
  const draftFocusVersion = workspaceShellHawkNextState.draftFocusVersion;
  const {
    isTerminalOpen,
    setIsTerminalOpen,
    sidePaneState,
  ...
  const notificationEnabled = useHawkNextStore((s) => s.notificationEnabled);
  ...
  const theme = useHawkNextStore((s) => s.theme);
  const setTheme = useHawkNextStore((s) => s.setTheme);
  const {
    isMacFullscreen,
  ...
  const {
    ...
    workspaceRemoteSessionId,
    workspaceIdentity,
    selectedProvider: workspaceShellHawkNextState.selectedProvider,
    intl,
  });
  ...
  const handleOpenProductDocs = useCallback(() => {
    platform.openExternal(HAWKNEXT_PRODUCT_DOCS_URL);
  }, [platform]);
  ...
  const handleStartDraftInWorkspace = useCallback(
    (
      ...
      createSource?: import("@hawknext/shared").SessionCreateSource,
    ) => {
      ...
      const targetSelectedProvider = resolveWorkspaceSwitchDraftProvider({
        currentSelectedProvider: workspaceShellHawkNextState.selectedProvider,
        targetWorkspacePath,
    ...
    [
      ...
      workspaceAbsPath,
      workspaceIdentity,
      workspaceShellHawkNextState.selectedProvider,
    ],
  );
  ...
  const testActions = useMemo<TestActions>(
    () => ({
      ...
      getPluginsOverview: (params) => services.hawknextAgentService.getPluginsOverview(params),
      addPluginMarketplace: (params) => services.hawknextAgentService.addPluginMarketplace(params),
      updatePluginMarketplace: (params) =>
        services.hawknextAgentService.updatePluginMarketplace(params),
      installPlugin: (params) => services.hawknextAgentService.installPlugin(params),
      listPlugins: (params) => services.hawknextAgentService.listPlugins(params),
      getPluginReferenceCatalog: (params) =>
        services.hawknextAgentService.getPluginReferenceCatalog(params),
    }),
    [locale, services.hawknextAgentService, setLocale, theme, setTheme, testMessages],
  );
  ...
  const handleSelectAdjacentConversation = useCallback(
    (direction: "previous" | "next") => {
      runVisibleWorkspaceCommand(() => {
        const sessionState = useHawkNextSessionStore.getState();
  ...
  return (
    <>
      ...
      <WorkspaceShellLayout
        ...
        workspaceShellHawkNextState={workspaceShellHawkNextState}
        theme={theme}
