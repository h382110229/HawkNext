...
import type { OnboardingRecordEntry } from "@hawknext/shared";
...
export function OccupationOnboarding({
  ...
  const requested = useHawkNextStore((state) => state.newUserOnboardingOpen);
  const setRequested = useHawkNextStore((state) => state.setNewUserOnboardingOpen);
  // 登录态变化（useRootOAuthEffects 登录成功后 setUser）时按 userId 重新判定是否触发引导。
  const userId = useHawkNextStore((state) => state.user?.id) ?? null;
  ...
  const savedInterfaceMode = useHawkNextStore((state) => state.interfaceMode);
  ...
  const requestOnboardingDialog = useHawkNextStore((state) => state.requestOnboardingDialog);
