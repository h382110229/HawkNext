...
import type { Locale, RemoteTarget, UserInfo, HawkNextTaskMeta } from "@hawknext/shared";
...
const EMPTY_WORKSPACE_TASK_ITEMS: HawkNextTaskMeta[] = [];
...
export const WorkspaceSidebar = memo(function WorkspaceSidebarComponent({
  ...
  const bumpTaskListVersion = useHawkNextSessionStore((state) => state.bumpTaskListVersion);
  ...
  const activeTaskId = useHawkNextSessionStore(
    (state) =>
      // Web 远程控制从全局 task 入口进入远端 workspace 时，会先按
      // workspaceIdentity 写入 activeTaskId；如果侧栏仍然只读 path-only 桶，
      // 当前任务高亮会丢失，也会把后续选择误判成未激活。
      selectWorkspaceHawkNextState(state, workspacePath, workspaceIdentity).activeTaskId,
  );
