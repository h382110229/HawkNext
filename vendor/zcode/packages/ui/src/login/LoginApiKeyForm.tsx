...
export function LoginApiKeyForm({ onCancel, onSaved, onSkipped }: LoginApiKeyFormProps) {
  ...
  const markApiKeyLoginSuccess = useHawkNextStore((state) => state.markApiKeyLoginSuccess);
          ...
          <div>
            ...
            >
              ...
              <SelectContent align="end" className="rounded-lg">
                ...
                >
                  {renderOAuthProviderIcon(CLOUD_INTL_PROVIDER_ID, "size-4")}
                  {intl.formatMessage({ id: "login.apiKey.provider.zai" })}
                </SelectItem>
                <SelectItem
                  value="cloud-cn"
                  className="rounded-md"
                  data-testid={testId(TID_LOGIN_API_KEY_PROVIDER_ITEM, "cloud-cn")}
                >
                  {renderOAuthProviderIcon(CLOUD_CN_PROVIDER_ID, "size-4")}
                  {intl.formatMessage({
                    id: "login.apiKey.provider.cloud-cn",
                  })}
                </SelectItem>
              </SelectContent>
            </Select>
