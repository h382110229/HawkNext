...
function LoginPanel({ active, onComplete }: LoginPanelProps) {
  ...
  const user = useHawkNextStore((s) => s.user);
  const oauthError = useHawkNextStore((s) => s.oauthError);
  const setOAuthError = useHawkNextStore((s) => s.setOAuthError);
  const oauthSuccessSeq = useHawkNextStore((s) => s.oauthSuccessSeq);
  const lastOAuthSuccessProvider = useHawkNextStore((s) => s.lastOAuthSuccessProvider);
  const loginEntryRequest = useHawkNextStore((s) => s.loginEntryRequest);
  const clearLoginEntryRequest = useHawkNextStore((s) => s.clearLoginEntryRequest);
  const markLoginEntryAttemptStatus = useHawkNextStore((s) => s.markLoginEntryAttemptStatus);
              ...
              <div className="space-y-2">
                {visibleProviders.map((provider) => (
                  <Button
                    ...
                    size="lg"
                    data-testid={
                      provider.id === CLOUD_CN_PROVIDER_ID
                        ? TID_OAUTH_LOGIN_BUTTON
          ...
          <div className="space-y-4">
            ...
            <Button
              ...
              onClick={() => {
                ...
                // 重新登录必须沿用刚才失败的渠道，不能因为 providers[0] 的原始顺序退回 CloudCN。
...
function LoginPanelLogo() {
  return (
    ...
    <div
      ...
      role="img"
    >
      <HawkNextAboutLogo className="h-auto w-10" />
    </div>
  );
}
...
function getLoginOAuthButtonMessageId(providerId: string): string {
  switch (providerId) {
    ...
    case CLOUD_CN_PROVIDER_ID:
      return "login.oauth.button.cloud-cn";
    default:
...
function getLoginOAuthRegionTagMessageId(providerId: string): string | null {
  switch (providerId) {
    ...
    case CLOUD_CN_PROVIDER_ID:
      return "login.oauth.regionTag.cloud-cn";
    default:
      return null;
  }
}
...
function getProviderPriority(provider: OAuthProviderMeta): number {
  switch (provider.id) {
    // Windows 登录入口里  入口需要固定排在最上面，
    // 之前把 CloudCN 设成更高优先级后，用户首屏会先看到次要入口。
...
function resolveVisibleLoginProviders(providers: OAuthProviderMeta[]): OAuthProviderMeta[] {
  // ZAI / CloudCN 现在共享 App 登录事实源，未登录时登录入口必须同时展示两个入口。
  // 不能临时隐藏 CloudCN，否则用户无法主动选择 CloudCN 作为 active provider。
