...
import { useHawkNextStore } from "@/store/StoreProvider.js";
...
function resolveSettingsUsageCodingPlanTabLabel({
  ...
  if (isTeamCodingPlanUsageSource(source)) {
    return source.label.replace(/^CloudCN\s*-\s*/i, "").trim() || source.label;
  }
...
export function SettingsPage({
  ...
  const { intl, localePreference, setLocalePreference } = useHawkNextIntl();
  ...
  >([]);
  const interfaceMode = useHawkNextStore((state) => state.interfaceMode);
  const setInterfaceMode = useHawkNextStore((state) => state.setInterfaceMode);
  const theme = useHawkNextStore((state) => state.theme);
  const setTheme = useHawkNextStore((state) => state.setTheme);
  const codePreviewSettings = useHawkNextStore((state) => state.codePreviewSettings);
  const setCodePreviewSettings = useHawkNextStore((state) => state.setCodePreviewSettings);
  const uiFontSizePx = useHawkNextStore((state) => state.uiFontSizePx);
  const setUiFontSizePx = useHawkNextStore((state) => state.setUiFontSizePx);
  const notificationEnabled = useHawkNextStore((state) => state.notificationEnabled);
  const setNotificationEnabled = useHawkNextStore((state) => state.setNotificationEnabled);
  const notificationSoundEnabled = useHawkNextStore((state) => state.notificationSoundEnabled);
  const setNotificationSoundEnabled = useHawkNextStore((state) => state.setNotificationSoundEnabled);
  ...
  const usageCloudCnProvider = usageProviderSettingsView?.providers.find(
    (provider) => provider.providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
  );
  const usageZaiProviderAccess = resolveEntitledAccountProviderAccess(
    usageProviderSettingsView,
    BUILTIN_MODEL_PROVIDER_IDS.zaiIndividualCodingPlan,
  );
  const usageCloudCnProviderAccess = resolveEntitledAccountProviderAccess(
    usageProviderSettingsView,
  ...
  const usageCloudCnProviderFingerprint = resolveEntitledAccountProviderAccessFingerprint(
    usageProviderSettingsView,
    BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
  );
  ...
  const usageCloudCnTeamProviderFingerprint = resolveEntitledAccountProviderAccessFingerprint(
    usageProviderSettingsView,
    BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan,
  );
  ...
  const usageCloudCnEntitlement = useUsageEntitlement({
    enabled:
      activeSection === "usage" &&
      !usageProviderSettingsLoading &&
      Boolean(usageCloudCnProviderFingerprint),
    includeSubscription: true,
    preferredProviderId: BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
    accountAccess: usageCloudCnProviderAccess?.access,
    allowDisabledPreferredProvider: true,
    requirePreferredProvider: true,
    allowEnvApiKey: false,
    cacheKey: buildUsageEntitlementCacheKey({
      providerId: BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
      providerFingerprint: usageCloudCnProviderFingerprint,
    }),
    refreshOnMount: true,
  });
  // 原只拉 cloud-cn family 的企业 pricing，zai team plan 在使用统计页
  ...
  const usageCloudCnEnterpriseProducts = useEnterpriseCodingPlanProducts({
    enabled: !usageProviderSettingsLoading && Boolean(usageCloudCnTeamProviderFingerprint),
    authenticated: true,
    family: "cloud-cn",
  });
  ...
  const usageSubscribedTeamProducts = useMemo(
    () => [
      ...(usageCloudCnEnterpriseProducts.snapshot?.productList.filter(
    ...
    [
      usageCloudCnEnterpriseProducts.snapshot?.productList,
  ...
  const usagePersonalCodingPlanSources = useMemo(() => {
    ...
    if (
      usageCloudCnProviderAccess &&
      hasActiveCodingPlanSnapshot(
        usageCloudCnEntitlement.snapshot,
        BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
      )
    ) {
      sources.push(
        buildPersonalCodingPlanUsageSource({
          providerId: BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan,
          accountAccess: usageCloudCnProviderAccess.access,
          label: usageCloudCnProvider?.providerName,
        }),
      );
    }
    return sources;
      ...
      buildCodingPlanUsageSources({
        accountAccesses: {
          ...
          ...(resolveEntitledAccountProviderAccess(
            ...
            ? {
                cloudCn: resolveEntitledAccountProviderAccess(
                  usageProviderSettingsView,
                  BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan,
                )!.access,
              }
  ...
  const checkingUsageCloudCodingPlanTab = Boolean(
    usageZaiProviderFingerprint &&
    (usageZaiEntitlement.loading || (!usageZaiEntitlement.snapshot && !usageZaiEntitlement.error)),
  );
  const checkingUsageCloudCnCodingPlanTab = Boolean(
    usageCloudCnProviderFingerprint &&
    (usageCloudCnEntitlement.loading ||
      (!usageCloudCnEntitlement.snapshot && !usageCloudCnEntitlement.error)),
  );
  const checkingUsageCodingPlanTab =
    usageProviderSettingsLoading ||
    checkingUsageCloudCodingPlanTab ||
    checkingUsageCloudCnCodingPlanTab ||
    usageCloudCnEnterpriseProducts.loading ||
  ...
  const setNewUserOnboardingOpen = useHawkNextStore((state) => state.setNewUserOnboardingOpen);
  ...
  const [hawknextInteractionBehavior, setHawkNextInteractionBehavior] =
    useState<HawkNextInteractionBehavior>("queue");
  ...
  useEffect(() => {
    services.settingService
      ...
      .then((settings: AppSettings) => {
        ...
        setHawkNextInteractionBehavior(settings.hawknextInteractionBehavior ?? "queue");
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    setHawkNextInteractionBehavior(sharedSettings.hawknextInteractionBehavior ?? "queue");
  ...
  const handleHawkNextInteractionBehaviorChange = useCallback(
    async (behavior: HawkNextInteractionBehavior) => {
      await runSettingsActionAsync({
        featureId: "settings.conversation",
        action: "change_interaction_behavior",
        trigger: "select",
        operation: () => updateSharedSettings({ hawknextInteractionBehavior: behavior }),
        completed: { resultSource: "shared_settings", valueAfter: behavior },
      });
      setHawkNextInteractionBehavior(behavior);
    },
    [updateSharedSettings],
  );
                      ...
                      <div className="space-y-8">
                        {activeSection === "general" ? (
                          <GeneralSectionContent
                            ...
                            hawknextInteractionBehavior={hawknextInteractionBehavior}
                            ...
                            onHawkNextInteractionBehaviorChange={handleHawkNextInteractionBehaviorChange}
