...
import type { IServiceAccessor } from "@hawknext/services";
...
export function useWorkspaceSessionReload({
  ...
  const handleReloadSession = useCallback(
    async (options?: { resumeTaskId?: string | null; provider?: HawkNextProvider | null }) => {
      ...
      const hawknextSessionStore = useHawkNextSessionStore.getState();
      const latestWorkspaceState = hawknextSessionStore.getWorkspaceState(
        workspaceAbsPath,
        workspaceIdentity,
      );
      const actionScope = resolveWorkspaceModelConfigSyncScope(latestWorkspaceState);
      const provider: HawkNextProvider = options?.provider ?? actionScope.provider;
      const resumeTaskId =
      ...
      hawknextSessionStore.setWorkspaceInitState(
        workspaceAbsPath,
        "initializing",
        null,
        workspaceIdentity,
      ...
      if (shouldPrepareWorkspace) {
        hawknextSessionStore.setConfigOptionsStatus(workspaceAbsPath, "loading", workspaceIdentity);
        // 草稿态点击 reload 后若不清空旧错误，输入区会继续显示上一轮失败提示，
        // 用户会误判本次重建仍失败。这里在新一轮重建开始时先清空草稿错误。
        hawknextSessionStore.setDraftError(workspaceAbsPath, null, workspaceIdentity);
        hawknextSessionStore.setTaskState(workspaceAbsPath, "idle", null, workspaceIdentity);
      }

      try {
        await services.hawknextTaskService.restartWorkspaceProcess({
        ...
        if (shouldPrepareWorkspace) {
          const prepareResult = await prepareWorkspaceWithHawkNextSessionService({
            ...
            workspaceIdentity,
            provider,
            hawknextSessionService: services.hawknextSessionService,
          });

          const latestAfterPrepare = hawknextSessionStore.getWorkspaceState(
            workspaceAbsPath,
            workspaceIdentity,
          );
          if (latestAfterPrepare.selectedProvider === provider) {
            const latestAfterResolve = hawknextSessionStore.getWorkspaceState(
              workspaceAbsPath,
              workspaceIdentity,
            );
            if (latestAfterResolve.selectedProvider === provider) {
              hawknextSessionStore.setConfigOptions(
                workspaceAbsPath,
                prepareResult.configOptions ?? [],
                workspaceIdentity,
              );
              hawknextSessionStore.setConfigOptionsStatus(
                workspaceAbsPath,
                "ready",
                workspaceIdentity,
              );
              hawknextSessionStore.setSlashCommands(
                workspaceAbsPath,
        ...
        hawknextSessionStore.setWorkspaceInitAttempts(workspaceAbsPath, 0, workspaceIdentity);
        hawknextSessionStore.setWorkspaceInitState(workspaceAbsPath, "ready", null, workspaceIdentity);
        ...
        if (shouldPrepareWorkspace) {
          hawknextSessionStore.setConfigOptionsStatus(workspaceAbsPath, "error", workspaceIdentity);
          // 草稿态下 reload 前会先清空旧错误；如果失败后不回填 draftError，
          // 聊天区只剩 toast，用户看不到可重试的详细报错。这里统一回填标准化错误到输入区。
          hawknextSessionStore.setDraftError(workspaceAbsPath, reloadDraftError, workspaceIdentity);
        }
    ...
    [
      ...
      services.hawknextSessionService,
      workspaceAbsPath,
      workspaceIdentity,
    ],
  );
