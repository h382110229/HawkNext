...
export function useWorkspaceTaskNavigation({
  ...
  const setActiveTaskId = useHawkNextSessionStore((s) => s.setActiveTaskId);
  const taskNavHistory = useHawkNextSessionStore((s) => s.taskNavHistory);
  const taskNavPushAutomations = useHawkNextSessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useHawkNextSessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useHawkNextSessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useHawkNextSessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useHawkNextSessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useHawkNextSessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.hawknextTaskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    const currentWorkspaceState = useHawkNextSessionStore
      .getState()
    ...
    while (entry) {
      ...
      const navWorkspaceState = useHawkNextSessionStore
        .getState()
  ...
  const currentWorkspaceState = useHawkNextSessionStore.getState().getWorkspaceState(workspaceAbsPath);
