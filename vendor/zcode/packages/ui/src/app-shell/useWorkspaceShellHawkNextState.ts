...
type WorkspaceShellHawkNextState = Pick<
...
export function useWorkspaceShellHawkNextState(workspaceAbsPath: string, workspaceIdentity?: string) {
  // App 之前直接订阅整个 workspaceHawkNextState，streaming 每个 chunk 都会改 taskMessagesByTaskId。
  // 这会把侧边栏、Header、Git 派生逻辑一起拖进同步重渲染，性能 trace 里那串 long task 就是这样被放大的。
  // 这里把外壳真正依赖的字段收敛成浅比较选择器，避免消息流惊动无关 UI。
  const workspaceShellHawkNextState = useHawkNextSessionStore(
    useShallow((state): WorkspaceShellHawkNextState => {
      const workspaceState = selectWorkspaceHawkNextState(state, workspaceAbsPath, workspaceIdentity);
  ...
  const reloadSessionDisabled = useHawkNextSessionStore((state) => {
    const workspaceState = selectWorkspaceHawkNextState(state, workspaceAbsPath, workspaceIdentity);
  ...
  return { workspaceShellHawkNextState, reloadSessionDisabled };
}
