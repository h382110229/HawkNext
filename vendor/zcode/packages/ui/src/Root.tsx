...
import { StoreProvider, useHawkNextStore } from "@/store/StoreProvider.js";
...
import { consumeHawkNextJwtInvalidRestartMarker } from "@/root/hawknextJwtInvalidRestartMarker.js";
...
function RootInner({
  ...
  const user = useHawkNextStore((state) => state.user);
  const isRestoringOAuthSession = useHawkNextStore((state) => state.isRestoringOAuthSession);
  const setUser = useHawkNextStore((state) => state.setUser);
  const setIsRestoringOAuthSession = useHawkNextStore((state) => state.setIsRestoringOAuthSession);
  const setOAuthError = useHawkNextStore((state) => state.setOAuthError);
  const oauthPollingActive = useHawkNextStore((state) => state.oauthPollingActive);
  const setOAuthPollingActive = useHawkNextStore((state) => state.setOAuthPollingActive);
  const markOAuthSuccess = useHawkNextStore((state) => state.markOAuthSuccess);
  const {
  ...
  const [welcomeScreenOpenReason, setWelcomeScreenOpenReason] =
    useState<WelcomeScreenOpenReason | null>(() =>
      consumeHawkNextJwtInvalidRestartMarker() ? "session-expired" : null,
    );
  const [providerFamilyDomainMigrationComplete, setProviderFamilyDomainMigrationComplete] =
    useState(false);
  const loginEntryRequest = useHawkNextStore((state) => state.loginEntryRequest);
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel === APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL) {
        ...
        void services.hawknextAgentService.syncAppRuntimePreferences(parsed.data).catch((error) => {
  ...
  }, [refreshAppSettings, services.broadcastService, services.hawknextAgentService]);

  useEffect(() => {
    if (!appSettings) {
      return;
    }
    void services.hawknextAgentService
  ...
  const totalUnreadTaskCount = useHawkNextSessionStore((state) =>
  ...
  // 这里把 provider 启动检查纳入 workspace 恢复门禁，避免未连接账号前启动 HawkNext session。
  ...
  if (isStartupRenderBlocked) {
    ...
    return (
      <RootShell>
        ...
        {/* HTML 启动壳已经展示 HawkNext SVG，但 React 接管 root 后旧壳会被整棵替换。
