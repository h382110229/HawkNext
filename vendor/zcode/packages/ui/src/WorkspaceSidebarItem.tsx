...
import { invalidateDeferredDraftSessionForSkillChange } from "@/lib/hawknextDraftSkillInvalidation.js";
...
const EMPTY_PINNED_TASKS: HawkNextTaskMeta[] = [];
...
export const WorkspaceSidebarItem = memo(function WorkspaceSidebarItem({
  ...
  const workspaceHawkNextState = useHawkNextSessionStore((state) =>
    selectWorkspaceHawkNextState(state, tab.workspacePath, tab.workspaceIdentity),
  );
  const activeTaskId = workspaceHawkNextState.activeTaskId;
  const removeTaskState = useHawkNextSessionStore((state) => state.removeTaskState);
  const upsertOptimisticTaskListItem = useHawkNextSessionStore(
    (state) => state.upsertOptimisticTaskListItem,
  );
  const removeOptimisticTaskListItem = useHawkNextSessionStore(
    (state) => state.removeOptimisticTaskListItem,
  );
  const setTaskUnreadIndicator = useHawkNextSessionStore((state) => state.setTaskUnreadIndicator);
  ...
  const hawknextTaskService = services.hawknextTaskService;
  const taskItemsRef = useRef(taskItems);
  taskItemsRef.current = taskItems;
  const workspaceHawkNextStateRef = useRef(workspaceHawkNextState);
  workspaceHawkNextStateRef.current = workspaceHawkNextState;
  ...
  const handleRemoveWorkspace = useCallback(async () => {
    ...
    if (
      hasRunningWorkspaceChat({
        workspaceState: workspaceHawkNextStateRef.current,
  ...
  const handleRenameTask = useCallback(
    async (taskId: string, title: string) => {
      ...
      let meta: HawkNextTaskMeta;
      try {
        meta = await hawknextTaskService.renameTask({
          taskId,
          workspacePath: tab.workspacePath,
          title,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleSetTaskPinned = useCallback(
    async (taskId: string, pinned: boolean) => {
      ...
      try {
        const meta = await hawknextTaskService.setTaskPinned({
          taskId,
          workspacePath: tab.workspacePath,
          pinned,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleArchiveTask = useCallback(
    async (taskId: string) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await hawknextTaskService.archiveTask({
        taskId,
        workspacePath: tab.workspacePath,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
      });
  ...
  const handleSetTaskUnread = useCallback(
    async (taskId: string, unread: boolean) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await hawknextTaskService.setTaskUnread({
        taskId,
        workspacePath: tab.workspacePath,
        unread,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  return (
    <li ref={itemRef} style={itemStyle} className="space-y-2">
      ...
      <RemoteSyncDialogs
        ...
        localHawkNextAgentService={baseServices.hawknextAgentService}
        remoteHawkNextAgentService={services.hawknextAgentService}
        remoteTarget={tab.remoteTarget}
        ...
        onPluginsSynced={async () => {
          await refreshWorkspacePluginCapabilitiesAfterRemoteSync({
            ...
            workspaceIdentity: tab.workspaceIdentity,
            workspacePath: tab.workspacePath,
            hawknextAgentService: services.hawknextAgentService,
            hawknextSessionService: services.hawknextSessionService,
          });
        }}
      />
