...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@hawknext/shared/hawknext-protocol-v4";
...
function readPermissionRuleScopes(option: HawkNextPermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: HawkNextPermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: HawkNextPermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<HawkNextProvider, Record<string, string>>
> = {
  glm: {
    // GLM/HawkNext Agent 通过 HawkNext Agent 发来的项目级记忆授权文案是英文原文。
...
function getProviderOptionNameMessageIds(
  provider: HawkNextProvider | undefined,
  name: string,
...
function getPermissionDisplayReason(request: HawkNextPermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: HawkNextPermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: HawkNextPermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  const respondWithOption = useCallback(
    (option: HawkNextPermissionOption) => {
  ...
  // 当前 HawkNext Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // HawkNext Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。
