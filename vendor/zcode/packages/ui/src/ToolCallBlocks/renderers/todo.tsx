...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@hawknext/shared";
import type { HawkNextPlanStep } from "@hawknext/shared";
...
const todoTextClasses: Record<HawkNextPlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): HawkNextPlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: HawkNextPlanStep["status"] }) {
  if (status === "completed") {
