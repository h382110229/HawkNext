import type { useHawkNextIntl } from "@/i18n/IntlProvider.js";
import type { ToolCallBlockRenderContext } from "@/ToolCallBlocks/shared.js";
import { isSubagentColor } from "@/lib/subagentColors.js";

type AgentIntl = ReturnType<typeof useHawkNextIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "hawknext", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "hawknext", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "hawknext", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "hawknext", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const hawknext = meta && isPlainRecord(meta.hawknext) ? meta.hawknext : null;
  const hawknextBackgroundAgent =
    hawknext && isPlainRecord(hawknext.backgroundAgent) ? hawknext.backgroundAgent : null;
  const taskNotification =
    hawknext && isPlainRecord(hawknext.taskNotification) ? hawknext.taskNotification : null;
  const backgroundAgent = hawknextBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "hawknext", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "hawknext", "taskNotification", "summary"]);
