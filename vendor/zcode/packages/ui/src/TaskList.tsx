...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@hawknext/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<HawkNextTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<HawkNextTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<HawkNextTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<HawkNextTaskMeta | null>;
  ...
  const startDraft = useHawkNextSessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: HawkNextTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareHawkNextTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {
