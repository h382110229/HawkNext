...
import type { Hook, HookConfig } from "@hawknext/shared";
import type { IHooksService } from "@hawknext/services";
...
function buildHawkNextHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildHawkNextHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    if (!source || source.location?.source === "hawknext") throw new Error("Hook is not importable");
    const imported: Hook = {
      ...source,
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildHawkNextHookLocation(workspacePath, source.location?.scope ?? "user"),
    };
