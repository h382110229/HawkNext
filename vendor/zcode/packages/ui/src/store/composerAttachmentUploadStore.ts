...
import type { AttachmentRef } from "@hawknext/shared/hawknext-protocol-v4";
...
declare global {
  interface Window {
    __hawknextComposerAttachmentUploadStoreE2E?: typeof useComposerAttachmentUploadStore;
    __hawknextCurrentComposerAttachmentScopeKeyE2E?: string;
  }
}

if (shouldExposeE2EStoreBridge()) {
  // E2E 只暴露当前唯一附件 owner，供 scope 切换用例准备状态；不再把附件塞回旧 Session Store。
  window.__hawknextComposerAttachmentUploadStoreE2E = useComposerAttachmentUploadStore;
}

export function exposeComposerAttachmentScopeKeyForE2E(scopeKey: string): void {
  if (window.__hawknextComposerAttachmentUploadStoreE2E) {
    window.__hawknextCurrentComposerAttachmentScopeKeyE2E = scopeKey;
  }
}
