...
import type { HawkNextTaskListItem } from "@hawknext/services";
...
import { mergeTaskMetaCandidates } from "@/lib/hawknextTaskMetaMerge.js";
import { compareHawkNextTaskListItems } from "@/lib/taskListOrdering.js";
...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: HawkNextTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: HawkNextTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: HawkNextTaskMeta;
    nextTask: HawkNextTaskMeta;
    previousState: TaskListMembershipState;
    nextState: TaskListMembershipState;
  }) => void;
  setTaskUnreadOverlay: (
    task: Pick<HawkNextTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">,
...
type CachedTaskListItem = HawkNextTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: HawkNextTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, HawkNextTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareHawkNextTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
