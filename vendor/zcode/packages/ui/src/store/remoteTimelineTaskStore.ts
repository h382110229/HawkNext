...
import type { HawkNextTaskListSortBy, IHawkNextTaskService } from "@hawknext/services";
...
interface RemoteTimelineTaskState {
  itemsByWorkspaceKey: Record<string, HawkNextTaskMeta[]>;
  ...
  refreshWorkspace: (params: {
    ...
    hawknextTaskService: Pick<IHawkNextTaskService, "listTaskList">;
    sortBy: HawkNextTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: HawkNextTaskMeta, sortBy?: HawkNextTaskListSortBy) => void;
...
function sortTimelineTasks(
  ...
  sortBy: HawkNextTaskListSortBy = "updated",
): HawkNextTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, hawknextTaskService, sortBy, limit }) {
    ...
    try {
      const result = await hawknextTaskService.listTaskList({
        kind: "timeline",
        workspaceScopes: [
          {
            workspacePath,
...
export async function refreshRemoteTimelineTasksForSession({
  ...
  await useRemoteTimelineTaskStore.getState().refreshWorkspace({
    workspacePath,
    workspaceIdentity,
    hawknextTaskService: session.services.hawknextTaskService,
    sortBy,
    limit,
  });
}
