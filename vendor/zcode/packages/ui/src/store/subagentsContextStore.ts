...
export function getSubagentsContextKey(
  ...
  return `${workspaceIdentity?.trim() || workspacePath}::${normalizeAgentProviderToHawkNextAgent(provider)}`;
}
...
async function loadContext(
  ...
  const provider = normalizeAgentProviderToHawkNextAgent(params.provider);
