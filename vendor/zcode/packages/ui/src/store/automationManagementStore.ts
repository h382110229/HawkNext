...
// 定时任务(automation)管理 store：走 hawknext-agent RPC（列表 / 创建 / 编辑 / 启停 / 重跑 / 删除 + 运行历史）。
...
interface AutomationManagementState {
  ...
  automations: HawkNextAutomation[];
  loading: boolean;
  error: string | null;
  ...
  refresh: (agentService: IHawkNextAgentService) => Promise<void>;
  createAutomation: (
  ...
  deleteAutomation: (automationId: string, agentService: IHawkNextAgentService) => Promise<void>;
  setEnabled: (
    automationId: string,
    enabled: boolean,
    agentService: IHawkNextAgentService,
  ) => Promise<void>;
  restartAutomation: (automationId: string, agentService: IHawkNextAgentService) => Promise<void>;
