...
import {
  ...
  type HawkNextSavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: HawkNextSavedWorkflowEntry[];
  invalid: HawkNextSavedWorkflowInvalidEntry[];
  runs: HawkNextSavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: HawkNextAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: HawkNextAgentSavedWorkflowTarget,
  agentService: IHawkNextAgentService,
): Promise<{ list: HawkNextWorkflowsListResult; runs: HawkNextSavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as HawkNextSavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: HawkNextAgentSavedWorkflowTarget | null,
