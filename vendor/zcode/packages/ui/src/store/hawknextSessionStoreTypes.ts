/* oxlint-disable eslint(max-lines) -- HawkNext Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * HawkNext Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 hawknextSessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
...
export interface WorkspaceInitState {
  status: HawkNextWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: HawkNextTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 HawkNext Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: HawkNextBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: HawkNextPermissionRequest | null;
  pendingPermissionRequests: HawkNextPermissionRequest[];
  elicitationRequest: HawkNextElicitationRequest | null;
  pendingElicitationRequests: HawkNextElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: HawkNextUiError | null;
}
...
export interface WorkspaceHawkNextUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * HawkNext Agent 的 workspace 初始化状态。
   *
   * 单 HawkNext Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  draftError: HawkNextUiError | null;
  ...
  taskConfigOptionsByTaskId: Record<string, HawkNextConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, HawkNextTaskMeta>;
  ...
  /** 当前 workspace 选中的 HawkNext Agent provider */
  ...
  /** HawkNext Agent configOptions（模型、模式、思考级别等） */
  configOptions: HawkNextConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: HawkNextTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface HawkNextSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceHawkNextUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceHawkNextUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: HawkNextPermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: HawkNextElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  /** 任务列表变更时调用（创建/删除任务），驱动 hawknextTaskMetaMerge 重新拉取列表 */
  bumpTaskListVersion: (workspacePath: string, workspaceIdentity?: string) => void;
  /** 更新已拉取的任务列表缓存 */
  setTaskListCache: (
    workspacePath: string,
    tasks: HawkNextTaskMeta[],
    workspaceIdentity?: string,
  ) => void;
...
const FALLBACK_PROVIDER: HawkNextProvider = HAWKNEXT_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceHawkNextUIState {
  // 单 HawkNext Agent 迁移后默认 provider 必须收敛到 glm。
