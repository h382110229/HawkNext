...
import type {
  ...
  HawkNextMcpListMode,
  HawkNextMcpServerStatusSnapshot,
  HawkNextMcpServer,
} from "@hawknext/shared";
import { convertToHawkNextAgentMcpServer } from "@hawknext/shared";
...
import {
  importLegacyCommonServersToHawkNextAgent,
  migrateStoredCommonMcpToHawkNextAgent,
...
interface McpStoreState {
  ...
  servers: HawkNextMcpServer[];
  statusSnapshots: Record<string, HawkNextMcpServerStatusSnapshot>;
  ...
  addHawkNextAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  updateHawkNextAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  deleteHawkNextAgentMcpServer: (name: string, projectPath?: string) => void;
  ...
  beginServerStatusListRefresh: (mode?: HawkNextMcpListMode) => number;
  ...
  mergeServerStatusSnapshots: (
    statuses: Record<string, HawkNextMcpServerStatusSnapshot>,
    requestEpoch: number,
    mode?: "connect" | "status",
  ) => void;
  getServer: (id: string) => HawkNextMcpServer | undefined;
  setCurrentProjectPath: (
    path: string,
    workspaceIdentity?: string,
    directoryService?: McpDirectoryService | null,
  ) => void;
  getEnabledMcpServersForHawkNext: (provider: string) => HawkNextAgentMcpServer[];
...
export const useMcpStore = create<McpStoreState>((set, get) => {
  ...
  const statusListEpochs: Record<HawkNextMcpListMode, number> = {
    connect: 0,
    status: 0,
  };
  ...
  function resolveMcpDirectoryService(
    ...
    if (!effectiveWorkspaceIdentity?.trim()) {
      ...
      // 到用户级 HawkNext Agent MCP 的迁移；目录服务只用于远端 workspace 覆盖路由。
      return null;
    }
  ...
  return {
    ...
    loadConfig: () => {
      ...
      // MCP 启停状态已经迁移到 ~/.hawknext/cli/config.json，不能再读取旧 localStorage，
      // 否则旧的本地开关会覆盖新的 HawkNext Agent 配置来源。
      ...
      loadMcpPromise = (async () => {
        try {
          ...
          if (!activeDirectoryService) {
            servers = await migrateStoredCommonMcpToHawkNextAgent(
              mcpPlatformService,
              servers,
              latestWorkspacePath,
            );
    ...
    addMcpServer: (name, config) => get().addHawkNextAgentMcpServer(name, config),
    updateMcpServer: (name, config) => get().updateHawkNextAgentMcpServer(name, config),
    deleteMcpServer: (name) => get().deleteHawkNextAgentMcpServer(name),
    addScopedMcpServer: async (source, name, config, projectPath) => {
      invalidateStatusListRequests();
      const targetSource: CliMcpSource = source === "mcp" ? "hawknextagentmcp" : source;
    ...
    addHawkNextAgentMcpServer: (name, config, projectPath) =>
      get().addScopedMcpServer("hawknextagentmcp", name, config, projectPath),
    updateHawkNextAgentMcpServer: (name, config, projectPath) =>
      get().updateScopedMcpServer("hawknextagentmcp", name, config, projectPath),
    deleteHawkNextAgentMcpServer: (name, projectPath) =>
      get().deleteScopedMcpServer("hawknextagentmcp", name, projectPath),
    ...
    beginServerStatusListRefresh: (mode = "connect") => {
      ...
      set((state) => ({
        servers: state.servers.map((server) => {
          ...
          if (
            server.source !== "hawknextagentmcp" ||
            !enabled ||
    ...
    markServerStatusListRefreshFailed: (error, requestEpoch, mode = "connect") => {
      ...
      set((state) => ({
        ...
        servers: state.servers.map((server) => {
          const enabled = state.enabledStates[server.id] ?? server.enabled;
          if (server.source !== "hawknextagentmcp" || !enabled || server.status !== "connecting") {
            return server;
          }
          return {
            ...server,
    ...
    getEnabledMcpServersForHawkNext: (_provider) => {
      const { servers, enabledStates, currentProjectPath } = get();
      const candidates = new Map<string, HawkNextMcpServer>();

      for (const server of servers) {
        const isEnabled = enabledStates[server.id] ?? server.enabled;
        if (!isEnabled) continue;
        if (server.source !== "hawknextagentmcp") continue;
      ...
      const result: HawkNextAgentMcpServer[] = [];
      for (const server of candidates.values()) {
        const hawknextAgentServer = convertToHawkNextAgentMcpServer(server.name, server.config);
        if (hawknextAgentServer) result.push(hawknextAgentServer);
      }
      return result;
    },
    ...
    mergePreloadedMcpServers: (preloaded) => {
      ...
      const targetSource: CliMcpSource = "hawknextagentmcp";
    ...
    migrateLegacyCommonMcp: async () => {
      ...
      const migration = await importLegacyCommonServersToHawkNextAgent(
        mcpPlatformService,
        result.servers ?? {},
