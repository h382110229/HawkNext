import type { McpServerConfig, NativeMcpServerRecord } from "@hawknext/shared";
...
function isHawkNextAgentUserServer(server: NativeMcpServerRecord): boolean {
  return (
    server.source === "hawknextagentmcp" &&
    server.scope === "user" &&
    (!server.location || server.location.source === "hawknext")
  );
}

export async function importLegacyCommonServersToHawkNextAgent(
  ...
  const existingNames = new Set(
    nativeServers.filter(isHawkNextAgentUserServer).map((server) => server.name),
  );
  let importedCount = 0;
  let skippedCount = 0;
...
export async function migrateStoredCommonMcpToHawkNextAgent(
  platform: McpPlatformService | null,
  nativeServers: NativeMcpServerRecord[],
  ...
  const migration = await importLegacyCommonServersToHawkNextAgent(
    ...
    legacyServers,
    nativeServers,
    "localStorage:hawknext-mcp-config",
  );
  if (migration.completed) {
    // 只有确认写入 hawknext agent 目录后才清理旧数据，避免 Web 端没有 desktop bridge 时丢配置。
    clearLegacyCommonMcpServers();
  }
  ...
  logger.info(
    `[mcpStore] migrated ${migration.importedCount} legacy common MCP servers to hawknext agent config`,
  );
