...
import type {
  ...
  HawkNextPluginDiagnostic,
  HawkNextPluginInfo,
  HawkNextPluginMarketplaceSummary,
  HawkNextPluginScope,
  HawkNextPluginsDescribeResult,
} from "@hawknext/shared";
import type { IPluginManagementService } from "@hawknext/services";
...
export interface PluginDescribeEntry {
  ...
  data?: HawkNextPluginsDescribeResult;
  error?: string;
}

// 设置页「插件管理」的数据源: 经 IPluginManagementService 薄服务由 hawknext-cli 提供 (list + enable/disable)。
// UI 不再直触 IHawkNextAgentService，plugins/* 旧协议词的消费收拢到服务实现一处。
// 与已 retired 的 marketplace pluginStore 无关, 故单独建一个精简 store。
export interface PluginManagementState {
  workspacePath: string | null;
  workspaceIdentity: string | null;
  configScope: HawkNextPluginScope | null;
  ...
  diagnostics: HawkNextPluginDiagnostic[];
  loading: boolean;
  error: string | null;
  /**
  ...
  initialize: (params: {
    ...
    configScope?: HawkNextPluginScope;
...
function throwForErrorDiagnostic(diagnostics: HawkNextPluginDiagnostic[]): void {
