...
import type { IBroadcastService } from "@hawknext/services";
import { createHawkNextStore, type HawkNextStore, type HawkNextState } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<HawkNextStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<HawkNextStore | null>(null);
  if (!storeRef.current) {
    storeRef.current = createHawkNextStore(broadcastService, {
 ...
 */
export function useHawkNextStore<T>(selector: (state: HawkNextState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useHawkNextStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useHawkNextStore（store 耦合剥离配套）。
 *
 ...
 */
export function useHawkNextStoreWithDefault<T>(
  selector: (state: HawkNextState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
