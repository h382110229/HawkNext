/**
 * HawkNext Session Store 选择器与内部辅助函数
 *
 * 从 hawknextSessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { HawkNextTaskRuntimeStatus, HawkNextTaskMeta } from "@hawknext/shared";
...
import {
  ...
  type HawkNextSessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceHawkNextUIState,
} from "./hawknextSessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceHawkNextUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceHawkNextUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: HawkNextSessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceHawkNextUIState) => WorkspaceHawkNextUIState,
  workspaceIdentity?: string,
  ...
  if (nextWorkspaceState === current) {
    // 单 HawkNext Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceHawkNextUIState,
  taskId: string,
): TaskRuntimeState {
...
interface WorkspaceDisplayedTaskState {
  taskStatus: HawkNextTaskRuntimeStatus;
...
export function getTaskUiState(workspaceState: WorkspaceHawkNextUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceHawkNextUIState
    | Partial<Pick<WorkspaceHawkNextUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, HawkNextTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<HawkNextTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceHawkNextState(
  state: HawkNextSessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceHawkNextUIState, "workspaceInit">,
): WorkspaceInitState {
