...
import type { IBroadcastService, BroadcastMessage } from "@hawknext/services";
import type { OAuthProviderId, UserInfo } from "@hawknext/shared";
import type { CodingPlanResetType } from "@hawknext/shared";
...
const CODE_PREVIEW_SETTINGS_KEY = "hawknext-code-preview-settings";
const PERFORMANCE_MODE_STORAGE_KEY = "hawknext-performance-mode";
...
export interface HawkNextState {
 ...
 */
export function createHawkNextStore(
  ...
  const useStore = create<HawkNextState>()((set, get) => ({
    ...
    theme: normalizeThemePreference((readSafeLocalStorage("hawknext-theme") as Theme) || "zai-dark"),
    setTheme: (theme: Theme) => {
      const normalizedTheme = normalizeThemePreference(theme);
      writeSafeLocalStorage("hawknext-theme", normalizedTheme);
    ...
    locale: readSafeLocalStorage("hawknext-locale") || "zh-CN",
    setLocale: (locale: string) => {
      writeSafeLocalStorage("hawknext-locale", locale);
      set({ locale });
    },
...
export type HawkNextStore = ReturnType<typeof createHawkNextStore>;
