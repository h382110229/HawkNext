...
interface RemotePinnedTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    hawknextTaskService: Pick<IHawkNextTaskService, "listPinnedTasks">;
  }) => Promise<void>;
  upsertTask: (task: HawkNextTaskMeta) => void;
...
function sortPinnedTasks(tasks: HawkNextTaskMeta[]): HawkNextTaskMeta[] {
...
export const useRemotePinnedTaskStore = create<RemotePinnedTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, hawknextTaskService }) {
    ...
    try {
      const items = await hawknextTaskService.listPinnedTasks({
        workspacePath,
