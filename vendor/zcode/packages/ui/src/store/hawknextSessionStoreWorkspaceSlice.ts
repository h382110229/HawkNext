...
} from "@/store/hawknextSessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/hawknextSessionStoreSelectors.js";

type SetFn = (
  partial:
    | HawkNextSessionStoreState
    | Partial<HawkNextSessionStoreState>
    | ((state: HawkNextSessionStoreState) => HawkNextSessionStoreState | Partial<HawkNextSessionStoreState>),
) => void;
...
function normalizeThoughtLevelConfigOption(option: HawkNextConfigOption): HawkNextConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: HawkNextConfigOption[]): HawkNextConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly HawkNextConfigOption[]): HawkNextConfigOption[] {
...
function isModeConfigOption(option: HawkNextConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<HawkNextConfigOption["options"]>;
...
function replaceModeConfigOption(
  currentOptions: HawkNextConfigOption[] | null,
  nextModeOption: HawkNextConfigOption | null,
): HawkNextConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceHawkNextUIState,
  updater: (options: HawkNextConfigOption[] | null) => HawkNextConfigOption[] | null,
): Pick<WorkspaceHawkNextUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceHawkNextUIState,
  taskId: string,
): { configOptions: HawkNextConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: HawkNextTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as HawkNextTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToHawkNextAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToHawkNextAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToHawkNextAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
