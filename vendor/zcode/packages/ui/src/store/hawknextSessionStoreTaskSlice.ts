...
import { getDefaultWorkspaceState } from "@/store/hawknextSessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly HawkNextTaskMeta[]): HawkNextTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: HawkNextPermissionRequest["options"],
  right: HawkNextPermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: HawkNextPermissionRequest,
  right: HawkNextPermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useHawkNextChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToHawkNextAgent(params.provider),
