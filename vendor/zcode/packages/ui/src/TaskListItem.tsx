...
import { isCronTask, isOffPeakTask, type HawkNextTaskMeta } from "@hawknext/shared";
import { TID_TASK_ARCHIVE, TID_TASK_ITEM, testId } from "@hawknext/shared";
...
function getTaskAutomationIdentity(task: HawkNextTaskMeta): string | undefined {
  return task.cronAutomationId ?? (task as HawkNextTaskMeta & { automationId?: string }).automationId;
}

function areTaskListItemTaskFieldsEqual(left: HawkNextTaskMeta, right: HawkNextTaskMeta) {
  if (left === right) {
    return true;
  }
...
export const MemoTaskItem = memo(function TaskListItem({
  ...
  const handleDragStart = useCallback(
    (event: React.DragEvent<HTMLLIElement>) => {
      ...
      const payload = {
        kind: "hawknext/session" as const,
        workspacePath,
        ...(task.workspaceIdentity?.trim() ? { workspaceIdentity: task.workspaceIdentity } : {}),
        ...(remoteSessionId ? { remoteSessionId } : {}),
        sessionId: task.taskId,
