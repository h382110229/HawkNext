...
export type { Locale, LocalePreference } from "@hawknext/shared";
...
const HAWKNEXT_INTERACTION_BEHAVIOR_OPTIONS: readonly HawkNextInteractionBehavior[] = ["queue", "guide"];
...
export function GeneralSectionContent({
  ...
  onHawkNextInteractionBehaviorChange,
  ...
  onHawkNextInteractionBehaviorChange: (behavior: HawkNextInteractionBehavior) => Promise<void>;
  ...
  return (
    <div className="space-y-4">
      ...
      <SettingsGroupCard>
        ...
        />
        {/* Wave 6: HawkBrain 皮肤「旧布局」回退（checklist §7） */}
        <SettingsRow
          label={intl.formatMessage({ id: "settings.legacyLayout" })}
          description={intl.formatMessage({ id: "settings.legacyLayout.description" })}
          control={
            <Switch
              checked={
                (typeof localStorage !== "undefined" &&
                  localStorage.getItem("hawknext.skinMode") === "legacy") ||
                false
              }
              onCheckedChange={(checked) => {
                const mode = checked ? "legacy" : "hawk";
                try {
                  localStorage.setItem("hawknext.skinMode", mode);
                } catch {
                  /* ignore */
                }
                document.documentElement.classList.toggle("hawk-legacy-layout", checked);
                document.documentElement.classList.toggle("hawk-skin", !checked);
                document.documentElement.dataset.hawkSkin = mode;
      ...
      <SettingsGroupCard>
        <SettingsRow
          label={intl.formatMessage({ id: "settings.hawknextInteractionBehavior" })}
          description={intl.formatMessage({
            id: "settings.hawknextInteractionBehaviorDescription",
          })}
          control={
            <Select
              value={hawknextInteractionBehavior}
              onValueChange={(value) => {
                void onHawkNextInteractionBehaviorChange(value as HawkNextInteractionBehavior);
              }}
            >
              <SelectTrigger size="lg" className="w-[260px] min-w-0 justify-between">
                <SelectValue />
              ...
              <SelectContent>
                {HAWKNEXT_INTERACTION_BEHAVIOR_OPTIONS.map((behavior) => (
                  <SelectItem key={behavior} value={behavior}>
                    {intl.formatMessage({
                      id: `settings.hawknextInteractionBehavior.option.${behavior}`,
                    })}
                  </SelectItem>
                ))}
              </SelectContent>
