...
import type { CuaAccessibilitySettingsResult, CuaPermissionKind } from "@hawknext/shared";
import { requiredCuaPermissionsForRequestAccessStatus } from "@hawknext/shared/hawknext-protocol-v4";
import {
  isCuaPermissionStatusAvailable,
  type CuaPermissionRestartOptions,
  type CuaPermissionStatusResult,
  type HawkNextAgentCuaPermissionObservation,
...
function cuaPermissionObservationKey(observation: HawkNextAgentCuaPermissionObservation): string {
...
export function CuaPermissionObservationAttachment() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      typeof services.hawknextAgentService.onDynamicCuaPermissionObservation !== "function"
    ) {
      return;
    }
    ...
    let disposed = false;
    const subscription = services.hawknextAgentService.onDynamicCuaPermissionObservation()(
      (observation) => {
        const missing = requiredCuaPermissionsForRequestAccessStatus(observation.permissionStatus);
