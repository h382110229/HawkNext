...
export function WorkspaceHeaderTitleSection({
  ...
  const buildHeaderTaskSnapshot = (
    fallbackTask: HawkNextTaskMeta,
    overrides?: Partial<HawkNextTaskMeta>,
  ): HawkNextTaskMeta => {
    if (activeTaskMeta) {
      return {
        ...activeTaskMeta,
  ...
  const handleSubmitRenameTask = async () => {
    ...
    try {
      const renamedTask = await services.hawknextTaskService.renameTask({
  ...
  const handleArchiveTask = async () => {
    ...
    void services.hawknextTaskService
      ...
      .then((meta) => {
        // Header 更多菜单不能只依赖 hawknextTaskMetaMerge：归档后只有旧列表状态被更新。
