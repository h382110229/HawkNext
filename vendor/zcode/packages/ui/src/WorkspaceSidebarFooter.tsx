...
import type { Locale, UserInfo } from "@hawknext/shared";
      ...
      <div className="flex min-w-0 gap-2">
        <DropdownMenu open={profileMenuOpen} onOpenChange={setProfileMenuOpen}>
          ...
          <DropdownMenuContent align="start" className="w-max min-w-50" forceMount>
            {/* Wave 6: 交付向导入口 — 侧栏常驻（首屏/任一布局可见） */}
            <DropdownMenuItem
              id="hawk-delivery-wizard"
              onSelect={() => {
                useHawkNextStore.getState().requestOnboardingDialog();
              }}
            >
              <Sparkles className="size-4" />
              {intl.formatMessage({ id: "delivery.wizard.entry" })}
            </DropdownMenuItem>
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>
