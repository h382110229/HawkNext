...
import type { HawkNextModelTrajectoryRecord } from "@hawknext/services";
import {
...
export function resolveTrajectoryTimelineItems(records: HawkNextModelTrajectoryRecord[]) {
...
export function resolveTrajectoryInputMessages({
  ...
  inputMessages: HawkNextModelTrajectoryRecord["request"]["messages"];
...
function shouldUseConversationDelta(record: HawkNextModelTrajectoryRecord): boolean {
  const kind = record.callSource?.kind;
  return kind === undefined || kind === "main" || kind === "subagent";
}

function computeDeltaMessages(
  messages: HawkNextModelTrajectoryRecord["request"]["messages"],
  previousMessageCount: number,
  index: number,
): HawkNextModelTrajectoryRecord["request"]["messages"] {
