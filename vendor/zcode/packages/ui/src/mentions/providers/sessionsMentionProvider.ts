// composer parity：`#` 会话候选的数据源从旧 hawknextSessionStore/taskQueryCache/remote*
...
import type { HawkNextProvider, HawkNextTaskMeta } from "@hawknext/shared";
...
function compareSessionTasks(
  left: HawkNextTaskMeta,
  right: HawkNextTaskMeta,
...
function getSessionLabel(task: HawkNextTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: HawkNextTaskMeta): string {
  const raw = task.workspacePath.trim() || task.workspaceIdentity?.trim() || "";
  return raw.split(/[\\/]/).filter(Boolean).at(-1) ?? raw;
}

function mapTaskToMentionItem(task: HawkNextTaskMeta, provider: HawkNextProvider): SessionMentionItem {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, HawkNextTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.hawknextAgentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.hawknextAgentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.hawknextAgentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.hawknextAgentService,
        },
      ];
    }
