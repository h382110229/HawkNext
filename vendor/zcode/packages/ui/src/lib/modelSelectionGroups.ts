...
function supportsRegistryApiFormat(
  ...
  // 仅剩 glm（HawkNext Agent）provider；三方 CLI 的 api format 差异已随 provider 下线。
  return isHawkNextAgentProvider(selectedProvider);
}
...
export function buildRegistryModelSelectGroups(
  ...
  return view.providers.flatMap((provider) => {
    ...
    const accountAccess = hawknextProviderAccountAccessSchema.safeParse(provider.config.access);
