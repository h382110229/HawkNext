import type { HawkNextWorkspaceTaskListChanged } from "@hawknext/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<HawkNextWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<HawkNextWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<HawkNextWorkspaceTaskListChanged, "reason">,
): boolean {
