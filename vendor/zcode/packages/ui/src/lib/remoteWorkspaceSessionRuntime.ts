import type { HawkNextTaskRuntimeStatus } from "@hawknext/shared";
import { getWorkspaceDisplayedTaskState } from "@/store/hawknextSessionStore.js";
import type { HawkNextSessionStoreState, WorkspaceHawkNextUIState } from "@/store/hawknextSessionStore.js";
...
interface MarkRemoteWorkspaceRunningTasksFailedParams {
  ...
  getWorkspaceState: HawkNextSessionStoreState["getWorkspaceState"];
  setTaskRuntimeState: HawkNextSessionStoreState["setTaskRuntimeState"];
  reason: string;
}

function isRunningRuntimeStatus(status: HawkNextTaskRuntimeStatus): boolean {
...
function collectRemoteWorkspaceRunningTaskIds(workspaceState: WorkspaceHawkNextUIState): string[] {
