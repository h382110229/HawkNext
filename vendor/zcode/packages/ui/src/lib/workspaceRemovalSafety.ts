import type { FileEntry, HawkNextTaskMeta, HawkNextTaskRuntimeStatus } from "@hawknext/shared";
import type { IFileService } from "@hawknext/services";
import type { WorkspaceHawkNextUIState } from "@/store/hawknextSessionStoreTypes.js";
...
export function hasRunningWorkspaceChat(params: {
  workspaceState: Pick<WorkspaceHawkNextUIState, "draftRuntime" | "taskRuntimeByTaskId">;
  taskItems: Pick<HawkNextTaskMeta, "taskId">[];
}): boolean {
...
function isWorkspaceRemovalBlockingRuntimeStatus(status: HawkNextTaskRuntimeStatus): boolean {
