import type {
  ...
  HawkNextTaskChangedFileSummary,
  HawkNextTaskMeta,
} from "@hawknext/shared";
import { computeLineChangeStat } from "@hawknext/shared";
...
export function getTaskChangeSummary(
  task: Pick<HawkNextTaskMeta, "changeSummary"> | null | undefined,
): HawkNextTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: HawkNextTaskChangeSummary,
...
export function buildTaskChangeSummary(
  fileChanges: readonly HawkNextPersistedFileChange[] | undefined,
): HawkNextTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: HawkNextTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: HawkNextPersistedFileChange | null | undefined,
): HawkNextTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: HawkNextTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly HawkNextPersistedFileChange[] | undefined,
): Map<number, HawkNextTaskChangeSummary> {
  const result = new Map<number, HawkNextTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }
