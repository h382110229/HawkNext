import type { UsageEntitlementSnapshot, UsageQuotaLimit } from "@hawknext/shared";
 ...
 * cloud-cn 业务后端用 `TOKENS_LIMIT`。两者 unit/number 语义完全一致
 *（unit=3,number=5 → 5 小时窗口；unit=6 → 每周），仅 type 枚举命名不同。
 * 这里把两个 type 视为等价，让 zai/cloud-cn team plan 在同一套 UI 消费方下都能命中。
...
/**
 * Tool 类配额的 type。
 * 目前 cloud-cn/zai 后端都用 TIME_LIMIT 表示每月工具调用配额，暂无需等价集合。
 */
