...
export type CodingPlanProviderFamily = "cloud-cn" | "zai" | "unknown";
...
export function resolveCodingPlanEntryPlanStateFromProviderSettings(
  ...
  const codingPlanProvider = accountProviders.find(
    (provider) =>
      ...
      provider.providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnIndividualCodingPlan ||
      // Team 同样持有 Coding Plan；不能用包含 Start 的宽泛 helper 判断套餐。
      provider.providerId === BUILTIN_MODEL_PROVIDER_IDS.zaiTeamCodingPlan ||
      provider.providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan,
  );
...
function resolveCodingPlanProviderFamily(providerId: string): CodingPlanProviderFamily {
  if (
    ...
    providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnTeamCodingPlan ||
    providerId === BUILTIN_MODEL_PROVIDER_IDS.cloudCnStartPlan
  ) {
    return "cloud-cn";
  }
  if (providerId === CLOUD_INTL_PROVIDER_ID || isCloudCodingPlanProviderId(providerId)) {
    return "zai";
  }
  return "unknown";
}
...
function resolveCodingPlanChannel(providerFamily: CodingPlanProviderFamily): string {
  return { cloudCn: "MaaS", zai: "Z_AI", unknown: "" }[providerFamily];
}
