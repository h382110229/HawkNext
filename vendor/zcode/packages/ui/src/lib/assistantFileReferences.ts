...
import { extractConversationPreviewFileReferences } from "@hawknext/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@hawknext/shared";
import {
  extractHawkNextFileCitationDirectives,
  resolveHawkNextFileCitationPreviewKind,
} from "@/lib/hawknextFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractHawkNextFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 hawknext-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
