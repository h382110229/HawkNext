// re-home 产物（为删除 hawknextChatMessages 铺路，模式同 hawknext-task-types-core）。
// 本文件承载旧消息转换层（hawknextChatMessages/hawknextChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（hawknextChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  HawkNextAssistantMessagePart,
  HawkNextAssistantMessageFeedback,
  HawkNextPromptAttachment,
  HawkNextTimelineMeta,
  HawkNextTaskSnapshotBodyRef,
  HawkNextTaskSnapshotToolFieldRef,
  HawkNextTaskSnapshotToolSlice,
...
export interface TaskChatToolCall {
  ...
  /** HawkNext 固定工具名；kind 仍保留给旧 HawkNext Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = HawkNextAssistantMessagePart;
