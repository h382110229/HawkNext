...
import { extractHawkNextFileCitations } from "@/lib/hawknextFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractHawkNextFileCitations(value);
...
export function createHawkNextFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function hawknextFileCitationRemarkPlugin() {
    return (tree: unknown) => {
