import type { HawkNextProvider, HawkNextTaskRuntimeStatus } from "@hawknext/shared";

function isBusyTaskRuntimeStatus(status: HawkNextTaskRuntimeStatus): boolean {
  return status === "creating" || status === "restoring" || status === "streaming";
}

function buildTaskProviderByTaskId(
  optimisticTaskMetaByTaskId: Record<string, { provider?: HawkNextProvider }>,
  taskListCache?: Array<{ taskId: string; provider?: HawkNextProvider }> | null,
): Record<string, HawkNextProvider | undefined> {
  const providerByTaskId: Record<string, HawkNextProvider | undefined> = {};
...
export function hasBusyTaskInWorkspaceProvider(
  ...
  taskRuntimeByTaskId: Record<string, { status: HawkNextTaskRuntimeStatus; provider?: HawkNextProvider }>,
  optimisticTaskMetaByTaskId: Record<string, { provider?: HawkNextProvider }>,
  taskListCache?: Array<{ taskId: string; provider?: HawkNextProvider }> | null,
