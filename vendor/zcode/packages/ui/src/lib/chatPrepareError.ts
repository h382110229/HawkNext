...
import { normalizeHawkNextUiError } from "@/lib/hawknextUiError.js";

export const MODEL_CONFIG_MISSING_UI_ERROR_CODE = "model_config_missing";

export type ModelConfigMissingUiError = HawkNextError & {
...
export function isProviderNotReadyError(error: unknown): boolean {
  return normalizeUnknownError(error).code === HAWKNEXT_AGENT_PROVIDER_NOT_READY_CODE;
}
...
): HawkNextError & { detail?: string } {
  const normalizedError = normalizeHawkNextUiError(
