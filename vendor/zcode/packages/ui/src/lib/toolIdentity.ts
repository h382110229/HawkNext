import {
  getHawkNextToolFamilyForName,
  isTodoPlanToolName,
  isHawkNextFileContentWriteToolName,
  normalizeHawkNextToolName,
  type HawkNextKnownToolName,
  type HawkNextToolFamily,
...
export interface ToolCallIdentity {
  toolName: HawkNextKnownToolName | string | null;
...
function identityFromKnownToolName(
  ...
  const family = getHawkNextToolFamilyForName(toolName);
  if (!family) {
    return null;
  }
  return {
...
function readRawToolNameCandidates(raw: unknown) {
  return {
    ...
    hawknext: readNestedString(raw, ["_meta", "hawknext", "toolName"]),
...
function isLegacyAgentTool(
  ...
  const rawHawkNext = normalizeLegacyToken(rawNames.hawknext);
...
export function resolveToolCallIdentity(toolCall: ToolIdentityLike): ToolCallIdentity {
  ...
  for (const candidate of [
    ...
    { value: rawNames.hawknext, source: "raw-hawknext-meta" as const },
  ]) {
  ...
  if (isLegacyAgentTool(toolCall, rawNames)) {
    return identityFromLegacyFamily(
      rawNames.direct ?? rawNames.hawknext ?? "Agent",
      "agent",
      "legacy-payload",
    );
  }

  // `Task` 现在是现役 Claude 兼容工具名，但历史 HawkNext Agent 投影会用
  ...
  if (
    // 当前 HawkNext Agent 传给 app 的 plan mode 退出工具是 ExitPlanMode，
...
export function isFileContentWriteToolCall(
  ...
  if (isHawkNextFileContentWriteToolName(identity.toolName)) {
    return true;
  }
