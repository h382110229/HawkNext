type HawkNextUserQuestionAnswers = Record<string, unknown>;
...
interface AskUserQuestionData {
  ...
  answers?: HawkNextUserQuestionAnswers;
}
...
function readQuestions(input: unknown): unknown[] {
  ...
  // HawkNext Agent 的单题输入与交互请求的多题输入共用展示管线。
  return typeof input.question === "string" && Array.isArray(input.options) ? [input] : [];
}

function readAnswers(input: unknown): HawkNextUserQuestionAnswers | undefined {
  if (!isPlainRecord(input)) {
    return undefined;
  }
...
function readNestedAskUserQuestionAnswers(input: unknown): HawkNextUserQuestionAnswers | undefined {
...
function parseHawkNextAskUserQuestionOutput(
  output: unknown,
  input: unknown,
): HawkNextUserQuestionAnswers | undefined {
...
export function readAskUserQuestionAnswers(value: {
  ...
  const parsedHawkNextOutputAnswers = parseHawkNextAskUserQuestionOutput(
    value.output,
    readAskUserQuestionInput(value),
  );
  if (parsedHawkNextOutputAnswers) {
    return parsedHawkNextOutputAnswers;
  }
  ...
  if (isPlainRecord(value.raw)) {
    ...
    const parsedRawHawkNextOutputAnswers = parseHawkNextAskUserQuestionOutput(
      value.raw.rawOutput,
      readAskUserQuestionInput(value),
    );
    if (parsedRawHawkNextOutputAnswers) {
      return parsedRawHawkNextOutputAnswers;
    }
    const nestedRawAnswers = readNestedAskUserQuestionAnswers(value.raw.output);
    if (nestedRawAnswers) {
      return nestedRawAnswers;
    }
    const parsedRawHawkNextAnswers = parseHawkNextAskUserQuestionOutput(
      value.raw.output,
      readAskUserQuestionInput(value),
    );
    if (parsedRawHawkNextAnswers) {
      return parsedRawHawkNextAnswers;
    }
...
export function getAskUserQuestionAnswerText(
  ...
  answers: HawkNextUserQuestionAnswers | undefined,
  noAnswerText: string,
) {
