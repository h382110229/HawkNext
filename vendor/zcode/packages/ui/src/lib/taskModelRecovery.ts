import type { HawkNextConfigOption, HawkNextTaskMeta } from "@hawknext/shared";
import { getHawkNextAgentModeSelectOptions } from "@hawknext/shared";
...
function resolveRecoveredTaskModelValue(
  taskMeta: Pick<HawkNextTaskMeta, "provider" | "model">,
): string | null {
...
function ensureModelOptionValue(option: HawkNextConfigOption, modelValue: string): HawkNextConfigOption {
...
function createRecoveredModelOption(modelValue: string): HawkNextConfigOption {
  return {
    category: "model",
    currentValue: modelValue,
    id: "model",
...
function createRecoveredModeOption(modeValue: string): HawkNextConfigOption {
  return {
    category: "mode",
...
function createRecoveredThoughtLevelOption(thoughtLevel: string): HawkNextConfigOption {
  return {
...
export function resolveTaskRestorePreloadConfigOptions({
  ...
  taskMeta: Pick<HawkNextTaskMeta, "provider" | "model"> &
    Partial<Pick<HawkNextTaskMeta, "mode" | "thoughtLevel">>;
  cachedTaskConfigOptions?: readonly HawkNextConfigOption[];
}): HawkNextConfigOption[] {
