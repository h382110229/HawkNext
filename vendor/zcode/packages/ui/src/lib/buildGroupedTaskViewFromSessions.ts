...
// sessions-index 只补实时 activity/detail，最终拼出旧 HawkNextGroupedTaskView 同形结果。
...
function taskKeyOf(task: HawkNextTaskMeta): string {
...
function taskOrderMapKey(task: HawkNextTaskMeta): string {
...
function nodeMapKey(node: HawkNextGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, HawkNextGroupedTaskViewStructureMember>,
): HawkNextTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: HawkNextTaskListItem; addedAt: number; key: string }> = [];
...
interface BuildGroupedTaskViewParams {
  ...
  taskIndexItems: HawkNextTaskMeta[];
  /** sessions-index 派生的会话 activity/detail，只 enrich 命中的持久行。 */
  sessions: HawkNextTaskMeta[];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, HawkNextTaskListItem>();
  ...
  const membersByGroupId = new Map<string, HawkNextGroupedTaskViewStructureMember[]>();
  ...
  const nodes: HawkNextGroupedTaskViewNode[] = structure.groups.map((group) => {
