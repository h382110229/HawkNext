...
> &
  Partial<Pick<IHawkNextTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<HawkNextTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: HawkNextTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: HawkNextTaskMeta[][]): HawkNextTaskMeta[] {
  const itemByEntityKey = new Map<string, HawkNextTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, HawkNextTaskMeta>();
