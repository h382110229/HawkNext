...
type CurrentSidebarCodingPlanUsageSource =
  ...
  | {
      audience: "team";
      // 原硬绑 cloudCnCodingPlan，zai team plan 的 currentUsageSource
      // 无法表达 zai providerId。松开为 SidebarUsageCodingPlanProviderId，
      // zai/cloud-cn team 都按各自 selectedKey 解析出的 providerId 表达。
      providerId: SidebarUsageCodingPlanProviderId;
      sourceId: SidebarUsageCodingPlanSourceId;
...
export function buildCodingPlanUsageSources({
  ...
  accountAccesses: Partial<Record<ProviderFamilyDomain, HawkNextProviderAccountAccess>>;
...
function buildTeamCodingPlanUsageSources(
  ...
  return subscribedTeamProducts.flatMap((product) => {
    ...
    return projectContexts.flatMap((projectContext, index) => {
      ...
      if (!label) {
        // Team Plan usage source 只展示组织名；缺失时不能生成 "CloudCN - " 空白来源。
        return [];
      }
      const projectKey = projectId || String(index);
      // 原 createCloudCNTeamPlanConnectionKey + cloudCnCodingPlan providerId
      // 硬编码 cloud-cn，zai team product 的 sourceId 用了 cloud-cn 前缀、providerId 也错。
...
function formatTeamUsageSourceLabel({
  ...
  // 原硬编码 "CloudCN - " 前缀，zai team source 显示出来品牌也错位。
