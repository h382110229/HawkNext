import type { HawkNextError, TraceId } from "@hawknext/shared";
import { errorAttributionSchema, type ErrorAttribution } from "@hawknext/shared/hawknext-protocol-v4";

export interface HawkNextUiError extends HawkNextError {
...
interface NormalizeHawkNextUiErrorOptions {
  fallbackCode?: string;
...
const GENERIC_HAWKNEXT_UI_ERROR_MESSAGES = new Set([
  "Internal error",
  "Turn execution failed",
  "Compact failed",
  "Rewind failed",
  "HawkNext session failed",
]);
...
function collectMessageCandidatesFromRecord(record: Record<string, unknown>): string[] {
  ...
  const messagePaths: Array<readonly string[]> = [
    ...
    ["data", "message"],
    ["data", "detail"],
    // HawkNext Agent 常把可读原因放在 data.details（复数）里；之前只识别 detail，
    ...
    ["data", "error", "detail"],
    ["data", "error", "details"],
    // hawknext-cli 会把模型/网络错误摘要放在 data.hawknext.error 下。
    // 之前 UI 只读 data.error，导致已经结构化好的 provider 根因仍被 “Internal error” 盖住。
    ["data", "hawknext", "error", "message"],
    ["data", "hawknext", "error", "detail"],
    ["data", "hawknext", "error", "details"],
  ];
...
function readFirstAttributionFromPaths(error: unknown): ErrorAttribution | undefined {
  ...
  const paths: Array<readonly string[]> = [
    ...
    ["data", "hawknext", "error", "attribution"],
  ];
  for (const path of paths) {
...
export function normalizeHawkNextUiError(
  error: unknown,
  options: NormalizeHawkNextUiErrorOptions = {},
): HawkNextUiError {
  const candidates = collectMessageCandidates(error);
  // hawknext-cli 已经把 provider/network 根因放进 detail 或 data.hawknext.error，
  // 外层仍可能保留 "Internal error" 这类包装文案。主提示优先选非泛化候选，避免根因被盖住。
  const primaryMessage =
    candidates.find((candidate) => !GENERIC_HAWKNEXT_UI_ERROR_MESSAGES.has(candidate)) ??
    candidates[0] ??
    options.fallbackMessage ??
    "Internal error";
  const detailMessage = candidates.find(
    (candidate) => candidate !== primaryMessage && !GENERIC_HAWKNEXT_UI_ERROR_MESSAGES.has(candidate),
  );
  ...
  const underlyingErrorMessage = readFirstStringFromPaths(error, [
    ...
    ["data", "hawknext", "error", "underlyingErrorMessage"],
  ]);
  const underlyingErrorDetail = readFirstStringFromPaths(error, [
    ...
    ["data", "hawknext", "error", "underlyingErrorDetail"],
  ]);
