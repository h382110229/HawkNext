...
// Vite 配置在 Node 加载期执行，不能导入 @hawknext/shared 根入口。
...
} from "@hawknext/shared/hawknextEndpoint";
...
export default defineConfig(({ mode }) => {
  ...
  const hawknextEndpointOrigin = resolveRuntimeHawkNextEndpointOrigin(endpointEnv);
  const cloudOAuthOrigin = resolveCloudOAuthOrigin(endpointEnv);
  // ZAI OAuth client_id 是公开标识，允许注入浏览器包；secret/token 不得走 VITE_。
  const cloudOAuthClientId = resolveCloudOAuthClientId(endpointEnv);
  ...
  return {
    ...
    define: {
      ...
      __HAWKNEXT_VERSION__: JSON.stringify(version),
      __HAWKNEXT_COMMIT__: JSON.stringify(env.HAWKNEXT_COMMIT || "unknown"),
      __HAWKNEXT_ENV__: JSON.stringify(hawknextEnv),
      "import.meta.env.VITE_HAWKNEXT_BASE_URL": JSON.stringify(hawknextEndpointOrigin),
      // 兼容旧 Web runtime 读取名；新代码统一读 VITE_HAWKNEXT_BASE_URL。
      ...
      "import.meta.env.VITE_CLOUD_INTL_OAUTH_CLIENT_ID": JSON.stringify(cloudOAuthClientId),
      "import.meta.env.VITE_CLOUD_INTL_OAUTH_ORIGIN": JSON.stringify(cloudOAuthOrigin),
    },
    build: {
