...
import { toUserInfo } from "./cloudWebOAuthProvider.js";

const ACTIVE_PROVIDER_KEY = "oauth:active_provider";
const HAWKNEXT_JWT_TOKEN_KEY = "hawknext-jwt";
const CLOUD_INTL_ACCESS_TOKEN_KEY = "oauth:cloud-intl:access_token";
const CLOUD_INTL_USER_INFO_KEY = "oauth:cloud-intl:user_info";
const CLOUD_CN_ACCESS_TOKEN_KEY = "oauth:cloud-cn:access_token";
const CLOUD_CN_USER_INFO_KEY = "oauth:cloud-cn:user_info";
...
export type WebOAuthProviderId = typeof CLOUD_INTL_PROVIDER_ID | typeof CLOUD_CN_PROVIDER_ID;

function isWebOAuthProviderId(value: unknown): value is WebOAuthProviderId {
  return value === CLOUD_INTL_PROVIDER_ID || value === CLOUD_CN_PROVIDER_ID;
}
 ...
 * 让所有已登录用户在发版当天掉线。加一段 cloud-cn 前缀是零风险的做法，代价只是多一个映射。
 */
function providerKeys(provider: WebOAuthProviderId): { accessToken: string; userInfo: string } {
  return provider === CLOUD_CN_PROVIDER_ID
    ? { accessToken: CLOUD_CN_ACCESS_TOKEN_KEY, userInfo: CLOUD_CN_USER_INFO_KEY }
    : { accessToken: CLOUD_INTL_ACCESS_TOKEN_KEY, userInfo: CLOUD_INTL_USER_INFO_KEY };
}
...
export class BrowserOAuthCredentialRepo {
  ...
  saveTokenSet(tokenSet: WebZaiTokenSet | OAuthTokenSet, provider: WebOAuthProviderId): void {
    const accessToken =
      "cloudAccessToken" in tokenSet ? tokenSet.cloudAccessToken : tokenSet.accessToken;
    const hawknextJwtToken = tokenSet.hawknextJwtToken;

    this.localStorage.setItem(providerKeys(provider).accessToken, accessToken);
    if (hawknextJwtToken) {
      this.localStorage.setItem(HAWKNEXT_JWT_TOKEN_KEY, hawknextJwtToken);
    } else {
      this.localStorage.removeItem(HAWKNEXT_JWT_TOKEN_KEY);
    }
  }
  ...
  loadCachedSessionState(): OAuthCachedSessionRestoreResult {
    ...
    const hawknextJwtToken = this.localStorage.getItem(HAWKNEXT_JWT_TOKEN_KEY);
    ...
    if (!keys || !hasText(hawknextJwtToken) || !hasText(accessToken) || !hasText(rawUserInfo)) {
    ...
    if (resolveJwtExpiration(hawknextJwtToken, this.now()).kind === "expired") {
  ...
  loadHawkNextJwtToken(): string | null {
    const session = this.loadCachedSessionState();
    if (session.status !== "authenticated") return null;
    return this.localStorage.getItem(HAWKNEXT_JWT_TOKEN_KEY)?.trim() || null;
  }

  private hasAnyStoredCredential(): boolean {
    return Boolean(
      this.localStorage.getItem(ACTIVE_PROVIDER_KEY) ||
      this.localStorage.getItem(HAWKNEXT_JWT_TOKEN_KEY) ||
      this.localStorage.getItem(CLOUD_INTL_ACCESS_TOKEN_KEY) ||
      this.localStorage.getItem(CLOUD_INTL_USER_INFO_KEY) ||
      this.localStorage.getItem(CLOUD_CN_ACCESS_TOKEN_KEY) ||
      this.localStorage.getItem(CLOUD_CN_USER_INFO_KEY),
    );
  }
  ...
  clearAll(): void {
    ...
    this.localStorage.removeItem(CLOUD_INTL_ACCESS_TOKEN_KEY);
    this.localStorage.removeItem(CLOUD_INTL_USER_INFO_KEY);
    this.localStorage.removeItem(CLOUD_CN_ACCESS_TOKEN_KEY);
    this.localStorage.removeItem(CLOUD_CN_USER_INFO_KEY);
  }
