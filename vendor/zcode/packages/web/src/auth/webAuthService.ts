import { CLOUD_INTL_PROVIDER_ID, type UserInfo } from "@hawknext/shared";
import {
...
import { WEB_ZAI_OAUTH_CONFIG, type WebCloudOAuthConfig } from "./webCloudOAuthConfig.js";
import { CloudWebOAuthProvider } from "./cloudWebOAuthProvider.js";
...
interface WebAuthServiceDependencies {
  config?: WebCloudOAuthConfig;
  provider?: CloudWebOAuthProvider;
...
export class WebAuthService {
  private readonly config: WebCloudOAuthConfig;
  private readonly provider: CloudWebOAuthProvider;
  ...
  constructor(dependencies: WebAuthServiceDependencies = {}) {
    ...
    this.provider = dependencies.provider ?? new CloudWebOAuthProvider(this.config);
  ...
  startLogin(options: WebAuthLoginOptions = {}): void {
    ...
    const provider = options.provider ?? CLOUD_INTL_PROVIDER_ID;
  ...
  async handleCallback(url: string): Promise<WebAuthCallbackResult | null> {
    ...
    const provider = this.repo.loadPendingProvider() ?? CLOUD_INTL_PROVIDER_ID;
  ...
  getHawkNextJwtToken(): string | null {
    return this.repo.loadHawkNextJwtToken();
  }
