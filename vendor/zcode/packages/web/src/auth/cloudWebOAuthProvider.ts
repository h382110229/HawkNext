...
import type { OAuthTokenSet, UserInfo } from "@hawknext/shared";
import type { WebOAuthProviderId } from "./browserOAuthCredentialRepo.js";

export interface WebCloudOAuthProviderConfig {
  authorizeUrl: string;
  tokenUrl: string;
  clientId: string;
  redirectUri: string;
  /** CloudCN 授权入口；参数名与 ZAI 不同（redirect/appId vs redirect_uri/client_id）。 */
  cloudCnAuthorizeUrl: string;
  cloudCnAppId: string;
}

interface WebCloudOAuthCallbackParams {
  code?: string;
  error?: string;
  state: string;
}
...
function normalizeTokenResponse(
  ...
  // 服务端按 provider 把 access_token 放在不同子对象里（GrantJWT 的 resp.Zai / resp.CloudCn）。
  const accessToken = (
    provider === CLOUD_CN_PROVIDER_ID
      ? payload.data?.cloudCn?.access_token
...
export class CloudWebOAuthProvider {
  constructor(
    private readonly config: WebCloudOAuthProviderConfig,
  ...
  buildAuthorizeUrl(params: {
    ...
    if (params.provider === CLOUD_CN_PROVIDER_ID) {
      const query = new URLSearchParams({
        redirect: redirectUri,
        appId: this.config.cloudCnAppId,
        state: params.state,
      });
      return `${this.config.cloudCnAuthorizeUrl}?${query.toString()}`;
    }
  ...
  parseCallbackParams(url: string): WebCloudOAuthCallbackParams {
  ...
  async exchangeToken(params: {
    ...
    const provider = params.provider ?? CLOUD_INTL_PROVIDER_ID;
