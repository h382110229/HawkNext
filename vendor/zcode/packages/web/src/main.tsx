...
import {
  AppErrorBoundary,
  Root,
  HawkNextIntlProvider,
  generateMobileDeviceFingerprint,
  playTaskNotificationSound,
  setStreamClientId,
  type Theme,
} from "@hawknext/ui";
import "@hawknext/ui/styles.css";
import { connectViaWebSocket } from "@hawknext/client";
import { WebCallbackPage } from "./auth/WebCallbackPage.js";
import { createWebAuthService } from "./auth/webAuthService.js";
import { WEB_ZAI_OAUTH_CONFIG, resolveWebAuthDevReturnTo } from "./auth/webCloudOAuthConfig.js";
...
import type { IPlatformService, RemoteTarget, ServerRemoteInfo } from "@hawknext/shared";
import { WEB_DEFAULT_THEME, resolveWebInitialTheme } from "./webThemeSeed.js";
import { readSkinMode, applyHawkSkin, applyTokenVars } from "../../../../../packages/hawk-ui/src/skin.ts";

function resolveWebThemePreference(defaultTheme: Theme = WEB_DEFAULT_THEME): Theme {
  const saved = localStorage.getItem("hawknext-theme");
...
// system 模式下需要查询系统偏好；非 system 模式直接用存储值
{
  ...
  // HawkNext · HawkBrain skin (Wave 6); 「旧布局」 via hawk-legacy-layout class
  const skinMode = readSkinMode();
  applyHawkSkin(skinMode);
  if (skinMode === "hawk") {
    applyTokenVars(document.documentElement, "hawk");
  }
}
...
function renderWebAuthCallbackPage(): void {
  document.title = "HawkNext - Sign In";
...
async function renderConversationSharePage(): Promise<void> {
  ...
  document.title = routeLocale === "zh-CN" ? "HawkNext 会话分享" : "HawkNext Conversation Share";
  ...
  const endpointOrigin =
    import.meta.env.VITE_HAWKNEXT_BASE_URL?.trim().replace(/\/+$/u, "") || window.location.origin;
  const mockMode =
  ...
  const getMockToken = () =>
    mockMode && window.sessionStorage.getItem("hawknext:share:mock-auth") === "owner"
      ? "mock-owner-token"
      : null;
  const onLogout = () => {
    if (mockMode) {
      window.sessionStorage.removeItem("hawknext:share:mock-auth");
  ...
  root.render(
    <ConversationShareLandingLoader
      shareCode={shareCode}
      client={client}
      getAccessToken={() => getMockToken() ?? webAuthService.getHawkNextJwtToken()}
      onLogin={(provider) => {
        if (mockMode) {
          window.sessionStorage.setItem("hawknext:share:mock-auth", "owner");
          window.location.reload();
          return;
        }
...
async function bootstrapWebApp() {
  ...
  try {
    ...
    document.title = "HawkNext - Web + Server";
