...
import type { ConversationSharePreview } from "@hawknext/shared";
import { ConversationShareReadonlyTimeline } from "@hawknext/ui/conversation-share-readonly";
import { renderOAuthProviderIcon } from "@hawknext/ui/oauth-provider-icon";
import { applyTheme, resolveTheme, type Theme } from "@hawknext/ui/useTheme";
...
interface Copy {
  ...
  retry: string;
  continueInHawkNext: string;
  switchToDarkTheme: string;
  switchToLightTheme: string;
  continueHelp: string;
  downloadHawkNext: string;
  retryOpen: string;
...
const COPY: Record<ConversationShareLandingLocale, Copy> = {
  ...
  "en-US": {
    ...
    loginWith: {
      zai: "Connect to ",
      cloudCn: "Connect to CloudCN",
    },
    loginRegion: { zai: "Global", cloudCn: "CN" },
    ...
    notFoundAccountHint:
      " and CloudCN do not share account data. Check whether you selected the wrong sign-in platform or used a different account.",
    ...
    outdatedTitle: "Update HawkNext to continue",
    outdatedDescription:
      "This share was created by a newer version of HawkNext. Please update to view it.",
    unavailableTitle: "Share unavailable",
    unavailableDescription:
      "This account is not allowed to view the share, or it no longer exists.",
    retry: "Try again",
    continueInHawkNext: "Continue in HawkNext",
    switchToDarkTheme: "Switch to dark theme",
    switchToLightTheme: "Switch to light theme",
    continueHelp: "If HawkNext did not open, download the app or try opening it again.",
    downloadHawkNext: "Download HawkNext",
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {importLink && headerView.continueVisible ? (
                      <a
                        ...
                        href={importLink}
                        aria-label={copy.continueInHawkNext}
                        title={copy.continueInHawkNext}
                        onClick={handleContinue}
                      >
                        <span className={headerView.continueCompact ? "sr-only" : "truncate"}>
                          {copy.continueInHawkNext}
                        </span>
                        <ArrowUpRightIcon
                          className={
                            headerView.continueCompact
                  ...
                  >
                    ...
                    {importLink ? (
                      <span className={SHARE_CONTINUE_MEASURE_CLASS}>
                        <span>{copy.continueInHawkNext}</span>
                        <ArrowUpRightIcon
        ...
        {importLink && showContinueHelp ? (
          ...
          <ShareContentInset>
            ...
            >
              ...
              <a className="text-brand underline underline-offset-2" href={HAWKNEXT_DOWNLOAD_URL}>
                {copy.downloadHawkNext}
              </a>
            </div>
          </ShareContentInset>
        ) : null}
          ...
          <ShareContentInset>
            ...
            >
              <a
                ...
                onClick={handleContinue}
              >
                <span className="truncate">{copy.continueInHawkNext}</span>
...
export function ConversationShareLandingLoader({
  ...
  const handleThemeChange = useCallback((nextTheme: Theme) => {
    localStorage.setItem("hawknext-theme", nextTheme);
