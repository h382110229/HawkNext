// Agent bundle 的暂存动作：把 apps/hawknext-cli/packages/cli/dist/hawknext.cjs 放进
...
// desktopRuntimeEnv.ts 的 resolveBundledHawkNextAgentBinaryPath() 解析，候选**只有**
...
export const AGENT_BUNDLE_SOURCE_RELATIVE = "apps/hawknext-cli/packages/cli/dist/hawknext.cjs";
...
export function resolveAgentBundlePaths({ repoRoot, platformKey }) {
  ...
  return {
    ...
    glmDir,
    stagedBundlePath: resolve(glmDir, "hawknext.cjs"),
 ...
 * （hawknext-agent / hawknext-acp 等）和旧 meta 会被一并打进安装包（CI 干净检出不会有，本地会）。
 */
