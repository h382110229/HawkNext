...
export function createElectronRuntimeMirrorEnv(mirror) {
  return {
    HAWKNEXT_ELECTRON_RUNTIME_MIRROR: mirror,
...
function resolveDefaultElectronBuilderBinariesMirror(env = process.env) {
  return env.HAWKNEXT_DEPS_BASE_URL?.trim() || env.INTRANET_MACHINE_HOST?.trim()
...
环境变量:
  HAWKNEXT_TARGET_OS              与 --os 等价
  HAWKNEXT_TARGET_ARCH            与 --arch 等价
`);
}
...
function parseArgs(argv) {
  const options = {
    os: process.env.HAWKNEXT_TARGET_OS ?? null,
    arch: process.env.HAWKNEXT_TARGET_ARCH ?? null,
    skipPrepare: process.env.HAWKNEXT_SKIP_PREPARE === "1",
    skipBuild: process.env.HAWKNEXT_SKIP_BUILD === "1",
    dryRun: false,
    positionals: [],
  };
...
async function main() {
  ...
  const buildEnv = {
    HAWKNEXT_TARGET_OS: os,
    HAWKNEXT_TARGET_ARCH: arch,
