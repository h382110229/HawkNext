...
if (process.platform === "darwin" && existsSync(electronBinary)) {
  ...
  // hawknext:// 交给一个没有项目入口的 Electron 默认壳。给本地启动副本补齐产品
  ...
  const devBundle = await prepareDevElectronAppBundle({
    electronAppPath,
    runtimeRoot: resolve(root, "../../.hawknext-runtime/desktop-dev"),
    electronVersion: electronPackage.version,
    arch: process.arch,
  });
  electronCommand = devBundle.executablePath;
  console.log(`[dev] Prepared macOS HawkNext Dev bundle: ${devBundle.appPath}`);
}
