...
{
    private const string Protocol = "HAWKNEXT_BROWSER_IMPORT_V1";
        ...
        try
        {
            if (HasArgument(args, "--version"))
            {
                ...
                Console.Out.WriteLine(
                    "HAWKNEXT_BROWSER_IMPORT_HELPER\t" + HelperVersion + "\t" + GetProcessArchitecture() +
    ...
    private static void RunBrokerCore()
    {
        ...
        try
        {
            ...
            pipeName = "hawknext-browser-import-broker-" + Guid.NewGuid().ToString("N");
            token = CreateToken();
    ...
    private static void RunTemporarySystemService(
        ...
        int brokerPid)
    {
        string serviceName = "HawkNextBrowserImport_" + Guid.NewGuid().ToString("N");
        string pipeName = "hawknext-browser-import-system-" + Guid.NewGuid().ToString("N");
