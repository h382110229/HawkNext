...
contextBridge.exposeInMainWorld("__HAWKNEXT_DEVICE_ID__", parseDeviceIdFromArgs());
...
import {
  InternalChannels,
  PlatformChannels,
  formatHawkNextRendererProcessName,
...
if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__hawknextFinalArmsCustomEventsE2E", {
...
function updateRendererProcessTitle(): void {
  process.title = formatHawkNextRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 hawknext-* title，便于按窗口角色筛选。
 ...
 */
contextBridge.exposeInMainWorld("hawknext", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@hawknext/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  /** 打开 HawkNext Computer Use 完整权限引导 */
  ...
  reportLocalTtftBatch: (batch: import("@hawknext/shared").LocalTtftBatch): void =>
  ...
  /** 导出日志：打包 ~/.hawknext/v2 及外部 agent 日志为 zip 并在 Finder 中显示 */
  exportLogs: (): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  ...
  importChromeBrowserData: (options?: import("@hawknext/shared").ChromeBrowserDataImportOptions) =>
  ...
  getHawkNextStdioTapDevState: (): Promise<HawkNextStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetHawkNextStdioTapDevState),
