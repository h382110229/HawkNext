...
// - 在官网页主世界挂 window.hawknextBridge，暴露三个能力：
...
// - getLang 读 main world 的 window.__hawknextLang__（由 App executeJavaScript 注入）；
//   onLangChange 在 main world 监听 hawknext-coding-plan-lang-change CustomEvent
...
const PUBLIC_BRIDGE_KEY = "hawknextBridge";
const NATIVE_BRIDGE_KEY = "__hawknextCodingPlanWebviewNativeBridge__";
const LANG_VAR = "__hawknextLang__";
const LANG_CHANGE_EVENT = "hawknext-coding-plan-lang-change";
const REPORT_CONTEXT_VAR = "__hawknextReportContext__";
  ...
  try {
    ...
    if (
      !isTrustedCodingPlanWebviewOrigin(url.origin, {
        e2eStoreBridgeEnabled: process.env.VITE_HAWKNEXT_E2E_STORE_BRIDGE === "1",
      })
    ) {
      return false;
    }
    ...
    // 该页支付成功后仍需 hawknextBridge.notifyPurchaseComplete 通知 App 刷新模型设置。
...
if (isTrustedCodingPlanBridgeLocation()) {
    ...
    notifyPurchaseComplete(payload: NotifyPurchaseCompletePayload) {
      try {
        ipcRenderer.sendToHost(CodingPlanWebviewChannels.PurchaseComplete, {
          ...
          timestamp: Date.now(),
        } satisfies import("@hawknext/shared").CodingPlanPurchaseCompletePayload);
      } catch {
  ...
  contextBridge.executeInMainWorld({
    func: (
      ...
      const bridge = {
        notifyPurchaseComplete(payload: NotifyPurchaseCompletePayload) {
          if (payload?.provider !== "zai" && payload?.provider !== "cloud-cn") {
            return;
          }
        ...
        // App locale 变化时用 executeJavaScript 派发 hawknext-coding-plan-lang-change 事件。
