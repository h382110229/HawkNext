...
export function createRemoteWorkspaceServiceCollection(params: {
  ...
  createReportingRemoteHawkNextTaskService: <T extends object>(service: T) => T;
  ...
  const localAccountProviderCredentialService = createAccountProviderCredentialService({
    ...
    async loadOAuthAccessToken(family) {
      const providerId = family === "zai" ? CLOUD_INTL_PROVIDER_ID : CLOUD_CN_PROVIDER_ID;
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    tokenProvider: async () =>
      (await localCredentialService.load(HAWKNEXT_JWT_TOKEN_KEY))?.trim() || null,
  });
  const conversationShareService = new ConversationShareService({
    hawknextAgentService: params.connectionServices.hawknextAgentService,
  ...
  const reportingRemoteHawkNextTaskService = params.createReportingRemoteHawkNextTaskService(
    params.connectionServices.hawknextTaskService,
  );
  ...
  const remoteHawkNextTaskService = params.createRemotePromptAttachmentTaskService(
    reportingRemoteHawkNextTaskService,
  );
  const remoteHawkNextSessionService = params.createRemotePromptAttachmentSessionService(
    params.connectionServices.hawknextSessionService,
  );
  ...
  params.connectionServices.hawknextAgentService.onDynamicSessionRuntimePreferencesRequest()(
    (request) => {
      const startedAt = Date.now();
      const requestContext = {
        event: "hawknext_protocol.runtime_preferences.host_request_received",
      ...
      void (async () => {
        ...
        await params.connectionServices.hawknextAgentService.respondSessionRuntimePreferences({
          requestId: request.requestId,
          resolution,
        });
  ...
  // 因此这里为 remote workspace host 补齐本地全局 channel；文件、终端、HawkNext Agent 仍来自远端，
  ...
  const services = new ServiceCollection()
    ...
    .register(IHawkNextTaskService, remoteHawkNextTaskService)
    .register(IHawkNextAgentService, params.connectionServices.hawknextAgentService)
    .register(IHawkNextSessionService, remoteHawkNextSessionService)
