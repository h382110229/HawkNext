...
import { matchesTaskListMembershipKind } from "@hawknext/shared/hawknext-protocol-v4";
import type {
...
interface ResolvedWindowHostControllerSource {
  ...
  taskService?: IHawkNextTaskService;
...
function taskKey(task: Pick<HawkNextTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: HawkNextTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@hawknext/shared/hawknext-protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: HawkNextTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  createId: () => string;
  resolveSource: (scope: HawkNextTaskListWorkspaceScope) => ResolvedWindowHostControllerSource | null;
  onSourceError?: (
    scope: WindowHostControllerSourceScope,
  ...
  const sourceTaskServices = new Map<string, IHawkNextTaskService>();
  const sourceSnapshotTaskServices = new Map<string, IHawkNextTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: IHawkNextTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: IHawkNextAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  function resolveQuerySources(
    scopes: HawkNextTaskListWorkspaceScope[],
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@hawknext/shared").WindowHostAttachmentScope;
