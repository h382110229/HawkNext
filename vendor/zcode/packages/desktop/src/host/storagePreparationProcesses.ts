...
import {
  ...
  hawknextStoragePreparationFrameSchema,
  type DatabaseStartupState,
} from "@hawknext/shared";
import { resolveDefaultHawkNextAgentCommand } from "@hawknext/services/storage-startup";
...
export async function prepareSessionStorage(options: {
  ...
  const command = resolveDefaultHawkNextAgentCommand({
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      try {
        ...
        const frame = hawknextStoragePreparationFrameSchema.parse(JSON.parse(line));
        clearTimeout(firstStateTimer);
