...
import {
  ...
  createHawkNextAgentConnectionScope,
  type HawkNextAgentV4ClientMode,
  collectServiceMemoryDiagnostics,
} from "@hawknext/services";
import {
  createLocalServices,
...
import {
  ...
  type HawkNextAutomationRunOutcome,
  type ModelSelection,
...
import type {
  ...
  RemoteConnection,
} from "@hawknext/server/remote";
import type { RemoteTarget } from "@hawknext/shared";
...
import { createRemoteConnectionProgressContext } from "@hawknext/server/remote/remoteConnectionProgressContext.js";
...
const remoteMediaRangePreviewEnabled =
  process.env["HAWKNEXT_REMOTE_MEDIA_RANGE_PREVIEW_ENABLED"] !== "0";
...
// 这里根据 main 传入的窗口 label 补一层稳定的 hawknext-* title，方便系统进程列表过滤。
process.title = formatHawkNextHostProcessName(process.env["HAWKNEXT_PROCESS_LABEL"]);
...
function writeHostLog(level: HostLogLevel, ...args: unknown[]): void {
  const prefix = formatLogPrefix("hawknext-host", process.pid);
  const consoleFn =
...
function createFullFeedbackLogArchiveViaMain(
  ...
  return new Promise((resolve, reject) => {
    ...
    // 收集范围和“导出日志”不一致，缺少 hawknext-cli 日志、rollout/debug 以及导出链路脱敏。
...
async function resolveOffPeakFilesChanged(params: {
  hawknextTaskService: IHawkNextTaskService;
  taskId: string;
  workspacePath: string;
  workspaceIdentity?: string;
}): Promise<number | undefined> {
  try {
    const snapshot = await params.hawknextTaskService.getTaskSnapshot({
...
async function finalizeOffPeakRun(params: {
  ...
  workspacePath: string;
  workspaceIdentity?: string;
  outcome: HawkNextAutomationRunOutcome;
  error?: string;
}): Promise<void> {
  ...
  void params.hawknextTaskService.setTaskUnread({
    taskId: params.taskId,
    workspacePath: params.workspacePath,
    ...(params.workspaceIdentity ? { workspaceIdentity: params.workspaceIdentity } : {}),
    unread: true,
...
function trackOffPeakRunOutcome(params: {
  ...
  const disposable = params.hawknextTaskService.onDynamicTaskTerminalOutcome(params.taskId)(
    (result) => {
      if (result.inputId !== params.traceId) return;
      disposeOffPeakRunSubscription(key);
      void finalizeOffPeakRun({
        hawknextTaskService: params.hawknextTaskService,
...
async function dispatchOffPeakRun(request: OffPeakRunDispatchRequest): Promise<{
  ...
  const hawknextTaskService = activeServices?.getOptional(IHawkNextTaskService);
  if (!hawknextTaskService) {
    throw new Error("HawkNext task service is not initialized.");
  }
  const runtime = await ensureOffPeakRuntime();
  if (!runtime) {
  ...
  try {
    ...
    if (dispatchKind === "bound-first-run") {
      ...
      const [deletedIds, tasks] = await Promise.all([
        hawknextTaskService.listDeletedTaskIds(workspaceScope),
        hawknextTaskService.listTasks(workspaceScope),
      ]);
      assertBoundSessionDispatchable({
        sessionId: taskId,
      ...
      const task = await hawknextTaskService.createTask({
        workspacePath: request.workspacePath,
        workspaceIdentity: request.workspaceIdentity,
        // 空 Session 沿用普通初始化；idle Selection 只在下方执行中注入。
        // 在此写入会让闲时轮结束后的普通消息继续使用无票的隐藏 Provider。
        mode: request.permissionMode as HawkNextTaskMode,
    ...
    await hawknextTaskService.sendPrompt({
      taskId,
      traceId,
...
async function dispatchCronRun(request: CronRunDispatchRequest): Promise<{
  ...
  const hawknextTaskService = targetServices.getOptional(IHawkNextTaskService);
  if (!hawknextTaskService) {
    throw new Error("HawkNext task service is not initialized.");
  }
  ...
  try {
    const task = request.targetTaskId
      ...
      : await hawknextTaskService.createTask({
          workspacePath: request.workspacePath,
          workspaceIdentity: request.workspaceIdentity,
...
function isHawkNextTaskMeta(value: unknown): value is HawkNextTaskMeta {
  return (
    typeof value === "object" &&
    value !== null &&
    "taskId" in value &&
...
function createReportingRemoteHawkNextTaskService<T extends object>(
  ...
  options?: {
    ...
    }) => Promise<{ content: string; attachments?: HawkNextPromptAttachment[] }>;
  },
): T {
  ...
  function subscribeSessionMessageRequests(target: T, meta: HawkNextTaskMeta): void {
  ...
  async function mirrorRemotePrompt(
    ...
    // hawknextSessionStore 的整条消费链词表都是 HawkNextStreamEvent。两个方案的评估结论：
    // a) relay 直接转发 v4 帧、手机端消费 v4 store（正解）：需要重做 relay stream-op
    //    协议 + 手机端 store；
    // b) 帧→HawkNextStreamEvent 薄映射：等价复刻 adapter mapSessionEvent，
    //    否决。
    // 结论：本镜像保持 legacy 源不动。
    ...
    try {
      ...
      // 远端 hawknext-server 没有 desktop realtime port；由窗口 Host 内的
  ...
  function finishWorkspaceTask(taskId: string, meta: HawkNextTaskMeta): void {
    workspaceProxyState.disposeTaskReadySubscription(taskId);
    workspaceTaskTracker.finish(taskId, meta);
  }

  function beginWorkspaceTask(target: T, taskId: string, meta: HawkNextTaskMeta): boolean {
    ...
    if (typeof onDynamicTaskReady !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote HawkNext task service does not expose onDynamicTaskReady");
    }
    const subscribe = onDynamicTaskReady.call(target, taskId);
    if (typeof subscribe !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote HawkNext task ready event is not subscribable");
    }
  ...
  // remote workspace 的 HawkNext Agent manager 跑在远端 server，desktop main 不能直接看到
  ...
  return new Proxy(service, {
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        let trackedTask: { taskId: string; meta: HawkNextTaskMeta; started: boolean } | undefined;
...
function warmUpHawkNextAgent(
  ...
  const hawknextSessionService = services.getOptional(IHawkNextSessionService);
  if (!hawknextSessionService) {
    return;
  }
  void hawknextSessionService
    .initializeWorkspace({
      workspacePath,
    ...
    .then((result) => {
      if (!result.available) {
        if (result.reasonCode === "provider_not_ready") {
          logger.info(
            `HawkNext agent warmup waiting for provider/model (${reason}) workspace=${workspacePath}`,
          );
          return;
        }
        logger.warn(
          `HawkNext agent warmup unavailable (${reason}) workspace=${workspacePath} reason=${result.reason ?? "unknown"}`,
        );
        return;
      }
      ...
      logger.info(
        `HawkNext agent warmup ready (${reason}) workspace=${workspacePath} transport=${result.transportKind ?? "unknown"}`,
      );
    })
    .catch((error) => {
      logger.warn(`HawkNext agent warmup failed (${reason}) workspace=${workspacePath}:`, error);
    });
}
...
const hostResourceUsageResponder = createHostResourceUsageResponder({
  getAgentService: () => activeServices?.getOptional(IHawkNextAgentService),
...
async function createWindowRemoteConnectionHandle(params: {
  ...
  const services = createRemoteWorkspaceServiceCollection({
    ...
    createReportingRemoteHawkNextTaskService: (service) =>
      createReportingRemoteHawkNextTaskService(service, {
  ...
  let disposed = false;
  // 远端 workspace 的 CLI 与 MCP 样本走与本地同一条路径：远端 hawknext-server → 本地 Host → main。
...
>({
  ...
  releaseWorkspace: async (services, context) => {
    await services.get(IHawkNextTaskService).releaseWorkspacePreparation({
  ...
  resolveSource: (scope) => {
    ...
    if (remoteSession?.workspacePath && remoteSession.workspaceIdentity) {
      ...
      return {
        ...
        taskService: services.get(IHawkNextTaskService),
        agentService: services.getOptional(IHawkNextAgentService),
    ...
    const taskService = activeServices?.getOptional(IHawkNextTaskService);
    if (!taskService) {
      return null;
    }
    return {
      ...
      taskService,
      agentService: activeServices?.getOptional(IHawkNextAgentService),
      sourceAvailability: "online" as const,
    };
  },
});
  ...
  return new Proxy(base, {
    get(target, property, receiver) {
      if (property === "setTaskPinned") {
        return async (params: Parameters<IHawkNextTaskService["setTaskPinned"]>[0]) => {
      ...
      if (property === "archiveTask" || property === "unarchiveTask") {
        return async (
          params:
            | Parameters<IHawkNextTaskService["archiveTask"]>[0]
            | Parameters<IHawkNextTaskService["unarchiveTask"]>[0],
        ) => {
      ...
      if (property === "deleteTask") {
        return async (params: Parameters<IHawkNextTaskService["deleteTask"]>[0]) => {
          await route(params, { kind: "delete" });
        };
      }
      if (property === "deleteArchivedTasks") {
        return async (params: Parameters<IHawkNextTaskService["deleteArchivedTasks"]>[0]) => {
      ...
      if (property === "deleteArchivedTask") {
        return async (params: Parameters<IHawkNextTaskService["deleteArchivedTask"]>[0]) =>
      ...
      if (property === "setTaskUnread") {
        return async (params: Parameters<IHawkNextTaskService["setTaskUnread"]>[0]) => {
...
function exposeServicesOnMessagePort(
  ...
  services: ServiceCollection,
  deferInit: boolean,
  clientMode: HawkNextAgentV4ClientMode = "desktop-continuous",
  ...
  const agentService = services.getOptional(IHawkNextAgentService);
  const connectionScope = agentService
    ? createHawkNextAgentConnectionScope(agentService, {
  ...
  const taskService = services.getOptional(IHawkNextTaskService);
  if (taskService) {
    overrides.set(
      IHawkNextTaskService.channelName,
      createControllerRoutedTaskService(taskService, attachmentScope),
    );
  }
  if (connectionScope) {
    overrides.set(IHawkNextAgentService.channelName, connectionScope.service);
  }
...
parentPort.on("message", async (e: Electron.MessageEvent) => {
    ...
    void (async () => {
      try {
        const dispatchResult = await dispatchCronRun({
          ...msg,
          mode: msg.mode as HawkNextTaskMode | undefined,
        });
        parentPort.postMessage({
          type: HostResponseTypes.CronRunResult,
          runId: msg.runId,
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliver) {
    ...
    if (!hawknextTaskService) {
      parentPort.postMessage({
        ...
        result: {
          error: "HawkNext task service is not initialized.",
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliveryResult) {
    const hawknextTaskService = activeServices?.getOptional(IHawkNextTaskService);
    if (!hawknextTaskService) {
      logger.warn("session message delivery result received before HawkNext task service initialized");
      return;
    }
    void hawknextTaskService.sendSessionMessageDeliveryResult(msg.result).catch((error) => {
  ...
  if (msg.type === HostMessageTypes.InitLocal) {
    ...
    databaseStartup = createHostDatabaseStartup({
      ...
      initializeServices: async () => {
        ...
        const services = await initializeHostApiNetworkTransportOwner({
          ...
          establishOwner: () => {
            const initializedServices = createLocalServices({
              ...
              hawknextAgentSpawnFallbackCwd: msg.agentSpawnFallbackCwd,
              hawknextBuiltinProviderConfigFilePath: msg.hawknextBuiltinProviderConfigFilePath,
              ...
              // browser-use：agent 的 interaction/browserExecute 经 hawknextAgentService 转到这个 executor，
        ...
        const hawknextTaskService = services.getOptional(IHawkNextTaskService);
        if (hawknextTaskService) {
          const reportingHawkNextTaskService = createReportingRemoteHawkNextTaskService(
            hawknextTaskService,
            {
              reportRunningPromptCount: false,
            },
          );
          services.register(IHawkNextTaskService, reportingHawkNextTaskService);
        }
...
async function setupRemoteConnection(
  ...
  const { createRemoteBackend, connectRemote, pickRemoteRuntimeEnv } =
    await import("@hawknext/server/remote");
  ...
  const connection = await connectRemote(backend, {
    ...
    // 这里显式透传编译期版本，避免漏导入后生成裸 HAWKNEXT_VERSION 引用导致 SSH 初始化直接 ReferenceError。
    appVersion: HAWKNEXT_VERSION,
    // 远端 hawknext-server/agent 是独立进程，不能继承 host 里的测试/生产 endpoint 选择。
