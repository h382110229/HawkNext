import type { HawkNextAutomationRunOutcome, HawkNextAutomationTrigger } from "@hawknext/shared";
...
interface CronRunLifecycleRepo {
  ...
  markRunOutcome(runId: string, outcome: HawkNextAutomationRunOutcome, error?: string): Promise<void>;
...
export async function settleCronRunTerminalOutcome(
  params: CronRunLifecycleIdentity & {
    repo: CronRunLifecycleRepo;
    outcome: Exclude<HawkNextAutomationRunOutcome, "running">;
    error?: string;
    logWarn: LogWarn;
  },
): Promise<void> {
