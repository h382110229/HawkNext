import type { IRemoteBackend, RemoteUploadOptions } from "@hawknext/server/remote";
import { quotePosixPathArg } from "@hawknext/server/remote/posixShell.js";
import type { TraceId, HawkNextPromptAttachment } from "@hawknext/shared";
import { randomUUID } from "node:crypto";

const REMOTE_PROMPT_ATTACHMENT_ROOT = "~/.hawknext/tmp/prompt-attachments";
const REMOTE_PROMPT_ATTACHMENT_RELATIVE_ROOT = ".hawknext/tmp/prompt-attachments";
...
export async function materializeRemotePromptAttachments(
  ...
  const nextAttachments: HawkNextPromptAttachment[] = [];
...
function getAttachmentLocalPath(attachment: HawkNextPromptAttachment): string | undefined {
      ...
      return async (...args: unknown[]) => {
        ...
        const prepared = await options.materializePromptAttachments({
          ...
          attachments: Array.isArray((params as { attachments?: unknown }).attachments)
            ? (params as { attachments?: HawkNextPromptAttachment[] }).attachments
            : undefined,
        });
