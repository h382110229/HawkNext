...
import { connectViaMessagePort, createMessagePortServiceConnection } from "@hawknext/client";
import {
  InternalChannels,
...
import type { Locale } from "@hawknext/shared";
...
).__HAWKNEXT_RENDERER_START__ = rendererStartedAt;
(window as Window & { __HAWKNEXT_LAUNCH_MARKS__?: LaunchMarks | null }).__HAWKNEXT_LAUNCH_MARKS__ =
  launchMarks;
...
function registerE2EStoreBridgesIfEnabled() {
  ...
  if (env.VITE_HAWKNEXT_E2E_STORE_BRIDGE !== "1") {
    return;
  }

  void import("@hawknext/ui/e2e-store-bridge").then(({ registerE2EStoreBridges }) => {
...
{
  const saved = localStorage.getItem("hawknext-theme") || "zai-dark";
  const resolved =
    saved === "system"
...
const isLocalDevelopmentRuntime =
  (globalThis as typeof globalThis & { __HAWKNEXT_LOCAL_DEVELOPMENT_RUNTIME__?: boolean })
    .__HAWKNEXT_LOCAL_DEVELOPMENT_RUNTIME__ === true;
...
function StartupReadyNotifier() {
  useEffect(() => {
    ...
    (window as Window & { __HAWKNEXT_REACT_COMMIT_AT__?: number }).__HAWKNEXT_REACT_COMMIT_AT__ =
      Date.now();
    // HTML 启动壳的弹出动画结束时，React 首屏可能还没 commit，直接移除壳会露出空白。
    // 这里在 React commit 后通知 index.html，再由启动壳统一判断动画和 React ready 两个条件后退场。
    window.dispatchEvent(new Event("hawknext-react-startup-ready"));
  }, []);
...
function initializeBusinessRoot(port: MessagePort): void {
  ...
  syncAppTelemetryContext({
    bridge: {
      syncTelemetryContext: (context) => window.hawknext.syncTelemetryContext(context),
    },
