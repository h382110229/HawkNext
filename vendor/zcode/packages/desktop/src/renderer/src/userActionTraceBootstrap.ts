...
import { RendererUserActionTelemetry, setUserActionTelemetry } from "@hawknext/ui";
...
export function initializeDesktopUserActionTrace(options: {
  ...
  const telemetry = new RendererUserActionTelemetry({
    ...
    resource: {
      ...
      serviceVersion: HAWKNEXT_VERSION || "unknown",
      deploymentEnvironment: options.isLocalDevelopmentRuntime ? "development" : HAWKNEXT_ENV,
      rendererInstanceId,
    },
