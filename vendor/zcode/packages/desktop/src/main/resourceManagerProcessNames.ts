...
import { formatHawkNextRendererProcessName } from "@hawknext/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatHawkNextRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatHawkNextRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatHawkNextRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
