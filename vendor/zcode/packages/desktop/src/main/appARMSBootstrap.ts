...
import { HAWKNEXT_AGENT_LIFECYCLE_LOG_MARKER } from "@hawknext/shared/process-diagnostic";
import {
...
function hasProductExecutable(
  ...
  // Linux Preview 使用 hawknext-preview。两者都必须精确匹配，不能放宽成前缀以免混入 helper dump。
...
const armsRumEnv = mapHawkNextEnvToArmsRumEnv(desktopRuntimeEnv);
...
function startArmsRum(): Promise<void> {
  return armsRum
    .init({
      ...
      endpoint: HAWKNEXT_ARMS_RUM_ENDPOINT,
      env: armsRumEnv,
          ...
          (payload?.events ?? []).filter(
            (event) =>
              !(
                ...
                event.message.includes(HAWKNEXT_AGENT_LIFECYCLE_LOG_MARKER)
              ),
          ),
    ...
    .then(() => {
      logger.info(`[arms] electron initialized env=${armsRumEnv} version=${HAWKNEXT_VERSION}`);
    })
    .catch((error) => {
...
export const armsInitPromise: Promise<void> =
  HAWKNEXT_TELEMETRY_ENABLED && HAWKNEXT_ARMS_RUM_ENDPOINT ? startArmsRum() : Promise.resolve();
