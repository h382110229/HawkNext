...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@hawknext/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "hawknext-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "hawknext.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "hawknext.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@hawknext/local-ttft", "1");
  const duration = meter.createHistogram("hawknext.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("hawknext.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("hawknext.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "hawknext.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("hawknext.local_ttft.system.duration", { unit: "ms" });
  const preparationDuration = meter.createHistogram("hawknext.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("hawknext.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("hawknext.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("hawknext.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("hawknext.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("hawknext.local_ttft.records");
  const drops = meter.createCounter("hawknext.local_ttft.dropped");
