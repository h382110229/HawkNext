...
import {
  ...
  DEFAULT_LOCALE,
  HAWKNEXT_VERSION,
  HAWKNEXT_TELEMETRY_ENABLED,
  HAWKNEXT_ARMS_RUM_ENDPOINT,
...
import {
  ...
  updateHawkNextStdioTapDevMenuState,
...
import { resolveHawkNextBuiltinProviderConfigFilePath } from "./desktopProviderConfig.js";
import {
  getCredentialsDir,
  ...
  resolveHawkNextEndpointEnvBaseOrigin,
  desktopRuntimeEnv,
...
import {
  registerDesktopHawkNextDataSizeTelemetry,
  stopDesktopHawkNextDataSizeTelemetry,
} from "./desktopHawkNextDataSizeTelemetry.js";
...
import { mapHawkNextEnvToArmsRumEnv } from "@hawknext/shared";
import {
...
// 会和开发态已打开的 HawkNext Dev 抢端口，导致 WebDriver session 创建前白屏超时。
// 仅本地开发运行默认开启远程调试端口，并允许 e2e 通过环境变量交给 Chromedriver 接管。
if (!app.isPackaged && process.env.HAWKNEXT_DISABLE_FIXED_REMOTE_DEBUGGING_PORT !== "1") {
...
const settingsFile = join(homedir(), ".hawknext", "v2", "setting.json");
...
async function resolveCurrentHawkNextEndpointOrigin() {
  return resolveHawkNextEndpointOrigin({
    env: HAWKNEXT_ENV,
    envBaseOrigin: resolveHawkNextEndpointEnvBaseOrigin(hostProcessLocalEnv),
    overrideOrigin: (await mainSettingService.get()).hawknextEndpointOrigin,
  });
}
...
app.on("browser-window-focus", (_event, win) => {
  ...
  rebuildMenu();
  // 设置/更新等无 Host 的 HawkNext 窗口也算前台：router 会先把旧 workspace Host 清成 null，
...
const readHelpConfig = createDesktopHelpConfigReader({
  appVersion: HAWKNEXT_VERSION || app.getVersion(),
  deviceMid,
...
const localTtftExporter = createLocalTtftExporter({
  ...
  version: HAWKNEXT_VERSION || app.getVersion(),
  logger,
});
...
const armsUserIdentitySync = createArmsUserIdentitySync({
  ...
  setUser:
    HAWKNEXT_TELEMETRY_ENABLED && HAWKNEXT_ARMS_RUM_ENDPOINT
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  appQuitPreparationInFlight = Promise.all([
    ...
    .finally(() => {
      ...
      // host 还没等到 agent 进程树的 SIGTERM/SIGKILL 兜底完成就被带走，hawknext-cli 会被 init 接管成残留进程。
...
function exitPreparedApp(reason: string): never | void {
  ...
  if (process.env.HAWKNEXT_E2E_RUN_ID?.trim()) {
...
function shouldConfirmAppQuit() {
  ...
  return HAWKNEXT_ENV === "production" && getRunningAgentSessionCount() > 0;
}

function confirmAppQuit(originWindow?: BrowserWindow | null) {
  if (!shouldConfirmAppQuit()) {
    logger.info(`[app-quit] quit confirmation skipped in ${HAWKNEXT_ENV}`);
    return true;
  }
  ...
  const dialogOptions = {
    ...
    message: isZh ? "确认退出 HawkNext?" : "Quit HawkNext?",
...
async function executeDesktopCommandForApp(
  ...
  return executeDesktopCommand({
    ...
    onHawkNextEndpointChanged: handleHawkNextEndpointChanged,
    hawknextEndpointEnvBaseOrigin: resolveHawkNextEndpointEnvBaseOrigin(hostProcessLocalEnv),
    onRelaunchApp: async () => {
...
async function resolveHawkNextEndpointSelection(): Promise<"production" | "test" | "custom"> {
  if (HAWKNEXT_ENV === "production") {
    return "production";
  }
  const origin = await resolveCurrentHawkNextEndpointOrigin();
  if (origin === DEFAULT_HAWKNEXT_ENDPOINT_ORIGIN) {
    return "production";
  }
  return "custom";
}

async function handleHawkNextEndpointChanged() {
  rebuildMenu();
}
...
function rebuildMenu() {
  void Promise.all([resolveHawkNextEndpointSelection(), mainSettingService.get()]).then(
    ([hawknextEndpointSelection, settings]) => {
      rebuildApplicationMenu({
        currentApplicationLocale,
        hawknextEndpointSelection,
  ...
  const win = createWindow({
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        {
          ...initMessage,
          hawknextBuiltinProviderConfigFilePath: resolveHawkNextBuiltinProviderConfigFilePath({
            env: { ...hostProcessLocalEnv, ...process.env },
          }),
        },
        {
...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32") {
    ...
    app.setAppUserModelId(
      resolveWindowsAppUserModelIdForFlavor(HAWKNEXT_PRODUCT_FLAVOR, { isPackaged: app.isPackaged }),
    );
  }
  ...
  // Preview 身份无论连接哪个后端都不自动更新：stable feed 上只分发正式 HawkNext 安装包，
  // 不向 Preview 渠道提供更新。
  void initAutoUpdater({
    enabled: HAWKNEXT_PRODUCT_FLAVOR === "production",
  ...
  registerRemoteIpcHandlers({
    ...
    armsCustomContext: {
      ...
      armsEnv: mapHawkNextEnvToArmsRumEnv(desktopRuntimeEnv),
    },
  ...
  if (HAWKNEXT_TELEMETRY_ENABLED && HAWKNEXT_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: HAWKNEXT_VERSION,
  ...
  registerDesktopHawkNextDataSizeTelemetry({
    ...
    isHawkNextBusy: () => getRunningAgentSessionCount() > 0,
    logger,
    rootPath: getHawkNextDataRootDir(),
    stateFile: join(app.getPath("userData"), "hawknext-data-size-telemetry.json"),
  });
  registerDesktopNetworkTelemetry(logger);

  // 本地未打包 dev 构建（app.isPackaged === false）必须跳过远端强制升级 gate。
  // 原因：force-update gate 只看 HAWKNEXT_ENV === "production"，但 dev 构建（如 dev:desktop:cua
  ...
  const forceUpdateGuardResult =
    HAWKNEXT_PRODUCT_FLAVOR === "production" && !skipForceUpdateForLocalDevRuntime
