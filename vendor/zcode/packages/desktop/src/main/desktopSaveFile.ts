...
import type { SaveFileRequest, SaveFileResult } from "@hawknext/shared";
import { PlatformChannels } from "@hawknext/shared";
...
async function downloadRemoteFile(sourceUrl: URL, destinationPath: string): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "hawknext-save-file-"));
