...
export type HawkNextDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type HawkNextDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: HawkNextDataSizePartialReason }
);

export interface HawkNextDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface HawkNextDataSizeScanOptions extends HawkNextDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("HawkNext data size scan aborted", "AbortError");
}
 ...
 */
export async function scanHawkNextDataDirectory(
  options: HawkNextDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<HawkNextDataSizePartialReason, "io_error"> | null = null;
...
export function isHawkNextDataSizeScanResult(value: unknown): value is HawkNextDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<HawkNextDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
