...
import { readHawkNextStdioTapDevState } from "@hawknext/services/node";
...
import {
  HELP_TOGGLE_HAWKNEXT_STDIO_TAP_MENU_ID,
  HELP_TOGGLE_DEV_TOOLS_MENU_ID,
} from "./desktopCommandHandlers.js";

const HELP_HAWKNEXT_ENDPOINT_PRODUCTION_MENU_ID = "help.hawknext-endpoint.production";
...
export function updateHawkNextStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_HAWKNEXT_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readHawkNextStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  hawknextEndpointSelection?: "production" | "test" | "custom";
  ...
  const stdioTapState = readHawkNextStdioTapDevState();
      ...
      ? [
          {
            ...
            submenu: [
              ...
              ...(HAWKNEXT_PRODUCT_FLAVOR === "production"
                ? [
                    {
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_HAWKNEXT_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleHawkNextStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(DesktopCommandIds.ToggleHawkNextStdioTapDevProxy),
              },
              { type: "separator" as const },
            ]
          : []),
        ...(HAWKNEXT_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpHawkNextEndpoint),
                submenu: [
                  {
                    id: HELP_HAWKNEXT_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpHawkNextEndpointProduction),
                    type: "radio" as const,
                    checked: (options.hawknextEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetHawkNextEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpHawkNextEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetHawkNextEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpHawkNextEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetHawkNextEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        hawknextEndpointSelection: options.hawknextEndpointSelection,
