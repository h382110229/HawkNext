...
import {
  ...
  HAWKNEXT_MCP_RESOURCE_SAMPLE_INTERVAL_MS,
...
interface StoredSample {
  sample: HawkNextMcpResourceSample;
...
function purgeExpired(now: number): void {
  for (const [key, entry] of latest) {
    if (now - entry.receivedAt > HAWKNEXT_MCP_RESOURCE_SAMPLE_INTERVAL_MS * 2) latest.delete(key);
  }
}
...
export function ingestMcpResourceSamples(
  ...
  const parsed = hawknextMcpResourceSamplesSchema.safeParse(raw);
  ...
  sample(context) {
    ...
    for (const [key, group] of groupSamples()) {
      recordExternalAppResourceSample({
        ...
        intervalMs: HAWKNEXT_MCP_RESOURCE_SAMPLE_INTERVAL_MS,
