...
/* eslint-disable max-lines -- host process 统一处理 main↔host 生命周期、日志、HawkNext Agent，拆分前先保持跨进程消息收口。 */
...
export interface HostInitMessage {
  ...
  /** Main 解析后的 HawkNext Built-in Provider Config 路径；Host/Services 不感知 Electron 安装布局。 */
...
export function spawnHostProcess(
  ...
  const execArgv = [
    ...(RUNTIME_HAWKNEXT_DEBUG ? [`--inspect-brk=${RUNTIME_HAWKNEXT_DEBUG}`] : []),
    "--no-warnings",
  ];
  const child = electronUtilityProcess.fork(hostModulePath, [], {
    serviceName: formatHawkNextHostProcessName(label),
    execArgv,
    env: {
      ...buildHostProcessEnv(dependencies.hostProcessLocalEnv),
      ...buildHostE2ECoverageEnv(),
      HAWKNEXT_PROCESS_LABEL: label,
      // macOS-only: the Computer Use Helper launcher runs inside this forked host utilityProcess, whose
      // code-signing identity is a nested Electron helper (NOT dev.hawknext.app). Publish THIS (main
      // Electron) process's pid — which IS dev.hawknext.app — so helperLauncher passes it as
      ...
      ...(process.platform === "darwin" ? { HAWKNEXT_CUA_LAUNCHER_PID: String(process.pid) } : {}),
      ...(dependencies.desktopContextPromptEnabled
        ? {
            [HAWKNEXT_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]: dependencies.desktopContextPromptEnabled()
              ? "1"
              : "0",
          }
        : {}),
  ...
  dependencies.logger.info(
    `[spawnHostProcess] CLOUD_CN_OAUTH_APP_SECRET source: ${process.env.CLOUD_CN_OAUTH_APP_SECRET ? "process" : dependencies.hostProcessLocalEnv.CLOUD_CN_OAUTH_APP_SECRET ? "dotenv" : "fallback"}`,
  );
...
export function disposeHostProcess(
  ...
  // 如果 main 仍按 150/300ms 强杀 host，host 会先退出，hawknext-cli/app-server 子进程就可能被 init 接管成孤儿。
