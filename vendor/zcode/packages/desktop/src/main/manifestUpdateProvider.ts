...
function buildElectronManifestUrl(options: {
  ...
  const url = options.manifestUrl?.trim()
    ...
    : new URL(ELECTRON_MANIFEST_API_PATH, normalizeHawkNextEndpointOrigin(options.endpointOrigin));
...
export class ManifestUpdateProvider extends Provider<UpdateInfo> {
  ...
  private resolveBaseUrl = new URL(DEFAULT_HAWKNEXT_ENDPOINT_ORIGIN);
  ...
  constructor(
    ...
    this.resolveBaseUrl = new URL(
      normalizeHawkNextEndpointOrigin(options.endpointOrigin ?? DEFAULT_HAWKNEXT_ENDPOINT_ORIGIN),
    );
  }
  ...
  override async getLatestVersion(): Promise<UpdateInfo> {
    ...
    return {
      ...
      hawknextReleaseChannel: releaseChannel,
    } as UpdateInfo;
  }
  ...
  private async resolveEndpointOrigin(): Promise<string> {
    const resolved =
      ...
      DEFAULT_HAWKNEXT_ENDPOINT_ORIGIN;
    return normalizeHawkNextEndpointOrigin(resolved);
  }
