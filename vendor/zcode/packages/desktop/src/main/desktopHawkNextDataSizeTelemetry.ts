...
import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@hawknext/shared";

import type { HawkNextDataSizeScanResult } from "./hawknextDataSizeScanner.js";
import {
  readHawkNextDataSizeTelemetryState,
  writeHawkNextDataSizeTelemetryState,
  type HawkNextDataSizeTelemetryState,
} from "./hawknextDataSizeTelemetryState.js";
import { scanHawkNextDataDirectoryInWorker } from "./hawknextDataSizeWorkerClient.js";

export type { HawkNextDataSizeTelemetryState } from "./hawknextDataSizeTelemetryState.js";

const HAWKNEXT_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const HAWKNEXT_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const HAWKNEXT_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const HAWKNEXT_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const HAWKNEXT_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const HAWKNEXT_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const HAWKNEXT_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const HAWKNEXT_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const HAWKNEXT_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const HAWKNEXT_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const HAWKNEXT_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const HAWKNEXT_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface HawkNextDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: HawkNextDataSizeTelemetryTiming = {
  abortedRetryMs: HAWKNEXT_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: HAWKNEXT_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: HAWKNEXT_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: HAWKNEXT_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: HAWKNEXT_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: HAWKNEXT_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: HAWKNEXT_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: HAWKNEXT_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: HAWKNEXT_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: HAWKNEXT_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface HawkNextDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface HawkNextDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isHawkNextBusy: () => boolean;
  logger: HawkNextDataSizeTelemetryLogger;
  readState: () => Promise<HawkNextDataSizeTelemetryState | null>;
  report: (result: HawkNextDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<HawkNextDataSizeScanResult>;
  timing?: Partial<HawkNextDataSizeTelemetryTiming>;
  writeState: (state: HawkNextDataSizeTelemetryState) => Promise<void>;
}
...
function buildHawkNextDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: HawkNextDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_hawknext_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      metric_kind: "hawknext_data_bytes",
      ...
      schema_version: 1,
      hawknext_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / HAWKNEXT_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createHawkNextDataSizeTelemetryScheduler(
  dependencies: HawkNextDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: HawkNextDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isHawkNextBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[hawknext-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<HawkNextDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: HawkNextDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[hawknext-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("HawkNext data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("HawkNext data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[hawknext-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[hawknext-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function evaluateDueCollection(): Promise<void> {
    ...
    if (dependencies.isHawkNextBusy()) {
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: HawkNextDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[hawknext-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createHawkNextDataSizeTelemetryScheduler> | null = null;

export function registerDesktopHawkNextDataSizeTelemetry(options: {
  context: Parameters<typeof buildHawkNextDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isHawkNextBusy: () => boolean;
  logger: HawkNextDataSizeTelemetryLogger;
  ...
  stopDesktopHawkNextDataSizeTelemetry();
  desktopScheduler = createHawkNextDataSizeTelemetryScheduler({
    ...
    isHawkNextBusy: options.isHawkNextBusy,
    logger: options.logger,
    readState: () => readHawkNextDataSizeTelemetryState(options.stateFile),
    report: (result) => {
      const payload = buildHawkNextDataSizeArmsPayload({ context: options.context, result });
      armsRum.sendCustom({
        group: payload.group,
        name: payload.name,
    ...
    scan: ({ signal }) =>
      scanHawkNextDataDirectoryInWorker(
        {
          maxDurationMs: HAWKNEXT_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: HAWKNEXT_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeHawkNextDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[hawknext-data-size] scheduler start failed", error);
  });
}

export function stopDesktopHawkNextDataSizeTelemetry(): void {
