...
export interface HawkNextDataSizeTelemetryState {
  lastReportedAt?: number;
  reportReservedAt?: number;
}

export async function readHawkNextDataSizeTelemetryState(
  stateFile: string,
): Promise<HawkNextDataSizeTelemetryState | null> {
  try {
    const parsed: unknown = JSON.parse(await readFile(stateFile, "utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      throw new TypeError("Invalid HawkNext data size telemetry state");
    }
    const record = parsed as Record<string, unknown>;
    const state: HawkNextDataSizeTelemetryState = {};
    if (record.lastReportedAt !== undefined) {
      if (typeof record.lastReportedAt !== "number" || !Number.isFinite(record.lastReportedAt)) {
        throw new TypeError("Invalid lastReportedAt in HawkNext data size telemetry state");
      }
    ...
    if (record.reportReservedAt !== undefined) {
      if (
        ...
        throw new TypeError("Invalid reportReservedAt in HawkNext data size telemetry state");
      }
...
export async function writeHawkNextDataSizeTelemetryState(
  stateFile: string,
  state: HawkNextDataSizeTelemetryState,
): Promise<void> {
