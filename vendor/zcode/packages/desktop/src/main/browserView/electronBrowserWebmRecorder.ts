...
const RECORDER_PORT_CHANNEL = "hawknext-browser-video-recorder:port";
...
export async function createElectronBrowserWebmRecorder(
  ...
  const recorderSession = session.fromPartition(`hawknext-browser-video-recorder-${randomUUID()}`);
