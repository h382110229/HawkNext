...
import type { DesktopTitleBarTheme, Locale } from "@hawknext/shared";
import {
  DEFAULT_LOCALE,
...
// Coding Plan 官网页专用 preload：挂 window.hawknextBridge 供官网回传购买完成信号。
...
function isCodingPlanPaypalNavigationUrl(url: string): boolean {
  try {
    ...
    // 后端下发的 PayPal approveUrl 可能先指向  支付 API 中转地址，
    // 由该地址再 302 到 PayPal。中转 URL 也必须留在当前 webview，否则会被系统浏览器接管。
    return (
      ["", resolveZaiBusinessBaseUrl()].includes(parsed.origin) &&
...
function attachEmbeddedBrowserWindowOpenHandler(options: {
  ...
  options.guestWebContents.on("will-navigate", (event, url) => {
    ...
    // 离开可信购买页时必须阻断 guest 导航并交给系统浏览器，避免第三方页面继承 hawknextBridge。
    event.preventDefault();
    void shell.openExternal(url).catch((error: unknown) => {
...
export function createBrowserWindow(options: {
  ...
  win.webContents.on("will-attach-webview", (event, webPreferences, params) => {
    ...
    //
    // Coding Plan 官网页例外：它需要 window.hawknextBridge 回传购买完成信号，
    ...
    // 如果放任 Electron 默认处理，又会创建脱离 HawkNext 的 BrowserWindow。这里由宿主重新打开
