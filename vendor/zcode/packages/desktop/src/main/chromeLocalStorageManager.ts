...
import type { ChromeBrowserDataImportError } from "@hawknext/shared";
...
export async function readChromeCookiesWithHelper(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "hawknext-chrome-cookie-helper-"));
...
export async function importChromeLocalStorage(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "hawknext-chrome-local-storage-"));
  const userDataPath = join(tempRoot, "User Data");
  const targetProfilePath = join(userDataPath, "Default");
  let originCount = 0;
  try {
