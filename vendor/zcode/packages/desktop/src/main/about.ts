...
const ABOUT_APPLICATION_NAME = "HawkNext Desktop App";
...
> = {
  "zh-CN": {
    aboutTitle: "关于 HawkNext",
    versionLabel: "版本",
    okButtonLabel: "确定",
    optimizedForAppleSilicon: "已针对 Apple Silicon 优化。",
    copyright: (year) => `版权所有 © ${year} HawkNext。`,
  },
  "en-US": {
    aboutTitle: "About HawkNext",
    versionLabel: "version",
    okButtonLabel: "OK",
    optimizedForAppleSilicon: "Optimized for Apple Silicon.",
    copyright: (year) => `Copyright © ${year} HawkNext.`,
  },
};
...
export function createAboutSnapshot(options: AboutSnapshotOptions = {}): AboutSnapshot {
  ...
  return {
    appVersion: normalizeValue(options.appVersion ?? buildMetadata?.appVersion ?? HAWKNEXT_VERSION),
    buildCommitId: normalizeValue(buildMetadata?.buildCommitId ?? HAWKNEXT_COMMIT),
    buildTime: normalizeValue(buildMetadata?.buildTime ?? HAWKNEXT_BUILD_TIME),
    environment: normalizeValue(options.environment ?? HAWKNEXT_ENV),
