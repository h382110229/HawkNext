...
import {
  ...
  normalizeHawkNextEndpointOrigin,
  resolveHawkNextEndpointOrigin,
} from "@hawknext/shared";
import { readHawkNextStdioTapDevState, setHawkNextStdioTapDevEnabled } from "@hawknext/services/node";
...
export const HELP_TOGGLE_HAWKNEXT_STDIO_TAP_MENU_ID = "help.toggle-hawknext-stdio-tap";
const HAWKNEXT_ENDPOINT_PROMPT_WIDTH = 460;
const HAWKNEXT_ENDPOINT_PROMPT_HEIGHT = 210;
const CODING_PLAN_WEBVIEW_PARTITION = "persist:hawknext-coding-plan";
...
async function clearAllDataAndRelaunch(options: {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: "确定要清除所有数据吗？",
    detail:
      "将删除 ~/.hawknext/v2（配置、凭据、日志）和浏览器缓存（localStorage）。操作不可恢复，清除后应用将自动重启。",
  });
  if (response !== 1) {
    return;
  }
  ...
  try {
    ...
    options.logger.info("[clear-all-data] deleted ~/.hawknext/v2");
  } catch (error) {
    options.logger.error("[clear-all-data] failed to delete ~/.hawknext/v2:", error);
  }
...
async function promptCustomHawkNextEndpoint(
  targetWindow: BrowserWindow | null | undefined,
  currentValue: string,
): Promise<string | undefined> {
  return showHawkNextEndpointPromptWindow({
    currentValue,
...
function buildHawkNextEndpointPromptHtml(currentValue: string): string {
  const value = escapeHtmlAttribute(currentValue);
  return `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
    <title>HawkNext Endpoint</title>
    <style>
  ...
  <body>
    <form id="form">
      <label for="endpoint">HawkNext endpoint origin</label>
    ...
    <script>
      ...
      const submit = (value) => { document.title = "hawknext-endpoint-submit:" + encodeURIComponent(value); };
      document.getElementById("form").addEventListener("submit", (event) => {
        event.preventDefault();
        submit(input.value);
      });
      document.getElementById("cancel").addEventListener("click", () => {
        document.title = "hawknext-endpoint-cancel";
      });
      input.focus();
      input.select();
    </script>
...
function showHawkNextEndpointPromptWindow(options: {
  currentValue: string;
  ...
  return new Promise((resolve) => {
    ...
    const promptWindow = new BrowserWindow({
      width: HAWKNEXT_ENDPOINT_PROMPT_WIDTH,
      height: HAWKNEXT_ENDPOINT_PROMPT_HEIGHT,
      parent: options.parentWindow,
      modal: Boolean(options.parentWindow),
      resizable: false,
      minimizable: false,
      maximizable: false,
      title: "HawkNext Endpoint",
      webPreferences: {
    ...
    promptWindow.on("page-title-updated", (event, title) => {
      if (title === "hawknext-endpoint-cancel") {
        event.preventDefault();
        finish(undefined);
        return;
      }
      if (!title.startsWith("hawknext-endpoint-submit:")) {
        return;
      }
      event.preventDefault();
      finish(decodeURIComponent(title.slice("hawknext-endpoint-submit:".length)));
    });
    ...
    void promptWindow.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(
        buildHawkNextEndpointPromptHtml(options.currentValue),
      )}`,
    );
  });
}

async function setHawkNextEndpointOverride(options: {
  value: string | undefined;
  settingService: { update(patch: { hawknextEndpointOrigin?: string | undefined }): Promise<void> };
  onHawkNextEndpointChanged: () => Promise<void> | void;
  logger: { warn: (...args: unknown[]) => void };
}) {
  if (HAWKNEXT_ENV === "production") {
    return;
  }
  const normalized = options.value ? normalizeHawkNextEndpointOrigin(options.value) : undefined;
  await options.settingService.update({ hawknextEndpointOrigin: normalized });
  await options.onHawkNextEndpointChanged();
}
...
function toggleHawkNextStdioTapDevProxy(options: {
  logger: { info: (...args: unknown[]) => void };
  updateHawkNextStdioTapDevMenuState: () => void;
}) {
  const current = readHawkNextStdioTapDevState();
  const next = setHawkNextStdioTapDevEnabled(!current.enabled);
  options.updateHawkNextStdioTapDevMenuState();
...
function resolveChangelogUrl(
  locale: Locale,
  endpointOrigin = DEFAULT_HAWKNEXT_ENDPOINT_ORIGIN,
): string {
  // 帮助菜单里的外链以前只有固定英文地址，切到中文界面后仍会落到英文 changelog。
  // 这里统一收口到主进程按当前应用语言分流，避免菜单模板里手写分支后续再出现多处不一致。
  const origin = buildHawkNextEndpointUrls(endpointOrigin).origin;
...
async function resolveCurrentHawkNextEndpointOrigin(settingService: {
  get(): Promise<{ hawknextEndpointOrigin?: string }>;
  ...
  return resolveHawkNextEndpointOrigin({
    ...
    overrideOrigin: settings.hawknextEndpointOrigin,
  });
}
...
export async function executeDesktopCommand(options: {
  ...
  settingService: {
    get(): Promise<Pick<AppSettings, "hawknextEndpointOrigin" | "desktopZoomLevel">>;
    update(
      patch: Partial<Pick<AppSettings, "hawknextEndpointOrigin" | "desktopZoomLevel">>,
    ): Promise<void>;
  };
  hawknextEndpointEnvBaseOrigin?: string | null;
  credentialsDir: string;
  ...
  switch (options.command) {
    ...
    case DesktopCommandIds.OpenChangelog:
      await openChangelog(
        ...
        await resolveCurrentHawkNextEndpointOrigin({
          ...options.settingService,
          envBaseOrigin: options.hawknextEndpointEnvBaseOrigin,
        }),
      );
      return;
    case DesktopCommandIds.CheckForUpdates:
      // 按产品身份而不是后端环境放行：生产后端的 Preview 同样没有更新器。
      if (HAWKNEXT_PRODUCT_FLAVOR === "production") {
    ...
    case DesktopCommandIds.OpenResourceManager:
      openResourceManager();
      return;
    case DesktopCommandIds.ToggleHawkNextStdioTapDevProxy:
      toggleHawkNextStdioTapDevProxy({
        logger: options.logger,
        updateHawkNextStdioTapDevMenuState: options.updateHawkNextStdioTapDevMenuState,
      });
      return;
    case DesktopCommandIds.SetHawkNextEndpointProduction:
      await setHawkNextEndpointOverride({
        value: DEFAULT_HAWKNEXT_ENDPOINT_ORIGIN,
        settingService: options.settingService,
        onHawkNextEndpointChanged: options.onHawkNextEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetHawkNextEndpointTest:
      await setHawkNextEndpointOverride({
        value: options.hawknextEndpointEnvBaseOrigin ?? resolveRuntimeHawkNextEndpointOrigin(),
        settingService: options.settingService,
        onHawkNextEndpointChanged: options.onHawkNextEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetHawkNextEndpointCustom: {
      const current =
        (await options.settingService.get()).hawknextEndpointOrigin ?? DEFAULT_HAWKNEXT_ENDPOINT_ORIGIN;
      const value = await promptCustomHawkNextEndpoint(targetWindow, current);
      if (!value) {
        return;
      }
      try {
    ...
    case DesktopCommandIds.ResetHawkNextEndpoint:
      await setHawkNextEndpointOverride({
        value: undefined,
        settingService: options.settingService,
        onHawkNextEndpointChanged: options.onHawkNextEndpointChanged,
