...
type PatchedIpcRenderer = IpcRenderer & { __hawknextArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __hawknextArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__hawknextArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__hawknextArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__hawknextArmsIpcPatched) {
    return;
  }
  ...
  patched.__hawknextArmsIpcPatched = true;
}
