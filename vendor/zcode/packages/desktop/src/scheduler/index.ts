...
function resolveScheduledAt(automation: HawkNextAutomation, now: number): number {
...
async function handleClaimed(automation: HawkNextAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: HawkNextAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: HawkNextOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: HawkNextAutomationTrigger =
