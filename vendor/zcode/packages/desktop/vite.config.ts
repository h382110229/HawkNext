...
import { resolveHawkNextEndpointOrigin, pickProductEndpointEnv } from "@hawknext/shared/hawknextEndpoint";
...
function resolveHawkNextEnv(value: string | undefined): "test" | "production" {
...
function createE2EUIRendererCoveragePlugin(repoRoot: string): Plugin {
  ...
  return {
    name: "hawknext:e2e-ui-source-coverage",
    enforce: "pre",
...
export default defineConfig(({ mode }) => {
  // `.env*` 只提供链接常量；当前产品环境由启动脚本或 CI 注入 HAWKNEXT_ENV。
  const env = { ...loadEnv(mode, "../..", ""), ...process.env };
  const repoRoot = resolve(__dirname, "../..");
  const hawknextEnv = resolveHawkNextEnv(env.HAWKNEXT_ENV);
  // 安装包身份与后端环境分轴；renderer 用它决定是否展示更新入口。
  const hawknextProductFlavor = resolveDesktopProductFlavor({
    ...process.env,
    ...env,
    HAWKNEXT_ENV: hawknextEnv,
  });
  const e2eCoverageEnabled =
    env.HAWKNEXT_E2E_COVERAGE === "1" || process.env.HAWKNEXT_E2E_COVERAGE === "1";
  const e2eStoreBridgeEnabled =
    env.VITE_HAWKNEXT_E2E_STORE_BRIDGE === "1" || process.env.VITE_HAWKNEXT_E2E_STORE_BRIDGE === "1";
  const hawknextEndpointOrigin = resolveHawkNextEndpointOrigin({
    env: hawknextEnv,
    envBaseOrigin: env.HAWKNEXT_BASE_URL ?? env.HAWKNEXT_ENDPOINT_ORIGIN,
  });
  ...
  return {
    ...
    define: {
      ...
      __HAWKNEXT_LOCAL_DEVELOPMENT_RUNTIME__: JSON.stringify(mode !== "production"),
      "import.meta.env.VITE_HAWKNEXT_BASE_URL": JSON.stringify(hawknextEndpointOrigin),
      // 兼容旧 renderer 读取名；新代码统一读 VITE_HAWKNEXT_BASE_URL。
      "import.meta.env.VITE_HAWKNEXT_ENDPOINT_ORIGIN": JSON.stringify(hawknextEndpointOrigin),
      ...
      // E2E store bridge 只能由 WDIO 专用变量打开，避免把 HAWKNEXT_ENV=test 产品环境误当成测试运行态。
      "import.meta.env.VITE_HAWKNEXT_E2E_STORE_BRIDGE": JSON.stringify(
