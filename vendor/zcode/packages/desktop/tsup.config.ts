import { pickProductEndpointEnv } from "@hawknext/shared/hawknextEndpoint";
...
function loadEnvFiles(): Record<string, string> {
  ...
  // 真实环境变量优先级最高
  if (process.env.HAWKNEXT_ENV) vars.HAWKNEXT_ENV = process.env.HAWKNEXT_ENV;
  if (process.env.HAWKNEXT_BASE_URL) vars.HAWKNEXT_BASE_URL = process.env.HAWKNEXT_BASE_URL;
  if (process.env.VITE_HAWKNEXT_BASE_URL) vars.VITE_HAWKNEXT_BASE_URL = process.env.VITE_HAWKNEXT_BASE_URL;
  // OAuth origin/client_id 由 host runtime 读取；这里保留覆盖入口，方便开发构建时观察统一 env 来源。
  if (process.env.CLOUD_INTL_OAUTH_CLIENT_ID) vars.CLOUD_INTL_OAUTH_CLIENT_ID = process.env.CLOUD_INTL_OAUTH_CLIENT_ID;
  if (process.env.CLOUD_INTL_OAUTH_ORIGIN) vars.CLOUD_INTL_OAUTH_ORIGIN = process.env.CLOUD_INTL_OAUTH_ORIGIN;
  if (process.env.CLOUD_INTL_BUSINESS_BASE_URL) {
    vars.CLOUD_INTL_BUSINESS_BASE_URL = process.env.CLOUD_INTL_BUSINESS_BASE_URL;
  }
  if (process.env.ZAI_BUSINESS_LOGIN_URL) {
    vars.ZAI_BUSINESS_LOGIN_URL = process.env.ZAI_BUSINESS_LOGIN_URL;
  }
  if (process.env.VITE_CLOUD_INTL_OAUTH_CLIENT_ID) {
    vars.VITE_CLOUD_INTL_OAUTH_CLIENT_ID = process.env.VITE_CLOUD_INTL_OAUTH_CLIENT_ID;
  }
  if (process.env.VITE_CLOUD_INTL_OAUTH_ORIGIN) {
    vars.VITE_CLOUD_INTL_OAUTH_ORIGIN = process.env.VITE_CLOUD_INTL_OAUTH_ORIGIN;
  }
  return {
    ...vars,
    ...Object.fromEntries(
...
const { environment: hawknextEnv } = await loadBuiltinProviderConfig();
// 安装包身份与后端环境分轴：HAWKNEXT_PREVIEW_IDENTITY=1 让生产后端的构建仍以 HawkNext Preview 身份打包运行。
const hawknextProductFlavor = resolveDesktopProductFlavor({ ...process.env, HAWKNEXT_ENV: hawknextEnv });
console.log(`[tsup] HAWKNEXT_ENV=${hawknextEnv} HAWKNEXT_PRODUCT_FLAVOR=${hawknextProductFlavor}`);

export function resolveDesktopTsupBundleSecurityOptions(
  runtimeEnv: Record<string, string | undefined> = process.env,
) {
  const isProduction = runtimeEnv.NODE_ENV === "production";
  const isE2ECoverageBuild = runtimeEnv.HAWKNEXT_E2E_COVERAGE === "1";
  return {
...
function createSharedDefines() {
  return {
    __HAWKNEXT_VERSION__: JSON.stringify(buildMetadata.appVersion),
    __HAWKNEXT_COMMIT__: JSON.stringify(buildMetadata.buildCommitId),
    __HAWKNEXT_BUILD_TIME__: JSON.stringify(buildMetadata.buildTime),
    __HAWKNEXT_ENV__: JSON.stringify(hawknextEnv),
    __HAWKNEXT_ENDPOINT_ENV__: JSON.stringify(pickProductEndpointEnv(env)),
    __HAWKNEXT_PRODUCT_FLAVOR__: JSON.stringify(hawknextProductFlavor),
    // Computer Use Helper build identity — helperInstaller 读它决定下载哪个 Helper bundle。
    // 缺失时 installer 抛 "Packaged HawkNext is missing its embedded Computer Use Helper build identity"。
    // CI 构建时通过 HAWKNEXT_CUA_HELPER_BUILD_ID env 注入；dev 为空串走兜底（dev helper 不走下载）。
    __HAWKNEXT_CUA_HELPER_BUILD_ID__: JSON.stringify(
      process.env.HAWKNEXT_CUA_HELPER_BUILD_ID?.trim() ?? "",
    ),
    // 客户端只有一个 CDN 配置，与发布端 OSS 目标列表分离。
    __HAWKNEXT_CDN_BASE_URL__: JSON.stringify(env.HAWKNEXT_CDN_BASE_URL?.trim() || ""),
  };
}
...
export default defineConfig([
  {
    ...
    entry: {
      ...
      "main/hawknextDataSizeWorker": "src/main/hawknextDataSizeWorker.ts",
    ...
    noExternal: [
      ...
      "@hawknext/provider",
      "@hawknext/provider-node",
      // services 已内联进 main，但其 producer import 曾被保留为裸包引用；
      // electron-builder 又会排除 node_modules/@hawknext，导致安装包启动即 ERR_MODULE_NOT_FOUND。
      // producer 的 JS broker 必须跟随 services 一起内联，原生 addon 仍只存在于独立 Helper。
      "@hawknext/hawknext-cua",
    ],
  ...
  },
  {
    ...
    platform: "node",
    target: "node22",
    // 与 host 同构：常驻 cron scheduler 进程复用 @hawknext/services（tasks-index + cron），
