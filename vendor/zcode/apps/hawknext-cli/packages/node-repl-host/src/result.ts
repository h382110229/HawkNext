import {
  HAWKNEXT_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY,
  HAWKNEXT_MCP_NODE_REPL_CUA_APP_META_KEY,
} from "@hawknext/contracts/mcp";
import { isOfficialCuaImageRefText } from "@hawknext/hawknext-cua/frame-contract";
import { CUA_APP_ASSOCIATIONS_META_KEY } from "@hawknext/hawknext-cua/host-display-contract";
import type { NodeReplRunResult } from "@hawknext/core/repl";
 ...
 * （hawknext-cua 的 frame-pixel-latest-only 测试即此契约），旧帧对模型没有使用价值。
...
export function toMcpRunResult(run: NodeReplRunResult): CallToolResult {
  ...
  delete responseMeta[HAWKNEXT_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
  ...
  delete responseMeta[HAWKNEXT_MCP_NODE_REPL_CUA_APP_META_KEY];
  if (run.cuaApp) responseMeta[HAWKNEXT_MCP_NODE_REPL_CUA_APP_META_KEY] = run.cuaApp;
  // Anthropic 兼容网关（如 cloud-cn MaaS）只解析 tool_result.content 开头的连续
      ...
      ? {
          _meta: {
            ...
            ...((run.images?.length ?? 0) > 0 || structuredContent.some((block) => block.type === "image")
              ? { "hawknext/nodeReplEmittedImage": true }
              : {}),
