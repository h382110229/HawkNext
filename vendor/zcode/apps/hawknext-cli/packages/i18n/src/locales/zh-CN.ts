...
export const zhCN: HawkNextCopy = {
  locale: "zh-CN",
  cli: {
    errors: {
...
不传 command 时，hawknext 会打开全屏 TUI。

命令:
  app-server 运行 HawkNext Protocol stdio app server
  commands   列出自定义 slash commands（\`commands list\`）
  doctor     检查运行时和打包假设
  login [zai|cloud-cn]  通过浏览器授权登录
  logout     删除共享的  登录凭据
...
`,
  ...
  tui: {
    ...
    terminal: {
      starting: "正在启动 HawkNext… Ctrl+C 退出",
