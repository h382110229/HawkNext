...
import type { CustomCommandRoot, CustomCommandSource } from "@hawknext/contracts";

const COMMANDS_DIR = "commands";
const GIT_MARKER = ".git";
const HOME_PREFIX = "~/";
const PRIORITY_STEP = 10;
const HAWKNEXT_DIR = ".hawknext";
...
export interface CustomCommandRootResolutionOptions {
  ...
  includeHawkNextCommands?: boolean;
}
...
export async function resolveDefaultCustomCommandRoots(
  ...
  const includeHawkNext = options.includeHawkNextCommands ?? true;
...
function commandRootsForBase(
  ...
  // 合并而不是 fallback：兼容 `.agents` 命令和原生 `.hawknext` 命令需要同时可见。
  // 同一级别 `.hawknext` 先扫描，命令同名时仍按“先到先赢”处理。
  return [
    root(join(baseDirectory, HAWKNEXT_DIR, COMMANDS_DIR), scope, "hawknext", nextPriority()),
