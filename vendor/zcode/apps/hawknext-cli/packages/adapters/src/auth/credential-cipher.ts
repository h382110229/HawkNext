...
const CREDENTIAL_SECRET_ENV_KEY = "HAWKNEXT_CREDENTIAL_SECRET";

export interface HawkNextCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface HawkNextCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createHawkNextCredentialCipher(
  options: HawkNextCredentialCipherOptions = {},
): HawkNextCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedHawkNextCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedHawkNextCredentialValue(value: string): boolean {
...
function resolveCredentialSecret(env: Record<string, string | undefined>): string {
  ...
  return `hawknext-credential-fallback:${platform()}:${homedir()}:${username}`;
}
