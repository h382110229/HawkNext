...
import { atomicWritePrivateTextFile, backupCorruptFile, withFileLock } from "@hawknext/shared/node";
import { createHawkNextCredentialCipher, type HawkNextCredentialCipher } from "./credential-cipher.js";

const HAWKNEXT_DATA_BASE_DIR_ENV_KEY = "HAWKNEXT_DATA_BASE_DIR";
const CLOUD_INTL_PROVIDER_ID = "zai";
const credentialChangeListeners = new Map<
  string,
  Set<() => void | Promise<void>>
>();

export const SHARED_HAWKNEXT_CREDENTIAL_KEYS = {
  activeProvider: "oauth:active_provider",
  cloudCnAccessToken: "oauth:cloud-cn:access_token",
  cloudCnRefreshToken: "oauth:cloud-cn:refresh_token",
  cloudCnUserInfo: "oauth:cloud-cn:user_info",
  cloudAccessToken: "oauth:cloud-intl:access_token",
  zaiRefreshToken: "oauth:cloud-intl:refresh_token",
  zaiUserInfo: "oauth:cloud-intl:user_info",
  hawknextJwtToken: "hawknext-jwt",
} as const;

export interface SharedHawkNextCredentialStoreOptions {
  baseDir?: string;
  cipher?: HawkNextCredentialCipher;
...
export interface SharedHawkNextCredentialStore {
...
export function createSharedHawkNextCredentialStore(
  options: SharedHawkNextCredentialStoreOptions = {},
): SharedHawkNextCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedHawkNextCredentialsPath(options);
  const cipher = options.cipher ?? createHawkNextCredentialCipher({ env });
  ...
  return {
    ...
    async clearZaiLoginCredentials(): Promise<void> {
      await mutateRawCredentialRecord(filePath, async (rawCredentials) => {
        const activeProviderRaw = rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.activeProvider];
        const activeProvider = activeProviderRaw ? cipher.decrypt(activeProviderRaw) : null;
        delete rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.cloudAccessToken];
        delete rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.zaiRefreshToken];
        delete rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.zaiUserInfo];
        delete rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.hawknextJwtToken];
        if (activeProvider === CLOUD_INTL_PROVIDER_ID) {
          delete rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.activeProvider];
        }
      });
    },
    ...
    async saveZaiLoginCredentials(payload: ZaiLoginCredentialPayload): Promise<void> {
      const encryptedCredentials = {
        activeProvider: cipher.encrypt(CLOUD_INTL_PROVIDER_ID),
      ...
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
        rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.activeProvider] =
          encryptedCredentials.activeProvider;
        rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.cloudAccessToken] =
          encryptedCredentials.accessToken;
        rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.hawknextJwtToken] = encryptedCredentials.jwtToken;
        rawCredentials[SHARED_HAWKNEXT_CREDENTIAL_KEYS.zaiUserInfo] = encryptedCredentials.userInfo;
      });
    },
  };
}

export function loadSharedHawkNextCredentialSync(
  key: string,
  options: SharedHawkNextCredentialStoreOptions = {},
): string | undefined {
  const filePath = resolveSharedHawkNextCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createHawkNextCredentialCipher({ env: options.env });
...
export function resolveSharedHawkNextCredentialsPath(
  options: SharedHawkNextCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[HAWKNEXT_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".hawknext", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared HawkNext credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared HawkNext credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}
