...
import { resolveCloudCNApiOrigin } from "@hawknext/shared";

const ZAI_API_HOST = "";
const JSON_CONTENT_TYPE = "application/json";
const HAWKNEXT_API_KEY_NAME = "hawknext-api-key";
const DEFAULT_ORG_NAME = "默认机构";
const DEFAULT_PROJECT_NAME = "默认项目";

export type CodingPlanFamily = "cloud-cn" | "zai";
...
async function resolveZaiBizToken(
  ...
  if (!token) {
    throw new CodingPlanApiKeyError(" biz token response is missing access_token.");
  }
  return token;
}
...
async function resolveBizApiKey(
  ...
  const keyEntry =
    keys.find((item) => item.name === HAWKNEXT_API_KEY_NAME) ??
    (await requestRemoteData<RemoteApiKeySummary>(
      input.httpClient,
      {
        body: new TextEncoder().encode(JSON.stringify({ name: HAWKNEXT_API_KEY_NAME })),
        headers: createBizAuthHeaders(input.authorization),
        method: "POST",
        trace: input.trace,
        url: listUrl,
