...
const DEFAULT_HAWKNEXT_OAUTH_BASE_URL = "/api/v1";
export type CliOAuthProviderId = "zai" | "cloud-cn";
...
export function createCliOAuthClient(options: CliOAuthClientOptions): CliOAuthClient {
  const baseUrl = normalizeBaseUrl(options.baseUrl ?? DEFAULT_HAWKNEXT_OAUTH_BASE_URL);
