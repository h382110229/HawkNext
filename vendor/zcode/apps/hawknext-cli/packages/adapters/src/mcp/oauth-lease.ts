...
import { acquireFileLock } from "@hawknext/shared/node";
import { HAWKNEXT_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@hawknext/shared";
import type { SharedHawkNextCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === HAWKNEXT_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;
