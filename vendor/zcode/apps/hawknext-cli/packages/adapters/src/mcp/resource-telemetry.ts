...
import { HAWKNEXT_MCP_RESOURCE_SAMPLE_INTERVAL_MS, type HawkNextMcpResourceSample } from "@hawknext/shared";
import {
  createProcessProbe,
  type ProcessProbe,
...
export interface McpResourceTelemetryOptions {
  arch: HawkNextMcpResourceSample["arch"];
  platform: HawkNextMcpResourceSample["platform"];
  now(): number;
  getProcesses(): McpResourceProcess[];
  onResourceSamples?(samples: HawkNextMcpResourceSample[]): void;
...
export function createMcpResourceTelemetry(options: McpResourceTelemetryOptions) {
  ...
  const sampleNow = async (): Promise<void> => {
    ...
    try {
      ...
      const groups = new Map<string, HawkNextMcpResourceSample>();
