...
import { sanitizeHawkNextRuntimeEnv } from "@hawknext/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeHawkNextRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.hawknext/server/node 启动，但登录环境 PATH 不含该目录，
