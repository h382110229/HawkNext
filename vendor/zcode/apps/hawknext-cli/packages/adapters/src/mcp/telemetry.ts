...
import type { HawkNextMcpTelemetryEvent } from "@hawknext/shared";
import {
...
type McpTelemetryEvent = HawkNextMcpTelemetryEvent;
...
interface CreateMcpTelemetryTrackerOptions {
  arch?: HawkNextMcpTelemetryEvent["arch"];
  idSalt: string;
  now?: () => number;
  onEvent(event: McpTelemetryEvent): void;
  platform?: HawkNextMcpTelemetryEvent["platform"];
  randomId?: () => string;
...
export function createMcpTelemetryTracker(
  ...
  const arch = options.arch ?? (process.arch as HawkNextMcpTelemetryEvent["arch"]);
  const now = options.now ?? Date.now;
  const platform = options.platform ?? (process.platform as HawkNextMcpTelemetryEvent["platform"]);
