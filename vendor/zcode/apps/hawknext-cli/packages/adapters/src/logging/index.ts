...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@hawknext/contracts";
import { LogLevel, LogLevelName } from "@hawknext/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.HAWKNEXT_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.HAWKNEXT_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".hawknext", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeHawkNextRuntimeEnv(env[HAWKNEXT_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `hawknext-${formatLocalLogDate(new Date())}.jsonl`;
}
