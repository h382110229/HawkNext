...
import { createOpenAI as createOfficialModel } from "@ai-sdk/official-model";
import { createOpenAICompatible as createOfficialModelCompatible } from "@ai-sdk/official-model-compatible";
import type { LanguageModel } from "ai";
import {
  compileModelOptionMaps,
  type CompiledModelOptionMaps,
  type ModelOptionValues,
} from "@hawknext/model-option-map";
import {
  type Logger,
  type ModelId,
...
import type { RegistryProviderConfig } from "@hawknext/provider";
import { withOpenRouterAttributionHeaders } from "@hawknext/shared";
import { createAnthropicCompatFetch } from "./anthropic-stream-compat.js";
import { createOfficialModelResponsesJsonCompatFetch } from "./official-model-responses-json-compat.js";
...
export type AiSdkProviderKind = "official-model" | "anthropic" | "official-model-compatible";
...
export class AiSdkModelExecution {
  ...
  private createFactory(
    ...
    switch (providerConfig.kind) {
      case "official-model": {
        const provider = createOfficialModel({
          apiKey,
          baseURL: providerConfig.baseURL,
          fetch: createOfficialModelResponsesJsonCompatFetch(optionFetch),
          headers,
        });
      ...
      case "official-model-compatible": {
        const provider = createOfficialModelCompatible<string, string, string, string>({
          name: providerConfig.name ?? providerId,
          baseURL: providerConfig.baseURL,
          apiKey,
          fetch: optionFetch,
          headers,
          // OfficialModel Compatible 流式 usage 需要显式请求，Usage 是执行结果的一部分。
          includeUsage: true,
...
function toAiSdkProviderConfig(
  ...
  switch (config.api.type) {
    ...
    case "official-model-responses":
      return { kind: "official-model", ...common };
    case "official-model-chat-completions":
      return { kind: "official-model-compatible", name: providerId, ...common };
  }
...
export function createProviderBusinessErrorFetch(
  ...
  return async (input, init) => {
    ...
    // hawknext-plan 安全校验拒绝（3007）等场景返回 HTTP 403 + JSON，但未必带
...
/**
 * 模型请求出口：官方 Coding Plan 端点经 HawkNext 平台网关发送（做套餐权益校验等平台侧处理），
...
/** 从 HTTP JSON body 解析 hawknext-plan 等业务错误（供 failure-classifier 在 APICallError 路径复用）。 */
export function readProviderBusinessFailureFromBody(body: unknown):
  ...
  const providerCode =
    ...
    // 二次包装后的外层 code 是 HawkNext 自己的 PROVIDER_BUSINESS_ERROR，
...
function emitProviderBusinessSseFrame(
  ...
  if (failure) {
    // 部分 OfficialModel-compatible provider 只在 HTTP 200 SSE error frame 中报告业务错误；
