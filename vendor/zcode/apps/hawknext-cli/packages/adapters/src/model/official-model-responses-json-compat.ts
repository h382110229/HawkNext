...
export function createOfficialModelResponsesJsonCompatFetch(baseFetch: ProviderFetch): ProviderFetch {
  return async (input, init) => {
    ...
    const normalized = normalizeOfficialModelResponsesJson(body);
    if (!normalized) {
      return response;
    }
...
function normalizeOfficialModelResponsesJson(value: unknown): Record<string, unknown> | undefined {
