import { resolveRuntimeHawkNextEndpointOrigin } from "@hawknext/shared";
import type { EnvRecord } from "./model-execution.js";

/**
 * 官方 Coding Plan 的模型请求经 HawkNext 平台网关发送。
 *
 *  / CloudCN Coding Plan 是 HawkNext 的官方订阅套餐，模型请求统一发往 HawkNext 平台网关，
...
const GATEWAY_PATH_BY_PROVIDER_ENDPOINT: ReadonlyMap<string, string> = new Map(
  OFFICIAL_CODING_PLAN_GATEWAY_ROUTES.map((route) => [
    endpointKey(safeParseUrl(route.providerEndpoint) ?? new URL("https://invalid.local/")),
    route.gatewayPath,
  ]),
);
...
export function resolveOfficialCodingPlanGatewayUrl(
  ...
  const gatewayUrl = new URL(gatewayPath, resolveRuntimeHawkNextEndpointOrigin(env));
