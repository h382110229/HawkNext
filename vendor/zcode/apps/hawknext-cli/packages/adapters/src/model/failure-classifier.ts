...
function classifyProviderBusinessFailureFromApiCallBody(
  ...
  return classifyProviderBusinessFailure(
    new ProviderBusinessError({
      ...
      providerId: "unknown",
      providerKind: "official-model-compatible",
...
function resolveProviderBusinessCode(error: ProviderBusinessError): string | undefined {
  ...
  // 真实 CloudCN 码（如 1234/1261）只保留在 responseBodySummary 的深层结构里。
...
function firstBracketedProviderCode(
  ...
  for (const record of records) {
    const code =
      readCloudCNBracketedProviderCode(record.message) ??
      readCloudCNBracketedProviderCode(record.providerMessage);
    if (code) {
      return code;
    }
  }
...
function readCloudCNBracketedProviderCode(value: unknown): string | undefined {
  if (typeof value !== "string") {
    return undefined;
  }
