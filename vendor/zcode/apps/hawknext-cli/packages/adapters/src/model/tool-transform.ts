...
import { ModelErrorCode, type JsonSchema, type ModelToolContract } from "@hawknext/contracts";
...
function hoistLocalJsonSchemaRefsToDefs(toolName: string, schema: JsonSchema): JsonSchema {
  ...
  for (const ref of refs) {
    let defKey = `hawknext_ref_${nextDefIndex}`;
    while (usedDefKeys.has(defKey)) {
      nextDefIndex += 1;
      defKey = `hawknext_ref_${nextDefIndex}`;
    }
    nextDefIndex += 1;
...
function providerOptionsForClientTool(options: AiSdkToolTransformOptions): {
  ...
  // Anthropic-compatible gateways reject that extra tool field, so HawkNext opts out
