...
import { containsOfficialCuaImageRefCredentialText } from "@hawknext/hawknext-cua/frame-contract";
...
const CHAT_STYLE_TOOL_RESULT_PROVIDER_KINDS = new Set(["official-model-compatible", "gateway"]);
...
export function shouldTextifyStructuredToolResults(
  ...
  if (options.apiFormat !== undefined) {
    return options.apiFormat === "official-model-chat-completions";
  }
  // 根因：official-model kind 使用 Responses API；按宽泛的 OfficialModel-like 家族判断会把它
 ...
 * provider 投影把 media 延后为独立 user 消息时（official-model-like textify），配对
