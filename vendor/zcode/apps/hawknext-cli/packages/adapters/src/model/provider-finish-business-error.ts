...
const CLOUD_CN_BRACKETED_BUSINESS_CODE_PATTERN = /^\[(\d{4})\](?=\[)/;
...
function normalizeProviderKind(value: AiSdkProviderKind | string | undefined): AiSdkProviderKind {
  return value === "official-model" || value === "anthropic" || value === "official-model-compatible"
    ? value
    : "official-model-compatible";
}
...
function collectCandidateRecords(record: Record<string, unknown>): Record<string, unknown>[] {
  ...
  while (queue.length > 0) {
    ...
    // hawknext-plan 等业务错误可能只出现在 AI SDK finish chunk 的深层 JSON（如 response.body），
...
function readProviderCode(record: Record<string, unknown>): string | undefined {
  ...
  const value =
    ...
    // CloudCN/ 的 SSE error chunk 有时只有 `[1302][...][request_id]` message，
    // 没有结构化 code；只解析这个强格式前缀，避免把普通 rate limit 文案误判成业务码。
    readCloudCNBracketedBusinessCode(record.message) ??
    readCloudCNBracketedBusinessCode(record.providerMessage) ??
    readCloudCNBracketedBusinessCode(errorRecord?.message) ??
    readCloudCNBracketedBusinessCode(errorRecord?.providerMessage) ??
    readCloudCNBracketedBusinessCode(contextRecord?.message) ??
    readCloudCNBracketedBusinessCode(contextRecord?.providerMessage);
  return value;
}
...
function readCloudCNBracketedBusinessCode(value: unknown): string | undefined {
  const message = normalizeStringish(value);
  const match = message?.match(CLOUD_CN_BRACKETED_BUSINESS_CODE_PATTERN);
  return match?.[1];
}
