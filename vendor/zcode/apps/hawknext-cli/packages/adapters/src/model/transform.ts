...
export interface AiSdkMessageTransformOptions {
  ...
  providerKind?: "official-model" | "anthropic" | "official-model-compatible" | "gateway" | "custom";
  stripMedia?: boolean;
...
export function toAiSdkMessages(
  ...
  const normalizedMessages =
    options.providerKind === "official-model-compatible"
...
function shouldStripStoredReasoningForOpenAiResponsesStatelessReplay(
  ...
  if (options.providerKind !== "official-model") return false;
  if (resolveApiFormat(options) !== "official-model-responses") return false;
  const officialModelOptions = objectRecord(options.providerOptions?.officialModel);
  if (typeof officialModelOptions.previousResponseId === "string") return false;
  if (typeof officialModelOptions.conversation === "string") return false;
  if (officialModelOptions.store === false) return false;
  return true;
}
...
function projectToolNameForProvider(
  ...
  // OfficialModel-compatible wire 不可靠接受空 function name，但历史中的原始
...
function hasOpenAiStoredReasoningItemId(providerOptions: unknown): boolean {
  const officialModelOptions = objectRecord(objectRecord(providerOptions).officialModel);
  return typeof officialModelOptions.itemId === "string" && officialModelOptions.itemId.length > 0;
}
...
function contentBlockToAiSdkUserParts(
  ...
  switch (block.type) {
    ...
    case "video": {
      ...
      // patch 后的 @ai-sdk/official-model-compatible / @ai-sdk/anthropic 转成 video_url / video block。
