...
import type { SkillRoot, SkillSource } from "@hawknext/contracts";
...
export async function resolveDefaultSkillRoots(
  ...
  const includeHawkNext = options.includeHawkNextSkills ?? true;
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.hawknext` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.hawknext` 优先，后续同名按 root 顺序解析。
  return [
    root(join(baseDirectory, HAWKNEXT_DIR, SKILLS_DIR), scope, "hawknext", nextPriority()),
