/* eslint-disable max-lines -- hawknext-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
...
export const LEGACY_CUA_PLUGIN_ID = "hawknext-cua@hawknext-plugins-official";
export const CANONICAL_CUA_PLUGIN_ID = "computer-use@hawknext-plugins-official";
...
export const HawkNextConfigFileSchema = z
  .object({
...
export type HawkNextConfigFile = z.infer<typeof HawkNextConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. HawkNext does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；HawkNext 运行态协议类型是 http。
    server.type = "http";
  ...
  if (
    ...
    // 兼容历史 CloudCN MCP 配置：旧字段名是 http_headers，agent runtime 只消费 headers。
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = HawkNextConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: HawkNextConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: HawkNextConfigFile["plugins"],
...
function parseSkillsRuntimeConfig(
  skills: HawkNextConfigFile["skills"],
