...
import type { McpServerConfig, RuntimeConfigPatch } from "@hawknext/contracts";
import {
    ...
    ...(hooks
      ? {
          hookCandidate: createWorkspaceHookSourceInput({
            ...
            // hooks 字段已由 loadFileConfig 经 HawkNextConfigFileSchema（shared 单源
...
function getProjectConfigBaseDir(path: string): string {
  ...
  return basename(configDirectory) === ".hawknext" ? dirname(configDirectory) : configDirectory;
}
