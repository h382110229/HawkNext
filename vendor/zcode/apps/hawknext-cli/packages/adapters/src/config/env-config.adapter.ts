// Env Config Adapter - Parse the intentionally small HAWKNEXT_* environment surface.
...
const DEFAULT_PREFIX = "HAWKNEXT_";

/**
 * Parse HAWKNEXT_* environment variables into config
 */
...
export function getToolConcurrencyConfig(): { maxConcurrency: number } {
  return {
    maxConcurrency: normalizeNumber(process.env.HAWKNEXT_MAX_TOOL_CONCURRENCY ?? "10"),
  };
}
