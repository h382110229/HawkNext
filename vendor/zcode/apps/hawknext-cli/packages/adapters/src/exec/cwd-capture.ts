...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `hawknext-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__hawknext_status=$?",
    `if [ "$__hawknext_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__hawknext_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__hawknext_status=%ERRORLEVEL%"',
    `if "%__hawknext_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__hawknext_status%",
  ].join("\r\n");
}
