...
import { windowsPathToGitBashPath } from "@hawknext/contracts";
import type { ExecutionShellDialect } from "@hawknext/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_HAWKNEXT_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_HAWKNEXT_SHELL_INIT_SNAPSHOT",
  ];
}
