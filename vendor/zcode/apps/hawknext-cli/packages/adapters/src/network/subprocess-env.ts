import {
  ...
  HAWKNEXT_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readHawkNextToolEnvPassthroughEnv,
...
const EXPLICIT_CA_SOURCE_KEYS = [HAWKNEXT_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [HAWKNEXT_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, HAWKNEXT_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readHawkNextToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const hawknextProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [HAWKNEXT_HTTP_PROXY_ENV_KEY], platform),
  );
  if (hawknextProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, hawknextProxy, platform);
    }
    return;
  }
}
