...
function createShellExpansionEnv(input: {
  ...
  set.HAWKNEXT_PROJECT_DIR = input.workingDirectory;
  if (input.sessionId) {
    set.CLAUDE_CODE_SESSION_ID = input.sessionId;
    set.CLAUDE_SESSION_ID = input.sessionId;
    set.HAWKNEXT_SESSION_ID = input.sessionId;
  }
  if (input.plugin) {
    set.CLAUDE_PLUGIN_DATA = input.plugin.dataPath;
    set.CLAUDE_PLUGIN_ROOT = input.plugin.rootPath;
    set.HAWKNEXT_PLUGIN_DATA = input.plugin.dataPath;
    set.HAWKNEXT_PLUGIN_ID = input.plugin.id;
    set.HAWKNEXT_PLUGIN_NAME = input.plugin.name;
    set.HAWKNEXT_PLUGIN_ROOT = input.plugin.rootPath;
  }
...
function assertShellExpansionContextAvailable(input: {
  ...
  for (const match of input.shellCommand.matchAll(SHELL_CONTEXT_VARIABLE_PATTERN)) {
    const name = match[1];
    if (!name) continue;
    if (name === "CLAUDE_SKILL_DIR" || name === "HAWKNEXT_SKILL_DIR") {
      throw new Error(
    ...
    if (
      ...
      (name === "CLAUDE_PLUGIN_DATA" ||
        ...
        name === "HAWKNEXT_PLUGIN_DATA" ||
        name === "HAWKNEXT_PLUGIN_ROOT")
    ) {
      throw new Error(
