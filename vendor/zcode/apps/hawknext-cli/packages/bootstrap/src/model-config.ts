...
export function createRuntimeAiSdkModelExecutionConfig(
  ...
  return {
    defaultHeaders: buildCliHawkNextSourceHeaders(env, options),
    env,
    ...(network ? { network } : {}),
  };
}
...
function buildCliHawkNextSourceHeaders(
  env: EnvRecord,
...
function resolveAppVersionForHeaders(
  ...
  return normalizePrintableHeaderValue(env[HAWKNEXT_APP_VERSION_ENV] ?? options.appVersion);
}
