...
import { createNodeCustomCommandAdapter } from "@hawknext/adapters/commands";
import type {
...
export interface ListHawkNextCustomCommandsOptions {
...
export interface InspectHawkNextCustomCommandOptions extends ListHawkNextCustomCommandsOptions {
  name: string;
}

export interface HawkNextCustomCommandInspection {
  command: CustomCommandContent;
  diagnostics: CustomCommandDiagnostic[];
}

export async function listHawkNextCustomCommands(
...
export async function inspectHawkNextCustomCommand(
  options: InspectHawkNextCustomCommandOptions,
): Promise<HawkNextCustomCommandInspection> {
  const discovery = createCustomCommandDiscovery(options);
...
export async function loadHawkNextCustomCommand(
  options: InspectHawkNextCustomCommandOptions,
...
function createCustomCommandDiscovery(options: ListHawkNextCustomCommandsOptions) {
