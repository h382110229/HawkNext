...
} from "@hawknext/adapters/plugins";
import type {
  Logger,
  PluginHookDetail,
  PluginLoadOutcome,
...
import { HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@hawknext/contracts";
import { HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID, isHawkNextCuaInternalFeatureEnabled } from "@hawknext/shared";
...
export interface ResolveHawkNextPluginsOptions {
...
export interface ListHawkNextPluginsOptions extends ResolveHawkNextPluginsOptions {}

export interface SetHawkNextPluginEnabledOptions extends ResolveHawkNextPluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetHawkNextPluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface HawkNextMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface HawkNextAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface HawkNextInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface HawkNextPluginsOverviewData {
  marketplaces: HawkNextMarketplaceSummaryData[];
  availablePlugins: HawkNextAvailablePluginData[];
  installedPlugins: HawkNextInstalledPluginData[];
  restorableBuiltins: HawkNextAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface HawkNextMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: HawkNextMarketplaceSummaryData[];
}

export interface AddHawkNextMarketplaceOptions extends ResolveHawkNextPluginsOptions {
...
export interface RemoveHawkNextMarketplaceOptions extends ResolveHawkNextPluginsOptions {
  marketplace: string;
}

export interface UpdateHawkNextMarketplaceOptions extends ResolveHawkNextPluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallHawkNextMarketplacePluginOptions extends ResolveHawkNextPluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallHawkNextMarketplacePluginOptions extends ResolveHawkNextPluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`hawknext plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateHawkNextMarketplacePluginOptions extends ResolveHawkNextPluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateHawkNextPluginPathOptions extends ResolveHawkNextPluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface HawkNextPluginUpdateData extends HawkNextPluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveHawkNextPluginsOptions {
  pluginId: string;
}

interface ConfigureHawkNextPluginOptions extends ResolveHawkNextPluginsOptions {
...
interface ResetHawkNextPluginConfigOptions extends ResolveHawkNextPluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateHawkNextPluginOptions extends ResolveHawkNextPluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeHawkNextPluginOptions extends ResolveHawkNextPluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface HawkNextPluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveHawkNextPlugins(options: ResolveHawkNextPluginsOptions = {}): PluginLoadOutcome {
...
export function getHawkNextPluginsOverview(
  options: ResolveHawkNextPluginsOptions = {},
): HawkNextPluginsOverviewData {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  const outcome = resolveHawkNextPlugins({
    ...options,
    configResult,
    pluginStorageRoot,
  });
  ...
  // hawknext-plugins-official 的内置与 CDN 分片已在 adapter 层合并为唯一 canonical manifest。
  const catalogs: Array<{
    summary: HawkNextMarketplaceSummaryData;
  ...
  const restorableBuiltins: HawkNextAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isHawkNextCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
      marketplace: HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE,
      version: def.version,
      installed: false,
      ...(listing ? { listing } : {}),
    };
...
export function listHawkNextPlugins(options: ListHawkNextPluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveHawkNextPlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setHawkNextPluginEnabled(
  options: SetHawkNextPluginEnabledOptions,
): Promise<SetHawkNextPluginEnabledResult> {
...
export async function addHawkNextPluginMarketplace(
  options: AddHawkNextMarketplaceOptions,
): Promise<HawkNextMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeHawkNextPluginMarketplace(
  options: RemoveHawkNextMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateHawkNextPluginMarketplace(
  options: UpdateHawkNextMarketplaceOptions,
): Promise<HawkNextMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installHawkNextMarketplacePlugin(
  options: InstallHawkNextMarketplacePluginOptions,
): Promise<HawkNextPluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallHawkNextMarketplacePlugin(
  options: UninstallHawkNextMarketplacePluginOptions,
): Promise<HawkNextInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const pluginId = resolvePluginIdForMutation(options);

    // 官方 CDN marketplace 与内置插件共享 hawknext-plugins-official id 空间，且其缓存
...
/**
 * `hawknext plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateHawkNextMarketplacePlugin(
  options: UpdateHawkNextMarketplacePluginOptions,
): Promise<HawkNextPluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateHawkNextPluginMarketplace({
    ...options,
  ...
  const installed = await installHawkNextMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `hawknext plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateHawkNextPluginPath(
  options: ValidateHawkNextPluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const hawknextCuaPluginId = HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === hawknextCuaPluginId &&
    !isHawkNextCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires HAWKNEXT_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureHawkNextPlugin(options: ConfigureHawkNextPluginOptions): Promise<void> {
...
export async function resetHawkNextPluginConfig(
  options: ResetHawkNextPluginConfigOptions,
...
export async function validateHawkNextPlugin(
  options: ValidateHawkNextPluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeHawkNextPlugin(
  options: DescribeHawkNextPluginOptions,
...
function resolvePluginContext(options: ResolveHawkNextPluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallHawkNextMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveHawkNextPluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.hawknext/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".hawknext", "config.json");
