...
import { openStartupSqliteSessionStore } from "@hawknext/adapters/storage";
import type { SessionInfo } from "@hawknext/contracts";
import type { ListHawkNextSessionsOptions, ResolveLatestSessionOptions } from "./app/types.js";
...
export async function listHawkNextSessions(
  options: ListHawkNextSessionsOptions = {},
): Promise<SessionInfo[]> {
  const ownsStore = options.sessionStore === undefined;
  let sessionStore = options.sessionStore;
  if (!sessionStore) {
