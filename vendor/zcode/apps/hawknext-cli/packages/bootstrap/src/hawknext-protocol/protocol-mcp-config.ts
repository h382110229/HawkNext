import type { McpServerConfig } from "@hawknext/contracts";
import type { HawkNextProtocolMcpServer } from "@hawknext/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: HawkNextProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // HawkNext Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
