...
import {
  ...
  hawknextAutomationCheckTaskBindingResultSchema,
  hawknextAutomationCreateResultSchema,
  hawknextAutomationDeleteResultSchema,
  hawknextAutomationListResultSchema,
  hawknextAutomationUpdateResultSchema,
  hawknextProtocolMethods,
  type HawkNextAutomationProtocol,
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => HawkNextProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              hawknextProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              hawknextProtocolMethods.automationList,
              {},
              hawknextAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          hawknextProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        hawknextProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        hawknextProtocolMethods.automationDelete,
        { automationId: input.id },
        hawknextAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: HawkNextAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: HawkNextAutomationProtocol): CronAutomation {
  return {
