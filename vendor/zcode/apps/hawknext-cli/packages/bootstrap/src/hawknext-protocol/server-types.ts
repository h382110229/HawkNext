...
import type { McpTelemetryTracker } from "@hawknext/adapters";
import type { WorkspaceHookPolicyProvider } from "@hawknext/core";
import type { AccountProviderConfigSnapshot } from "@hawknext/provider";
import {
  hawknextProtocolErrorCodes,
  type HawkNextDeliveryKind,
  type ModelSelection,
  type HawkNextModelContextBudgetStrategy,
  type HawkNextProtocolMessage,
  type HawkNextProtocolMethod,
  type HawkNextProtocolNotification,
  type HawkNextProtocolRequest,
  type HawkNextProtocolRequestId,
...
import type { HawkNextApp, HawkNextAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../hawknext-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../hawknext-protocol-v4/v4-gateway.js";
...
export interface HawkNextProtocolAgentDependencies {
  createHawkNextApp(options?: Omit<HawkNextAppOptions, "providerRegistry">): HawkNextApp | Promise<HawkNextApp>;
  /**
...
export type HawkNextProtocolAgentResolvedDependencies = HawkNextProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface HawkNextProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface HawkNextProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface HawkNextProtocolSessionRecord {
  app: HawkNextApp;
  ...
  protocolEventSequences: Map<string, HawkNextProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, HawkNextProtocolToolInputTransmissionState>;
...
export interface HawkNextProtocolClientRequestOptions {
  signal?: AbortSignal;
  timeoutMs?: number;
...
export interface HawkNextProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: HawkNextProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: HawkNextProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: HawkNextProtocolMessage): message is HawkNextProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: HawkNextProtocolMessage,
): message is HawkNextProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: HawkNextProtocolMessage,
): message is { id: HawkNextProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: HawkNextProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): HawkNextProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: HawkNextDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("HawkNext Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "hawknext_protocol.session.require_missing",
...
export function createProtocolLogger(deps: HawkNextProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("hawknext").child({
    module: "bootstrap.hawknext_protocol",
  });
}
