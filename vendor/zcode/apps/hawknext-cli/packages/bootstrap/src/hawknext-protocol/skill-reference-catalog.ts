...
import {
  hawknextSkillsReferenceCatalogParamsSchema,
  type HawkNextSkillReferenceCatalogEntry,
  type HawkNextSkillsReferenceCatalogResult,
} from "@hawknext/shared";
import type { SkillLoadOutcome, SkillMetadata } from "@hawknext/contracts";
import { listHawkNextSkills } from "../skills.js";
import {
  parseParams,
  requireSession,
...
export async function getSkillReferenceCatalog(
  ...
  const params = parseParams(hawknextSkillsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
  ...
  const outcome = await listHawkNextSkills({
...
function toResult(
  authority: HawkNextSkillsReferenceCatalogResult["authority"],
...
function toReferenceCatalogEntry(skill: SkillMetadata): HawkNextSkillReferenceCatalogEntry {
  const scope =
