...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@hawknext/contracts";
import {
  hawknextMcpListParamsSchema,
  hawknextMcpListResultSchema,
  type HawkNextMcpListResult,
...
export async function listMcpServers(
  ...
  rawParams: unknown,
): Promise<HawkNextMcpListResult> {
  const params = parseParams(hawknextMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.hawknext_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.hawknext` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return hawknextMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return hawknextMcpListResultSchema.parse({
