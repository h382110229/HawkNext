...
import {
  hawknextPluginsCancelOperationParamsSchema,
  hawknextProtocolMethods,
  hawknextWorkspaceCancelGenerateTextParamsSchema,
  hawknextWorkspaceHookTrustGrantParamsSchema,
} from "@hawknext/shared";
import type { BrowserControlPort } from "@hawknext/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@hawknext/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  HawkNextProtocolError,
  HawkNextProtocolMessage,
  HawkNextProtocolMethod,
  HawkNextProtocolNotification,
  HawkNextProtocolRequest,
  HawkNextProtocolRequestId,
  HawkNextProtocolResponse,
} from "@hawknext/shared";
import {
...
} from "../hawknext-protocol-v4/interaction-registry.js";
...
import {
  ...
  parseParams,
  toProtocolError,
  type HawkNextProtocolClientRequestOptions,
  type HawkNextProtocolAgentDependencies,
  type HawkNextProtocolAgentServerContext,
  type HawkNextProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@hawknext/contracts";

export type { HawkNextProtocolAgentDependencies, HawkNextProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type HawkNextProtocolOutboundMessage = HawkNextProtocolNotification | HawkNextProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, HawkNextProtocolSessionRecord>;
}): Promise<void> {
...
interface HawkNextProtocolPostResponseBatch {
  readonly messages: readonly HawkNextProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class HawkNextProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<HawkNextProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: HawkNextProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: HawkNextProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    HawkNextProtocolRequestId,
    HawkNextProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: HawkNextProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createHawkNextApp);
    const resolvedDeps = {
      ...deps,
      createHawkNextApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, HawkNextProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: HawkNextProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("HawkNext Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "hawknext_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: HawkNextProtocolRequestId): HawkNextProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: HawkNextProtocolRequestId): HawkNextProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: HawkNextProtocolMessage,
  ): Promise<HawkNextProtocolError | HawkNextProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("HawkNext Protocol notification ignored", {
        event: "hawknext_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: HawkNextProtocolRequest,
  ): Promise<HawkNextProtocolError | HawkNextProtocolResponse> {
  ...
  private async dispatchRequest(request: HawkNextProtocolRequest) {
      ...
      case hawknextProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: hawknextWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? hawknextWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(hawknextPluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(hawknextWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: HawkNextProtocolRequestId, result: unknown): HawkNextProtocolResponse {
    return { id, result };
  }

  private fail(
    id: HawkNextProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): HawkNextProtocolError {
  ...
  private requestClient<T>(
    method: HawkNextProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: HawkNextProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No HawkNext Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: HawkNextProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: HawkNextProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
