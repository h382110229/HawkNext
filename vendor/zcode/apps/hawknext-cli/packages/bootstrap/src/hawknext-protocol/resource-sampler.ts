...
import {
  createHawkNextProcessResourceSampler,
  type HawkNextProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: HawkNextProtocolAgentServer,
  send: (message: HawkNextProtocolNotification) => void,
  logger: Logger,
): HawkNextProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createHawkNextProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: hawknextProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "hawknext_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
