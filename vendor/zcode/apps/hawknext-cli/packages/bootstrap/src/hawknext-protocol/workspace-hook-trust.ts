...
import { workspaceHookPolicySchema } from "@hawknext/contracts";
import type { WorkspaceHookPolicyProvider } from "@hawknext/core";
import {
  hawknextWorkspaceHookTrustGrantParamsSchema,
  hawknextWorkspaceHookTrustGrantReasonCodeSchema,
  hawknextWorkspaceHookTrustGrantResultSchema,
  type HawkNextWorkspaceHookTrustGrantReasonCode,
  type HawkNextWorkspaceHookTrustGrantResult,
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = hawknextWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return hawknextWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): HawkNextWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): HawkNextWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = hawknextWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
