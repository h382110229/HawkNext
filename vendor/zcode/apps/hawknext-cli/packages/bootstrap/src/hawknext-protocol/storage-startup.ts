...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@hawknext/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  hawknextProtocolNotifications,
  hawknextStorageStartupStateSchema,
  type HawkNextStorageStartupState,
} from "@hawknext/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: HawkNextStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = hawknextStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: hawknextProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { hawknextStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@hawknext/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = hawknextStoragePathReadySchema.parse(JSON.parse(line));
