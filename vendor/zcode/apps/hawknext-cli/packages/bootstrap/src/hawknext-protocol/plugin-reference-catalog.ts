...
import {
  HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE_ID,
  hawknextProtocolNotifications,
  hawknextPluginsReferenceCatalogParamsSchema,
  hawknextPluginsResolveSuggestedReferenceParamsSchema,
  type HawkNextPluginReferenceCatalogEntry,
  type HawkNextPluginDiagnostic as SharedPluginDiagnostic,
  type HawkNextPluginsReferenceCatalogResult,
  type HawkNextPluginsResolveSuggestedReferenceResult,
} from "@hawknext/shared";
import type { PluginReferenceCatalogEntry } from "@hawknext/contracts";
import { buildPluginReferenceCatalog } from "@hawknext/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(hawknextPluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<HawkNextPluginsResolveSuggestedReferenceResult> {
  const params = parseParams(hawknextPluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  if (
    ...
    return unavailable(
      ...
      "推荐插件不是受信任的官方 hawknext-plugins-official 来源",
    );
  }

  const workingDirectory = params.workspace.workspacePath;
  const readState = () => {
    const outcome = resolveHawkNextPlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: hawknextProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateHawkNextPluginMarketplace({
    ...
    const refreshTimeoutRequest = new Promise<never>((_, reject) => {
      ...
      refreshTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("刷新 hawknext-plugins-official 超时（10000 ms）");
  ...
  const overview = getHawkNextPluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getHawkNextPluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): HawkNextPluginReferenceCatalogEntry {
