import type { HawkNextMessagePart, HawkNextMessageWithParts, HawkNextToolState } from "@hawknext/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@hawknext/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): HawkNextMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): HawkNextMessagePart {
  const base = {
...
function mapToolState(state: ToolState): HawkNextToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // HawkNext protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
