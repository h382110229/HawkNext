...
import { hawknextProtocolMessageSchema, hawknextProtocolMethods } from "@hawknext/shared";
import type { Logger } from "@hawknext/contracts";

type HawkNextProtocolOutgoingMessage =
  | HawkNextProtocolError
  | HawkNextProtocolNotification
  | HawkNextProtocolRequest
  | HawkNextProtocolResponse;

type HawkNextProtocolMessageHandler = (
  message: HawkNextProtocolMessage,
) => Promise<HawkNextProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface HawkNextProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: HawkNextProtocolMessageHandler;
  ...
  ) => readonly HawkNextProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: HawkNextProtocolRequestId) => {
    readonly messages: readonly HawkNextProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class HawkNextProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: HawkNextProtocolNdjsonConnectionOptions) {
  ...
  send(message: HawkNextProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("HawkNext Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("HawkNext Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): HawkNextProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("HawkNext Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "hawknext_protocol.parse.failed",
    ...
    const parsed = hawknextProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid HawkNext Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: HawkNextProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === hawknextProtocolMethods.sessionStop ||
        message.method === hawknextProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: HawkNextProtocolMessage): Promise<void> {
    if (this.terminal) return;
