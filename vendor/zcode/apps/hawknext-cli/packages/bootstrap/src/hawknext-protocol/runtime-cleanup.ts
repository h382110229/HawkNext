import type { Logger, McpPort } from "@hawknext/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@hawknext/adapters/mcp";
import type { SqliteSessionStore } from "@hawknext/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@hawknext/telemetry";
...
import type { HawkNextProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<HawkNextProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<HawkNextProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`HawkNext Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `hawknext_protocol.${resource}.shutdown.failed`,
