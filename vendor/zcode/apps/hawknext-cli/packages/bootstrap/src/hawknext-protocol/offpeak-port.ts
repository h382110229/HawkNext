import type { OffPeakPort, OffPeakTaskSummary } from "@hawknext/contracts";
import {
  hawknextOffPeakCreateResultSchema,
  hawknextOffPeakListResultSchema,
  hawknextProtocolMethods,
  type HawkNextOffPeakTaskProtocolSnapshot,
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            hawknextProtocolMethods.offPeakList,
            {},
            hawknextOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        hawknextProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: HawkNextOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
