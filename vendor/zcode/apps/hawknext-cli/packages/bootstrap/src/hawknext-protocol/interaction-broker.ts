...
import {
  ...
  hawknextUserInputResponseSchema,
  type HawkNextPermissionOption,
  type HawkNextPermissionResponse,
  type HawkNextUserInputQuestion,
...
import type { HawkNextProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        hawknextProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → HawkNextPermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: HawkNextPermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 hawknextPermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        hawknextProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): HawkNextUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): HawkNextUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): HawkNextUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): HawkNextUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "hawknext_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
