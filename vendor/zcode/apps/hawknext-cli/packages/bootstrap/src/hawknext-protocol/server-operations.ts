/* eslint-disable max-lines -- HawkNext Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../hawknext-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@hawknext/adapters/exec";
import { inputIntentMetadata } from "../hawknext-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@hawknext/core";
import {
  CoreErrorType,
  ...
  type UsageStorePort,
  type WorkspaceId,
} from "@hawknext/contracts";
import {
  DEFAULT_HAWKNEXT_MODEL_CONTEXT_BUDGET_STRATEGY,
  HAWKNEXT_SESSION_RUNTIME_PREFERENCES_REQUEST_TIMEOUT_MS,
  hawknextProtocolErrorCodes,
  hawknextProtocolMethods,
  hawknextSessionCancelBackgroundTaskParamsSchema,
  hawknextSessionCompactParamsSchema,
  hawknextSessionCloseParamsSchema,
  hawknextSessionCreateParamsSchema,
  hawknextSessionEventsParamsSchema,
  hawknextSessionForkParamsSchema,
  hawknextSessionGoalParamsSchema,
  hawknextSessionListParamsSchema,
  hawknextSessionMessagesParamsSchema,
  hawknextSessionReadParamsSchema,
  hawknextSessionRuntimePreferencesResultSchema,
  hawknextSessionResumeParamsSchema,
  hawknextSessionSendParamsSchema,
  hawknextSessionSetModeParamsSchema,
  hawknextSessionSetModelParamsSchema,
  hawknextSessionSetThoughtLevelParamsSchema,
  hawknextSessionStopParamsSchema,
  hawknextSessionSubscribeParamsSchema,
  hawknextSessionSubagentsParamsSchema,
  hawknextTaskTokenUsageParamsSchema,
  hawknextUsageStatsParamsSchema,
  hawknextWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type HawkNextSessionCreateParams,
  type HawkNextDeliveryKind,
  type IntegratedTerminalShellSelection,
  type HawkNextSessionRuntimePreferencesScope,
  type HawkNextSessionRuntimePreferencesResult,
  type HawkNextModelContextBudgetStrategy,
  type HawkNextProtocolTrace,
  type HawkNextSessionEvent,
  type HawkNextSessionHistoryTarget,
  type HawkNextSessionResumeParams,
  type HawkNextSessionPersistence,
...
import {
  ...
  parseParams,
  requireSession,
  type HawkNextProtocolAgentServerContext,
  type HawkNextProtocolSessionRecord,
  type HawkNextProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceHawkNextApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
import { runSessionModelConfigMutation } from "../hawknext-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<HawkNextProtocolSessionRecord["app"]["readTarget"]>>>
>;

type HawkNextSessionRecordParams = (
  | HawkNextSessionCreateParams
  | (HawkNextSessionResumeParams & {
      mode?: HawkNextSessionCreateParams["mode"];
      model?: HawkNextSessionCreateParams["model"];
      parentSessionId?: HawkNextSessionCreateParams["parentSessionId"];
      thoughtLevel?: HawkNextSessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: HawkNextSessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: HawkNextModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: HawkNextProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<HawkNextProtocolSessionRecord["app"], "listThoughtLevels">,
...
const liveProtocolStreamingDeltaBatches = new WeakMap<
  HawkNextProtocolSessionRecord,
  Map<string, LiveProtocolStreamingDeltaBatch>
>();

function protocolEventSequenceKey(deliveryKind?: HawkNextDeliveryKind): string {
  return deliveryKind ?? DEFAULT_PROTOCOL_EVENT_SEQUENCE_KEY;
}

function getProtocolEventSequenceState(
  record: HawkNextProtocolSessionRecord,
...
function getProtocolToolInputTransmissionState(
  record: HawkNextProtocolSessionRecord,
  deliveryKind?: HawkNextDeliveryKind,
): HawkNextProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: HawkNextProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const hawknext = asRecord(meta.hawknext);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(hawknext.parentToolUseId) ??
    stringValue(hawknext.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: HawkNextSessionEvent,
  toolInputTransmissions: HawkNextProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
  context: HawkNextProtocolAgentServerContext,
  record: HawkNextProtocolSessionRecord,
  event: SessionEvent,
  deliveryKind?: HawkNextDeliveryKind,
  options: {
...
): HawkNextSessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("HawkNext Protocol session event filtered", {
      event: "hawknext_protocol.session_event.filtered",
      eventId: String(event.id),
      module: "bootstrap.hawknext_protocol",
  ...
  // tool_input_delta 等高频模型中间态。HawkNext Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<HawkNextSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: HawkNextSessionEvent[] = [];
  const toolInputTransmissions: HawkNextProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("HawkNext Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "hawknext_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<HawkNextSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<HawkNextProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<HawkNextSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: HawkNextProtocolSessionRecord;
  sessionId: SessionId;
  createParams: HawkNextSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("HawkNext Protocol imported history persisted", {
    event: "hawknext_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: HawkNextProtocolAgentServerContext,
  rawParams: unknown,
  trace: HawkNextProtocolTrace | undefined,
  project: (
    record: HawkNextProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(hawknextSessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("HawkNext Protocol session/create started", {
    event: "hawknext_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("HawkNext Protocol session/create skipped unsupported thought level", {
          event: "hawknext_protocol.session_create.thought_level_skipped",
          module: "bootstrap.hawknext_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 HawkNext session。
    ...
    context.logger?.info("HawkNext Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "hawknext_protocol.session_create.completed",
    ...
    context.logger?.warn("HawkNext Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "hawknext_protocol.session_create.failed",
      module: "bootstrap.hawknext_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: HawkNextProtocolAgentServerContext,
  params: HawkNextSessionResumeParams,
  ...
  context.logger?.debug("HawkNext Protocol session resume started", {
    activeBeforeWait,
    event: "hawknext_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("HawkNext Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "hawknext_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.hawknext_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      hawknextProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("HawkNext Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "hawknext_protocol.session.resume_completed",
    module: "bootstrap.hawknext_protocol",
...
export async function resumeSession(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "hawknext_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "hawknext_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.hawknext_protocol",
      sessionId: session.id,
...
export async function listSessions(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(hawknextSessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("HawkNext Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "hawknext_protocol.session.persisted_missing",
      module: "bootstrap.hawknext_protocol",
...
export async function getUsageStats(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionReadParamsSchema, rawParams);
...
export async function readMessages(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(hawknextSessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("HawkNext Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("HawkNext Protocol session/send accepted", {
...
export async function compactSession(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("HawkNext Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("HawkNext Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("HawkNext Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("HawkNext Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("HawkNext Protocol goal paused"));
    }
...
export async function forkSession(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: HawkNextProtocolAgentServerContext,
  record: HawkNextProtocolSessionRecord,
  fork: Awaited<ReturnType<HawkNextProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: HawkNextProtocolAgentServerContext,
  parentRecord: HawkNextProtocolSessionRecord,
  forkRecord: HawkNextProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("HawkNext Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("HawkNext Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("HawkNext Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("HawkNext Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("HawkNext Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("HawkNext Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("HawkNext Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("HawkNext Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(hawknextSessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("HawkNext Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(hawknextSessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: HawkNextProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(hawknextSessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: HawkNextSessionPersistence,
  expectedPersistence?: HawkNextSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(hawknextWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceHawkNextApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: HawkNextProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: HawkNextProtocolAgentServerContext,
  _record: HawkNextProtocolSessionRecord,
...
async function resolveForkTarget(
  context: HawkNextProtocolAgentServerContext,
  record: HawkNextProtocolSessionRecord,
  target: HawkNextSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(hawknextTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: HawkNextProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: hawknextProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "hawknext_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "hawknext_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // HawkNextProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("HawkNext Protocol runtime preferences request started", {
    event: "hawknext_protocol.runtime_preferences.request_started",
    module: "bootstrap.hawknext_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      hawknextProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      hawknextSessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("HawkNext Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "hawknext_protocol.runtime_preferences.response_received",
      module: "bootstrap.hawknext_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "hawknext_protocol.runtime_preferences.request_failed",
      module: "bootstrap.hawknext_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("HawkNext Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("HawkNext Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "HawkNext Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_HAWKNEXT_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: HawkNextProtocolAgentServerContext,
  params: HawkNextSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: HawkNextProtocolTrace,
): Promise<HawkNextProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("HawkNext Protocol createRecord MCP config", {
    event: "hawknext_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: HawkNextProtocolSessionRecord | undefined;
  const app = await createWorkspaceHawkNextApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.hawknext/.agents 的 enabled MCP，
      ...
      // HawkNext Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // HawkNext Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: HawkNextProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): HawkNextSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isHawkNextSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isHawkNextSessionMode(
  value: unknown,
): value is NonNullable<HawkNextSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("HawkNext Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "hawknext_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "hawknext_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "hawknext_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "hawknext_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "hawknext_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: HawkNextStateUpdatedNotification = {
    patch,
    reason,
