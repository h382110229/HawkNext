...
import { createInMemorySessionEventStore } from "@hawknext/adapters/storage";
import type { ModelSelection } from "@hawknext/contracts";
import {
  hawknextProviderTestModelConnectivityParamsSchema,
  hawknextWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(hawknextWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(hawknextProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function createWorkspaceHawkNextApp(
  context: HawkNextProtocolAgentServerContext,
  ...
  return context.deps.createHawkNextApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createHawkNextApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<HawkNextProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: HawkNextProtocolAgentServerContext,
  record: Pick<HawkNextProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {
