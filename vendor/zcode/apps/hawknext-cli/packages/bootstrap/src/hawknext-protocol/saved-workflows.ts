...
// 解析器与序列化器只从 @hawknext/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.hawknext/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@hawknext/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  HAWKNEXT_WORKFLOWS_RUNS_MAX_LIMIT,
  hawknextWorkflowsDeleteParamsSchema,
  hawknextWorkflowsGetParamsSchema,
  hawknextWorkflowsListParamsSchema,
  hawknextWorkflowsMoveParamsSchema,
  hawknextWorkflowsRunsParamsSchema,
  hawknextWorkflowsUpdateMetaParamsSchema,
  type HawkNextSavedWorkflowRun,
  type HawkNextSavedWorkflowScope,
  type HawkNextWorkflowsDeleteResult,
  type HawkNextWorkflowsGetResult,
  type HawkNextWorkflowsListResult,
  type HawkNextWorkflowsMoveResult,
  type HawkNextWorkflowsRunsResult,
  type HawkNextWorkflowsUpdateMetaResult,
} from "@hawknext/shared";
import type { JournalStorePort } from "@hawknext/dynamic-workflow";
...
function scopeOf(params: { scope?: HawkNextSavedWorkflowScope }): HawkNextSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  rawParams: unknown,
): Promise<HawkNextWorkflowsListResult> {
  const params = parseParams(hawknextWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: HawkNextProtocolAgentServerContext,
  rawParams: unknown,
): Promise<HawkNextWorkflowsGetResult> {
  const params = parseParams(hawknextWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(hawknextWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(hawknextWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(hawknextWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(HAWKNEXT_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: HawkNextSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, HawkNextSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, HawkNextSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: HawkNextProtocolAgentServerContext,
  rawParams: unknown,
): Promise<HawkNextWorkflowsMoveResult> {
  const params = parseParams(hawknextWorkflowsMoveParamsSchema, rawParams);
