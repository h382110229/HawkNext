import {
  hawknextPluginsConfigureParamsSchema,
  hawknextPluginsResetConfigParamsSchema,
  hawknextPluginsInstallParamsSchema,
  hawknextPluginsMarketplaceAddParamsSchema,
  hawknextPluginsMarketplaceRemoveParamsSchema,
  hawknextPluginsMarketplaceUpdateParamsSchema,
  hawknextPluginsOverviewParamsSchema,
  hawknextPluginsListParamsSchema,
  hawknextPluginsSetEnabledParamsSchema,
  hawknextPluginsUninstallParamsSchema,
  hawknextPluginsUpdateParamsSchema,
  hawknextPluginsValidateParamsSchema,
  hawknextPluginsDescribeParamsSchema,
  hawknextPluginsRestoreBuiltinParamsSchema,
  type HawkNextAvailablePluginSummary,
  type HawkNextInstalledPluginSummary,
  type HawkNextPluginComponentGroup,
  type HawkNextPluginDiagnostic,
  type HawkNextPluginInfo,
  type HawkNextPluginMarketplaceSummary,
  type HawkNextPluginsConfigureResult,
  type HawkNextPluginsDescribeResult,
  type HawkNextPluginsInstallResult,
  type HawkNextPluginsListResult,
  type HawkNextPluginsMarketplaceMutationResult,
  type HawkNextPluginsOverviewResult,
  type HawkNextPluginsRestoreBuiltinResult,
  type HawkNextPluginsSetEnabledResult,
  type HawkNextPluginsUninstallResult,
  type HawkNextPluginsValidateResult,
} from "@hawknext/shared";
import type { PluginDiagnostic, PluginMetadata } from "@hawknext/contracts";
import {
  addHawkNextPluginMarketplace,
  configureHawkNextPlugin,
  describeHawkNextPlugin,
  getHawkNextPluginsOverview,
  installHawkNextMarketplacePlugin,
  removeHawkNextPluginMarketplace,
  resolveHawkNextPlugins,
  resetHawkNextPluginConfig,
  ...
  validateHawkNextPlugin,
} from "../plugins.js";
import { listInstalledPluginRecords } from "@hawknext/adapters/plugins";
...
import { createConfig, resolvePath, type ConfigResult } from "@hawknext/adapters/config";
import { parseParams, type HawkNextProtocolAgentServerContext } from "./server-types.js";

// 把 CLI 的 PluginMetadata 投影成协议可序列化的 HawkNextPluginInfo (只保留 UI 需要的字段)。
function toPluginInfo(plugin: PluginMetadata, configResult?: ConfigResult): HawkNextPluginInfo {
...
function toPluginDiagnostic(diagnostic: PluginDiagnostic): HawkNextPluginDiagnostic {
  return {
...
export async function listPlugins(
  context: HawkNextProtocolAgentServerContext,
  rawParams: unknown,
): Promise<HawkNextPluginsListResult> {
  const params = parseParams(hawknextPluginsListParamsSchema, rawParams);
...
): Promise<HawkNextPluginsSetEnabledResult> {
  const params = parseParams(hawknextPluginsSetEnabledParamsSchema, rawParams);
  abortSignal?.throwIfAborted();
  const result = await setHawkNextPluginEnabled({
...
export async function getPluginsOverview(
  context: HawkNextProtocolAgentServerContext,
  rawParams: unknown,
): Promise<HawkNextPluginsOverviewResult> {
  const params = parseParams(hawknextPluginsOverviewParamsSchema, rawParams);
  const overview = getHawkNextPluginsOverview({
...
export async function addPluginMarketplace(
  ...
  const params = parseParams(hawknextPluginsMarketplaceAddParamsSchema, rawParams);
...
export async function removePluginMarketplace(
  ...
  const params = parseParams(hawknextPluginsMarketplaceRemoveParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function updatePluginMarketplace(
  ...
  const params = parseParams(hawknextPluginsMarketplaceUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function installPlugin(
  ...
  rawParams: unknown,
  abortSignal?: AbortSignal,
): Promise<HawkNextPluginsInstallResult> {
  const params = parseParams(hawknextPluginsInstallParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    installHawkNextMarketplacePlugin({
      abortSignal,
...
export async function uninstallPlugin(
  context: HawkNextProtocolAgentServerContext,
  rawParams: unknown,
): Promise<HawkNextPluginsUninstallResult> {
  const params = parseParams(hawknextPluginsUninstallParamsSchema, rawParams);
  const removed = await uninstallHawkNextMarketplacePlugin({
    logger: context.logger,
    marketplace: params.marketplace,
...
export async function updatePlugin(
  ...
  const params = parseParams(hawknextPluginsUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  ...
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const installedPlugins: HawkNextInstalledPluginSummary[] = [];
    const dependencyClosure: string[] = [];
    // 聚合每条记录重装产生的诊断：installHawkNextMarketplacePlugin 失败时不抛错，而是返回
    // CLI 形态的 PluginDiagnostic（见其错误分支的 toMarketplaceInstallDiagnostic），
    // 这里逐条经协议侧 toPluginDiagnostic 投影成 HawkNextPluginDiagnostic 回传，
    // 让失败的重装显式暴露，而不是静默"成功"。
    const diagnostics: HawkNextPluginDiagnostic[] = [];
    for (const record of installed) {
      const result = await installHawkNextMarketplacePlugin({
        logger: context.logger,
...
export async function restoreBuiltinPlugin(
  ...
  const params = parseParams(hawknextPluginsRestoreBuiltinParamsSchema, rawParams);
...
export async function configurePlugin(
  ...
  const params = parseParams(hawknextPluginsConfigureParamsSchema, rawParams);
  await configureHawkNextPlugin({
...
export async function resetPluginConfig(
  ...
  const params = parseParams(hawknextPluginsResetConfigParamsSchema, rawParams);
  await resetHawkNextPluginConfig({
    logger: context.logger,
    pluginId: params.pluginId,
    scope: params.scope,
    workingDirectory: params.workspace.workspacePath,
...
export async function validatePlugin(
  context: HawkNextProtocolAgentServerContext,
  rawParams: unknown,
): Promise<HawkNextPluginsValidateResult> {
  const params = parseParams(hawknextPluginsValidateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function describePlugin(
  context: HawkNextProtocolAgentServerContext,
  rawParams: unknown,
): Promise<HawkNextPluginsDescribeResult> {
  const params = parseParams(hawknextPluginsDescribeParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    describeHawkNextPlugin({
      logger: context.logger,
  ...
  const components: HawkNextPluginComponentGroup[] = result.components.map((group) => ({
    kind: group.kind,
    items: group.items.map((item) => ({
      name: item.name,
      ...(item.description ? { description: item.description } : {}),
...
function toMarketplaceSummary(input: {
  ...
  isOfficial?: boolean;
  featured?: string[];
  refreshFailure?: HawkNextPluginMarketplaceSummary["refreshFailure"];
}): HawkNextPluginMarketplaceSummary {
  return {
    id: input.id,
    name: input.name,
    source: input.source,
...
function toAvailablePluginSummary(input: {
  ...
  listing?: HawkNextAvailablePluginSummary["listing"];
}): HawkNextAvailablePluginSummary {
  return {
    id: input.id,
    name: input.name,
...
function toInstalledPluginSummary(input: {
  ...
  installedAt?: string;
  componentTypes?: string[];
  hookDetails?: HawkNextInstalledPluginSummary["hookDetails"];
  updateStatus?: "none" | "update-available" | "version-changed";
  latestVersion?: string;
  listing?: HawkNextInstalledPluginSummary["listing"];
}): HawkNextInstalledPluginSummary {
  return {
    id: input.id,
    name: input.name,
    marketplace: input.marketplace,
