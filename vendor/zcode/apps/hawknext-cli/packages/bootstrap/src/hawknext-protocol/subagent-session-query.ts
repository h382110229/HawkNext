...
import type { HawkNextSessionEndedSubagent, HawkNextSessionRunningSubagent } from "@hawknext/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: HawkNextSessionRunningSubagent[];
  ended: HawkNextSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): HawkNextSessionRunningSubagent["status"] | undefined {
...
): HawkNextSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): HawkNextSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: HawkNextSessionRunningSubagent[] = [];
  const ended: HawkNextSessionEndedSubagent[] = [];
...
function encodeCursor(item: HawkNextSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly HawkNextSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: HawkNextSessionEndedSubagent[]; nextCursor?: string } {
