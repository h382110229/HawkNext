import {
  ...
  type HawkNextActiveToolCall,
  type HawkNextContextUsageBreakdownItem,
  type HawkNextDeliveryKind,
  type HawkNextPendingPermission,
  type HawkNextSessionContextUsage,
  type HawkNextSessionEvent,
  type HawkNextSessionGoal,
  type HawkNextSessionGoalVerification,
  type HawkNextSessionGoalVerificationTimeline,
  type HawkNextSessionGoalStats,
  type HawkNextSessionInfo,
  type HawkNextSessionKind,
  type HawkNextSessionProjection,
  type HawkNextSessionRuntimeState,
  type HawkNextSessionSettingsState,
  type HawkNextSessionStateSnapshot,
  type HawkNextSessionTodoGroup,
...
import type { HawkNextApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: HAWKNEXT_PROTOCOL_NAME,
      version: HAWKNEXT_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<HawkNextApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 hawknext-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("hawknext-artifact://") ? artifactUri : undefined;
}
...
): Promise<HawkNextSessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<HawkNextApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as HawkNextSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is HawkNextSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = hawknextApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const hawknextMeta = asRecord(meta.hawknext);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = hawknextApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): HawkNextSessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<HawkNextApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: HawkNextDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): HawkNextSessionContextUsage | undefined {
...
): HawkNextSessionGoalVerification[] {
...
): HawkNextSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: HawkNextSessionContextUsage["cache"] | undefined,
): HawkNextSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): HawkNextSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): HawkNextPendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 hawknextPendingPermissionSchema（shared/src/hawknext-protocol/index.ts:1139）和
  // hawknextPermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 hawknextPermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): HawkNextActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): HawkNextSessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): HawkNextSessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): HawkNextSessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, HawkNextSessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, HawkNextSessionTodoGroup>,
  input: {
  ...
  const group: HawkNextSessionTodoGroup = {
    id: input.groupId,
...
function addOrUpdateTodoInGroup(
  group: HawkNextSessionTodoGroup,
  fingerprint: string,
  todo: TodoItem,
): void {
...
function mapSessionEventType(type: SessionEvent["type"]): HawkNextSessionEvent["type"] {
  switch (type) {
