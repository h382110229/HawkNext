...
import {
  ...
  hawknextProviderRuntimeHeadersResponseSchema,
  type HawkNextWorkspaceRef,
} from "@hawknext/shared";
import type { HawkNextAppOptions } from "../app/types.js";
import {
...
): NonNullable<HawkNextAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          hawknextProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: hawknextProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
      ...
      // hawknext-plan 的账号鉴权材料是每次请求刷新的运行时配置，
