...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@hawknext/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@hawknext/core";
import type { Logger, PluginMetadata } from "@hawknext/contracts";
import {
  createAgentStateId,
...
interface LoadHawkNextAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadHawkNextAgentProfilesResult {
...
export async function loadHawkNextAgentProfiles(
  input: LoadHawkNextAgentProfilesInput,
): Promise<LoadHawkNextAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".hawknext", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .hawknext/agents/*.md 是仓库内容，不能通过 frontmatter
...
): LoadHawkNextAgentProfilesResult {
  const diagnostics: AgentProfileParseDiagnostic[] = [];
