...
import {
  getCapturedHawkNextCuaBrokerCredentials,
  isHawkNextCuaMcpCommand,
  isHawkNextCuaMcpPackageArg,
  HAWKNEXT_CUA_BROKER_SOCKET_ENV_KEY,
  HAWKNEXT_CUA_NODE_REPL_HOST_ENV_KEY,
  HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID,
  HAWKNEXT_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  HAWKNEXT_PLUGIN_ID_ENV_KEY,
} from "@hawknext/shared";

export { HAWKNEXT_CUA_BROKER_SOCKET_ENV_KEY as HAWKNEXT_CUA_BROKER_SOCKET_ENV } from "@hawknext/shared";
// CLI 入口会先清理 broker 凭据；shared node_repl 的可信配置随后从进程内捕获快照恢复它们。
function resolveHawkNextCuaBrokerSocket(): string | undefined {
  // captured 优先；运行时残留的 stale socket 不能覆盖可信快照。
  return (
    getCapturedHawkNextCuaBrokerCredentials().socket ||
    process.env[HAWKNEXT_CUA_BROKER_SOCKET_ENV_KEY]?.trim()
  );
}

function resolveHawkNextCuaBrokerToken(): string | undefined {
  return undefined;
}

const NODE_REPL_SERVER_NAME = "node_repl";
const REFRESH_MARKER_ENV = "HAWKNEXT_CUA_PERMISSION_BROKER_REFRESH_MARKER";
    ...
    Object.entries(pluginServers)
      .filter(
        ([name, config]) =>
          ...
          config.type === "stdio" &&
          config.env?.[HAWKNEXT_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() ===
            HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID,
      )
      .map(([name]) => name),
  );
...
export async function listMcpServerStatuses(
  ...
  for (const [name, config] of Object.entries(servers)) {
    // 迁移后旧的 hawknext-cua MCP 只是历史配置，不再进入状态投影；CUA
...
export function omitMcpServers(
  ...
  return injectHawkNextCuaBrokerMcpServers(
    kept,
    resolveHawkNextCuaBrokerSocket(),
    resolveHawkNextCuaBrokerToken(),
    trustedOfficialCuaServerNames,
  );
}

function injectHawkNextCuaBrokerMcpServers(
  servers: Record<string, McpServerConfig>,
...
function injectCuaCredentialsIntoNodeRepl(
  ...
  const captured = getCapturedHawkNextCuaBrokerCredentials();
  ...
  return {
    ...
    env: {
      ...config.env,
      [HAWKNEXT_CUA_BROKER_SOCKET_ENV_KEY]: socketPath,
      ...(refreshMarker ? { [REFRESH_MARKER_ENV]: refreshMarker } : {}),
      ...(pluginAuthority ? { [HAWKNEXT_CUA_PLUGIN_AUTHORITY_ENV_KEY]: pluginAuthority } : {}),
      [HAWKNEXT_CUA_NODE_REPL_HOST_ENV_KEY]: "1",
      [HAWKNEXT_PLUGIN_ID_ENV_KEY]: HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID,
    },
  };
}

function isHawkNextCuaStdioServer(
  name: string,
  config: McpServerConfig,
): config is McpStdioServerConfig {
  if (config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 内置 official hawknext-cua plugin 的 MCP server 走 __hawknext-plugin-host，command 是 Helper
  // (非 hawknext-cua)、args 是 [hawknext.cjs, __hawknext-plugin-host, server.js]（非 hawknext-cua package arg），
  // 上面的 name/command/args 三条都匹配不到。_plugin id 由 adapters resolver 权威写入 env
  // （manifest/user env 不可覆盖），用它识别 official plugin server。
  if (
    config.env?.[HAWKNEXT_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() === HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID
  ) {
    return true;
  }
  // 判定与 desktop/services 共用 @hawknext/shared 的单一事实源，避免两条注入入口漂移。
  if (isHawkNextCuaMcpCommand(config.command)) return true;
  return (config.args ?? []).some(isHawkNextCuaMcpPackageArg);
}
...
function isRetiredCuaMcpServer(name: string, config: McpServerConfig | undefined): boolean {
  ...
  return (
    name !== NODE_REPL_SERVER_NAME && config !== undefined && isHawkNextCuaStdioServer(name, config)
  );
}
