import { PERMISSION_FULL_ACCESS_OPTION_ID } from "@hawknext/shared/hawknext-protocol-v4";
...
import { verdictWorkspaceHookReviewRequest } from "@hawknext/shared/workspace-hook-review-monotonicity";
import {
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  isHawkNextModelRetryRecoveryProgressPayload,
  isHawkNextFileStreamingToolInputPreviewTool,
  parseHawkNextBackgroundTaskNotificationText,
  resolveHawkNextBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseHawkNextBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
    const row = this.findToolRow(parsed.toolUseId);
    if (!row) return [];

    const notificationStatus = hawknextBackgroundTaskNotificationToolUpdateStatus(
  ...
  private openToolRow(
    ...
    if (isHawkNextFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private appendStreamingToolInput(
    ...
    if (
      state.lastPublishedAt !== null &&
      now - state.lastPublishedAt < HAWKNEXT_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    ) {
      return [];
    }
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveHawkNextBackgroundTaskControlKind(payload);
  ...
  //
  // 归约本体在 @hawknext/shared 的 workflow-runs-reducer（与状态 schema 同居）：TUI 镜像要用
  // 同一份归约，两处各写一份就是两个时钟。
