...
import { getModelUsageTotalTokens, SessionEventType } from "@hawknext/contracts";
import { parseAutomationRunId } from "@hawknext/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const hawknext = recordValue(meta.hawknext);
  return (
    ...
    optionalString(hawknext.parentToolCallId) ??
    optionalString(hawknext.parentToolUseId)
  );
}
