...
import type { EventId, SessionEvent, SessionId, TraceId, TurnId } from "@hawknext/contracts";
import {
...
const LEGACY_PROTOCOL_SESSION_STOPPED_MESSAGE = "HawkNext Protocol session stopped";
...
function isPersistedAssistantCancellation(error: AssistantErrorInfo): boolean {
  ...
  // 只能用 HawkNext 自身生成的标准 name/message 二元组兼容恢复；不泛化匹配 provider 文案。
  return (
    code === undefined &&
