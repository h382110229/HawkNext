...
import { createNodeModelSelectionFacade } from "@hawknext/provider-node";
import { createNodeLoggerFactory } from "@hawknext/adapters/logging";
import {
  createMcpAdapterConnectionPool,
  createMcpTelemetryTracker,
  type McpConnectionPool,
  type McpTelemetryTracker,
} from "@hawknext/adapters/mcp";
import {
  hawknextProtocolNotifications,
  type HawkNextMcpResourceSample,
  type HawkNextMcpTelemetryEvent,
} from "@hawknext/shared";
import type { SqliteSessionStore } from "@hawknext/adapters/storage";
import { traceContextToLogContext, createRootTraceContext } from "@hawknext/contracts";
import type { McpPort, ModelSelection } from "@hawknext/contracts";
import type { PresentationSurface } from "@hawknext/core";
import type { RunHawkNextProtocolAgentOptions, HawkNextAppOptions } from "./app/types.js";
import { createHawkNextApp } from "./app/create-app.js";
import {
...
} from "./hawknext-protocol/storage-startup.js";
...
import { installHawkNextProtocolAiSdkWarningLogger } from "./hawknext-protocol/ai-sdk-warning-logger.js";
import {
  createOfficialMcpAuthHeadersPort,
  type OfficialMcpAuthRequestContext,
} from "./hawknext-protocol/official-mcp-auth-port.js";
import {
  createOfficialMcpTrustedOriginRegistry,
  OFFICIAL_MCP_DEV_TRUSTED_ORIGINS_ENV,
  HAWKNEXT_WORKSPACE_IDENTITY_ENV,
  resolveRuntimeHawkNextEndpointOrigin,
} from "@hawknext/shared";
import { HawkNextProtocolAgentServer } from "./hawknext-protocol/server.js";
import { HawkNextProtocolNdjsonConnection } from "./hawknext-protocol/transport.js";
import { cleanupProtocolRuntime } from "./hawknext-protocol/runtime-cleanup.js";
import { startProtocolResourceSampler } from "./hawknext-protocol/resource-sampler.js";
import { acquireProtocolStartupResource } from "./hawknext-protocol/startup-resource.js";
import type { HawkNextProcessResourceSampler } from "./process-resource-sampler.js";
import { prepareHawkNextTelemetryEnv, shutdownHawkNextTelemetry } from "./telemetry-bootstrap.js";

function applyProtocolPresentationSurface(
  options: Omit<HawkNextAppOptions, "providerRegistry">,
  presentationSurface: PresentationSurface,
): Omit<HawkNextAppOptions, "providerRegistry"> {
  return {
    ...options,
    runtimeConfig: {
...
function applyProtocolProviderRegistry(
  options: Omit<HawkNextAppOptions, "providerRegistry">,
  providerRegistry: HawkNextAppOptions["providerRegistry"],
...
export async function runHawkNextProtocolAgent(
  options: RunHawkNextProtocolAgentOptions = {},
): Promise<void> {
  ...
  const traceContext = createRootTraceContext({
    attributes: {
      entrypoint: "hawknext_protocol",
    },
  });
  const logger = loggerFactory.createLogger("hawknext").child({
    ...traceContextToLogContext(traceContext),
    module: "bootstrap.hawknext_protocol",
  });
  installHawkNextProtocolAiSdkWarningLogger(logger);
  const startupTimer = new StartupTimer(
    logger,
    {
      ...traceContextToLogContext(traceContext),
      module: "bootstrap.hawknext_protocol",
      startupKind: "hawknext_protocol_agent",
    },
    startupStartedAt,
  );
  startupTimer.start("HawkNext Protocol agent startup started", {
    context: { version: options.version },
    event: "hawknext_protocol.startup.started",
    stage: "start",
  });

  let sessionStore: SqliteSessionStore | undefined;
  let serverForCleanup: HawkNextProtocolAgentServer | undefined;
  ...
  let mcpResourceSink: ((samples: HawkNextMcpResourceSample[]) => void) | undefined;
  let mcpTelemetrySink: ((event: HawkNextMcpTelemetryEvent) => void) | undefined;
  let processResourceSampler: HawkNextProcessResourceSampler | undefined;
  ...
  try {
        ...
        openProtocolStartupStorage({
          ...
          onProgress: (progress) =>
            logger.info("SQLite startup state", {
              event: "hawknext_protocol.startup.storage_state",
              ...progress,
            }),
        }),
    });
    ...
    logger.info("Worker Provider Registry 已就绪", {
      ...
      event: "hawknext_protocol.provider_registry.ready",
      module: "bootstrap.hawknext_protocol",
    ...
    const telemetryEnv = await acquireProtocolStartupResource({
      signal: options.lifecycle?.signal,
      logger,
      disposeLate: () => shutdownHawkNextTelemetry(),
      create: () =>
        prepareHawkNextTelemetryEnv(runtimeEnv, {
          cliVersion: options.version,
          productVersion: options.env?.HAWKNEXT_APP_VERSION,
          runtimeSurface,
        }),
    });
    const telemetryDeviceMid = telemetryEnv.HAWKNEXT_TELEMETRY_DEVICE_MID;
    mcpTelemetryTracker =
    ...
    // 与下面 trustedOrigins 的 resolveHawkNextApiOrigin 必须是同一个表达式，否则两侧判定分叉。
    const resolveHawkNextApiOrigin = (): string =>
      resolveRuntimeHawkNextEndpointOrigin(options.env ?? process.env);
    const workspaceIdentity = (options.env ?? process.env)[HAWKNEXT_WORKSPACE_IDENTITY_ENV]?.trim();
    const officialMcpAuth = {
      ...
      resolveHawkNextApiOrigin,
      ...(workspaceIdentity ? { workspaceIdentity } : {}),
      // 信任判定只看一条：目标 origin 等于当前 HawkNext API origin（https）。pluginId 不参与。
    ...
    options.lifecycle?.signal.throwIfAborted();
    const server = (serverForCleanup = new HawkNextProtocolAgentServer({
      createHawkNextApp: (appOptions = {}) =>
        createHawkNextApp({
          ...
          env: {
            ...telemetryEnv,
            ...appOptions.env,
            ...(telemetryDeviceMid ? { HAWKNEXT_TELEMETRY_DEVICE_MID: telemetryDeviceMid } : {}),
          },
          ...
          onToolExecResource: (params) =>
            connection.send({ method: hawknextProtocolNotifications.toolExecResource, params }),
        }),
      cwd: options.cwd,
      env: options.env,
      loggerFactory,
    ...
    const connection = new HawkNextProtocolNdjsonConnection({
      signal: options.lifecycle?.signal,
    ...
    mcpResourceSink = (samples) =>
      connection.send({
        method: hawknextProtocolNotifications.mcpResourceSamples,
        params: samples,
      });
    mcpTelemetrySink = (event) => {
      ...
      connection.send({
        method: hawknextProtocolNotifications.mcpTelemetry,
        params: event,
      });
    };
    connection.start();
    ...
    startupTimer.complete("HawkNext Protocol agent startup completed", {
      event: "hawknext_protocol.startup.completed",
      stage: "total",
    });
    ...
    startupTimer.fail("HawkNext Protocol agent startup failed", error, {
      event: "hawknext_protocol.startup.failed",
      stage: "total",
    });
    throw error;
  } finally {
    ...
    logger.info("HawkNext Protocol agent shutdown completed", {
      ...traceContextToLogContext(traceContext),
      event: "hawknext_protocol.shutdown.completed",
      module: "bootstrap.hawknext_protocol",
      status: "completed",
    });
  }
...
function resolveProtocolRuntimeSurface(
  ...
  return env.HAWKNEXT_TELEMETRY_RUNTIME_SURFACE?.trim() === "remote_workspace_host"
