...
import {
  HAWKNEXT_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type HawkNextProcessResourceSample,
} from "@hawknext/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const HAWKNEXT_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = HAWKNEXT_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateHawkNextProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: HawkNextProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: HawkNextProcessResourceSample["platform"];
  arch?: HawkNextProcessResourceSample["arch"];
...
export interface HawkNextProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createHawkNextProcessResourceSampler(
  options: CreateHawkNextProcessResourceSamplerOptions,
): HawkNextProcessResourceSampler {
  const platform = options.platform ?? (process.platform as HawkNextProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as HawkNextProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: HawkNextProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, HAWKNEXT_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
