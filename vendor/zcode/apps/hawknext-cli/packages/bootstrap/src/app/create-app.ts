...
} from "@hawknext/adapters/storage";
import { createNodeLoggerFactory } from "@hawknext/adapters/logging";
import { createConfig, resolvePath } from "@hawknext/adapters/config";
import {
  createNodeExecutionAdapter,
  resolveEffectiveBashShellSelection,
} from "@hawknext/adapters/exec";
import { createNodeFileSystemAdapter } from "@hawknext/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@hawknext/adapters/http";
import { createJimpImageProcessorAdapter } from "@hawknext/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@hawknext/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@hawknext/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@hawknext/adapters/context";
import { createNodeSkillAdapter } from "@hawknext/adapters/skills";
import { createMcpAdapter } from "@hawknext/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@hawknext/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveHawkNextRuntimeEnv } from "@hawknext/shared";
import {
...
import { resolveHawkNextCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveHawkNextBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadHawkNextAgentProfiles } from "../subagents.js";
...
export async function createHawkNextApp(options: HawkNextAppOptions): Promise<HawkNextApp> {
  if (!options?.providerRegistry) {
    throw new Error("createHawkNextApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("hawknext").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveHawkNextRuntimeEnv(options.env ?? process.env) === "development",
    );
    const hawknextSubagentProfileOutcome = await loadHawkNextAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const hawknextSubagentProfiles = hawknextSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: hawknextSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: hawknextSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...hawknextSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          hawknextSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("HawkNext runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("HawkNext runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).HAWKNEXT_MAILBOX_ROOT ?? "~/.hawknext/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveHawkNextBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveHawkNextCustomCommandPrompt(text, {
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("hawknext-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("hawknext-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new HawkNextAttachmentFaultError(HAWKNEXT_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new HawkNextAttachmentFaultError(HAWKNEXT_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("HawkNext app startup failed", error, {
