import type { HawkNextToolExecResource, BackgroundBashOutputResult } from "@hawknext/shared";
import type { AiSdkModelAdapter } from "@hawknext/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { HawkNextModelOption } from "@hawknext/shared";
import type { EffectiveModelSelectionResult } from "@hawknext/shared/model-selection";
export type { HawkNextModelOption } from "@hawknext/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { HawkNextInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@hawknext/contracts";
...
export type HawkNextAppRuntimeConfigInput = AgentRuntimeConfig;

export interface HawkNextAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: HawkNextAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: HawkNextToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@hawknext/contracts").TurnBackgroundAttribution;
...
export interface HawkNextPluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface HawkNextPluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: HawkNextInstalledPluginData | null;
}
...
export interface HawkNextApp {
  ...
  getCurrentModelOption?(): HawkNextModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): HawkNextModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@hawknext/shared").HawkNextSessionSubagentsResult>;
  ...
  listModels(): HawkNextModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<HawkNextPluginSetResult>;
  uninstallPlugin(plugin: string): Promise<HawkNextPluginUninstallResult>;
  /**
...
export interface RunHawkNextProtocolAgentOptions {
...
export interface ListHawkNextSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
