...
import {
  ...
  downloadHawkNextBuiltinRelease,
...
export interface ProcessProviderRegistryRuntimeOptions {
  ...
  readonly standalone?: {
    ...
    readonly onBuiltinRefreshResult?: (event: HawkNextBuiltinRefreshEvent) => void;
  };
}
...
export async function startProcessProviderRegistryRuntime(
  ...
  if (!paths) {
    throw new Error("缺少进程 Provider Registry 的 HawkNext Built-in / Personal Config 路径");
  }

  const accountSource = new MutableAccountProviderConfigSource();
  const credentialStore = options.standalone
    ? (options.standalone.credentialStore ?? createSharedHawkNextCredentialStore({ env: { ...env } }))
    : undefined;
  let standaloneAccount: AccountProviderService | undefined;
  const bundledFile = options.standalone
    ? env[HAWKNEXT_BUILTIN_PROVIDER_BUNDLED_CONFIG_FILE_ENV]?.trim()
    : undefined;
  const runtime = new NodeProviderRegistryRuntime({
    ...paths,
    ...(bundledFile
      ? {
          hawknextBuiltinFilePath: bundledFile,
          hawknextBuiltinActiveFilePath: paths.hawknextBuiltinFilePath,
          hawknextBuiltinRemote: {
            controlFilePath: join(
              dirname(paths.hawknextBuiltinFilePath),
              "hawknext-builtin-refresh.json",
            ),
            resolveEndpointKey: () => resolveRuntimeHawkNextEndpointOrigin(env),
            fetchRelease: (endpointOrigin, signal) =>
              downloadHawkNextBuiltinRelease({
                endpointOrigin,
                signal,
                appVersion: HAWKNEXT_VERSION,
                platform: resolveHawkNextBuiltinClientPlatform(),
      ...
      : {}),
    onHawkNextBuiltinRefreshError: options.standalone?.onBuiltinRefreshError,
    accountSource,
    ...(credentialStore
      ? {
  ...
  const disposeRecovery = standaloneAccount
    ? runtime.onDidCheckHawkNextBuiltin(async () => {
        const [config, account] = await Promise.all([
          runtime.configService.read(),
          standaloneAccount!.read(),
        ]);
        if (config.hawknextBuiltinRevision !== account.basedOnHawkNextBuiltinRevision)
...
export function parseProcessAccountProviderConfigSnapshot(input: {
  ...
  readonly basedOnHawkNextBuiltinRevision: string;
  ...
  const basedOnHawkNextBuiltinRevision = input.basedOnHawkNextBuiltinRevision.trim();
  if (!basedOnHawkNextBuiltinRevision) {
  ...
  return Object.freeze({
    revision,
    basedOnHawkNextBuiltinRevision,
    providers,
