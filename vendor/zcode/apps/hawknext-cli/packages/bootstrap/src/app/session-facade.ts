import { updateUiLocaleInFileConfig, type ConfigResult } from "@hawknext/adapters/config";
import type { AgentRuntime } from "@hawknext/core";
import { resolveLocale } from "@hawknext/i18n";
import { normalizeModelSelection, type ModelSelection } from "@hawknext/provider";
import {
...
import type { PrepareUserExecutionBoundary, HawkNextApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<HawkNextApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<HawkNextApp["updateTargetStatus"]>
      >,
