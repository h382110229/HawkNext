...
import type { PluginLoadOutcome } from "@hawknext/contracts";
...
import type {
  HawkNextApp,
  HawkNextAppOptions,
  HawkNextPluginSetResult,
  HawkNextPluginUninstallResult,
} from "./types.js";
...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    listPlugins: async (): Promise<PluginLoadOutcome> => listHawkNextPlugins(commonOptions()),
    setPluginEnabled: async (
      plugin: string,
      enabled: boolean,
    ): Promise<HawkNextPluginSetResult> => {
    ...
    uninstallPlugin: async (plugin: string): Promise<HawkNextPluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE) {
