...
import { BUILTIN_PROVIDER_TEMPLATE_IDS } from "@hawknext/shared";
import { getDefaultConfigPath } from "@hawknext/adapters/config";
import {
...
function resolveLegacyProviderApiType(
  kind: "anthropic" | "official-model" | "official-model-compatible" | undefined,
): ProviderApiType {
  switch (kind) {
    case "anthropic":
...
function importLegacyCliPersonalProviderConfig(
  ...
  // 通用 HawkNextConfigFileSchema 刻意把旧 provider 留在 passthrough unknown，
