...
import type { SharedHawkNextCredentialStore } from "@hawknext/adapters/auth";
import type { ProviderRuntimeHeadersPort } from "@hawknext/core";
import {
...
async function readStandaloneCodingPlanCatalog(
  ...
  readonly hawknextBuiltinRevision: string;
  readonly providers: readonly StandaloneCodingPlanProvider[];
}> {
  if (config)
    return {
      hawknextBuiltinRevision: config.revision,
  ...
  const filePath = env[HAWKNEXT_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  if (!filePath) {
    throw new Error(`${HAWKNEXT_BUILTIN_PROVIDER_CONFIG_FILE_ENV} is required for login`);
  }
...
export async function resolveStandaloneCodingPlanProvider(
  ...
  if (matches.length !== 1) {
    throw new Error(
      `HawkNext Built-in Config 必须为 ${family} 声明唯一 Individual Coding Plan Provider`,
    );
  }
  return matches[0]!;
}
...
export async function readStandaloneAccountProviderConfigSnapshot(
  credentialStore: Pick<SharedHawkNextCredentialStore, "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
  config?: Pick<ProviderConfigLayerSnapshot, "revision" | "providers">,
  ...
  return createAccountProviderConfigSnapshot(catalog.hawknextBuiltinRevision, providers);
}
...
export function createStandaloneProviderRuntimeHeadersPort(
  credentialStore: Pick<SharedHawkNextCredentialStore, "load" | "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
