...
import {
  HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE,
  HAWKNEXT_PLUGIN_HOST_COMMAND,
...
export function createBundledMcpRuntimeConfig(input: {
  ...
  return {
    ...
    env: {
      ...input.env,
      // 桌面打包态 process.execPath 是 HawkNext Helper；缺少 Node 模式会误进 Electron main。
...
export function writeOfficialPluginRuntimeManifest(input: OfficialRuntimeManifestInput): void {
  const manifestPath = join(input.rootPath, ".hawknext-plugin", "plugin.json");
  ...
  // 把 seed 流程整个阻断，连带 listHawkNextSkills 拉不出 plugin skill。
  if (manifest.mcpServers === undefined) return;
  const mcpServers = asRecord(manifest.mcpServers);
  const hostPrefixArgs = officialPluginHostPrefixArgs();
  if (!hostPrefixArgs) return;

  // 保留对其他历史 official plugin MCP 的通用重写；hawknext-cua 当前是 skill/SDK-only，
  ...
  for (const [serverKey, serverRaw] of Object.entries(mcpServers)) {
    ...
    mcpServer.env = {
      ...mcpServerEnv,
      // 桌面打包态的 process.execPath 是 HawkNext Helper。
      ...
      [HAWKNEXT_PLUGIN_ID_ENV_KEY]: `${input.pluginName}@${HAWKNEXT_OFFICIAL_PLUGIN_MARKETPLACE}`,
    };
...
export function officialPluginHostPrefixArgs(): string[] | undefined {
  if (isSeaRuntime()) return [HAWKNEXT_PLUGIN_HOST_COMMAND];

  const entrypoint = process.argv[1];
  if (!entrypoint) return undefined;

  return [...process.execArgv, resolve(entrypoint), HAWKNEXT_PLUGIN_HOST_COMMAND];
}
