import type { ConfigResult } from "@hawknext/adapters/config";
import { discoverNodePluginsSync } from "@hawknext/adapters/plugins";
import type { AgentRuntimeConfig } from "@hawknext/core";
import type { Logger, McpServerConfig, PluginLoadOutcome } from "@hawknext/contracts";
...
import type { HawkNextAppOptions } from "./types.js";
...
export function resolveStartupPlugins(input: {
  ...
  logger?: Logger;
  options: Pick<HawkNextAppOptions, "officialPluginRoots" | "pluginStorageRoot">;
  ...
  const pluginOutcome = discoverNodePluginsSync({
    ...
    // (resolveHawkNextPlugins) 和应用启动路径都要传，否则 `/skill skill-creator` 在会话里
    // 报 "Skill not found: skill-creator"，但 `hawknext plugins list` 却显示它是 enabled。
    officialPluginsEnabledByDefault: DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS,
    storageRoot: pluginStorageRoot,
    workingDirectory: input.workingDirectory,
  });
  input.startupTimer.mark("HawkNext plugins resolved", {
    context: {
...
export function startAppStartup(input: {
  ...
  input.startupTimer.start("HawkNext app startup started", {
    context: {
...
export function markConfigurationLoaded(input: {
  ...
  input.startupTimer.mark("HawkNext app configuration loaded", {
    context: {
...
export function markStorageAdaptersInitialized(input: {
  ...
  input.startupTimer.mark("HawkNext storage adapters initialized", {
    context: {
...
export function markMcpAdapterInitialized(input: {
  ...
  input.startupTimer.mark("HawkNext MCP adapter initialized", {
    context: {
...
export function markRuntimeConstructed(input: {
  ...
  input.startupTimer.mark("HawkNext runtime constructed", {
    context: {
      hasInjectedModelAdapter: input.hasInjectedModelAdapter,
...
export function completeAppStartup(input: {
  ...
  input.startupTimer.complete("HawkNext app startup completed", {
    context: {
      sessionId: input.sessionId,
      workingDirectory: input.workingDirectory,
    },
