...
import { detectLocale, resolveLocale } from "@hawknext/i18n";
import type { RuntimeConfigPatch, SupportedLocale, UiLocale } from "@hawknext/contracts";
import type { HawkNextAppOptions } from "./types.js";

export function isMessageEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.HAWKNEXT_MESSAGE_ENABLED === "1" || env.HAWKNEXT_MESSAGE_ENABLED === "true";
}

export function createConfigCliOverrides(options: HawkNextAppOptions): RuntimeConfigPatch | undefined {
...
function detectAppLocale(options: HawkNextAppOptions): SupportedLocale | undefined {
