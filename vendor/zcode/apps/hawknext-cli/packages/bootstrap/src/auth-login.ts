import {
  ...
  createSharedHawkNextCredentialStore,
  createCliOAuthClient,
  createCliOAuthPollToken,
  openUrlInBrowser,
  SHARED_HAWKNEXT_CREDENTIAL_KEYS,
  type BrowserOpenResult,
  type SharedHawkNextCredentialStore,
  type CliOAuthClient,
  type CliOAuthInitData,
  type CliOAuthPollData,
  type CliOAuthUser,
} from "@hawknext/adapters";
import { createConfig } from "@hawknext/adapters/config";
import { createNodeHttpClientAdapter } from "@hawknext/adapters/http";
import type { EnvRecord } from "@hawknext/adapters/model";
import { buildHawkNextEndpointUrls, resolveRuntimeHawkNextEndpointOrigin } from "@hawknext/shared";
import {
...
export type CodingPlanProviderId = "cloud-cn" | "zai";

export interface LoginHawkNextCliOptions {
  providerId?: CodingPlanProviderId;
  abortSignal?: AbortSignal;
  apiKeyResolver?: ReturnType<typeof createCodingPlanApiKeyResolver>;
  baseUrl?: string;
  credentialStore?: SharedHawkNextCredentialStore;
  env?: EnvRecord;
...
export interface LoginHawkNextCliResult {
...
export type LoginCloudCnCodingPlanOptions = Omit<LoginHawkNextCliOptions, "providerId">;
export type LoginCloudCnCodingPlanResult = LoginHawkNextCliResult & { providerId: "cloud-cn" };
...
export interface LogoutHawkNextCliOptions {
  credentialStore?: SharedHawkNextCredentialStore;
  env?: EnvRecord;
}

export interface LogoutHawkNextCliResult {
  credentialsPath: string;
}
...
export async function hasConfiguredStandaloneCodingPlan(
  ...
  const credentialStore =
    options.credentialStore ?? createSharedHawkNextCredentialStore({ env: options.env });
  return hasStandaloneCodingPlanAccess(credentialStore, options.env ?? process.env);
}

export class HawkNextCliLoginError extends Error {
  readonly code:
    | "auth_failed"
    | "auth_timeout"
  ...
  constructor(
    code: HawkNextCliLoginError["code"],
    message: string,
    options: { cause?: unknown } = {},
  ) {
    super(message, options);
    this.name = "HawkNextCliLoginError";
    this.code = code;
  }
}

export async function loginHawkNextCli(
  options: LoginHawkNextCliOptions = {},
): Promise<LoginHawkNextCliResult> {
  ...
  const timeoutError = () =>
    new HawkNextCliLoginError("auth_timeout", "Authorization timed out. Please retry login.");
  ...
  try {
    ...
    const credentialStore = options.credentialStore ?? createSharedHawkNextCredentialStore({ env });
    ...
    const readyData = await pollUntilReady({
      ...
      createError: (code) =>
        code === "auth_timeout"
          ? timeoutError()
          : new HawkNextCliLoginError(code, "Authorization failed. Please retry login."),
    });
    ...
    try {
      if (providerId === "zai") {
        ...
        await credentialStore.saveMany({
          [SHARED_HAWKNEXT_CREDENTIAL_KEYS.activeProvider]: providerId,
          [SHARED_HAWKNEXT_CREDENTIAL_KEYS.hawknextJwtToken]: readyData.token,
          [SHARED_HAWKNEXT_CREDENTIAL_KEYS.cloudCnAccessToken]: readyData.accessToken,
          ...(readyData.refreshToken
            ? { [SHARED_HAWKNEXT_CREDENTIAL_KEYS.cloudCnRefreshToken]: readyData.refreshToken }
            : {}),
          [SHARED_HAWKNEXT_CREDENTIAL_KEYS.cloudCnUserInfo]: JSON.stringify({
      ...
      throw new HawkNextCliLoginError(
        "credential_write_failed",
    ...
    try {
      ...
      throw new HawkNextCliLoginError(
        "config_update_failed",
        "Login succeeded but updating HawkNext config failed.",
        { cause: error },
      );
    }
    return {
...
export async function loginCloudCnCodingPlan(
  options: LoginCloudCnCodingPlanOptions = {},
): Promise<LoginCloudCnCodingPlanResult> {
  return {
    ...(await loginHawkNextCli({ ...options, providerId: "cloud-cn" })),
    providerId: "cloud-cn",
  };
}
...
export async function configureCodingPlanApiKey(
  ...
  if (!apiKey) {
    throw new HawkNextCliLoginError("config_update_failed", "API key must not be empty.");
  }
  const credentialStore =
    options.credentialStore ?? createSharedHawkNextCredentialStore({ env: options.env });
...
export async function logoutHawkNextCli(
  options: LogoutHawkNextCliOptions = {},
): Promise<LogoutHawkNextCliResult> {
  const credentialStore =
    options.credentialStore ?? createSharedHawkNextCredentialStore({ env: options.env });
  ...
  const keys = [
    ...Object.values(SHARED_HAWKNEXT_CREDENTIAL_KEYS),
    ...identityKeys,
    ...dynamicApiKeyKeys,
  ];
...
async function persistStandaloneCodingPlanConnection(input: {
  ...
  readonly credentialStore: SharedHawkNextCredentialStore;
  ...
  const path =
    ...
    input.env[HAWKNEXT_PERSONAL_PROVIDER_CONFIG_FILE_ENV]?.trim() ??
...
function createOAuthClient(options: LoginHawkNextCliOptions, env: EnvRecord): CliOAuthClient {
  return createCliOAuthClient({
    baseUrl:
      options.baseUrl ?? buildHawkNextEndpointUrls(resolveCliHawkNextEndpointOrigin(env)).apiBaseUrl,
    providerId: options.providerId ?? "zai",
    httpClient: options.httpClient ?? createDefaultHttpClient(env),
  });
}

function resolveCliHawkNextEndpointOrigin(env: EnvRecord): string {
  return resolveRuntimeHawkNextEndpointOrigin(env);
}
