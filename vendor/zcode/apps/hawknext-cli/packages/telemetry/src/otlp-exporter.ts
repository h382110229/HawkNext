...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@hawknext/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@hawknext/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("hawknext.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("hawknext.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "hawknext.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "hawknext.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("hawknext.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("hawknext.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "hawknext.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("hawknext.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("hawknext.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("hawknext.model.attempt.tokens"),
    cardinalityView("hawknext.model.attempt.stream_stall.count"),
    cardinalityView("hawknext.telemetry.creation_drop.count"),
    cardinalityView("hawknext.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "hawknext.build.commit_id": resource.buildCommitId,
    "hawknext.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "hawknext.product.version": resource.productVersion,
    "hawknext.runtime.distribution": resource.runtimeDistribution,
    "hawknext.runtime.surface": resource.runtimeSurface,
    "hawknext.telemetry.schema_owner": "cli",
    "hawknext.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
