...
import type { ModelStatusSink } from "@hawknext/contracts/model";
...
export async function prepareModelTelemetryEnv(
  ...
  if (!resolveOtlpTraceEndpoint(env) || isExplicitlyDisabled(env.HAWKNEXT_MODEL_TELEMETRY_ENABLED)) {
    return env;
  }
  const existingInstallationId = normalizeTelemetryDeviceMid(env.HAWKNEXT_TELEMETRY_DEVICE_MID);
  const installationId =
    existingInstallationId ?? (await resolveStandaloneDeviceMid(env.HAWKNEXT_HOME?.trim()));
  const preparedEnv = installationId ? { ...env, HAWKNEXT_TELEMETRY_DEVICE_MID: installationId } : env;
...
async function createPreparedOwner(
  ...
  const resource: TelemetryResourceContext = {
    buildCommitId: normalizeBuildCommitId(options.buildCommitId ?? env.HAWKNEXT_BUILD_COMMIT_ID),
    cliVersion: normalizeVersion(options.cliVersion),
    deploymentEnvironment:
      env.HAWKNEXT_ENV ??
      env.HAWKNEXT_RUNTIME_ENV ??
      resourceAttributesFromEnv(env)["deployment.environment.name"],
    installationId: normalizeTelemetryDeviceMid(env.HAWKNEXT_TELEMETRY_DEVICE_MID),
    productVersion: normalizeVersion(options.productVersion ?? env.HAWKNEXT_APP_VERSION),
    runtimeDistribution:
      options.runtimeDistribution ??
      resolveRuntimeDistribution(env.HAWKNEXT_TELEMETRY_RUNTIME_DISTRIBUTION),
    runtimeSurface:
      options.runtimeSurface ?? resolveRuntimeSurface(env.HAWKNEXT_TELEMETRY_RUNTIME_SURFACE),
    serviceInstanceId: randomUUID(),
    serviceName: env.OTEL_SERVICE_NAME?.trim() || "hawknext-cli-agent",
  };
  try {
...
function resolveTelemetryIdentity(env: EnvRecord): TelemetryIdentitySnapshot {
  const userSubjectId = normalizeTelemetryId(env.HAWKNEXT_TELEMETRY_USER_SUBJECT_ID);
  const configuredState = env.HAWKNEXT_TELEMETRY_IDENTITY_STATE?.trim();
  const identityState =
    ...
    configuredState === "unknown"
      ...
      : userSubjectId
        ? "authenticated"
        : normalizeTelemetryDeviceMid(env.HAWKNEXT_TELEMETRY_DEVICE_MID)
          ? "anonymous"
          : "unknown";
...
async function resolveStandaloneDeviceMid(
  hawknextHome: string | undefined,
): Promise<string | undefined> {
  const stateFile = hawknextHome
    ? join(hawknextHome, "v2", "telemetry-state.json")
    : join(homedir(), ".hawknext", "v2", "telemetry-state.json");
