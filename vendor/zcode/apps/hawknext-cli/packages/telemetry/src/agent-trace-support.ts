...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@hawknext/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 HawkNext Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "hawknext.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "hawknext.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "hawknext.execution.turn_id": safeId(correlation.turnId),
    "hawknext.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "hawknext.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "hawknext.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "hawknext.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "hawknext.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "hawknext.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "hawknext.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
