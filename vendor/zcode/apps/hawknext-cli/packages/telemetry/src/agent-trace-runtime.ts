...
import type {
  ...
  ToolTraceStart,
} from "@hawknext/contracts/telemetry";
import {
  activeWriterContext,
  BaseSpanWriter,
  compactAttributes,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "hawknext.agent_turn.turn_number": integer(input.turnNumber),
            "hawknext.agent_turn.input_source": safeEnum(input.inputSource),
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "hawknext.agent_step.step_id": safeId(input.stepId),
            "hawknext.agent_step.step_index": integer(input.stepIndex),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "hawknext.execution.tool_call_id": safeId(input.toolCallId),
            "hawknext.tool_execution.tool_name": toolName,
            ...toolCompatibilityAttributes({
    ...
    return this.safeCreate(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent?.correlation),
            "hawknext.context_compaction.trigger": safeEnum(input.trigger),
            "hawknext.context_compaction.phase": safeEnum(input.phase),
            "hawknext.context_compaction.model_mode": safeEnum(input.modelMode),
            "hawknext.context_compaction.outer_attempt": integer(input.outerAttempt),
            "hawknext.context_compaction.max_attempts": integer(input.maxAttempts),
            "hawknext.context_compaction.triggering_step_index": integer(input.triggeringStepIndex),
            "hawknext.context_compaction.policy_context_window_tokens": finiteNonNegative(
              input.policyContextWindowTokens,
            ),
            "hawknext.context_compaction.threshold_tokens": finiteNonNegative(input.thresholdTokens),
            "hawknext.context_compaction.token_source": safeEnum(input.tokenSource),
            "hawknext.context_compaction.recovered_from_logical_call_id": safeId(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "hawknext.detached_operation.operation": safeEnum(input.operation),
            "hawknext.detached_operation.execution_kind": safeEnum(input.executionKind),
            "hawknext.detached_operation.trigger": safeEnum(input.trigger),
            "hawknext.detached_operation.target_kind": safeEnum(input.targetKind),
            "hawknext.detached_operation.goal_iteration": integer(input.goalIteration),
            "hawknext.detached_operation.chunk_index": integer(input.chunkIndex),
            "hawknext.detached_operation.chunk_count": integer(input.chunkCount),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "hawknext.execution.logical_call_id": safeId(input.logicalCallId),
            "hawknext.model_call.operation": safeEnum(input.operation),
            "hawknext.model_call.streaming": input.streaming,
            "hawknext.model_call.model_role": safeEnum(input.modelRole),
            "hawknext.model_call.requested_provider_id": safeString(input.requested.providerId, 128),
            "hawknext.model_call.requested_model": safeString(input.requested.requestedModel, 128),
            "hawknext.model_call.reasoning_capability": safeEnum(input.requested.reasoning.capability),
            "hawknext.model_call.reasoning_requested_state": safeEnum(
              input.requested.reasoning.requestedState,
            ),
            "hawknext.model_call.reasoning_requested_control": safeEnum(
              input.requested.reasoning.requestedControl,
            ),
            "hawknext.model_call.reasoning_requested_level": safeString(
              input.requested.reasoning.requestedLevel,
              128,
            ),
            "hawknext.model_call.reasoning_requested_budget_tokens": integer(
              input.requested.reasoning.requestedBudgetTokens,
            ),
            "hawknext.model_call.call_cause": safeEnum(input.callCause),
            "hawknext.model_call.previous_logical_call_id": safeId(input.previousLogicalCallId),
          }),
          context: parentContext,
          correlation: parent?.correlation,
          parent,
    ...
    return this.safeCreate(
      ...
      () => {
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent.correlation),
            "hawknext.execution.tool_call_id": safeId(parent.toolCallId),
            "hawknext.command_execution.safe_name": safeString(input.safeName, 128),
            "hawknext.command_execution.category": safeEnum(input.category),
            "hawknext.command_execution.command_count": integer(input.commandCount),
            "hawknext.command_execution.shell_kind": safeEnum(input.shellKind),
            "hawknext.command_execution.sandboxed": input.sandboxed,
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "hawknext.execution.tool_call_id": safeId(parent.toolCallId),
            "hawknext.execution.logical_call_id": parentWriter.logicalCallId,
            "hawknext.execution.model_operation": parentWriter.operation,
            "hawknext.execution.model_role": parentWriter.modelRole,
            "hawknext.model_attempt.request_id": safeId(input.requestId),
            "hawknext.model_attempt.attempt_number": integer(input.attemptNumber),
            "hawknext.model_attempt.max_attempts": integer(input.maxAttempts),
            "hawknext.model_attempt.attempt_cause": safeEnum(input.attemptCause),
            "hawknext.model_attempt.previous_request_id": safeId(input.previousRequestId),
            "hawknext.model_attempt.retry_delay_ms": finiteNonNegative(input.retryDelayMs),
            "hawknext.model_attempt.provider_id": safeString(target.providerId, 128),
            "hawknext.model_attempt.provider_kind": safeEnum(target.providerKind),
            "hawknext.model_attempt.provider_origin": safeString(target.providerOrigin),
            "hawknext.model_attempt.provider_route": safeString(target.providerRoute),
            "hawknext.model_attempt.requested_model": safeString(target.requestedModel, 128),
            "hawknext.model_attempt.transport": safeEnum(input.transport),
            "hawknext.model_attempt.api_operation": safeEnum(input.apiOperation),
            "hawknext.model_attempt.reasoning_capability": safeEnum(target.reasoning.capability),
            "hawknext.model_attempt.reasoning_requested_state": safeEnum(
              target.reasoning.requestedState,
            ),
            "hawknext.model_attempt.reasoning_requested_control": safeEnum(
              target.reasoning.requestedControl,
            ),
            "hawknext.model_attempt.reasoning_requested_level": safeString(
              target.reasoning.requestedLevel,
              128,
            ),
            "hawknext.model_attempt.reasoning_requested_budget_tokens": integer(
...
function causationLink(
  ...
  return {
    attributes: {
      "hawknext.link.relation": relation,
    },
...
class TurnWriter extends TrackedBaseWriter implements AgentTurnSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() => this.setAttribute("hawknext.agent_turn.result_type", resultType));
  }
...
class StepWriter extends TrackedBaseWriter implements AgentStepSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() =>
      this.setAttribute("hawknext.agent_step.terminal_reason", terminalReason),
    );
  }
...
class ToolWriter extends TrackedBaseWriter implements ToolExecutionSpanWriter {
  ...
  setPermissionDecision(decision: "granted" | "denied" | "not_required"): void {
    this.setAttribute("hawknext.tool_execution.permission_decision", decision);
  ...
  setOutputBytes(bytes: number): void {
    this.setAttribute("hawknext.tool_execution.output_bytes", finiteNonNegative(bytes));
  }

  setOutputTruncated(truncated: boolean): void {
    this.setAttribute("hawknext.tool_execution.output_truncated", truncated);
  }
  ...
  finishDenied(reason: "user_denied" | "policy_denied" | "unavailable" | "unknown"): void {
    this.finishDomainOutcomeIfOpen("denied", () =>
      this.setAttribute("hawknext.tool_execution.permission_denial_reason", reason),
    );
  }
...
class CommandWriter extends TrackedBaseWriter implements CommandExecutionSpanWriter {
  ...
  markFirstOutput(): void {
    ...
    this.setAttribute("hawknext.command_execution.first_output_ms", elapsed);
  ...
  setExitCode(exitCode: number): void {
    ...
    this.setAttribute("hawknext.command_execution.exit_code", normalized);
  ...
  setSignal(signal: string): void {
    ...
    this.setAttribute("hawknext.command_execution.signal", normalized);
    this.setAttributes(commandCompatibilityAttributes({ signal: normalized }));
  }

  setOutputBytes(bytes: number): void {
    this.setAttribute("hawknext.command_execution.output_bytes", finiteNonNegative(bytes));
  }

  setTimedOut(timedOut: boolean): void {
    this.setAttribute("hawknext.command_execution.timed_out", timedOut);
  }
...
class CompactionWriter extends TrackedBaseWriter implements ContextCompactionSpanWriter {
  ...
  setInputTokens(tokens: number): void {
    this.setAttribute("hawknext.context_compaction.input_tokens", finiteNonNegative(tokens));
  }

  setOutputTokens(tokens: number): void {
    this.setAttribute("hawknext.context_compaction.output_tokens", finiteNonNegative(tokens));
  }
...
class DetachedWriter extends TrackedBaseWriter implements DetachedOperationSpanWriter {
  ...
  setResultType(resultType: "text" | "boolean" | "metadata" | "other"): void {
    this.setAttribute("hawknext.detached_operation.result_type", resultType);
  }
...
class ModelAttemptWriter extends TrackedBaseWriter implements ModelAttemptSpanWriter {
  ...
  setProviderRequestId(requestId: string): void {
    this.setAttribute("hawknext.model_attempt.provider_request_id", safeString(requestId, 256));
  }

  setResponseModel(model: ResponseModelTelemetryDescriptor): void {
    const value = safeString(model.model, 128);
    this.setAttribute("hawknext.model_attempt.response_model", value);
  ...
  setEffectiveReasoningState(state: ModelReasoningState): void {
    this.setAttribute("hawknext.model_attempt.reasoning_effective_state", safeEnum(state));
  }

  setEffectiveReasoningControl(control: ModelReasoningControlType): void {
    this.setAttribute("hawknext.model_attempt.reasoning_effective_control", safeEnum(control));
  }

  setEffectiveReasoningLevel(level: string): void {
    this.setAttribute("hawknext.model_attempt.reasoning_effective_level", safeString(level, 128));
  }

  setEffectiveReasoningBudgetTokens(tokens: number): void {
    this.setAttribute(
      "hawknext.model_attempt.reasoning_effective_budget_tokens",
  ...
  setFinishReason(reason: ModelFinishReason): void {
    ...
    this.setAttribute("hawknext.model_attempt.finish_reason", value);
    this.setAttributes(modelResponseCompatibilityAttributes({ finishReason: value }));
  }

  setInputTokens(tokens: number): void {
    this.setUsage("inputTokens", "input", tokens, "hawknext.model_attempt.input_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ inputTokens: tokens }));
  }

  setOutputTokens(tokens: number): void {
    this.setUsage("outputTokens", "output", tokens, "hawknext.model_attempt.output_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ outputTokens: tokens }));
  }

  setReasoningTokens(tokens: number): void {
    this.setUsage("reasoningTokens", "reasoning", tokens, "hawknext.model_attempt.reasoning_tokens");
  }

  setCacheReadTokens(tokens: number): void {
    this.setUsage("cacheReadTokens", "cache_read", tokens, "hawknext.model_attempt.cache_read_tokens");
  }
  ...
  setCacheWriteTokens(tokens: number): void {
    this.setUsage(
      ...
      "cache_write",
      tokens,
      "hawknext.model_attempt.cache_write_tokens",
    );
  }

  setStreamOutputCommitted(committed: boolean): void {
    this.setAttribute("hawknext.model_attempt.stream_output_committed", committed);
  }

  setHttpStatusCode(statusCode: number): void {
    const normalized = integer(statusCode);
    this.setAttribute("hawknext.model_attempt.http_status_code", normalized);
    this.setAttributes(httpResponseCompatibilityAttributes(statusCode));
  }

  setProviderErrorCode(code: string): void {
    this.setAttribute("hawknext.model_attempt.provider_error_code", safeString(code, 128));
  }

  setProviderErrorMessage(message: string): void {
    this.setAttribute("hawknext.model_attempt.provider_error_message", sanitizeErrorMessage(message));
  }

  setRetryAfterMs(delayMs: number): void {
    this.setAttribute("hawknext.model_attempt.retry_after_ms", finiteNonNegative(delayMs));
  }
  ...
  markFirstProviderEvent(): void {
    ...
    this.setAttribute("hawknext.model_attempt.time_to_first_provider_event_ms", elapsed);
  ...
  markFirstContent(): void {
    ...
    this.setAttribute("hawknext.model_attempt.time_to_first_content_ms", elapsed);
  ...
  markFirstText(): void {
    ...
    this.setAttribute("hawknext.model_attempt.time_to_first_text_ms", elapsed);
...
function lifecycleKeys(spanName: string): WriterLifecycleKeys {
  const prefix = `hawknext.${spanName}`;
  return {
