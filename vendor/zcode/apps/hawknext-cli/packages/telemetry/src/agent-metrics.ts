...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "hawknext.agent.turn.duration"),
      agent_step: duration(meter, "hawknext.agent.step.duration"),
      tool_execution: duration(meter, "hawknext.tool.execution.duration"),
      command_execution: duration(meter, "hawknext.command.execution.duration"),
      context_compaction: duration(meter, "hawknext.context.compaction.duration"),
      detached_operation: duration(meter, "hawknext.detached.operation.duration"),
      model_call: duration(meter, "hawknext.model.call.duration"),
      model_attempt: duration(meter, "hawknext.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("hawknext.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("hawknext.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "hawknext.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "hawknext.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "hawknext.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("hawknext.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "hawknext.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "hawknext.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("hawknext.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("hawknext.telemetry.abandoned.count", {
