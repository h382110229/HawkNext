...
import { resolveWorkspaceHookTimeoutMs } from "@hawknext/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "hawknext-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 HawkNext camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  const set: Record<string, string> = {
    ...
    HAWKNEXT_PROJECT_DIR: input.cwd || workingDirectory,
    HAWKNEXT_SESSION_ID: input.sessionId,
  };
  ...
  return {
    set: {
      ...
      HAWKNEXT_PLUGIN_DATA: plugin.dataPath,
      HAWKNEXT_PLUGIN_ID: plugin.id,
      HAWKNEXT_PLUGIN_NAME: plugin.name,
      HAWKNEXT_PLUGIN_ROOT: plugin.rootPath,
    },
  };
}
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.HAWKNEXT_PLUGIN_DATA = plugin.dataPath;
    replacements.HAWKNEXT_PLUGIN_ROOT = plugin.rootPath;
  }
  return value.replace(
    /\$\{(CLAUDE_CODE_SESSION_ID|CLAUDE_PLUGIN_DATA|CLAUDE_PLUGIN_ROOT|CLAUDE_PROJECT_DIR|CLAUDE_SESSION_ID|CLAUDE_SKILL_DIR|HAWKNEXT_PLUGIN_DATA|HAWKNEXT_PLUGIN_ROOT|HAWKNEXT_PROJECT_DIR|HAWKNEXT_SESSION_ID|HAWKNEXT_SKILL_DIR)\}/gu,
    (_match, key: string) => {
      if (key === "CLAUDE_SKILL_DIR" || key === "HAWKNEXT_SKILL_DIR") {
