import { CompactTrigger } from "@hawknext/contracts";
...
import { modelMessageContentBlockToText, modelMessageContentToText } from "@hawknext/contracts";
...
function stringifyToolCallInputForTokenEstimate(input: unknown): string {
  try {
    ...
    // HawkNext 的模型输入仍可能包含未知内容；异常输入不能让本地预算估算中断 compact。
