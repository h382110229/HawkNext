...
export function computeOfficialCuaServerNames(
  ...
  const expectedAuthority = getCapturedHawkNextCuaBrokerCredentials().pluginAuthority;
  ...
  for (const [name, config] of Object.entries(servers)) {
    ...
    if (
      config.env?.[HAWKNEXT_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() !==
        HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID ||
      config.env?.[HAWKNEXT_CUA_PLUGIN_AUTHORITY_ENV_KEY]?.trim() !== expectedAuthority
    ) {
      continue;
    }
    names.add(name);
