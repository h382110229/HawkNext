...
import { RESPOND_TO_COORDINATOR_TOOL_NAME } from "@hawknext/contracts";
import type { SubagentRunOptions } from "@hawknext/contracts";
import {
  defaultScheduler,
  PermissionService,
    ...
    runExploreAgent: async (request, options) => {
      ...
      const childRuntime = new AgentRuntime(
        request.sessionId,
        {
          ...
          // 开启 modelStreaming，子请求仍会退回 generateText。部分 OfficialModel-compatible 端点在
          ...
          agentName: `hawknext-${request.agentType}`,
