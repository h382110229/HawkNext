...
import type { FileSystemPort, Logger, TraceContext } from "@hawknext/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  return input.scope === "project"
    ? join(workspace, ".hawknext", "agent-memory", agentKey)
    : join(workspace, ".hawknext", "agent-memory-local", agentKey);
}
