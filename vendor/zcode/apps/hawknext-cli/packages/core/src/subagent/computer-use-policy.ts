...
import {
  HAWKNEXT_CUA_OFFICIAL_MCP_NAMESPACE_NAME as HAWKNEXT_CUA_OFFICIAL_MCP_SERVER_NAME,
  HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID,
} from "@hawknext/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@hawknext/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(HAWKNEXT_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === HAWKNEXT_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(HAWKNEXT_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === HAWKNEXT_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === HAWKNEXT_CUA_OFFICIAL_PLUGIN_ID,
  );
