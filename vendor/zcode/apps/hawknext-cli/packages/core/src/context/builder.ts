...
import type { ModelInputMessage } from "@hawknext/contracts";
import type {
...
export class ContextBuilder {
  ...
  build(): ContextBuildResult {
    ...
    // 「You are HawkNext, an interactive coding agent」对一个
    ...
    if (!hasCustomSystemPrompt) {
      if (!isWorkflowActor && this.config.presentationSurface === "hawknext_desktop") {
  ...
  private assembleSystemMessages(sections: ContextSection[]): ModelInputMessage[] {
    ...
    if (dynamicSystemContent) {
      messages.push({
        role: "system",
        // HawkNext by design：Main Agent 的 dynamic system block 自带左边界，所有 provider 保持一致。
