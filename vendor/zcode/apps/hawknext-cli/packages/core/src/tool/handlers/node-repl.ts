...
import type { SessionId } from "@hawknext/contracts";
...
/**
 * 每个 hawknext session 一个持久 NodeReplSession，跨多次 js 调用保持内存状态（globalThis）。
...
function isBrowserSurfaceSideEffect(command: import("@hawknext/contracts").BrowserCommand): boolean {
...
function isAutoScreenshotTriggerCommand(
  command: import("@hawknext/contracts").BrowserCommand,
): boolean {
  if (
        ...
        execute: async (browserId, browserGeneration, command) => {
          ...
          if (result.meta) {
            ...
            onBrowserResponseMeta({
              "hawknext/browserUse": true,
              "hawknext/toolSurface": {
                kind: "browserUse",
              ...
              ...(result.ok && meta.tabId && isAutoScreenshotTriggerCommand(command)
                ? {
                    "hawknext/browserTurnScreenshot": {
