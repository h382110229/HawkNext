...
import type { ModelCatalogEntry, ModelSelection } from "@hawknext/contracts";
import {
  HAWKNEXT_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@hawknext/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(HAWKNEXT_MODEL_REASONING_SEPARATOR, searchFrom);
