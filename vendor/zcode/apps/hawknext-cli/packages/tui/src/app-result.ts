import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@hawknext/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getHawkNextCopy(result.locale ?? input.fallback.locale).tui;
