...
import type { PlaywrightChromiumModule } from "@hawknext/adapters/browser";
...
const ASSET_PREFIX = "hawknext-playwright-runtime/";
...
export async function loadCliPlaywrightChromium(): Promise<PlaywrightChromiumModule> {
  ...
  const require = createRequire(join(runtimeDirectory, "hawknext-playwright-loader.cjs"));
...
function cacheBaseDirectory(): string {
  ...
  if (platform() === "darwin" && home) {
    return join(home, "Library", "Caches", "hawknext", "sea-assets");
  }
  if (platform() === "win32") {
    return join(
