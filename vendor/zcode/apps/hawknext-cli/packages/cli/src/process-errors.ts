...
import {
  HAWKNEXT_PROCESS_DIAGNOSTIC_PREFIX,
  ...
  type HawkNextProcessDiagnostic,
...
function writeProcessErrorDiagnostic(
  ...
  try {
    const detail = formatProcessError(reason).slice(0, HAWKNEXT_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS);
    const diagnostic: HawkNextProcessDiagnostic = {
      version: 1,
      errorId: randomUUID(),
      kind,
    ...
    stderr.write(
      `${HAWKNEXT_PROCESS_DIAGNOSTIC_PREFIX}${JSON.stringify(diagnostic)}\n[hawknext] process error kind=${kind} origin=${origin}\n${detail}\n`,
    );
  } catch {
    // 诊断输出不能再次击穿进程级异常边界。
  }
