...
import {
  ...
  normalizeHawkNextRuntimeEnv,
  sanitizeHawkNextRuntimeEnv,
  sanitizeHawkNextRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeHawkNextRuntimeEnv(env),
    ...buildHawkNextToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildHawkNextToolEnvPassthroughEnv(env);
  sanitizeHawkNextRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[HAWKNEXT_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): HawkNextRuntimeEnv {
  const explicit = normalizeHawkNextRuntimeEnv(env[HAWKNEXT_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.HAWKNEXT_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.HAWKNEXT_BETA === "1" || env.HAWKNEXT_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])hawknext-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.HAWKNEXT_STORAGE_DIR = join(homedir(), ".hawknext-beta");
}
