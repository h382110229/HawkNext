...
// 品牌类型（SessionId）和 createHawkNextApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createHawkNextApp ?? bootstrapModule?.createHawkNextApp;
  if (!createAppFactory) throw new Error("HawkNext app factory is unavailable.");
  ...
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownHawkNextTelemetry ?? bootstrapModule?.shutdownHawkNextTelemetry;
  }
