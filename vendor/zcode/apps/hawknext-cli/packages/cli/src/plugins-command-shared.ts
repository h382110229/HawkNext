...
import type { listHawkNextPlugins } from "@hawknext/bootstrap";
import { createInterface } from "node:readline";
import type { GlobalOptions, RunContext } from "@hawknext/shared-types";
import type { CliEnv } from "./env.js";

export type BootstrapModule = typeof import("@hawknext/bootstrap");
export type PluginListOutcome = ReturnType<typeof listHawkNextPlugins>;
 ...
 * 避免 `hawknext plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addHawkNextPluginMarketplace",
  getPluginsOverview: "getHawkNextPluginsOverview",
  installPlugin: "installHawkNextMarketplacePlugin",
  listPlugins: "listHawkNextPlugins",
  removeMarketplace: "removeHawkNextPluginMarketplace",
  setPluginEnabled: "setHawkNextPluginEnabled",
  uninstallPlugin: "uninstallHawkNextMarketplacePlugin",
  updateMarketplace: "updateHawkNextPluginMarketplace",
  updatePlugin: "updateHawkNextMarketplacePlugin",
  validatePluginPath: "validateHawkNextPluginPath",
...
/** `hawknext plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@hawknext/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by hawknext. Use: user, project");
  }
