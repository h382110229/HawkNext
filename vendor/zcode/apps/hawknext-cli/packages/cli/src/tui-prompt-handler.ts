import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@hawknext/adapters/config";
import type { SessionEvent } from "@hawknext/contracts";
import type { HawkNextAppOptions } from "@hawknext/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@hawknext/i18n";
import type { TuiRequestPermission } from "@hawknext/tui";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createHawkNextApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<HawkNextAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createHawkNextApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createHawkNextApp"]>>>;
    try {
  ...
  const commandCenter = createCommandCenter({
    ...
    loginCloudCn: (options) => loginCloudCnForTui(deps, options),
