...
import { createDenyPermissionBroker } from "@hawknext/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  HawkNextAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `hawknextSessionEventTypeSchema`（`shared/src/hawknext-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `hawknextSessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createHawkNextApp` 注入点边界）。
 *
