import type { TuiSessionMetadata } from "@hawknext/tui";
...
export async function listSkillsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listHawkNextSkillsForTui = deps.listSkills ?? (await loadBootstrapModule()).listHawkNextSkills;
  return await listHawkNextSkillsForTui({
    env,
    logger: deps.logger,
    workingDirectory,
  });
...
export async function listSessionsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listHawkNextSessionsForTui =
    deps.listSessions ?? (await loadBootstrapModule()).listHawkNextSessions;
  const sessions = await listHawkNextSessionsForTui({
