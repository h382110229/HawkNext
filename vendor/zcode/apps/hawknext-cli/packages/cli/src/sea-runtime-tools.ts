...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@hawknext/shared/runtime-tool-runtime";
...
const assetPrefix = "hawknext-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".hawknext-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.HAWKNEXT_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || join(homedir(), ".hawknext");
