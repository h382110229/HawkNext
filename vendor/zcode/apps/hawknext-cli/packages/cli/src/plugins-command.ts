...
import type { SetHawkNextPluginEnabledResult, HawkNextPluginInstallData } from "@hawknext/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: hawknext plugins <command> [options]
...
Scopes: user (default), project. \`hawknext plugin\` is an alias of \`hawknext plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart hawknext to apply.\n`,
  );
...
function reportInstallOutcome(
  ctx: RunContext,
  options: GlobalOptions,
  result: HawkNextPluginInstallData,
  pluginId: string,
): number {
  const failed = hasErrors(result.diagnostics);
  if (options.json) {
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetHawkNextPluginEnabledResult; error?: string }> = [];
