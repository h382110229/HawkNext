...
import { createNodeLoggerFactory } from "@hawknext/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@hawknext/core";
import { color, formatJson, supportsColor } from "@hawknext/core";
import { getHawkNextCopy, isUiLocale, type UiLocale } from "@hawknext/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@hawknext/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getHawkNextCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "hawknext_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("hawknext doctor", colors)}\n`);
...
const runHawkNextProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 HawkNext agent transport closed。
    ...
    const runProtocolAgent =
      deps.runHawkNextProtocolAgent ?? (await loadBootstrapModule()).runHawkNextProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("hawknext").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runHawkNextProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
