...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listHawkNextCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: hawknext commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listHawkNextCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@hawknext/bootstrap"));
  return (await bootstrap()).inspectHawkNextCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: HawkNextCustomCommandInspection,
  options: GlobalOptions,
): string {
