...
type BootstrapModule = typeof import("@hawknext/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listHawkNextSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: hawknext skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listHawkNextSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@hawknext/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectHawkNextSkill;
...
function formatHumanSkillInspection(
  inspection: HawkNextSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: HawkNextSkillInspection, cwd: string): string =>
  formatJson({
    cwd,
