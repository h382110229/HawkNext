...
import { formatJson, type PresentationSurface } from "@hawknext/core";
import type { RunContext, GlobalOptions } from "@hawknext/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createHawkNextApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createHawkNextApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createHawkNextApp ?? bootstrapModule?.createHawkNextApp;
    if (!createApp) throw new Error("HawkNext app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareHawkNextTelemetryEnv ?? bootstrapModule?.prepareHawkNextTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownHawkNextTelemetry ?? bootstrapModule?.shutdownHawkNextTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.HAWKNEXT_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `hawknext -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveHawkNextCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedHawkNextSlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listHawkNextCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadHawkNextCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: hawknext hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
