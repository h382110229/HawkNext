...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@hawknext/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.HAWKNEXT_STORAGE_DIR?.trim() || join(homedir(), ".hawknext");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "hawknext-clipboard-"));
