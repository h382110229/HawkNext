...
export async function runLoginCommand(
  ...
  try {
    ...
    if (args.length > 1 || (providerId !== "zai" && providerId !== "cloud-cn")) {
      throw new Error("Usage: hawknext login [zai|cloud-cn] [--no-browser]");
    }
...
function writeAuthorizeUrl(
  ...
  target.write(
    `Opening browser for ${providerId === "cloud-cn" ? "CloudCN" : ""} authorization.\nFallback URL:\n${authorizeUrl}\n`,
  );
}
