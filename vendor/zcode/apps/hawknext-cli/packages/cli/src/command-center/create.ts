import type { TuiSubmitPrompt } from "@hawknext/tui";
import {
      ...
      if (command.name === "login") {
        ...
        if (command.args === "zai-coding-plan") {
          if (!deps.login) {
            return {
              mode: deps.getMode?.(),
              response: " Coding Plan login is not available in this client.",
            };
          }
        ...
        if (command.args === "cloud-cn-coding-plan") {
          if (!deps.loginCloudCn) {
            return {
              mode: deps.getMode?.(),
              response: "CloudCN Coding Plan login is not available in this client.",
            };
          }
        ...
        return {
          mode: deps.getMode?.(),
          response:
            "Usage: /login [zai-coding-plan|cloud-cn-coding-plan|zai-coding-plan-api-key <api-key>|cloud-cn-coding-plan-api-key <api-key>]",
        };
      }
