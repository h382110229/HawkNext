...
export const DEFAULT_DESTINATION_ROOT = resolve(DEFAULT_MOUNT_ROOT, "hawknext", "deps");
export const RELEASE_DIRECTORY_PREFIX = "hawknext-cli-";
...
const usage = `Usage:
  ...
  node packages/cli/scripts/upload-sea-smb.mjs --dest-root /Volumes/shared/hawknext/deps
...
Options:
  ...
  --dest-root <path>  Mounted SMB release root. Defaults to /Volumes/shared/hawknext/deps.
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing SEA binaries in ${distDir}: ${missing.join(", ")}. ` +
        "Run `pnpm --filter @hawknext/cli build:sea` first.",
    );
  }
