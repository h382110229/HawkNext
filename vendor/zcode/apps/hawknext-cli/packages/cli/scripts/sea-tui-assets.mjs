...
export const seaTuiAssetPrefix = "hawknext-tui-runtime/";
...
const workspacePackageParentDirectories = (root) => {
  ...
  return [
    ...
    // TUI runtime 闭包会经由 @hawknext/contracts 依赖仓库根的 @hawknext/shared；
    // 只扫描 apps/hawknext-cli 子 workspace 会把这个合法 workspace 误判为缺失。
    resolve(repositoryRoot, "packages"),
  ];
};

const runtimePackageNames = async ({ root, target, workspacePackageDirectories }) => {
  const tuiDirectory = workspacePackageDirectories.get("@hawknext/tui");
  if (!tuiDirectory) {
    throw new Error("Missing @hawknext/tui workspace package.");
  }
  const queue = [
    { fromDirectory: tuiDirectory, packageName: "@hawknext/tui" },
  ...
  while (queue.length > 0) {
    ...
    const assetPackagePath = placeRuntimePackage({
      ...
      packageDirectory,
      placements,
      fromAssetPath: entry.fromAssetPath ?? "node_modules/@hawknext/tui",
    });
    ...
    // SEA 缓存目录无法解析 pnpm workspace 链接；旧的 @hawknext 白名单漏掉 i18n
