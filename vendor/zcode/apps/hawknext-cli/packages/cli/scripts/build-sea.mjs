...
const cliBundle = resolve(dist, "hawknext.cjs");
const seaBlobForTarget = (target) => resolve(dist, `hawknext-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "hawknext-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/hawknext.cjs",
        output: `dist/hawknext-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "hawknext-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        HAWKNEXT_STORAGE_DIR: storageRoot,
      };
      delete env.HAWKNEXT_BFS_BINARY;
      delete env.HAWKNEXT_RG_BINARY;
      delete env.HAWKNEXT_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/hawknext.cjs. Run `pnpm build` first.");
  }
