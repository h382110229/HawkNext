...
const traceHeaderNames = ["x-hawknext-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          HAWKNEXT_HTTP_PROXY: url,
          HAWKNEXT_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.hawknextCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.HAWKNEXT_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.HAWKNEXT_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.HAWKNEXT_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.HAWKNEXT_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.HAWKNEXT_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
