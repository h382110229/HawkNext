...
} from "./official-model-response-assembler.js";
...
async function writeMockResponse(input: {
  ...
  if (mockResponse === undefined) {
    throw new Error(`Missing mock OfficialModel response for request ${input.requestIndex}.`);
  }
...
function parseRequestBody(bodyText: string): JsonObject {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("Provider proxy only supports JSON OfficialModel request bodies.");
  }
