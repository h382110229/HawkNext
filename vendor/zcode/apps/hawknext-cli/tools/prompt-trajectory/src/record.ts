...
import { startOpenAiProviderProxy } from "./official-model-provider-proxy.js";

interface HawkNextBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createHawkNextApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface HawkNextContractsModule {
...
interface HawkNextProviderModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadHawkNextModules();
    ...
    const app = await bootstrap.createHawkNextApp({
...
async function loadHawkNextModules(): Promise<{
  bootstrap: HawkNextBootstrapModule;
  contracts: HawkNextContractsModule;
  provider: HawkNextProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<HawkNextBootstrapModule>,
      import(contractsPath) as Promise<HawkNextContractsModule>,
      import(providerConfigPath) as Promise<Omit<HawkNextProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<HawkNextProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built hawknext-cli packages. Run `pnpm --filter @hawknext/bootstrap^... build && pnpm --filter @hawknext/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
