...
export class MessageLedger {
  ...
  async recordRequestBody(body: JsonObject): Promise<number> {
    ...
    if (!messages) {
      throw new Error("OfficialModel request body must contain a messages or input array.");
    }
    ...
    for (let messageIndex = 0; messageIndex < messages.length; messageIndex += 1) {
      ...
      if (!isOpenAiMessage(message)) {
        throw new Error(`OfficialModel request message ${messageIndex} is missing role.`);
      }
      await this.write({
        kind: "message",
        message,
