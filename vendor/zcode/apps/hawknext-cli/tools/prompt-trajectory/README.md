# @hawknext/prompt-trajectory

OfficialModel protocol trajectory recorder for inspecting hawknext-cli prompt assembly.
...
```bash
pnpm --filter @hawknext/bootstrap^... build
pnpm --filter @hawknext/bootstrap build

pnpm --filter @hawknext/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/hawknext-prompt-trajectory/basic-live

pnpm --filter @hawknext/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @hawknext/prompt-trajectory derive -- \
  --out /tmp/hawknext-prompt-trajectory/basic-live

pnpm --filter @hawknext/prompt-trajectory model-io -- \
  --input ~/.hawknext/cli/debug/model-io-<session>.jsonl \
  --out /tmp/hawknext-prompt-trajectory/model-io-session
```
...
uses the same hawknext model config resolution as the CLI. The upstream request is
...
out/promptYYYYMMDD-HHMMSS/trajectories/*.official-model_request_body.json
out/promptYYYYMMDD-HHMMSS/trajectories/*.anthropic_request_body.json
```

The OfficialModel-compatible snapshot keeps the legacy comparison format. The Anthropic
...
`model-io` reads a real HawkNext `model-io-*.jsonl` file and turns the main
conversation into a reusable Anthropic trajectory:

```text
out/<run>/anthropic_trajectory.json
out/<run>/manifest.json
out/<run>/trajectories/*.official-model_request_body.json
