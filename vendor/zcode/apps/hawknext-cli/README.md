...
- `npm run build` produces `dist/hawknext.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
hawknext plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.hawknext/cli/plugins`:
...
- `marketplaces/hawknext-plugins-official/`: bundled and CDN partitions plus the merged metadata for the single official marketplace.

This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and HawkNext Guide content plugins are default-enabled and appear as `browser-use@hawknext-plugins-official`, `document-skills@hawknext-plugins-official`, `skill-creator@hawknext-plugins-official`, and `hawknext-guide@hawknext-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@hawknext-plugins-official`, `android-emulator@hawknext-plugins-official`, and `restore-legacy-sessions@hawknext-plugins-official`, are discovered by hawknext but stay disabled until the user enables them.

```sh
hawknext plugins list
hawknext plugins enable ios-simulator
hawknext plugins disable browser-use
hawknext plugins enable restore-legacy-sessions
hawknext plugins disable ios-simulator
```
...
MCP config can live directly in `.hawknext-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current hawknext plugin surface:
...
Example `.hawknext-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${HAWKNEXT_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${HAWKNEXT_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${HAWKNEXT_PLUGIN_DATA}",
...
- `${HAWKNEXT_PLUGIN_ROOT}`
- `${HAWKNEXT_PLUGIN_DATA}`
- `${HAWKNEXT_PROJECT_DIR}`
- `${user_config.key}`
- `${HAWKNEXT_SOME_ENV}`

Only environment variables with the `HAWKNEXT_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.

### Recommended Layout

```txt
my-plugin/
  .hawknext-plugin/plugin.json
  .mcp.json
  skills/
    my-skill/SKILL.md
  commands/
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting hawknext-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

hawknext reads MCP servers from the main JSON config. The default user config path is `~/.hawknext/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits hawknext's environment plus any `env` overrides.
...
hawknext reads hooks from the same main JSON config file as MCP, usually `~/.hawknext/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
