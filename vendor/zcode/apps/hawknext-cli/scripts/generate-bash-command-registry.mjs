...
if (check) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "hawknext-bash-registry-"));
  ...
  try {
    ...
    if (!actual.equals(expected)) {
      throw new Error(
        "Generated Bash command registry is stale. Run `pnpm --dir apps/hawknext-cli registry:generate`.",
      );
    }
  } finally {
