...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "hawknext-native-search-build-"));
  ...
  console.log("==> HawkNext native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DHAWKNEXT_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DHAWKNEXT_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DHAWKNEXT_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DHAWKNEXT_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DHAWKNEXT_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DHAWKNEXT_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DHAWKNEXT_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DHAWKNEXT_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(
