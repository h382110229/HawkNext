...
assert.ok(archive, "Usage: node scripts/hawknext-distribution-smoke.mjs <archive.tar.gz>");
const directory = await realpath(await mkdtemp(join(tmpdir(), "hawknext-release-smoke-")));
const root = join(directory, "hawknext");
const runner = join(root, "bin/hawknext.mjs");
const workspace = join(directory, "workspace");
const env = {
  ...process.env,
  HAWKNEXT_DATA_BASE_DIR: join(directory, "data"),
  NODE_PATH: "",
  NODE_OPTIONS: "",
  TERM: "xterm-256color",
};
...
try {
  ...
  await writeFile(
    runtimeCheck,
    'import { runTui } from "@hawknext/tui"; if (typeof runTui !== "function") throw new Error("Missing TUI export"); console.log("tui-runtime-ok");',
  );
  ...
  await until(
    () => /HawkNext/.test(screen) && /(?:登录|\/login|输入提示词|Type a prompt)/i.test(screen),
