...
BASE_URL="\${HAWKNEXT_DIST_BASE_URL:-${baseUrl}}"
INSTALL_DIR="\${HAWKNEXT_DIST_HOME:-$HOME/.hawknext/runtime}"
BIN_DIR="\${HAWKNEXT_DIST_BIN_DIR:-$HOME/.local/bin}"

need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "hawknext install requires $1" >&2
    exit 1
  fi
}
...
cat > "$BIN_DIR/hawknext" <<SH
#!/usr/bin/env sh
exec node "$INSTALL_DIR/current/bin/hawknext.mjs" "\\$@"
SH
chmod +x "$BIN_DIR/hawknext"

echo "HawkNext $VERSION installed."
echo "Run: hawknext (TUI) or hawknext --web (Web)"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
