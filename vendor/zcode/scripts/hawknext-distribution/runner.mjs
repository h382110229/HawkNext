...
const agentEntry = join(root, "agent", "hawknext.cjs");

function usage() {
  return `Usage:
  hawknext --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  hawknext --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      HAWKNEXT_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      HAWKNEXT_AGENT_SERVER_COMMAND: process.execPath,
      HAWKNEXT_SERVER_HOST: options.host,
      HAWKNEXT_SERVER_WORKSPACE: options.workspace,
      HAWKNEXT_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      HAWKNEXT_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("HawkNext Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: hawknext --web [options] (hawknext --web --help for details)\n");
    }
