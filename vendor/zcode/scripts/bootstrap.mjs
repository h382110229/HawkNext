...
function resolveBootstrapWithRemoteEnv(baseEnv = process.env) {
  ...
  return {
    ...
    HAWKNEXT_BOOTSTRAP_WITH_REMOTE: "1",
...
function runBootstrapServerBuild() {
  ...
  // bootstrap:with-remote 的最终构建过去复用 build:bootstrap，导致 @hawknext/server build
...
function runBootstrapWithRemoteBuild() {
  for (const filter of ["@hawknext/rpc", "@hawknext/web", "@hawknext/formal-proof"]) {
...
runGit(["submodule", "update", "--init", "--recursive", "apps/hawknext-cli"]);
