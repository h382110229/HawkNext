...
const defaultEntryPath = join(repoRoot, "apps/hawknext-cli/packages/cli/dist/hawknext.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.HAWKNEXT_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "hawknext.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `hawknext.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `hawknext.bytecode-runtime-${runtimeHash}.cjs`;
