  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    const child = spawn(command, spawnArgs, {
      ...
      env: withPinnedNodePath(
        {
          ...process.env,
          HAWKNEXT_ENV: requestedEnv,
          HAWKNEXT_DESKTOP_AGENT_BYTECODE: agentBytecode ? "1" : "0",
        },
        process.execPath,
      ),
      stdio: "inherit",
...
try {
  ...
  await run(pnpmCommand, ["--filter", "@hawknext/desktop", "pre-dev"]);
  ...
  await run(pnpmCommand, ["--filter", "@hawknext/desktop", "dev:runtime"]);
} catch (error) {
