...
function printUsageAndExit() {
  process.stderr.write(
    [
      "Usage: hawknext-stdio-tap.mjs --workspace-key <key> [--log-dir <dir>] -- <command> [...args]",
      "",
    ].join("\n"),
  );
  process.exit(2);
...
function parseArgs(argv) {
  ...
  let workspaceKey = "";
  let logDir = process.env.HAWKNEXT_STDIO_TAP_LOG_DIR?.trim() || "";
  ...
  return {
    ...
    logDir: logDir || join(homedir(), ".hawknext", "v2", "dev", "stdio-traffic"),
    workspaceKey,
  };
}
...
child.on("error", (error) => {
  ...
  process.stderr.write(`[hawknext-stdio-tap] spawn failed: ${error.message}\n`);
});
