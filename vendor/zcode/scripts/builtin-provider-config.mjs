...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.HAWKNEXT_ENV !== undefined) value = parsed.HAWKNEXT_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid HAWKNEXT_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.HAWKNEXT_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/hawknext-builtin.json",
  );
  try {
    ...
    const { decodeHawkNextBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/hawknext-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeHawkNextBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "hawknext-builtin.json"), config.content, "utf8");
  return config;
}
