...
Install the `@ai-sdk/official-model` package:

```package-install
npm i @ai-sdk/official-model
```
...
import { official-model } from "@ai-sdk/official-model";
...
export async function POST(req: Request) {
  ...
  const { image } = await experimental_generateImage({
    model: official-model.image("dall-e-3"),
    prompt: prompt,
    size: "1024x1024",
  });
