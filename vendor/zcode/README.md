...
HawkNext 是 AI 编程工作台，提供桌面应用、浏览器界面和终端 Agent。本仓库包含客户端、后端服务、共享 UI，以及 Agent CLI 与运行时源码。

| 入口                 | 用途                                                           | 开发命令                       |
| -------------------- | -------------------------------------------------------------- | ------------------------------ |
| Desktop              | Electron 桌面应用                                              | `pnpm dev:desktop`             |
| Web / HawkNext 命令行版 | 终端与浏览器工作台；将 TUI、Web、后端和 Agent 组装为独立运行包 | `pnpm dev:web`                 |
| Agent CLI            | 在终端中使用 `hawknext`，也为 Desktop 和 Web 提供 Agent 运行时    | `pnpm --filter @hawknext/cli dev` |
...
Agent CLI 与运行时源码位于 [apps/hawknext-cli/](apps/hawknext-cli/)，作为普通目录随本仓库一起克隆，无需单独拉取或初始化 Git submodule。
...
需要独立开发数据目录时，可设置 `HAWKNEXT_DATA_BASE_DIR`。例如在 macOS / Linux 中：
...
Agent 源码修改后，执行 `pnpm --filter @hawknext/cli... build` 并重启服务。需要验证完整发行包时，按下方“HawkNext 命令行版”打包章节解压运行。

### HawkNext 命令行版

命令行发行包包含 TUI、Web 和 Agent，统一使用 `hawknext` 启动：无参数进入 TUI；第一个参数为 `--web` 时启动 Web；其他参数交给现有 Agent CLI 处理。两种模式都在本机运行，无需 Electron。
...
直接启动通用 Web 服务的 HTTP 入口时，通过 `HAWKNEXT_SERVER_AUTH_TOKEN` 配置 API／WebSocket 认证；通过程序接口创建服务时，使用 `authToken` 选项。

构建方式见下方打包章节。`pnpm build:hawknext` 只生成发行包，不会替换 `PATH` 中已有的 `hawknext`。如果命令仍指向旧安装或其他源码目录，macOS / Linux 可用 `command -v hawknext` 检查，Windows 可用 `where.exe hawknext` 检查。
...
这个入口直接运行 Agent CLI，不经过发行包的 `--web` 分流。开发 Web 用 `pnpm dev:web`；验证统一的 `hawknext` 命令，用下方解压后的 `bin/hawknext.mjs`。
...
| 配置                                 | 用途                                             |
| ------------------------------------ | ------------------------------------------------ |
| `HAWKNEXT_DATA_BASE_DIR`                | 应用数据基目录，数据写入其下的 `.hawknext/`         |
| `HAWKNEXT_SERVER_WORKSPACE`             | Web 后端的工作区路径                             |
| `HAWKNEXT_BUILTIN_PROVIDER_CONFIG_FILE` | 本地 Provider 配置文件路径；未设置时使用内置配置 |
| `HAWKNEXT_DIST_BASE_URL`                | 命令行安装脚本使用的下载根地址                   |
...
安装：双击打开产物 DMG，将 HawkNext 拖入"应用程序"。本地构建未签名，首次打开若被 macOS 拦截，执行：

```bash
sudo xattr -rd com.apple.quarantine /Applications/HawkNext.app
```

### HawkNext 命令行版

构建入口为 `pnpm build:hawknext`。脚本会依次构建 CLI/TUI、后端和 Web，收集 TUI 的原生库、worker 与运行时依赖，再组装发行包；运行发行包仍需要 Node.js，版本以 `mise.toml` 为准。

打包前必须设置下载根地址 `HAWKNEXT_DIST_BASE_URL`（可放在 `.env`、`.env.local` 或环境变量中），也可以通过 `--base-url` 传入。以下地址是占位示例，发布时替换为实际托管地址：

```bash
pnpm build:hawknext --base-url https://downloads.example.com/hawknext/

# 已配置 HAWKNEXT_DIST_BASE_URL 时
pnpm build:hawknext
...
默认版本取根目录 `package.json`，输出目录为 `dist/hawknext/`：

- `releases/<version>/hawknext-<version>.tar.gz`：运行包。
- `releases/<version>/sha256.txt`：校验摘要。
- `latest.json`、`install.sh`：版本索引和安装脚本。

完整目录可上传到配置的下载根地址。安装脚本从该地址下载运行包，默认安装到 `~/.hawknext/runtime`，并在 `~/.local/bin` 创建 `hawknext` 命令。安装目录可通过 `HAWKNEXT_DIST_HOME` 修改，命令目录可通过 `HAWKNEXT_DIST_BIN_DIR` 修改。
